...
from superverse.annotators.base import BaseAnnotator, ImageType
from superverse.annotators.utils import (
    ColorLookup,
    Trace,
    resolve_color,
    resolve_text_background_xyxy,
)
from superverse.config import CLASS_NAME_DATA_FIELD, ORIENTED_BOX_COORDINATES
from superverse.detection.core import Detections
from superverse.detection.utils import clip_boxes, mask_to_polygons, spread_out_boxes
from superverse.draw.color import Color, ColorPalette
from superverse.draw.utils import draw_polygon
...
from superverse.utils.internal import deprecated
...
class BoxAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![bounding-box-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/bounding-box-annotator-example-purple.png)
        """
...
@deprecated(
    ...
    " `BoundingBoxAnnotator` will be removed in superverse-0.26.0."
)
...
class MaskAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![mask-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/mask-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
...
class PolygonAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![polygon-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/polygon-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
        if detections.mask is None:
            return scene
...
class ColorAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![box-mask-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/box-mask-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
...
class HaloAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![halo-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/halo-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
        if detections.mask is None:
            return scene
...
class EllipseAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![ellipse-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/ellipse-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
        for detection_idx in range(len(detections)):
            x1, y1, x2, y2 = detections.xyxy[detection_idx].astype(int)
...
class BoxCornerAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![box-corner-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/box-corner-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
        for detection_idx in range(len(detections)):
            x1, y1, x2, y2 = detections.xyxy[detection_idx].astype(int)
...
class CircleAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![circle-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/circle-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
        for detection_idx in range(len(detections)):
            x1, y1, x2, y2 = detections.xyxy[detection_idx].astype(int)
...
class DotAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![dot-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/dot-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
...
class LabelAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![label-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/label-annotator-example-purple.png)
        """
...
class IconAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        Example:
            ...
            available_icons = ["khulnasoft.png", "lenny.png"]
        ...
        ![icon-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/icon-annotator-example.png)
        """
        assert isinstance(scene, np.ndarray)
...
class BlurAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![blur-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/blur-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
...
class TraceAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![trace-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/trace-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
        if detections.tracker_id is None:
            raise ValueError(
                "The `tracker_id` field is missing in the provided detections."
                " See more: https://superverse.khulnasoft.com/latest/how_to/track_objects"
            )
...
class HeatMapAnnotator(BaseAnnotator):
    ...
    def annotate(self, scene: ImageType, detections: Detections) -> ImageType:
        ...
        ![heatmap-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/heat-map-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
...
class PixelateAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![pixelate-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/pixelate-annotator-example-10.png)
        """
        assert isinstance(scene, np.ndarray)
        image_height, image_width = scene.shape[:2]
        clipped_xyxy = clip_boxes(
...
class TriangleAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![triangle-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/triangle-annotator-example.png)
        """
        assert isinstance(scene, np.ndarray)
        xy = detections.get_anchors_coordinates(anchor=self.position)
        for detection_idx in range(len(detections)):
...
class RoundBoxAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![round-box-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/round-box-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
        for detection_idx in range(len(detections)):
            x1, y1, x2, y2 = detections.xyxy[detection_idx].astype(int)
...
class PercentageBarAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        ![percentage-bar-example](https://media.khulnasoft.com/
        superverse-annotator-examples/percentage-bar-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
...
class BackgroundOverlayAnnotator(BaseAnnotator):
    ...
    def annotate(self, scene: ImageType, detections: Detections) -> ImageType:
        ...
        ![background-overlay-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/background-color-annotator-example-purple.png)
        """
        assert isinstance(scene, np.ndarray)
