...
class SuperverseWarnings(Warning):
    """Superverse warning category.
    Set the deprecation warnings visibility for Superverse library.
...
if os.getenv("SUPERVISON_DEPRECATION_WARNING") == "0":
    warnings.simplefilter("ignore", SuperverseWarnings)
else:
    warnings.simplefilter("always", SuperverseWarnings)
...
def warn_deprecated(message: str):
    ...
    """
    warnings.warn(message, category=SuperverseWarnings, stacklevel=2)
