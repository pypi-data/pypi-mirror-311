...
from superverse.config import (
    CLASS_NAME_DATA_FIELD,
    ORIENTED_BOX_COORDINATES,
)
from superverse.detection.lmm import (
    LMM,
    from_florence_2,
    from_paligemma,
...
from superverse.detection.tools.transformers import (
...
from superverse.detection.utils import (
    ...
    merge_data,
    merge_metadata,
    process_khulnasoft_result,
    xywh_to_xyxy,
)
from superverse.geometry.core import Position
from superverse.utils.internal import get_instance_variables
from superverse.validators import validate_detections_fields


@dataclass
class Detections:
    """
    The `sv.Detections` class in the Superverse library standardizes results from
    various object detection and segmentation models into a consistent format. This
    class simplifies data manipulation and filtering, providing a uniform API for
    integration with Superverse [trackers](/trackers/), [annotators](/latest/detection/annotators/), and [tools](/detection/tools/line_zone/).

    === "Inference"

        Use [`sv.Detections.from_inference`](/detection/core/#superverse.detection.core.Detections.from_inference)
    ...
    === "Ultralytics"
        ...
        Use [`sv.Detections.from_ultralytics`](/detection/core/#superverse.detection.core.Detections.from_ultralytics)
        method, which accepts model results from both detection and segmentation models.
    ...
    === "Transformers"
        ...
        Use [`sv.Detections.from_transformers`](/detection/core/#superverse.detection.core.Detections.from_transformers)
        method, which accepts model results from both detection and segmentation models.
    ...
    @classmethod
    def from_inference(cls, khulnasoft_result: Union[dict, Any]) -> Detections:
        """
        Create a `sv.Detections` object from the [Khulnasoft](https://khulnasoft.com/)
        API inference result or the [Inference](https://inference.khulnasoft.com/)
        package results. This method extracts bounding boxes, class IDs,
        confidences, and class names from the Khulnasoft API result and encapsulates
        them into a Detections object.

        Args:
            khulnasoft_result (dict, any): The result from the
                Khulnasoft API or Inference package containing predictions.
        ...
        with suppress(AttributeError):
            khulnasoft_result = khulnasoft_result.dict(exclude_none=True, by_alias=True)
        xyxy, confidence, class_id, masks, trackers, data = process_khulnasoft_result(
            khulnasoft_result=khulnasoft_result
        )
    ...
    def with_nmm(
        ...
        ![non-max-merging](https://media.khulnasoft.com/superverse-docs/non-max-merging.png){ align=center width="800" }
        """  # noqa: E501 // docs
        if len(self) == 0:
            return self
