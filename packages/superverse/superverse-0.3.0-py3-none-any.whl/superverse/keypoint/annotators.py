...
from superverse.annotators.base import ImageType
from superverse.detection.utils import pad_boxes, spread_out_boxes
from superverse.draw.color import Color
from superverse.draw.utils import draw_rounded_rectangle
from superverse.geometry.core import Rect
from superverse.keypoint.core import KeyPoints
from superverse.keypoint.skeletons import SKELETONS_BY_VERTEX_COUNT
from superverse.utils.conversion import ensure_cv2_image_for_annotation
...
class VertexAnnotator(BaseKeyPointAnnotator):
    ...
    def annotate(self, scene: ImageType, key_points: KeyPoints) -> ImageType:
        ...
        ![vertex-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/vertex-annotator-example.png)
        """
...
class EdgeAnnotator(BaseKeyPointAnnotator):
    ...
    def annotate(self, scene: ImageType, key_points: KeyPoints) -> ImageType:
        ...
        ![edge-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/edge-annotator-example.png)
        """
        assert isinstance(scene, np.ndarray)
        if len(key_points) == 0:
            return scene
...
class VertexLabelAnnotator:
    ...
    def __init__(
        self,
        color: Union[Color, List[Color]] = Color.KHULNASOFT,
    ...
    def annotate(
        ...
        ![vertex-label-annotator-example](https://media.khulnasoft.com/
        superverse-annotator-examples/vertex-label-annotator-example.png)
        ...
        Example:
            ...
            ```
        ![vertex-label-annotator-custom-example](https://media.khulnasoft.com/
        superverse-annotator-examples/vertex-label-annotator-custom-example.png)
        """
        assert isinstance(scene, np.ndarray)
