...
from ipfabric.models.security import Machine, DefaultAction
...
class Diagram(BaseModel):
    ...
    def visio(
        ...
        if not self.ipf.get("os/feature-flags").json().get("ENABLE_VSDX_EXPORT", False):
            raise ValueError("Visio export feature flag is not enabled in this version of IP Fabric.")
        return self._query(
            self._swap_src_dst(parameters, unicast_swap_src_dst),
            snapshot_id=snapshot_id,
            overlay=overlay,
                        ...
                        if event.type == "security check":
                            ...
                            if event.securityType == "zoneFw":
                                ...
                                if zone_firewall_rules:
                                    ...
                                    graph_device.securityModelRule = self.find_default_rule(
                                        machine_zones, event.decidingPolicyName
                                    )
    ...
    @staticmethod
    def find_default_rule(data: Machine, chain_name: str) -> Optional[DefaultAction]:
        """
        Find the matching rule from a given dataset based on the chain name and index path.

        Args:
            data (Machine): The dataset containing rule chains and rules.
            chain_name (str): The name of the chain to look for.
        Returns:
            Rule: The matching rule if found, otherwise None.
        """
        rule_chain = data.ruleChains.get(chain_name)
        return rule_chain.defaultAction if rule_chain else None
