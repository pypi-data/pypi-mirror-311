...
from ipfabric.scripts.shared import check_snapshot_arg
...
def main():
    ...
    arg_parser.add_argument(
        ...
        "with or without `$` for *nix compatability; default '$last'.",
        default="$last",
    )
    arg_parser.add_argument(
        "-n",
    ...
    arg_parser.add_argument(
        "-t", "--timeout", help="Timeout for NIST API (Int, Float, or None); default 60 seconds.", default=60
    )
    arg_parser.add_argument(
    ...
    ipf = IPFClient(snapshot_id=check_snapshot_arg(args.snapshot))
