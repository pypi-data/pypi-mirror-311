...
class SRAG:
    ...
    def __process_data(self, filepath, old_filter):
        ...
        data_processed = (data.assign(
            ...
            , MES_SIN_PRI=lambda x: (x['DT_SIN_PRI'].dt.strftime('%m')).astype(int)
            ...
            , SEM_EPI=lambda x: x['SEM_PRI'].astype(int)
            , ANO_EPI = lambda x: np.where( (x['MES_SIN_PRI'] == 12) & (x['SEM_EPI'] == 1), x['ANO_SIN_PRI'] + 1, x['ANO_SIN_PRI'])
            ...
            , POS_ANY=lambda x: np.where(x['POS_SUM'] > 0, 1, 0)
                ...
                , 'DT_SIN_PRI', 'ANO_SIN_PRI','MES_SIN_PRI', 'SEM_SIN_PRI', 'ANO_SEM_SIN_PRI', 'DT_SIN_PRI_SEM', 'DIF_SEM_FILE_SIN_PRI'
                , 'SEM_EPI', 'ANO_EPI'
    ...
    def __load_file_data(self, old_filter):
        ...
        if isinstance(self.__filepath, list):
            data = pd.concat([ self.__process_data(fp, old_filter) for fp in self.__filepath ]).reset_index(drop=True)
        else:
            data = self.__process_data(self.__filepath, old_filter)
    ...
    def generate_training_weeks(self, lag = None):
        ...
        df = data[['DT_SIN_PRI_SEM', 'ANO_SIN_PRI', 'SEM_SIN_PRI', 'ANO_SEM_SIN_PRI', 'ANO_EPI', 'SEM_EPI']].drop_duplicates().reset_index(drop=True)
