...
class StreamBufferOverflowData:
    '''Provides data the @ref EYETRACKER_NOTIFICATION_STREAM_BUFFER_OVERFLOW callback.

    You will get data of this type when you subscribe to @ref EYETRACKER_NOTIFICATION_STREAM_BUFFER_OVERFLOW
    with EyeTracker.subscribe_to.
    '''

    def __init__(self, data):
        if not isinstance(data, dict):
            raise ValueError("You shouldn't create StreamBufferOverflowData objects yourself.")

        self.__system_time_stamp = data["system_time_stamp"]
        self.__notification_source_stream = data["notification_source_stream"]
    ...
    @property
    def notification_source_stream (self):
        '''Gets the name of the stream which was the source of the buffer overflow notification.
        '''
        return self.__notification_source_stream