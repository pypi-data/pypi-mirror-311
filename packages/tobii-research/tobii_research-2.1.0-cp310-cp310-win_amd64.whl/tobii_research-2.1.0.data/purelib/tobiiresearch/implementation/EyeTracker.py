...
from tobiiresearch.implementation.Notifications import EyeTrackingModeChangedData
from tobiiresearch.implementation.Notifications import DeviceFaultsData, DeviceWarningsData
from tobiiresearch.implementation.Notifications import StreamBufferOverflowData
...
##
# Indicates that the device can deliver an eye openness data stream.
#
# Value in tuple EyeTracker.device_capabilities
# <CodeExample>create_eyetracker.py</CodeExample>
...
##
# Used in EyeTracker.subscribe_to and EyeTracker.unsubscribe_from for stream buffer overflow messages.
#
# You will get a callback when a stream buffer encounters a buffer overflow. This happens when stream data
# is not consumed in a timely manner and the SDK receives data from the tracker faster than it is consumed
# by the relevant stream callback.
#
# Callbacks will receive a StreamBufferOverflowData
# object or a dictionary with values if as_dictionary is True.
# The warnings information consists of a comma seprated string.
# See @ref find_all_eyetrackers or EyeTracker.__init__ on how to create an EyeTracker object.
# <CodeExample>notifications.py</CodeExample>
EYETRACKER_NOTIFICATION_STREAM_BUFFER_OVERFLOW = "eyetracker_notification_stream_buffer_overflow"
...
#
# You will get a callback when time synchronized eye openness is received. Time synchronized eye openness is not
# supported on all eye trackers, other eye trackers need additional license to activate this support. Callbacks will
# receive an EyeOpennessData object or a dictionary with values if as_dictionary is True.
# See @ref find_all_eyetrackers or EyeTracker.__init__ on how to create an EyeTracker object.
...
_available_notification_subscriptions =\
    {EYETRACKER_NOTIFICATION_CONNECTION_LOST: ConnectionLostData,
     ...
     EYETRACKER_NOTIFICATION_STREAM_BUFFER_OVERFLOW: StreamBufferOverflowData,
     }
...
class EyeTracker(object):
    ...
    def subscribe_to(self, subscription_type, callback, as_dictionary=False):
        ...
        if subscription_type in _available_notification_subscriptions.keys():
            with self.__subscription_lock and self.__notification_subscription_lock:
                first_notification_subscription = self.__subscribe_to_notification(
                    subscription_type, callback, as_dictionary)
                if first_notification_subscription:
                    self.__subscribe_to_stream(
                        _EYETRACKER_NOTIFICATIONS, self.__notification_callback)
        else:
            with self.__subscription_lock:
                self.__subscribe_to_stream(
                    subscription_type, callback, as_dictionary)

    def __subscribe_to_notification(self, subscription_type, callback, as_dictionary=False):
        '''
        ...
        num_notification_subscriptions = len(self.__notification_subscriptions)
        self.__notification_subscriptions.setdefault(subscription_type, {})[
            callback] = as_dictionary
        return num_notification_subscriptions == 0

    def __subscribe_to_stream(self, subscription_type, callback, as_dictionary=False):
        ...
        self.__subscriptions.setdefault(subscription_type, {})[
            callback] = as_dictionary
    ...
    def unsubscribe_from(self, subscription_type, callback=None):
        ...
        if subscription_type in _available_notification_subscriptions.keys():
            with self.__subscription_lock and self.__notification_subscription_lock:
                self.__unsubscribe_from_notification(
                    subscription_type, callback)
                no_more_notifications = len(
                    self.__notification_subscriptions) == 0
                if (no_more_notifications):
                    self.__unsubscribe_from_stream(
                        _EYETRACKER_NOTIFICATIONS, None)
        else:
            with self.__subscription_lock:
                self.__unsubscribe_from_stream(subscription_type, callback)

    def __unsubscribe_from_notification(self, subscription_type, callback=None):
    ...
    def __unsubscribe_from_stream(self, subscription_type, callback=None):
        if subscription_type not in _subscription_types:
            _on_error_raise_exception(_invalid_parameter)
