...
def _filter_decision_makers_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    """Find all duplicates, keep the row with the most infromation in the email and linkedin_url columns."""
    ...
    df_with_dm_only = df[df["decision_maker_firstname"].notna() | df["decision_maker_lastname"].notna()]
    # Count the number of NaNs in the relevant columns for each row
    df_with_dm_only["nan_count"] = (
        df_with_dm_only[["decision_maker_email", "decision_maker_linkedin_url"]].isna().sum(axis=1)
    )
    # Find the index of the row with the minimum NaN count in each group
    idx = df_with_dm_only.groupby(group_columns)["nan_count"].idxmin()
    # Select the rows with the least NaNs within each group
    result = df.loc[idx].drop(columns=["nan_count"], errors="ignore")
