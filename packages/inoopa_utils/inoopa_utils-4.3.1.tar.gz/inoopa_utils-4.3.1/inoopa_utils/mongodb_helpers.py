...
def _filter_decision_makers_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    group_columns = [
        col for col in [id_column, "decision_maker_firstname", "decision_maker_lastname"] if col in df.columns
    ]
    # Count the number of NaNs in the relevant columns for each row
    df["nan_count"] = df[["decision_maker_email", "decision_maker_linkedin_url"]].isna().sum(axis=1)
    # Find the index of the row with the minimum NaN count in each group
    idx = df.groupby(group_columns)["nan_count"].idxmin()
    # Select the rows with the least NaNs within each group
    result = df.loc[idx].drop(columns=["nan_count"])
