...
def import_veh_dyn_info_2(filepath2localgg: str = "") -> np.ndarray:
    ...
    if not filepath2localgg:
        raise RuntimeError(
            "Missing path to file which contains vehicle acceleration limits!"
        )
    ...
    if data_localggfile.ndim == 1:
        ...
        if data_localggfile.size != 5:
            raise RuntimeError(
                "TPA MapInterface: wrong shape of localgg file data -> five columns required!"
            )
        ...
        if np.any(tpamap[:, 0] < 0.0):
            raise RuntimeError(
                "TPA MapInterface: one or more s-coordinate values are smaller than zero!"
            )

        if np.any(np.diff(tpamap[:, 0]) <= 0.0):
            raise RuntimeError(
                "TPA MapInterface: s-coordinates are not strictly monotone increasing!"
            )

        # check whether endpoint and start point of s is close together in xy
        if not np.isclose(
            np.hypot(tpamap[0, 1] - tpamap[-1, 1], tpamap[0, 2] - tpamap[-1, 2]), 0.0
        ):
            raise RuntimeError(
                "TPA MapInterface: s-coordinates representing the race track are not closed; "
    ...
    if np.any(tpamap[:, 3:] > 20.0):
        raise RuntimeError(
            "TPA MapInterface: max. acceleration limit in localgg file exceeds 20 m/s^2!"
        )

    if np.any(tpamap[:, 3:] < 1.0):
        raise RuntimeError(
            "TPA MapInterface: min. acceleration limit in localgg file is below 1 m/s^2!"
        )
