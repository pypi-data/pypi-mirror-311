...
def opt_min_curv(
    ...
    num_splines = num_points

    # calculate allowed deviation from refline
    dev_max_right = reftrack.w_tr_right_m - w_veh / 2
    dev_max_left = reftrack.w_tr_left_m - w_veh / 2

    if not reftrack.is_closed:
        reftrack.close_racetrack()
    ...
    if (num_points * 4 != A.shape[0]) or (A.shape[0] != A.shape[1]):
    ...
    # information
    A_ex_b = np.zeros((num_points, num_splines * 4), dtype=int)
    ...
    # information
    A_ex_c = np.zeros((num_points, num_splines * 4), dtype=int)
    ...
    M_x = np.zeros((num_splines * 4, num_points))
    M_y = np.zeros((num_splines * 4, num_points))
    ...
    q_x = np.zeros((num_splines * 4, 1))
    q_y = np.zeros((num_splines * 4, 1))

    for i in range(num_splines):
        j = i * 4

        if i < num_points - 1:
            q_x[j, 0] = reftrack.x_m[i]
            q_x[j + 1, 0] = reftrack.x_m[i + 1]

            q_y[j, 0] = reftrack.y_m[i]
            q_y[j + 1, 0] = reftrack.y_m[i + 1]
        else:
            q_x[j, 0] = reftrack.x_m[i]
            q_x[j + 1, 0] = reftrack.x_m[0]

            q_y[j, 0] = reftrack.y_m[i]
            q_y[j + 1, 0] = reftrack.y_m[0]

    # set up P_xx, P_xy, P_yy matrices
    x_prime = np.eye(num_points, num_points) * np.matmul(np.matmul(A_ex_b, A_inv), q_x)
    y_prime = np.eye(num_points, num_points) * np.matmul(np.matmul(A_ex_b, A_inv), q_y)
    ...
    con_ge = np.ones((num_points, 1)) * kappa_bound - k_kappa_ref
    con_le = -(
        np.ones((num_points, 1)) * -kappa_bound - k_kappa_ref
    ...
    G = np.vstack((np.eye(num_points), -np.eye(num_points), E_kappa, -E_kappa))
    ...
    lateral_shifts = quadprog.solve_qp(H, -f, -G.T, -h, 0)[0]
    ...
    q_x_tmp = q_x + np.matmul(M_x, np.expand_dims(lateral_shifts, 1))
    q_y_tmp = q_y + np.matmul(M_y, np.expand_dims(lateral_shifts, 1))

    x_prime_tmp = np.eye(num_points, num_points) * np.matmul(
        np.matmul(A_ex_b, A_inv), q_x_tmp
    )
    y_prime_tmp = np.eye(num_points, num_points) * np.matmul(
        np.matmul(A_ex_b, A_inv), q_y_tmp
    )

    x_prime_prime = np.squeeze(
        np.matmul(T_c, q_x) + np.matmul(T_nx, np.expand_dims(lateral_shifts, 1))
    )
    y_prime_prime = np.squeeze(
        np.matmul(T_c, q_y) + np.matmul(T_ny, np.expand_dims(lateral_shifts, 1))
    )

    curv_orig_lin = np.zeros(num_points)
    curv_sol_lin = np.zeros(num_points)
    ...
    return lateral_shifts, curv_error_max


def old_opt_min_curv(
    reftrack: np.ndarray,
    normvectors: np.ndarray,
    A: np.ndarray,
    kappa_bound: float,
