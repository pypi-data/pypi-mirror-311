...
class APIManager:
    ...
    def search(
        ...
        """
        if not self.api_key:
            logger.warning("API key is required for search functionality.")
            return None
        ...
        if response:
            ...
            logger.warning("Failed to retrieve search results.")
            return None
...
class TTSPipelineManager:
    ...
    def setup_keyboard_controls(self) -> None:
        ...
        keyboard.on_press_key('f', create_key_handler(self.perform_search))
    ...
    def perform_search(self) -> None:
        """
        Interactive search functionality triggered by keyboard
        """
        if not self.api_key:
            logger.warning("API key is required for search functionality.")
            return
        ...
        filter_type: str = input("Enter filter (optional, press Enter to skip): ")
        self.api_manager.search(query, filter_type or None)
