...
def test_sum_exogenous_demand_and_electric_vehicle_demand():
    ...
    calibration_profile_df = read_csv_dataset("ev_calibration_profile.csv")
    ev_fleet_df = read_csv_dataset("ev_fleets.csv")
    exo_demand_df = read_csv_dataset("demand.csv")
    ...
    consumption_xr = create_consumption_per_million_ev_xarray_dataset(
        temperature_xr, calibration_profile_df, profile_df, 15, 0, True
    )
    ev_fleet_xr = create_area_ev_fleet_xarray(ev_fleet_df, time_df)

    total_consumption_ev_xr = create_total_consumption_xarray_dataset(
    ...
    save_xarray_into_csv_file(global_demand_xr, "global_demand_xarray.csv")
...
def test_extract_total_demand_from_xarray_dataset():
    ...
    consumption_xr = create_total_consumption_xarray_dataset(
        consumption_xr, ev_fleet_xr
    )
    ...
    save_xarray_into_csv_file(demand_xr, "demand_xarray.csv")
...
def test_create_total_consumption_xarray_dataset():
    ...
    save_xarray_into_csv_file(consumption_xr, "consumption_xarray.csv")
...
def test_create_consumption_xarray_dataset():
    ...
    print(consumption_xr)
    save_xarray_into_csv_file(consumption_xr, "consumption_xr.csv")
