...
class Coordinate:
    ...
    def validate(coordinate: tuple):
        ...
        if not Coordinate.is_valid(coordinate):
            raise Exception(f'The provided "coordinate" parameter is not a valid tuple of 2 elements that are values between our limits {str(NORMALIZATION_MIN_VALUE)} and {str(NORMALIZATION_MAX_VALUE)}. Please, provide a valid coordinate.')
