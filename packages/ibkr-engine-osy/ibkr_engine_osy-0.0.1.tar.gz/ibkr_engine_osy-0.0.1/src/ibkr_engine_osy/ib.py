...
class IB:
    ...
    **user code may not block for too long**.
    ...
    def __init__(self, host: str, port: int, client_id: int):
        ...
        self.client.api_end += self.disconnectedEvent
    ...
    def connect(self, timeout: float = 4, readonly: bool = False, account: str = '', raiseSyncErrors: bool = False):
        ...
        """
        return self._run(self.connectAsync(timeout, readonly, account, raiseSyncErrors))
    ...
    def disconnect(self):
        ...
        if not self.client.is_connected:
            return
        stats = self.client.connection_stats
        self._logger.info(
            f'Disconnecting from {self.client.host}:{self.client.port}, '
            f'{formatSI(stats.numBytesSent)}B sent '
            f'in {stats.numMsgSent} messages, '
            f'{formatSI(stats.numBytesRecv)}B received '
            f'in {stats.numMsgRecv} messages, '
            f'session time {formatSI(stats.duration)}s.')
    ...
    timeRange = staticmethod(timeRange)
    timeRangeAsync = staticmethod(timeRangeAsync)
    waitUntil = staticmethod(waitUntil)

    def _run(self, *awaitables: Awaitable):
        return run(*awaitables, timeout=self.RequestTimeout)
    ...
    def waitOnUpdate(self, timeout: float = 0) -> bool:
        ...
        if timeout:
            try:
                run(asyncio.wait_for(self.updateEvent, timeout))
            except asyncio.TimeoutError:
                return False
        else:
            run(self.updateEvent)
        return True
    ...
    def pnl(self, account='', modelCode='') -> List[PnL]:
        ...
        return [v for v in self.wrapper.reqId2PnL.values() if
                ...
                and (not modelCode or v.model_code == modelCode)]
    ...
    def pnlSingle(
        ...
        return [v for v in self.wrapper.reqId2PnlSingle.values() if
                (not account or v.account == account)
                and (not modelCode or v.model_code == modelCode)
                and (not conId or v.con_id == conId)]
    ...
    def placeOrder(self, contract: Contract, order: Order) -> Trade:
        ...
        """
        orderId = order.order_id or self.client.getReqId()
        self.client.placeOrder(orderId, contract, order)
        now = datetime.now(timezone.utc)
        key = self.wrapper.orderKey(
            self.wrapper.client_id, orderId, order.perm_id)
    ...
    def cancelOrder(self, order: Order, manualCancelOrderTime: str = '') \
        ...
        """
        self.client.cancelOrder(order.order_id, manualCancelOrderTime)
        now = datetime.datetime.now(datetime.timezone.utc)
        key = self.wrapper.orderKey(
            order.client_id, order.order_id, order.perm_id)
        trade = self.wrapper.trades.get(key)
        if trade:
            if not trade.isDone():
            ...
            self._logger.error(f'cancelOrder: Unknown orderId {order.order_id}')
        return trade
    ...
    def reqRealTimeBars(
        ...
        bars.bar_size = barSize
        bars.what_to_show = whatToShow
        bars.use_rth = useRTH
        bars.real_time_bars_options = realTimeBarsOptions or []
    ...
    def cancelRealTimeBars(self, bars: RealTimeBarList):
        ...
        """
        self.client.cancelRealTimeBars(bars.req_id)
        self.wrapper.endSubscription(bars)

    def reqHistoricalData(
            self, contract: Contract,
            endDateTime: Union[datetime, datetime.date, str, None],
    ...
    def cancelHistoricalData(self, bars: BarDataList):
        ...
        """
        self.client.cancelHistoricalData(bars.req_id)
        self.wrapper.endSubscription(bars)
    ...
    def reqHeadTimeStamp(self, contract: Contract, whatToShow: str, useRTH: bool, formatDate: int = 1) -> datetime:
        """
    ...
    def reqScannerSubscription(
        ...
        dataList.req_id = reqId
        dataList.subscription = subscription
        dataList.scanner_subscription_options = scannerSubscriptionOptions or []
        dataList.scanner_subscription_filter_options = \
    ...
    def cancelScannerSubscription(self, dataList: ScanDataList):
        ...
        """
        self.client.cancelScannerSubscription(dataList.req_id)
    ...
    async def connectAsync(
        ...
        self.internal_logger(msg=f'AT Func:{self.connectAsync.__name__};Async Connecting...')
        clientId = int(self.client_id)
        self.wrapper.client_id = clientId
        timeout = timeout or None
        try:
            # establish API connection
            await self.client.connect_async(clientId, timeout)
            ...
            self.internal_logger(
                msg=f'AT Func:{self.connectAsync.__name__}; getting accounts: {accounts}; account: {account}'
            )
            ...
            if len(accounts) <= self.MaxSyncedSubAccounts:
                for acc in accounts:
                    reqs[f'account updates for {acc}'] = self.reqAccountUpdatesMultiAsync(acc)

            self.internal_logger(msg=f'AT Func:{self.connectAsync.__name__}; getting requests: {reqs}')
    ...
    async def reqHistoricalDataAsync(
            ...
            chartOptions: List[TagValue] = [], timeout: float = 60) \
            -> BarDataList:
        self.internal_logger(f'AT Func:{self.reqHistoricalDataAsync.__name__}; requesting historical data...')
        reqId = self.client.getReqId()
        self.internal_logger(f'AT Func:{self.reqHistoricalDataAsync.__name__}; getting the reqID from the `Client`: {reqId}')
        bars = BarDataList()
        bars.req_id = reqId
        bars.contract = contract
        bars.end_date_time = endDateTime
        bars.duration_str = durationStr
        bars.bar_size_setting = barSizeSetting
        bars.what_to_show = whatToShow
        bars.use_rth = useRTH
        bars.format_date = formatDate
        bars.keep_up_to_date = keepUpToDate
        bars.chart_options = chartOptions or []

        self.internal_logger(f'AT Func:{self.reqHistoricalDataAsync.__name__}; starting a request from `Wrapper`')
        future = self.wrapper.startReq(reqId, contract, container=bars)

        if keepUpToDate:
            self.wrapper.startSubscription(reqId, bars, contract)

        end = formatIBDatetime(endDateTime)

        self.internal_logger(f'AT Func:{self.reqHistoricalDataAsync.__name__}; requesting historical data from `Client`')
    ...
    def reqHistoricalTicksAsync(
        ...
        reqId = self.client.getReqId()
        future = self.wrapper.startReq(reqId, contract)
        start = formatIBDatetime(startDateTime)
        end = formatIBDatetime(endDateTime)
    ...
    async def reqHeadTimeStampAsync(self, contract: Contract, whatToShow: str, useRTH: bool, formatDate: int) -> datetime:
        reqId = self.client.getReqId()
        future = self.wrapper.startReq(reqId, contract)
    ...
    async def reqScannerDataAsync(
        ...
        dataList = self.reqScannerSubscription(
            ...
            scannerSubscriptionFilterOptions or [])
        future = self.wrapper.startReq(dataList.req_id, container=dataList)
        await future
        self.client.cancelScannerSubscription(dataList.req_id)
        return future.result()
