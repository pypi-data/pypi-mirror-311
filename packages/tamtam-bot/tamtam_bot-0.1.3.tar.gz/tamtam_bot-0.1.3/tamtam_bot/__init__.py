...
class BotHandler:
    ...
    def subscribe(self, url, update_types, version):
        ...
        data = {"url": url, "update_types": update_types, "version": version}
        try:
            response = requests.post(
                self.url + method, params=params, data=json.dumps(data)
            )
            if response.status_code == 200:
    ...
    def get_all_chats(self, count=50, marker=None):
        ...
        """
        method = "chats"
        params = {"access_token": self.token, "count": count, "marker": marker}
        try:
            response = requests.get(self.url + method, params=params)
            if response.status_code == 200:
                chats = response.json()
    ...
    def get_chat_admins(self, chat_id):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    "It's not a chat or the bot is not an administrator. Chat_id: {}".format(
                        chat_id
                    )
                )
                chat_admins = None
    ...
    def edit_chat_info(
        self, chat_id, icon=None, icon_url=None, title=None, pin=None, notify=True
    ):
        """
        ...
        data = {"icon": icon_res, "title": title, "pin": pin, "notify": notify}
        try:
            response = requests.patch(
                self.url + method, params=auth, data=json.dumps(data)
            )
            if response.status_code == 200:
    ...
    def get_messages(
        self, chat_id, message_ids="", time_from=None, time_to=None, count=50
    ):
        """
    ...
    def get_text(self, update):
        ...
        if update:
            ...
            if (
                type == "message_edited"
                or type == "message_callback"
                or type == "message_created"
                or type == "message_constructed"
            ):
                try:
        ...
        if update:
            ...
            if (
                ...
                try:
                    ...
                    try:
                        attachments = update["message"]["link"]["message"][
                            "attachments"
                        ]
                    except Exception as e:
    ...
    def get_chat_id(self, update=None):
        ...
        if update is None:
            ...
            if (
                ...
                type == "message_constructed" or type == "message_construction_request"
            ):
                chat_id = None
            elif type:
                try:
        ...
        if update:
            ...
            if type == "message_chat_created":
                ...
                try:
                    ...
                    logger.info(
                        "get_user_id (message_created...) sender is None: %s", e
                    )
            elif type == "message_chat_created":
                try:
                ...
                if not user_id:
                    try:
                        ...
                        logger.info(
                            "get_user_id (message_chat_created - pinned) is None: %s", e
                        )
    ...
    def get_message_id(self, update):
        ...
        if update:
            ...
            if "message" in upd.keys():
                ...
                "message_id" in upd.keys()
            ):  # type == 'message_chat_created' or type == 'message_removed':
                mid = upd["message_id"]
        return mid
    ...
    def edit_message(
        self, message_id, text, attachments=None, link=None, notify=True, format=None
    ):
        """
            ...
            try:
                ...
                if "code" in upd.keys():
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(
                            "Error edit message: {}".format(response.status_code)
                        )
            except Exception as e:
    ...
    def pin_message(self, chat_id, message_id, notify=True):
        ...
        params = (("access_token", self.token),)
        data = {"message_id": message_id, "notify": notify}
        try:
            response = requests.put(
                self.url + method, params=params, data=json.dumps(data)
            )
    ...
    def delete_message(self, message_id):
        ...
        """
        method = "messages"
        params = {"message_id": message_id, "access_token": self.token}
        try:
    ...
    def attach_buttons(self, buttons):
        ...
        if isinstance(buttons, list):
            ...
            attach = [{"type": "inline_keyboard", "payload": {"buttons": [[buttons]]}}]
        return attach
    ...
    def button_link(self, text, url):
        ...
        :return: возвращает подготовленную кнопку для последующего формирования массива
        """
        button = {"type": "link", "text": text, "url": url}
        return button
    ...
    def button_contact(self, text):
        ...
        :return: возвращает подготовленную кнопку для последующего формирования массива
        """
        button = {"type": "request_contact", "text": text}
        return button
    ...
    def button_location(self, text, quick=False):
        ...
        :return: возвращает подготовленную кнопку для последующего формирования массива
        """
        button = {"type": "request_geo_location", "text": text, "quick": quick}
        return button

    def button_chat(
        self, text, chat_title, chat_description=None, start_payload=None, uuid=None
    ):
        """
    ...
    def markup(self, type, from_posit, length):
        ...
        """
        markup = [{"type": type, "from": from_posit, "length": length}]
        return markup
    ...
    def send_reply_message(self, text, mid, chat_id, dislinkprev=None, format=None):
        ...
        update = self.send_message(
            text, chat_id, link=link, dislinkprev=dislinkprev, format=format
        )
        return update
    ...
    def token_upload_content(self, type, content, content_name=None):
        ...
        try:
            response = requests.post(
                url, files={"files": (content_name, content, "multipart/form-data")}
            )
            if response.status_code == 200:
                token = response.json()
        except Exception as e:
            ...
            try:
                ...
                if "code" in upd.keys():
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(
                            "Error sending message: {}".format(response.status_code)
                        )
            except Exception as e:
                logger.error("Error send_message: %s.", e)
        return update

    def send_answer_callback(
        self,
        callback_id,
        notification,
        text=None,
        ...
        if text is None and attachments is None and link is None:
            message = None
        data = {"message": message, "notification": notification}
        flag = "attachment.not.ready"
        while flag == "attachment.not.ready":
            try:
                response = requests.post(
                ...
                if "code" in upd.keys():
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(
                            "Error answer callback: {}".format(response.status_code)
                        )
            except Exception as e:
                logger.error("Error answer_callback: %s.", e)
        return update

    def send_construct_message(
        self,
        session_id,
        hint,
        text=None,
        ...
        while flag == "attachment.not.ready":
            try:
                response = requests.post(
                    self.url + method, params=params, data=json.dumps(datas)
                )
                upd = response.json()
                if "code" in upd.keys():
                    flag = upd.get("code")
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(
                            "Error construct_message: {}".format(response.status_code)
                        )
            except Exception as e:
