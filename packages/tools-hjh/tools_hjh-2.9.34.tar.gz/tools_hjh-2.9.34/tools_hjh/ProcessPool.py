...
class ProcessPool():
    ...
    def run_nowait(self, func, args, process_name=None): 
        p = Process(target=func, args=args)
        self.running_p.append(p)
        p.start()
        self.p_pname[p] = process_name
        return p.pid

    def run(self, func, args, process_name=None):
        """ 主线程命令当前线程池从空闲线程中取一个线程执行给入的方法，如果池满，则主线程等待 """     
        while self.get_running_num() >= self.size:
            time.sleep(self.while_wait_time)
        return self.run_nowait(func, args, process_name=process_name)
