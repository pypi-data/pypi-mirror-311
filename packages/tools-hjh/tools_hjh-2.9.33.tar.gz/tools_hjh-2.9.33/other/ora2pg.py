...
help_mess = '''version 1.4.1
使用方法
python3 ora2pg.py example.conf run_mode example.log
run_mode : ↓
copy_table, copy_data, copy_index, copy_pk, copy_fk, copy_uk, copy_sequence, copy_from_sql
compare_data_number, compare_index, compare_constraint
get_table_ddl, get_table_oracle_ddl
drop_fk

注意事项
copy_table，表名列名无论oracle中是大小写，转为pg中一律为小写
copy_table，会提前drop table，如果表存在外键关联第一次可能删不掉，多执行几次copy_table即可
...
copy_data，收集元数据时，会count表，且强制走全表，避免索引与表不同步的情况，因此大表可能比较慢
copy_index，if_clear决定是否会先drop已存在索引
copy_index，如果索引名已被占用（被其他对象占用），会自动重命名
...
4.如果tables中的源表是一个通过dblink同步过来的视图或同义词，会失败，但是却可以通过copy_from_sql抽取
5.对于tables中的对象是视图的情况，可以同步表结构，但是无法同步分区、注释、索引、约束等信息
'''
...
def copy_data():
    ...
    # 监听，tasks有数据就会启动get_data_from_oracle
    is_over = False
        ...
        while(True):
            ...
            for task in tasks.copy():
                ...
                if task[4] is not None:
                    mess = task[1] + '(' + str(task[4]) + ')'
                else:
                    mess = task[1]
                pp.run(get_data_from_oracle, task, process_name=mess)
            if is_over:
                break
    ...
    if if_clear == 'true':
        ...
        tp = ThreadPool(parallel_num, while_wait_time=0.1)
    ...
    tp = ThreadPool(parallel_num, while_wait_time=0.1)
    pp = ProcessPool(parallel_num, while_wait_time=0.1)
    
    # 启动一个复制数据的监听线程
    tp_get_data = ThreadPool(1, while_wait_time=0.1)
    tp_get_data.run(run_get_data, (pp,))
    
    # 多线程启动表分析程序
    for table_map in table_map_list:
        tp.run(get_table_metadata, (src_db, table_map, global_scn), thread_name=table_map[0] + '.' + table_map[1])
        
    # 监控元数据获取未完成的表
    while True:
        log.info('获取元数据，尚未完成的表：' + str(tp.get_running_name()))
    ...
    # while True:
        # running = list(set(pp.get_running_name()))
        # log.info('复制数据，尚未完成的任务：' + str(running))
...
def get_data_from_oracle(table_map, mess, table_scn, table_metadata=None, page=None, page_rn=None):
    ...
    try:
        ...
        i = 1
        tp = ThreadPool(save_parallel, while_wait_time=0.1)
        while True:
            if page is None:
                my_mess = mess + '(' + str(i) + ')'
            else:
...
def save_to_pg(dst_db, table_map, rows, cols, mess, select_time, table_scn, if_exists_blob):
    ...
    insert_sql = 'insert into ' + dst_table + '(' + cols_str + ') values(' + wenhaos + ')'
    ...
    if src_where == '1=1':
        src_where = ''
    log.info('写入PG成功', mess, src_where, num, '大小=' + str(round(rows_size, 2)) + 'MB', '读速=' + str(round(rows_size / select_time, 2)) + 'MB/s', '写速=' + str(round(rows_size / save_time, 2)) + 'MB/s', 'scn=' + table_scn)
...
def copy_from_sql():
    ...
    # 多进程启动导数
    tp = ProcessPool(parallel_num, while_wait_time=0.1)
    for table_map in table_map_list:
...
def copy_table(run_mode):
    ...
    if run_mode == 'get_table_ddl':
        tp = ThreadPool(1, while_wait_time=0.1)
