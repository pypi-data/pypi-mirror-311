...
import os, sys, warnings, fnmatch
...
class CodataBase(Database):
    ...
    def show_filetimes(self, ax: Optional[Axes] = None, duration=1*60.):
        """
        ...
        timestamp, nfiles = self.select2arrays(f'''
            select AVG(timestamp), count(*) as nfiles from  
                TRACES 
...
if __name__ == "__main__":
    ...
    if args.build is not None:
        ...
        with CodataBase(args.database_filename, create=True, verbose=args.verbose) as db:
            ...
            db.extract_header(acquisition_system="CODA", timezone="Europe/Paris")
        sys.exit(0)
