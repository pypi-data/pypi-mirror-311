...
class IdeGuiManager extends IdeZipManager {
  ...
  constructor(editorId){
    ...
    this.guiIdeFlags = {
      ...
      initTermH: 0,
      initGlobH: 0,
      aceMenuOpened: false,           // Has auto-completion, F1 or settings menu opened
      inFullScreen: false,            // Inner flag to handle mixed requests between full and split screen.
  ...
  storeInitHeightsIfNeeded(){
    if(!this.isFullScreen && !this.isSplitScreen){
...
class IdeFullScreenManager extends IdeGuiManager {
  ...
  build(){
    ...
    */
    ...
    if(IdeFullScreenManager.OBSERVER_MISSING){
      IdeFullScreenManager.OBSERVER_MISSING = false
      ...
      const unsetMenuFlagLater =()=> setTimeout(()=>{ this.guiIdeFlags.aceMenuOpened=false }, 150)
      ...
      new MutationObserver((records)=>{
        for(const record of records) {
          ...
          for (const node of record.removedNodes) {
            ...
            if(kind && kind!='auto-complete') unsetMenuFlagLater()
          ...
          for(let node of record.addedNodes) {
            ...
            this.guiIdeFlags.aceMenuOpened = true
            switch(kind){

              case 'menu':
                node.find('table')
                    .addClass('dummyclass')     // To deactivate a pmt background rule
                    .css('font-size','10px')
                break
              ...
              case 'auto-complete':
                new MutationObserver((records)=>{
                  for(const record of records){
                    ...
                    if(isVisible) this.guiIdeFlags.aceMenuOpened=true
                    else          unsetMenuFlagLater()
                  }
                }).observe(node[0], {attributeFilter:['style']})
                break
            }

            if(this.isFullScreen || kind=='auto-complete'){
              node.detach().appendTo(this.global)
  ...
  /** Identify if the given node is one related to a context menu (ACE auto-completion, ACE menu,
   * or commands).
   * */
  isSomeAceMenu(jNode){
    return (
  ...
  switchFullScreenFromEscape(event){
    if(event.key=='Escape' && !this.isFullScreen && !this.guiIdeFlags.aceMenuOpened){
  ...
  requestFullScreen(){
    this.global[0].requestFullscreen().then(async _=>{
      ...
      this.guiIdeFlags.inFullScreen = false
...
class IdeSplitScreenManager extends IdeFullScreenManager {
  ...
  async switchSplitScreenFromButton(e){
    ...
    if(wasFullScreen){
      ...
      // minLines.
      while(this.guiIdeFlags.inFullScreen) await sleep(50)
      await sleep(50)
