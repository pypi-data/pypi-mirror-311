...
from smack_app.utils.utils import APPNAME, REPOPATH
...
def test_wd():
    yield TEST_WD
    clear_folder_contents(TEST_WD)


@pytest.fixture()
def test_h5_dir():
    yield TEST_h5_DIR
    clear_folder_contents(TEST_h5_DIR)


@pytest.fixture()
def app_runner():
    with patch("smack.cli_validation.available_cpu_count") as mock_cpu_count:
...
def test_full_app_run_with_mock_data(app_runner, test_wd, test_h5_dir):
    ...
    for final_path in ["heteroplasmy_matrix", "variant_statistics"]:
