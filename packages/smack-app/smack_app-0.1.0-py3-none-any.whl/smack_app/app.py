...
from smack_app.utils.utils import APPNAME, REPOPATH, UMI_Mode
...
def setup(
    ...
    ] = f"{repopath}/smack_working_directory",
):
    ...
    you can run! See our wiki <> for more information!
...
def _run_consensus_calling(
    ...
    command_path = (
        f"{repopath}/{appname}/{appname}/genotyping/run_consensus_variant_calling.py"
    )
    command = [
        "python",
        command_path,
...
def genotype(
    ...
    (genome_txt_path, genome_name, mito_length) = create_refAllele_file(
        genome_fasta_path,
        os.path.join(f"{repopath}/{appname}/{appname}", f"reference/{mito_genome}.txt"),
    )
    if state["verbose"]:
...
def filter_variants(
    ...
    h5_directory: Annotated[
        str,
        typer.Argument(
            help="Path to h5 directory. Should be the same path output by `genotype` command. Can also be a list of directories if multi-sample=True",
    ...
    command_path = f"{repopath}/{appname}/{appname}/filter_barcodes.py"
    ...
    command_path = f"{repopath}/{appname}/{appname}/collect_variants.py"
    command = [
        "python",
        command_path,
        "-h5dir",
    ...
    command_path = f"{repopath}/{appname}/{appname}/filter_variants.py"
    command = [
        "python",
        command_path,
        "-variant-statistics",
