...
def gene_roles_heatmap(vals):
    ...
    plt.figure(dpi=900)
    # Create a heatmap
    sns.heatmap(degree_data, annot=False, annot_kws={'size': 100}, cmap=plt.cm.Greens, square=True)
    plt.xlabel('Gene roles')
    plt.ylabel('Interaction levels')
    plt.title('Intercations Heatmap')  
    plt.show()
    return
...
def scatter_plot(x, y, x_sd, y_sd):
    ...
    for i in range(len(x)):
        plt.errorbar(x[i], y[i], xerr=x_err[i], yerr=y_err[i], fmt='o', color=colors[i], markersize=5, capsize=2, capthick=2, label=f'Data Point {i+1}')
    plt.xlabel('log10(bf)')
...
def box_plot3(data_bf1, data_bf2, data_bs1, data_bs2, data_cv21, data_cv22, fig_name):
    ...
    for i, group_data in enumerate(data):
        for j, group in enumerate(group_data):
            ax.boxplot(group, positions=[positions[i][j]], patch_artist=True, boxprops=dict(facecolor=colors[j]), showfliers=False)
    ax.set_xticks([1.5, 4.5, 7.5])
    ax.set_xticklabels(['BF', 'BS', 'CV2'])
...
def scatter_2d(x, y, color, x_label, y_label, figname):
    fig, ax = plt.subplots(dpi=900)
