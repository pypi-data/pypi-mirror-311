...
def genepair_interactionsinference(data, geneinfo, filter_eps, figflag, verbose1, verbose2, verbose3):
   ...
   """
    ...
    if (verbose1 == True): heatmap(density, figflag, 1, verbose2, geneinfo[0, 0], geneinfo[0, 1])
...
def heatmap(vals, figflag, figindex, verbose2, genename1, genename2):
    ...
    if (verbose2 == True):
        plt.savefig(os.path.abspath('example/'+str(figflag)+'-'+str(figindex)+'.pdf'))  
        plt.close()
        plt.clf()
    else: plt.show()
    return
...
def Binary_PoBe_Interactions(params, vals, figflag, genename1, genename2, verbose1, verbose2, verbose3): 
    ...
    if (verbose1 == True) & ((verbose3 == 0) | (verbose3 == 2)):
        ...
        im = ax1.pcolormesh(rescale_py_givenx, cmap=plt.cm.viridis, alpha=0.9)
        fig1.colorbar(im1)
        plt.xlabel(genename1)
        plt.ylabel(genename2)
        if (verbose2 == True):
            plt.savefig(os.path.abspath('example/'+str(figflag)+'-6.pdf'))  
            plt.close()
            plt.clf()
        else: plt.show()
        fig2, ax2 = plt.subplots(dpi=900)
        ax2.set_title('Theory Inference Rescaled Conditional Probability Given $X_2$')
        im2 = ax2.pcolormesh(rescale_px_giveny, cmap=plt.cm.viridis, alpha=0.9)
        fig2.colorbar(im4)
        plt.xlabel(genename2)
        plt.ylabel(genename1)
        if (verbose2 == True):
            plt.savefig(os.path.abspath('example/'+str(figflag)+'-5.pdf'))  
            plt.close()
            plt.clf()
        else: plt.show()
    return
