...
class MongoDBRepository(GenericRepository[T], ABC):
    ...
    async def get_list(self, limit: int = 0, order_by: str | List[str] | None = None, **filters) -> List[T]:
        ...
        # Apply sorting
        if order_by:
            cursor = cursor.sort(self._build_sort(order_by))
    ...
    async def get_page_list(self, page_number: int = 1, page_size: int = 10, order_by: str | List[str] | None = None,
        ...
        return Page(elements=documents, page_number=page_number, page_size=page_size, total_records=total_records)
    ...
    async def add(self, record: BaseMongoModel) -> T:
        """Insert a new record."""
    ...
    async def update(self, record: BaseMongoModel) -> T:
        """Update an existing record."""
        query = {"id": record.id}
        update_data = {"$set": record.model_dump(by_alias=True, exclude={"id"})}
    ...
    def _build_query(self, *args: Any, **kwargs: Any) -> dict:
        ...
        if args:
            if len(args) > len(field_order):
                raise ValueError(
                    f"Too many positional arguments. Expected at most {len(field_order)}, got {len(args)}.")

            # 将 positional arguments 和 field_order 结合成查询条件
            query.update({field: value for field, value in zip(field_order, args)})
