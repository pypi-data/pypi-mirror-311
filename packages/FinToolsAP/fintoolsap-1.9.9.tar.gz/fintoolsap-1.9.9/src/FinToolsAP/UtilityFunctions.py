...
def calibrate_MertonModel(df: pandas.DataFrame,
                          horizon: int = 1) -> pandas.DataFrame:
    
    raise NotImplementedError
    
    # this implements the method described in Vassalou & Xing (2004)
    # https://doi.org/10.1111/j.1540-6261.2004.00650.x
    
    # assumes a pandas dataframe is given with daily observations
    # of a firms equity value, face value of debt, and risk free rate
    # with names 'me', 'fvd', and 'rf' respectively inclduing a 'date' 
    # column that can be converted to a datetime64[ns]
    
    # only works on one firm at a time
    
    # additonally the horizon is required
    
    # value of firm
    def _firm_value(V: float, E: float, 
                    D: float, sigmaV: float, 
                    r: float, T: float) -> float:
        
        d1 = (numpy.log(V / D) + (r + 0.5 * sigmaV ** 2) * T) / (sigmaV * numpy.sqrt(T))
        d2 = d1 - sigmaV * numpy.sqrt(T)
        
        # black scholes call
        _term1 = V * scipy.stats.norm(d1)
        _term2 = numpy.exp(-r * T) * D * scipy.stats.norm(d2)
        calculated_equity_value = _term1 - _term2
        
        # return the difference between calculated and actual value
        return calculated_equity_value - E
    
    # copy data
    df_internal = df.copy()
    
    # filter out columns not needed
    df_internal = df_internal[['date', 'me', 'fvd', 'rf']]
    
    # calculate the returns on the firm
    df['ret'] = numpy.log(df.me / df.me.shift(1))
        
    # initial guess for volatility of firm value
    avgE = df.me.mean()
    avgD = df.fvd.mean()
    sigmaE = df.ret.std()
    sigmaV_init = sigmaE * (avgE / (avgE + avgD))
