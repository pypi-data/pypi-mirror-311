...
def test_transport_no_transport(parameters_transport_single_horizon):
    ...
    assert_equal(s.operation_conversion_power_capacity.to_numpy(), np.array([20, 20]))
...
def test_transport_one_hour(parameters_transport_single_horizon):
    ...
    assert_equal(s.operation_conversion_power_capacity.to_numpy(), np.array([40, 0]))
...
def test_transport_no_transport_min_capacity(parameters_transport_single_horizon):
...
def test_transport_multi_hour(parameters_transport):
    ...
    p = p.update(
        dict(conversion_availability=p.conversion_availability.expand_dims(area=p.area).copy())
    )
    ...
    assert_equal(s.operation_conversion_power_capacity.to_numpy(), np.array([20, 20]))
    assert_equal(s.operation_conversion_power.to_numpy(), np.array([[12, 6, 2], [8, 14, 18]]))
    assert_equal_epsilon(s.operation_transport_power_capacity.to_numpy(), np.array([2, 8]), 12)
...
def test_transport_multi_horizon(parameters_transport_multi_horizon):
    ...
    assert model.objective_value == 5 * 10 * 2 + 10 * 10 * 2 + 0.01 * 10
...
def test_transport_early_decommissioning(parameters_transport_multi_horizon):
    ...
    np.testing.assert_array_equal(s.operation_transport_power.to_numpy(), np.array([10, 0]))
    ...
    assert model.objective_value == 5 * 10 * 2 + 10 * 10 + 0.01 * 10


def test_transport_annuity_perfect_foresight(parameters_transport_multi_horizon):
    p = parameters_transport_multi_horizon.copy(deep=True)
