...
def test_combined_single_hour(parameters_combined_single_horizon):
    ...
    assert model.objective_value == 10 * 2 + 100 * 2 + 1 * 2 + 1.5 * 5
...
def test_combined_multi_hour(parameters_combined_single_horizon):
    ...
    assert_equal(s.operation_conversion_power.to_numpy(), np.array([5, 7, 1, 0, 2, 4, 8, 9, 6, 3]))

    assert_equal(
        s.operation_spillage.transpose("resource", "hour"),
        np.array([[3, 5, 0, 0, 0, 2, 6, 7, 4, 1]] + [[0] * len(p.hour)] * (len(p.resource) - 1)),
    )

    assert_equal(
        s.operation_combined_power.to_numpy(),
        np.array([[2, 2, 1, 0, 2, 2, 2, 2, 2, 2], [0, 0, 1, 2, 0, 0, 0, 0, 0, 0]]),
    )

    assert model.objective_value == 10 * 2 + 100 * 2 + 1 * 2 * 10 + 1.5 * 5 * 3
...
def test_combined_multi_horizon(parameters_combined_multi_horizon):
    ...
    np.testing.assert_array_equal(s.planning_combined_costs.to_numpy(), np.array([200, 200]))
    ...
    np.testing.assert_array_equal(s.operation_combined_power_capacity.to_numpy(), np.array([2, 2]))
    ...
    assert model.objective_value == 10 * 2 * 2 + 100 * 2 * 2 + 1 * 2 + 1.5 * 5
...
def test_combined_early_decommissioning(parameters_combined_multi_horizon):
    ...
    np.testing.assert_array_equal(s.operation_combined_power_capacity.to_numpy(), np.array([2, 0]))
    ...
    assert model.objective_value == 10 * 2 * 1 + 100 * 2 * 2 + 1 * 2 + 1.5 * 5
...
def test_combined_annuity_perfect_foresight(parameters_combined_multi_horizon):
    ...
    np.testing.assert_array_equal(s.planning_combined_costs.to_numpy(), np.array([400, 0]))
