...
def test_conversion_single_hour(parameters_conversion_single_horizon):
    ...
    assert_equal(s.planning_conversion_power_capacity.to_numpy(), np.array([0, 20]))
    assert model.objective_value == 200
...
def test_conversion_multi_hour(parameters_conversion_single_horizon):
    ...
    assert_equal(s.planning_conversion_power_capacity.to_numpy(), np.array([10, 20]))
    ...
    assert_equal(
        s.operation_conversion_power.sel(conversion_tech="wind_onshore").to_numpy(),
    ...
    assert model.objective_value == 740
...
def test_conversion_multi_hour_energy(parameters_conversion_single_horizon):
    ...
    # objective value = (300 + 290 + 290) + (0 + 145 + 0) + 8 * sum([max(14.5 - 29 * 0.1 * k, 0) for k in range(10)])
    assert model.objective_value == 1373
...
def test_load_shedding(parameters_conversion_single_horizon):
    ...
    assert_equal(s.planning_conversion_power_capacity.to_numpy(), np.array([10]))
    assert_equal(s.operation_conversion_power.to_numpy(), np.array([10.0]))
    assert_equal(s.operation_load_shedding.sel(resource="methane").to_numpy(), np.array([15]))

    assert model.objective_value == 15000 + 200 + 100 + 80
...
def test_conversion_must_run(parameters_conversion_single_horizon):
    ...
    p.conversion_power_capacity_investment_min.loc[dict(conversion_tech="ocgt")] = 2
    p.conversion_power_capacity_investment_max.loc[dict(conversion_tech="ocgt")] = 2
    ...
    assert model.objective_value == 20 * 2 + 10 * 40 + 10 * 2 + 3 * 8
...
def test_conversion_multi_horizon(parameters_conversion_multi_horizon):
    ...
    np.testing.assert_array_equal(s.planning_conversion_costs.to_numpy(), np.array([1000, 1000]))
    ...
    assert model.objective_value == 4000
...
def test_conversion_early_decommissioning(parameters_conversion_multi_horizon):
    ...
    assert model.objective_value == 3000


def test_conversion_annuity_perfect_foresight(parameters_conversion_multi_horizon):
    p = parameters_conversion_multi_horizon.copy(deep=True)
    ...
    np.testing.assert_array_equal(s.planning_conversion_costs.to_numpy(), np.array([2000, 0]))
...
def test_conversion_power_capacity_investment_min_early_decommissioning(parameters_dispatch_invest):
    ...
    p.conversion_power_capacity_investment_max.loc[dict(conversion_tech="smr", year_inv=2010)] = 10
    ...
    assert_equal(s.operation_conversion_costs, np.array([93.0, 62.0, 124.0, -0.0]))
    assert_equal(s.planning_conversion_costs, np.array([150.0, 100.0, 300.0, 0]))

    assert model.objective_value == 829.0


def test_conversion_power_capacity_max_early_decommissioning(parameters_dispatch_invest):
    p = parameters_dispatch_invest
    p = p.sel(
        area=["area_1"],
        conversion_tech=["smr"],
    ...
    p.conversion_power_capacity_max.loc[dict(conversion_tech="smr", year_op=2040)] = 3
    ...
    assert model.objective_value == 898.0
