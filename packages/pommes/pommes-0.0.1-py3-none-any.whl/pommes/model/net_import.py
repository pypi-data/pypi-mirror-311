...
def add_net_import(model, model_parameters, annualised_totex_def, operation_adequacy_constraint):
    """
    ...
    m.add_constraints(
        operation_net_import_import.sum(["area", "hour"]) <= p.net_import_max_yearly_energy_import,
        mask=np.isfinite(p.net_import_max_yearly_energy_import),
        name="operation_net_import_import_yearly_max_constraint",
    )

    m.add_constraints(
        operation_net_import_export.sum(["area", "hour"]) <= p.net_import_max_yearly_energy_export,
    ...
    m.add_constraints(
        ...
        + (
            ...
            + p.net_import_import_price
            * operation_net_import_export  # TODO same export and import price ??
        ).sum("hour")
        == 0,
