...
def test_carbon(parameters_dispatch_invest, carbon):
    ...
    assert_equal(s.operation_conversion_power_capacity.to_numpy(), np.array([10, 22]))
    ...
    assert_equal_epsilon(
        model.dual.operation_carbon_goal_constraint.to_numpy(), np.array([-2.0]), 10
    )
    assert model.objective_value == 772
...
def test_carbon_import(parameters_dispatch_invest, carbon, net_import):
    ...
    assert_equal(s.operation_conversion_power_capacity.to_numpy(), np.array([10, 50]))
    assert_equal_epsilon(
        s.operation_conversion_power.sel(conversion_tech="ocgt").to_numpy(),
    ...
    assert_equal_epsilon(model.dual.operation_carbon_goal_constraint.to_numpy(), np.array([0]), 10)
    assert model.objective_value == 1394.0
