...
from googleads_housekeeper.adapters.orm import preview_task_and_results
...
def find_placements_for_exclusion(
  ...
  for account in task_obj.accounts:
    ...
    if not (
      placements := fetcher.get_placements_for_account(account, task_obj)
    ):
      continue
...
def add_async_preview_to_db(
  preview_task_and_results: preview_task.PreviewTaskAndResults,
  uow: unit_of_work.AbstractUnitOfWork,
):
  views.upsert_async_preview_to_db(
    task_and_results=preview_task_and_results, uow=uow
  )
