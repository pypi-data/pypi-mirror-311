...
from googleads_housekeeper.domain.core.preview_task import (
  PreviewTaskAndResults,
  PreviewTaskStatus,
)
...
from googleads_housekeeper.domain.placement_handler.placement_service import (
  PAGE_SIZE,
)
...
def run_task(
...
) -> tuple[dict, MessagePayload | None]:
  with copy(uow) as uow:
    ...
    uow.commit()
    if task_obj.output in (
      task.TaskOutput.NOTIFY,
...
def run_preview_placements(
  ...
  Args:
    ...
    save_to_db:
      ...
      Defaults to True.
  """
  new_task = PreviewTaskAndResults(preview_command=json.dumps(cmd))
  add_new_async_preview_task(task_and_results=new_task, uow=uow)
  report_fetcher = AdsReportFetcher(api_client=ads_api_client)
  task_obj = task.Task(
    name='',
  ...
  new_task.status = PreviewTaskStatus.DONE
  update_completed_async_preview_task(task_and_results=new_task, uow=uow)


def add_new_async_preview_task(
  task_and_results: PreviewTaskAndResults,
  uow: unit_of_work.AbstractUnitOfWork,
):
  """Adds a new asynchronous preview task to the unit of work.

    - Adds the given task to the preview task and results repository.
    - Commits the changes within the unit of work.

  Args:
      task_and_results:
        The task and its associated results to be added.

      uow:
        The unit of work managing database transactions.
  """
  with uow:
    uow.preview_task_and_results.add(task=task_and_results)
    uow.commit()


def update_completed_async_preview_task(
  task_and_results: PreviewTaskAndResults,
  uow: unit_of_work.AbstractUnitOfWork,
):
  """Updates an existing asynchronous preview task as completed in the unit of work.

     - Updates the specified task in the preview task and results repository.
     - Commits the changes within the unit of work.

  Args:
      task_and_results:
        The task and its associated results to be updated.

      uow:
        The unit of work managing database transactions.
  """
  with uow:
    existing_task_and_results = uow.preview_task_and_results.get_by_condition(
      'id', task_and_results.id
    )
    if existing_task_and_results:
      uow.preview_task_and_results.update(
        task_id=task_and_results.id,
        update_dict={
          'status': task_and_results.status,
          'results': task_and_results.results,
        },
      )
      uow.commit()
...
def get_preview_tasks_table(
  cmd: commands.GetPreviewTasksTable, uow: unit_of_work.AbstractUnitOfWork
) -> list[PreviewTaskAndResults]:
  return views.preview_tasks_table(uow)


def get_results_for_specific_preview_task(
  cmd: commands.GetResultsForSpecificPreviewTask,
  uow: unit_of_work.AbstractUnitOfWork,
  preview_pagination_size: int | None = PAGE_SIZE,
) -> GaarfReport:
  return views.results_for_specific_preview_task(
    preview_task_id=cmd.preview_task_id,
    uow=uow,
    preview_pagination_index_zero_based=cmd.preview_pagination_index_one_based
    - 1,
    preview_pagination_size=preview_pagination_size,
  )
...
COMMAND_HANDLERS = {
  ...
  commands.GetPreviewTasksTable: get_preview_tasks_table,
  commands.GetResultsForSpecificPreviewTask: get_results_for_specific_preview_task,
}
