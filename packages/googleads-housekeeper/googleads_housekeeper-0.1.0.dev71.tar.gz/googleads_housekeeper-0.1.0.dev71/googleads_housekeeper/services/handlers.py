...
def run_manual_exclusion_task(
  ...
  with uow:
    ...
    excluder = placement_excluder.PlacementExcluder(
      ads_api_client, cmd.exclusion_level
    )
...
def preview_placements(
  ...
  placement_service.add_async_preview_to_db(
    preview_task_and_results=new_task, uow=uow
  )
  report_fetcher = AdsReportFetcher(api_client=ads_api_client)
  task_obj = task.Task(
    name='',
...
def get_results_for_specific_preview_task(
  ...
  with uow:
    return views.get_placements_for_specific_preview_task(
