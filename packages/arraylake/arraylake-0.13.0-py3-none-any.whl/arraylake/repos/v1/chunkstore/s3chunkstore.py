...
class ClientKey:
    ...
    anon: bool
    fetch_credentials_func: Optional[Callable[..., S3Credentials]]
...
async def get_client(
    ...
    key = ClientKey(loop, anon, fetch_credentials_func, tuple(sorted(client_kws.items())))
    async with _LOCK:
...
class S3ObjectStore(ObjectStore):
    ...
    def __getstate__(self):
        return self._bucket_name, self._fetch_credentials_func, self.constructor_kwargs

    def __setstate__(self, state):
        self._bucket_name, self._fetch_credentials_func, self.constructor_kwargs = state
        self._set_props()
    ...
    async def _open(self):
        ...
        self._session_client = await get_client(loop, self.constructor_kwargs, self._fetch_credentials_func)
    ...
    async def is_anonymous_session(self) -> bool:
        ...
        assert hasattr(self._session_client, "_client_config"), "session client does not have _client_config"
