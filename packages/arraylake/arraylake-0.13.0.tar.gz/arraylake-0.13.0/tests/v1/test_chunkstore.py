...
async def test_chunkstore_repr(cstore):
    ...
    await cstore.object_store._open()
    repr(cstore.object_store)
...
async def test_s3chunkstore_client_lifecycle():
    ...
    key = s3chunkstore.ClientKey(loop, False, None, tuple({}))
    assert key in s3chunkstore._GLOBAL_CLIENTS

    s3chunkstore.close_client(key)
    assert key not in s3chunkstore._GLOBAL_CLIENTS
