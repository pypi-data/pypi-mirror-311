...
@pytest.mark.asyncio
async def test_user_auth_refresh_request(test_token_file) -> None:
    # Note: while this test does not explicitly await anything, the UserAuth class does include an asyncio.Lock
    ...
    request = auth.build_refresh_request()
    assert request.method == "GET"
    assert request.url == url + f"/refresh_token?token={refresh_token}"


@pytest.mark.asyncio
async def test_user_auth_refreshes_on_401(test_token_file, respx_mock) -> None:
    url = "https://foo.com"
    ...
    respx_mock.get(url + "/refresh_token", params={"token": orig_tokens["refresh_token"]}).mock(
        return_value=httpx.Response(httpx.codes.OK, json=new_tokens)
    )
...
@pytest.mark.asyncio
async def test_token_refresh_from_metastoredb(test_token_file, respx_mock) -> None:
    ...
    branches_route = respx_mock.get(api_url + "/repos/my-org/my-repo/branches").mock(
        side_effect=[httpx.Response(httpx.codes.UNAUTHORIZED), httpx.Response(httpx.codes.OK, json=branch_json)]
    )
    refresh_token_route = respx_mock.get(api_url + "/refresh_token").mock(
...
async def test_config_verify_ssl(respx_mock, verify_ssl):
    ...
    assert client._client._transport.wrapped_transport._pool._ssl_context.verify_mode == verify_mode
...
async def test_config_custom_ssl_cert(respx_mock, cafile):
    ...
    with config.set({"service.ssl.cafile": cafile}):
        ...
        if not cafile:
            _ = await client._request("GET", "")
            assert client._client._transport.wrapped_transport._pool._ssl_context.verify_mode == ssl.CERT_REQUIRED
        else:
