...
class HttpMetastore(ArraylakeHttpClient, Metastore):
    ...
    async def create_database(self, name: str, bucket_nickname: str | None = None, kind: RepoKind = RepoKind.V1):
        """
        Params:
        ...
        """
        body = RepoCreateBody(name=name, bucket_nickname=bucket_nickname, kind=kind)
    ...
    async def get_s3_bucket_credentials(self, name: str) -> S3Credentials:
    ...
    async def get_api_client_id_from_token(self, token: str) -> str:
        ...
        handle_response(response)
        auth_resp = TokenAuthenticateResponse.model_validate_json(response.content)
        return str(auth_resp.client_id)
    ...
    async def get_permission_check(self, principal_id: str, resource: str, action: OrgActions | RepoActions) -> bool:
        ...
        handle_response(response)
        decision = PermissionCheckResponse.model_validate(response.content)
