...
from rwkvfla.ops.utils import chunk_global_reversed_cumsum
from rwkvfla.utils import contiguous, device_capacity, check_pytorch_version, device, detect_tf32
from rwkvfla.utils import autocast_custom_fwd, autocast_custom_bwd
...
def chunk_rwkv6_bwd_kernel_intra(
    ...
    for j in range(0, BC):
        ...
        # [BC, BK]
        m_i = o_i[:, None] < j
        b_dq_dk_shared += tl.where(m_i, b_dA[:, None] * b_qj[None, :] * safe_exp_in_intra(b_gqj[None, :] - b_gk), 0.).to(TLTYPE) * scale
...
if __name__ == "__main__":
    ...
    from rwkvfla.ops.rwkv6 import fused_recurrent_rwkv6, native_recurrent_rwkv6
    from rwkvfla.ops.rwkv6.recurrent_naive import naive_recurrent_rwkv6
    scale = 1.0
    use_h = True
    u_2d = False
    B = 1
