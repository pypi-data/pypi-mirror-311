...
class ScrapyScraperSpider(scrapy.Spider):
	...
	def closed(self, reason: typing.Any):
		...
		self.__collected = array.unique(self.__collected)
	...
	def __extract_links(self, url: str, response: typing.Any):
		tmp = []
		try:
			for link in array.unique(scrapy.linkextractors.LinkExtractor(
				tags          = ["a", "link", "script"],
				attrs         = ["href", "src"],
				allow_domains = [] if self.__links else self.allowed_domains
