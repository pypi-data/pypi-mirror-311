...
def render_tabs(self, tabs, default, entity=None):
    ...
    self.w('<div id="entity-tabs-%s" class="entity-tabs">', uid)
    self.w('<ul class="nav nav-tabs">')
    for i, (tabid, domid, tabkwargs) in enumerate(tabs):
        if domid == active_tab:
            self.w('<li class="active">')
        else:
            self.w("<li>")
        self.w('<a href="#%s" data-toggle="tab">', domid)
        self.w(tabkwargs.pop("label", self._cw._(tabid)))
        self.w("</a>")
        self.w("</li>")
    self.w("</ul>")
    self.w('<div class="tab-content">')
    for tabid, domid, tabkwargs in tabs:
        active = "active" if domid == active_tab else ""
        self.w('<div id="%s" class="tab-pane %s">', domid, active)
        if self.lazy:
    ...
    if self.lazy:
        ...
        """
            % {"domid": active_tab, "uid": uid, "cookiename": self.cookie_name}
        )
