...
class EyesRunner(object):
    ...
    def __init__(
        ...
        self._commands = CommandExecutor.get_instance(self.Protocol)
        ...
        if is_legacy:
            self._ref = self._commands.core_make_manager(
                manager_type, legacy_concurrency=concurrency, agent_id=agent_id
            )
        else:
            self._ref = self._commands.core_make_manager(
                manager_type, concurrency=concurrency, agent_id=agent_id
            )
...
class ClassicEyesRunner(EyesRunner):
    def __init__(self):
        super(ClassicEyesRunner, self).__init__(ManagerType.CLASSIC)
