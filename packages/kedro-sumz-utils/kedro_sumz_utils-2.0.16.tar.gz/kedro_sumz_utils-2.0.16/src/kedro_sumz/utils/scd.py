...
def scd1_audit_cols(
    ...
    source_cols = [
        c.upper() for c in source.columns if (c.upper() not in key_cols) and (c.upper() not in filter_cols)
    ]
...
def scd2_delta_write(
    ...
    source_cols = [c.upper() for c in source.columns if c.upper() not in audit_cols] + audit_cols
    ...
    staging = updates.select(
        ...
        F.col("src.*"),
        get_current_timestamp().alias("FECHA_INICIO_VIGENCIA"),
    ).unionByName(
        source.alias("src").select(
            ...
            F.coalesce(
                F.col(f"src.{date_col}")
                if date_col and date_col.upper() in [s.upper() for s in source.columns]
                else F.lit(None),
