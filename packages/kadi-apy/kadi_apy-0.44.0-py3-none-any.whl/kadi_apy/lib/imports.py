...
def import_eln(manager, file_path):
    ...
    with zipfile.ZipFile(file_path) as ro_crate, TemporaryDirectory() as tmpdir:
        ...
        root_parts = _extract_property(root_entity, "hasPart", as_list=True)
        ...
        for root_part in root_parts:
            ...
            dataset_parts = _extract_property(dataset_entity, "hasPart", as_list=True)
            ...
            if not has_json_export:
                property_values = _extract_property(
                    dataset_entity, "variableMeasured", default=[], as_list=True
                )
...
def _extract_description(entity, entities):
    ...
    if isinstance(description, dict):
        # To stay compatible with the previous (non-flattened) description format for
        # now.
        if description.get("@type") == "TextObject":
...
def _extract_property(entity, name, default=None, as_list=False):
    property = entity.get(name, default)

    if property is None:
        _raise_invalid_content(f"Property '{name}' does not exist.")

    if not isinstance(property, (list, dict)):
        _raise_invalid_content(f"Property '{name}' is neither an array nor an object.")

    if isinstance(property, list):
        if not as_list:
            if len(property) == 0:
                _raise_invalid_content(f"Property '{name}' is empty.")

            return property[0]

        return property

    return [property] if as_list else property
