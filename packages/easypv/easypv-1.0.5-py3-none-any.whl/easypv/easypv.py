...
def load_app(package_name):
	...
	package_names=csdir+package_name+".py"
	return_pkg=csdir+'easypv.py'
	#current_directory =str(os.path.dirname(file_path)).replace("pandas",package_name)
	#ile_path = os.path.join(current_directory, package_names)
	root.destroy()
...
if __name__ == '__main__':
	ver="1.0.5"	
	root = Tk.Tk()
	root.title("药物警戒数据分析工作平台 EasyPV"+" "+ver)
