...
class Collection:
	...
	def add(self, contenu_uniprot):
		if any(p.id == Uniprot(contenu_uniprot).id for p in self.prot):
			...
			print(f"La protéine avec l'ID '{self.prot[-1].id}' a été ajoutée à la collection.")
	...
	def __add__(self, other):
		...
		total_collection = Collection(None) # total_collection = coll_1 + coll_2
	...
	def find_prot_by_id(self, uniprot_id:str)-> Uniprot:
		for p in self.prot:
			if p.id == uniprot_id:
				return p
		raise ValueError(f"Aucune protéine avec l'ID '{uniprot_id}' n'a été trouvée dans la collection.")
