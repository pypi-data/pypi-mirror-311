...
from .validators import validate_byom_dataset
import snowflake
import pandas as pd
...
def generate_model_name(name,source):
    ...
    if not source or source.lower() in {"byom", "notebook-byom", "notebook"}:
...
def validate_datasets_for_sf(model_obj,name,model_type,flavour, snowflake_df, sample_input_data, x_train, y_train, 
                             x_test, y_test, y_pred, prob, dataset_name, dataset_source,prediction_column, source):
    ...
    try:
        ...
        if source.lower() in {"byom","notebook-byom"}:
            if sample_input_data is None or not isinstance(sample_input_data, (pd.DataFrame)):
                raise ValueError("Please provide sample_input_data as pandas dataframe for BYOM models")
            validate_column_names(sample_input_data)
            validate_byom_dataset(sample_input_data)
            return True, "BYOM Model dataset is validated successfully."

        elif flavour == ModelFlavours.snowflake_model_flavour :
...
def is_path(variable):
    if isinstance(variable, str):
        return os.path.exists(variable)
    return False

def update_source(model_obj, source):
    source_lower = source.lower()
    if source_lower in {"byom", "experiment"}:
        return source_lower
    return "notebook-byom" if is_path(model_obj) else "notebook"