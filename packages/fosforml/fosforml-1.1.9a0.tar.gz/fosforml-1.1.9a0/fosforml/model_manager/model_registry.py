...
from .utilities import DatasetManager, Metadata, BYOMRegisrty, log_snowflake_model
...
class ModelRegistry:
    ...
    def register_model(self,
        ...
        if source.upper() is not "EXPERIMENT":
        ...
        try:
            ...
            if source.lower() in {"byom", "notebook-byom"}:
                if score:
                    is_valid,msg,score_func = BYOMValidations.load_validate_score_function(score, conda_dependencies)
                    if not is_valid:
                        return False, msg
                else:
                    score_func = None
                byom_obj = BYOMRegisrty(model,score_func,model_type,model_flavour,sample_input_data, conda_dependencies)
                model = byom_obj.get_byom_model()
                byom_model_sig = byom_obj.get_model_signature(model)
            else:
                byom_model_sig = None
        ...
        if source not in {"byom", "notebook-byom"}:
    ...
    def get_model_metrics(self,
        ...
        metrics['dataset_details'] = model_dataset_details.get('ui_display', [])

        
        metrics['model_metrics'] = buildtime_metrics

        metrics['hyper_parameters'] = model_dataset_details.get('hyper_parameters', [])
    ...
    def get_source_details(self,source):
        ...
        elif source is not None and source.upper() in {"NOTEBOOK","NOTEBOOK-BYOM"} :
            source_info['status'] = "Registered"
            source_info['source'] = "NOTEBOOK"
            source_info['repo_details'] = self.get_repo_details()    
        else:
