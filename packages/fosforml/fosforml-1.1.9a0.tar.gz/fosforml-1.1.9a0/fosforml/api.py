...
from fosforml.utils import (
    generate_model_name,
    is_path,
    validate_datasets_for_sf,
    validate_privileges,
    update_source
)
...
def register_model(
    ...
    **kwargs
    ...
    Args:
        model_obj (object/string): The model object to be registered or path to model file in case of byom
        session (snowflake.snowpark.session.Session): The Snowflake session object.
        scoring_func (string): The path to scoring function file.
        ...
        sample_input_data (pandas.DataFrame): The sample input data for the BYOM model.
    ...
    """
    print("Starting model registration via fosforml")
    ...
    source = update_source(model_obj, source)
    
    # validate model object and score
    if model_obj is None:
        return False,"Model object is required, please provide the model object to be registered."
    if is_path(model_obj):
        model = BYOMValidations.load_validate_model(model_obj, flavour, conda_dependencies)
    if scoring_func and not is_path(scoring_func):
        return False,"Invalid scoring function path, please provide the path to the scoring function file."
    if not is_path(model_obj) and flavour not in str(type(model_obj)):
