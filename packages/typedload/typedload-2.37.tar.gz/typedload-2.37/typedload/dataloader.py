...
try:
    # A dirty trick
    #
    # The alias module has a syntax that won't work on
...
def _dataclassload(l: Loader, value: Dict[str, Any], type_) -> Any:
    ...
    try:
        ...
        if l.pep563:
            ...
            if unalias:
                type_hints = {k: unalias(v.type) for k,v in type_.__dataclass_fields__.items()}
            else:
...
def _namedtupleload(l: Loader, value: Any, type_) -> Any:
    ...
    try:
        ...
        if l.pep563:
            ...
            if unalias:
                type_hints = {k: unalias(v) for k,v in type_.__annotations__.items()}
            else:
...
def _typeddictload(l: Loader, value: Any, type_) -> Any:
    ...
    try:
        ...
        # Resolve the NotRequired/Required stuff
        for k, v in type_hints.items():
            while True:
                if is_notrequired(v):
                    v = type_hints[k] = notrequiredtype(v)
                    necessary_fields.discard(k)
                elif is_required(v):
                    v = type_hints[k] = requiredtype(v)
                    necessary_fields.add(k)
                elif is_readonly(v):
                    v = type_hints[k] = readonlytype(v)
                else:
                    break
        if unalias:
...
def _attrload(l: Loader, value: Any, type_) -> Any:
    ...
    try:
        ...
        necessary_fields = set()
        if unalias:
            type_hints = {i.name: (_get_attr_converter_type(i.converter) if i.converter else unalias(i.type)) for i in type_.__attrs_attrs__}
        else:
