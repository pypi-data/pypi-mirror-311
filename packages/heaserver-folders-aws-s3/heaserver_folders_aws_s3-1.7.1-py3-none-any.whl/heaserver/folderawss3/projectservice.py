...
from heaobject.root import DesktopObject, Permission
...
from .util import client_line_ending, response_folder_as_zip, set_file_source, get_result_or_see_other, MONGODB_AWS_S3_FOLDER_METADATA_COLLECTION, move, rename, copy
    ...
    async with DesktopObjectActionLifecycle(request=request,
        ...
        async with mongo.MongoContext(request) as mongo_client:
            ...
            return await move(activity, request, mongo_client, sub, volume_id, bucket_id, id_, key,
                              new_volume_id, target_bucket_id, target_key_parent, AWSS3Project)
...
async def delete_project(request: web.Request) -> web.Response:
    ...
    if async_requested:
        async def done(resp: web.Response):
            logger.debug('Delete finished with %s', resp)
            if resp.status == 204:
                async with aws.S3ClientContext(request=request, volume_id=volume_id) as s3_client:
                    ...
                    async with mongo.MongoContext(request) as mongo_client:
                        if versioning.get('Status') != 'Enabled':
                            ...
                            version = await awsservicelib.get_latest_object_version(s3_client, bucket_id, key)
                            metadata = await mongo_client.get_admin_nondesktop_object(MONGODB_AWS_S3_FOLDER_METADATA_COLLECTION,
                                                                                      {'bucket_id': bucket_id, 'encoded_key': id_, '$or': [{'deleted': False}, {'deleted': {'$exists': False}}]})
                            if metadata:
                                metadata['deleted'] = True
                                metadata['version'] = version
                                await mongo_client.update_admin_nondesktop_object(metadata, MONGODB_AWS_S3_FOLDER_METADATA_COLLECTION)
        status_location = f'{request.url}/deleterasyncstatus'
        response_ = await awsservicelib.delete_object_async(request, status_location, recursive=True, activity_cb=publish_desktop_object_and_clear_cache, done_cb=done)
    else:
        response_ = await awsservicelib.delete_object(request, recursive=True, activity_cb=publish_desktop_object_and_clear_cache)
