...
def _validate_version(version) -> bool:
    version_regex = re.compile(r"(^v\d+\.\d+\.\d+(-\d+)?(-?(rc(\.?\d+)?|alpha(\.\d+)?)?)?$)")
    if len(version) > 32:
        raise InvalidConfiguration("A semantic version should not be longer than 32 characters.")
    if not version_regex.findall(version):
        raise InvalidConfiguration(
            f"The version provided {version}, does not match the pattern '{version_regex.pattern}'. You can try it out on "
            + "https://regex101.com/r/b9fCxa/1, which existed at the time of writing. A common issue is you forgot to add a 'v' at "
            + "the start of the version. To see examples of what works and what doesn't, check out the tests at "
            + "https://github.com/astronomer/astronomer_e2e_test/blob/master/tests/unit/test_github.py"
        )
    return True
...
def _get_client():
    ...
    if not token:
        raise InvalidConfiguration("Please provide the GITHUB_TOKEN environment variable")
    return Github(token)
...
def publish_release(version, repo, org, commitish, pre_release=False, github_client=None):
    ...
    for tag in generate_release_names(repo, org, github_client=github_client):
        if tag == version:
            raise UnsafeBehavior(f"We found an existing release with version {version}. Aborting!")
