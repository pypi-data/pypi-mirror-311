...
from achatbot.types.ai_conf import TOGETHER_LLM_MODEL, TOGETHER_LLM_URL, ASRConfig, LLMConfig, TTSConfig, AIConfig
...
class AIBot(IBot):
    ...
    def get_openai_llm_processor(self, llm: LLMConfig | None = None) -> LLMProcessor:
        from achatbot.processors.llm.openai_llm_processor import OpenAILLMProcessor, OpenAIGroqLLMProcessor
        if not llm:
            llm = self._bot_config.llm
    ...
    def get_moshi_voice_processor(self, llm: LLMConfig | None = None) -> VoiceProcessorBase:
        ...
        if llm and llm.tag and "moshi" in llm.tag:
            from achatbot.processors.voice.moshi_voice_processor import MoshiVoiceOpusStreamProcessor, MoshiVoiceProcessor
