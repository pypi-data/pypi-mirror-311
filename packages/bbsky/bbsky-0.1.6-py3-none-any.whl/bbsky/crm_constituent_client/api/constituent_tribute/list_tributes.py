from http import HTTPStatus
from typing import Any, Dict, Optional, Union, cast

import httpx

from ... import errors
from ...client import AuthenticatedClient, Client
from ...models.list_tributes_site_filter_mode import ListTributesSiteFilterMode
from ...models.problem_details import ProblemDetails
from ...models.tribute_list_collection import TributeListCollection
from ...types import UNSET, Response, Unset


def _get_kwargs(
    constituent_id: str,
    *,
    site_filter_mode: Union[Unset, ListTributesSiteFilterMode] = UNSET,
    sites_selected: Union[Unset, str] = UNSET,
    limit: Union[Unset, int] = UNSET,
    session_key: Union[Unset, str] = UNSET,
    infinity_session: Union[Unset, str] = UNSET,
    more_rows_range_key: Union[Unset, str] = UNSET,
    start_row_index: Union[Unset, int] = UNSET,
) -> Dict[str, Any]:
    params: Dict[str, Any] = {}

    json_site_filter_mode: Union[Unset, str] = UNSET
    if not isinstance(site_filter_mode, Unset):
        json_site_filter_mode = site_filter_mode.value

    params["site_filter_mode"] = json_site_filter_mode

    params["sites_selected"] = sites_selected

    params["limit"] = limit

    params["session_key"] = session_key

    params["infinity_session"] = infinity_session

    params["more_rows_range_key"] = more_rows_range_key

    params["start_row_index"] = start_row_index

    params = {k: v for k, v in params.items() if v is not UNSET and v is not None}

    _kwargs: Dict[str, Any] = {
        "method": "get",
        "url": f"/constituents/{constituent_id}/tributes",
        "params": params,
    }

    return _kwargs


def _parse_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Optional[Union[Any, ProblemDetails, TributeListCollection]]:
    if response.status_code == 200:
        response_200 = TributeListCollection.from_dict(response.json())

        return response_200
    if response.status_code == 400:
        response_400 = ProblemDetails.from_dict(response.json())

        return response_400
    if response.status_code == 403:
        response_403 = cast(Any, None)
        return response_403
    if response.status_code == 404:
        response_404 = cast(Any, None)
        return response_404
    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[Any, ProblemDetails, TributeListCollection]]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    constituent_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
    site_filter_mode: Union[Unset, ListTributesSiteFilterMode] = UNSET,
    sites_selected: Union[Unset, str] = UNSET,
    limit: Union[Unset, int] = UNSET,
    session_key: Union[Unset, str] = UNSET,
    infinity_session: Union[Unset, str] = UNSET,
    more_rows_range_key: Union[Unset, str] = UNSET,
    start_row_index: Union[Unset, int] = UNSET,
) -> Response[Union[Any, ProblemDetails, TributeListCollection]]:
    """List tributes.

     Returns a list of tributes where the current constituent is the tributee or the acknowledgee.

    Args:
        constituent_id (str):
        site_filter_mode (Union[Unset, ListTributesSiteFilterMode]):
        sites_selected (Union[Unset, str]):
        limit (Union[Unset, int]):
        session_key (Union[Unset, str]):
        infinity_session (Union[Unset, str]):
        more_rows_range_key (Union[Unset, str]):
        start_row_index (Union[Unset, int]):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[Any, ProblemDetails, TributeListCollection]]
    """

    kwargs = _get_kwargs(
        constituent_id=constituent_id,
        site_filter_mode=site_filter_mode,
        sites_selected=sites_selected,
        limit=limit,
        session_key=session_key,
        infinity_session=infinity_session,
        more_rows_range_key=more_rows_range_key,
        start_row_index=start_row_index,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    constituent_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
    site_filter_mode: Union[Unset, ListTributesSiteFilterMode] = UNSET,
    sites_selected: Union[Unset, str] = UNSET,
    limit: Union[Unset, int] = UNSET,
    session_key: Union[Unset, str] = UNSET,
    infinity_session: Union[Unset, str] = UNSET,
    more_rows_range_key: Union[Unset, str] = UNSET,
    start_row_index: Union[Unset, int] = UNSET,
) -> Optional[Union[Any, ProblemDetails, TributeListCollection]]:
    """List tributes.

     Returns a list of tributes where the current constituent is the tributee or the acknowledgee.

    Args:
        constituent_id (str):
        site_filter_mode (Union[Unset, ListTributesSiteFilterMode]):
        sites_selected (Union[Unset, str]):
        limit (Union[Unset, int]):
        session_key (Union[Unset, str]):
        infinity_session (Union[Unset, str]):
        more_rows_range_key (Union[Unset, str]):
        start_row_index (Union[Unset, int]):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[Any, ProblemDetails, TributeListCollection]
    """

    return sync_detailed(
        constituent_id=constituent_id,
        client=client,
        site_filter_mode=site_filter_mode,
        sites_selected=sites_selected,
        limit=limit,
        session_key=session_key,
        infinity_session=infinity_session,
        more_rows_range_key=more_rows_range_key,
        start_row_index=start_row_index,
    ).parsed


async def asyncio_detailed(
    constituent_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
    site_filter_mode: Union[Unset, ListTributesSiteFilterMode] = UNSET,
    sites_selected: Union[Unset, str] = UNSET,
    limit: Union[Unset, int] = UNSET,
    session_key: Union[Unset, str] = UNSET,
    infinity_session: Union[Unset, str] = UNSET,
    more_rows_range_key: Union[Unset, str] = UNSET,
    start_row_index: Union[Unset, int] = UNSET,
) -> Response[Union[Any, ProblemDetails, TributeListCollection]]:
    """List tributes.

     Returns a list of tributes where the current constituent is the tributee or the acknowledgee.

    Args:
        constituent_id (str):
        site_filter_mode (Union[Unset, ListTributesSiteFilterMode]):
        sites_selected (Union[Unset, str]):
        limit (Union[Unset, int]):
        session_key (Union[Unset, str]):
        infinity_session (Union[Unset, str]):
        more_rows_range_key (Union[Unset, str]):
        start_row_index (Union[Unset, int]):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[Any, ProblemDetails, TributeListCollection]]
    """

    kwargs = _get_kwargs(
        constituent_id=constituent_id,
        site_filter_mode=site_filter_mode,
        sites_selected=sites_selected,
        limit=limit,
        session_key=session_key,
        infinity_session=infinity_session,
        more_rows_range_key=more_rows_range_key,
        start_row_index=start_row_index,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    constituent_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
    site_filter_mode: Union[Unset, ListTributesSiteFilterMode] = UNSET,
    sites_selected: Union[Unset, str] = UNSET,
    limit: Union[Unset, int] = UNSET,
    session_key: Union[Unset, str] = UNSET,
    infinity_session: Union[Unset, str] = UNSET,
    more_rows_range_key: Union[Unset, str] = UNSET,
    start_row_index: Union[Unset, int] = UNSET,
) -> Optional[Union[Any, ProblemDetails, TributeListCollection]]:
    """List tributes.

     Returns a list of tributes where the current constituent is the tributee or the acknowledgee.

    Args:
        constituent_id (str):
        site_filter_mode (Union[Unset, ListTributesSiteFilterMode]):
        sites_selected (Union[Unset, str]):
        limit (Union[Unset, int]):
        session_key (Union[Unset, str]):
        infinity_session (Union[Unset, str]):
        more_rows_range_key (Union[Unset, str]):
        start_row_index (Union[Unset, int]):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[Any, ProblemDetails, TributeListCollection]
    """

    return (
        await asyncio_detailed(
            constituent_id=constituent_id,
            client=client,
            site_filter_mode=site_filter_mode,
            sites_selected=sites_selected,
            limit=limit,
            session_key=session_key,
            infinity_session=infinity_session,
            more_rows_range_key=more_rows_range_key,
            start_row_index=start_row_index,
        )
    ).parsed
