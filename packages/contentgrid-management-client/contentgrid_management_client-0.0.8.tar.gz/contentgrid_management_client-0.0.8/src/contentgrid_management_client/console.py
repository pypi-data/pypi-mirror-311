...
class Project(InteractiveHALResponse):
    ...
    def get_bookmark(self) -> str:
        return self.get_link("bookmark").uri
...
class Blueprint(InteractiveHALResponse):
    ...
    def get_webhooks(self) ->List[HALResponse]:
        return self.client.follow_link(self.get_link("webhooks")).get_embedded_objects_by_key(key="webhooks")

    # Entity actions
    def get_entity_definitions(self) -> List["EntityDefinition"]:
        return self.client.follow_link(self.get_link("entities"), infer_type=InteractiveHALResponse).get_embedded_objects_by_key("entities", infer_type=EntityDefinition)
    ...
    def create_entity(self, entity_name : str, entity_description : str=None ) -> "EntityDefinition":
        payload = {
            "name" : entity_name
        }
        ...
        response = self.client.post(self.get_link("entities").uri, json=payload, headers={"Content-Type" : "application/json"})
    ...
    def get_automations_link(self) -> HALLink:
        return HALLink(self.get_self_link().uri + "/automations")
...
class Automation(InteractiveHALResponse):
    ...
    def get_annotations_link(self) -> HALLink:
        return self.get_link("annotations")
    ...
    def _create_annotation(self, json_payload : dict) -> "AutomationAnnotation":
        ...
        data = self.client._validate_json_response(response=response)
...
class EntityDefinition(InteractiveHALResponse):
    ...
    def create_attribute(self, attribute_name: str, type:AttributeType, *, attribute_description: str=None, unique: bool=False, required: bool=False, indexed:bool=False, natural_id:bool=False):
        payload = {
            "indexed" : indexed,
            "name" : attribute_name,
        ...
        response = self.client.get_method_from_string(add_attribute_template["method"])(add_attribute_template["target"], json=payload, headers={"Content-Type" : "application/json"})
        data = self.client._validate_json_response(response)
        self.refetch()
        return AttributeDefinition(data=data, client=self.client, curie_registry=self.curie_registry)

    def patch(self, new_entity_name: str=None, new_entity_description: str=None):
        payload = {}
        if new_entity_name != None:
            payload["name"] = new_entity_name
        
        if new_entity_description != None:
    ...
    def create_relation(self, relation_name: str, target:str, many_source_per_target:bool, many_target_per_source:bool, relation_description: str= None, *, required: bool=False):
        payload = {
            "name" : relation_name,
            "required" : required,
            "target" : target,
        ...
        response = self.client.get_method_from_string(add_relation_template["method"])(add_relation_template["target"], json=payload, headers={"Content-Type" : "application/json"})
        data = self.client._validate_json_response(response)
        self.refetch()
...
class AttributeDefinition(InteractiveHALResponse):
    def __init__(self, data: dict, client: HALFormsClient, curie_registry: CurieRegistry = None) -> None:
        ...
        self.indexed : bool = data["indexed"]
    ...
    def patch(self, attribute_name :str=None, indexed :bool=None, attribute_description :str=None, attribute_type :AttributeType=None, required :bool=None, unique :bool=None):
        payload = {}
        if attribute_name != None:
            payload["name"] = attribute_name
        if indexed != None:
            payload["indexed"] = indexed
        if attribute_description != None:
            payload["description"] = attribute_description
    ...
    def add_allowed_values_constraint(self, allowed_values: List[str]) -> "AttributeConstraint":
        if "set-allowed-values" in self.templates.keys():
            ...
            values_property_name = next(filter(lambda prop : "required" in prop.keys() and prop["required"] == True, set_allowed_values_template["properties"]))["name"]
            response = self.client.get_method_from_string(set_allowed_values_template["method"])(set_allowed_values_template["target"], json={values_property_name : allowed_values}, headers={"Content-Type" : "application/json"})
            data = self.client._validate_json_response(response)
            self.refetch()
    ...
    def get_constraints_of_type(self, constraint_type: ConstraintType) -> "AttributeConstraint":
    ...
    def enable_exact_search(self) -> None:
        ...
        if template_name in self.templates.keys():
            ...
            response = self.client.get_method_from_string(template["method"])(template["target"])
    ...
    def get_search_options_of_type(self, search_type: SearchType) -> "AttributeSearchOption":
    ...
    def __str__(self) -> str:
        ...
        return f"[Attribute {self.name}] type={self.type.name} | required={self.required} | unique={self.unique} | indexed={self.indexed} | exact_search={exact} | prefix_search={prefix} | natural_id={self.natural_id}"
...
class AttributeConstraint(InteractiveHALResponse):
    ...
    def __str__(self) -> str:
        return f"[AttributeConstraint {self.type.name}]"
...
class RelationDefinition(InteractiveHALResponse):
    ...
    def patch(self, new_relation_name: str=None, new_relation_description: str=None):
        payload = {}
        if new_relation_name != None:
            payload["name"] = new_relation_name
        
        if new_relation_description != None:
    ...
    def __str__(self) -> str:
        return f"[Relation {self.name}] required={self.required} | source={self.source} | target={self.target} | many_source_per_target={self.many_source_per_target} | many_target_per_source={self.many_target_per_source}"
