...
class _MethodsGenerator(ABCMeta):
    ...
    def _create_save_method_from(cls, method: dict) -> dict:
        ...
        result['extra_params'] = (
            method['extra_params'] + ['filepath']
            if method.get('extra_params')
            else ['filepath']
        )
        result['docstring'] = (
            f"{method.get('docstring')}\nSaves the data to a file."
    ...
    def _generate_signature(cls, method_details: MethodDetails) -> str:
        ...
        if method_details.extra_params:
            func_signature += (
                f'{arg_separator.join(method_details.extra_params)}{arg_separator}'
            )
    ...
    def _get_modified_method(cls, func, method_details: MethodDetails):
        ...
        def modified_method(*args, **kwargs):
            ...
            if 'method' in kwargs:
                kwargs.pop('method')
                logger.warning(
                    'Method parameter is not authorized and defined in the methods binding dictionary.'
                )
            if 'endpoint' in kwargs:
                kwargs.pop('endpoint')
                logger.warning(
                    'Endpoint parameter is not authorized and defined in the methods binding dictionary.'
                )
            if method_details.resource_name in kwargs:
                frozen_kwargs['resource_id'] = kwargs.pop(method_details.resource_name)
    ...
    def __new__(cls, classname, supers, cls_dict):
        ...
        if client._methods_binding:
            for method_details in client.get_and_extend_methods():
                ...
                func_name = method_details.name.strip('_')
                cls_dict[func_name] = client._generate_method(
