...
def run():
    ...
    # 初始化组件模块
    init_module_parser = init_subparsers.add_parser("m", help="初始化组件模块")
    init_module_parser.add_argument(
        "-m", "--moduleDirPath", type=str, nargs="?", help="组件工程路径", metavar=""
    )
    init_module_parser.add_argument(
        "-g", "--group", type=str, help="maven group", metavar=""
    )
    init_module_parser.add_argument(
        "-a", "--artifactId", type=str, help="maven artifactId", metavar=""
    )
