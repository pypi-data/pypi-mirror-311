...
def distance_method_routing_analysis(series_a, series_b, Codebook_mes, report = False, export_dir = '.'):
    ...
    series_a = series_a + max(series_b)
    fig_2 = go.Figure()
    fig_2.add_trace(
        go.Scatter(
    ...
    fig.add_trace(
            go.Scatter(
                x= time_range, y=series_a, line_width = 3, opacity = 0.7, name = 'series A', line_color = THEME_COLOR[1]
            )
        )
    fig.add_trace(
            go.Scatter(
                x= time_range, y=series_b, line_width = 3, opacity = 0.7, name = 'series B', line_color = THEME_COLOR[2]
            )
        )
    ...
    fig.layout.yaxis.title = "Net Demand (kWh)" 
