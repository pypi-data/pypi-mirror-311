...
def distance_method_routing_analysis(series_a, series_b, Codebook_mes, report = False, export_dir = '.'):
    ...
    series_a_shift = series_a + max(series_b)*1.5

    fig = go.Figure()
    fig.add_trace(
            go.Scatter(
                x= time_range, y=series_a_shift, line_width = 4, opacity = 0.7, name = 'Series A', line_color = THEME_COLOR[1], hoverinfo='none'
            )
        )
    fig.add_trace(
            go.Scatter(
                x= time_range, y=series_b, line_width = 4, opacity = 0.7, name = 'Series B', line_color = THEME_COLOR[2], hoverinfo='none'
            )
        )
    ...
    if report:
        ...
        """
        with open(f"{export_dir}//fd_routing_report.html", "w") as file:
            file.write(html_template)