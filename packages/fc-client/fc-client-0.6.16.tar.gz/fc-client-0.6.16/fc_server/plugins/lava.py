...
from prettytable import PLAIN_COLUMNS  # pylint: disable=no-name-in-module
...
class Plugin(FCPlugin, Lava):
    ...
    async def __get_job_tags(self, job_id, semaphore):
        """
    ...
    async def booking_info(self, driver):
        """
        Current booking info of this framework
        """

        devices = [
            ["lava", device["current_job"], device["hostname"]]
            for device in await self.lava_get_devices()
            if device["hostname"] in driver.managed_resources
            and device["current_job"] is not None
        ]
        ...
        table.field_names = ["User", "Job", "Place"]
        table.max_width = 80

        return table.get_string(sortby="Place")

    async def get_comments(self, driver):
        """
        Produce comments for lava
        """

        comments = {
            device["hostname"]: f"lava#{device['current_job']}"
            for device in await self.lava_get_devices()
            if device["hostname"] in driver.managed_resources
            and device["current_job"] is not None
        }
    ...
    async def schedule(
        ...
        job_tags_semaphore = asyncio.Semaphore(30)
        job_tags_list = await asyncio.gather(
            *[
                self.__get_job_tags(queued_job["id"], job_tags_semaphore)
