...
def variable_type(self):
    ...
    if isinstance_m(self.parent, ['VarTuple']):
        ...
        datatypes = self.parent.parameter.type_.datatype
        return next(dt for var, dt in zip(vars_, datatypes) if var is self)
    ...
    if textx_isinstance(self.function, metamodel['ObjectImport']):
        if textx_isinstance(self.parent, metamodel['Operand']):
            ...
            if not ((textx_isinstance(other, metamodel['FunctionCall']) and
                ...
                ret_type = None
        elif textx_isinstance(self.parent, metamodel['VarTuple']):
            typespec = {'datatype': tuple([None]*len(self.parent.variables))}
            ret_type = DType('Tuple', (tuple,), typespec)
        else:
            ret_type = None
    else:
...
def var_tuple_type(self):
    """evaluate / check the types in a variables tuple object"""
    if self.parameter.type_ is None:
        return DType('Tuple', (tuple,), {})
    if (not issubclass(self.parameter.type_, tuple) or
            len(self.variables) != len(self.parameter.type_.datatype)):
        message = f'Expected {len(self.variables)} variable parameters'
        if issubclass(self.parameter.type_, tuple):
            message += f' but {len(self.parameter.type_.datatype)} were given'
        raise StaticValueError(message)
    return self.parameter.type_
...
def table_type(self):
    ...
    names = [get_reference(c).name for c in self.columns if isinstance_r(c, ['Series'])]
    if len(set(names)) != len(names):
        msg = 'Repeating column names were found in table'
        raise StaticValueError(msg)
    for column in self.columns:
        if not issubclass(column.type_, typemap['Series']):
...
def array_type(self):
    ...
    typespec = {'datatype': dtype, 'arraytype': 'True',
                'datalen': get_array_datalen(self)}
...
def range_type(self):
    ...
    for par in (self.start, self.stop, self.step):
        type_ = par.type_
        if type_ is not None and not issubclass(type_, typemap['Quantity']):
            msg = f'Range parameter {par} must be numeric real type'
            raise_exception(par, StaticTypeError, msg)
        if type_ is not None and type_.datatype is not None:
...
def sum_type(self):
    ...
    if self.parameter is not None:
        ...
        assert self.params
        if not all(issubclass(p.type_, typemap['Quantity']) for p in self.params):
            msg = 'Sum accepts only inputs of numerical types'
            raise StaticTypeError(msg)
...
def get_par_datatype(obj, datatype):
    ...
    if datatype is not None and issubclass(datatype, scalar_type):
...
def boolean_reduce_type(self):
    ...
    if self.parameter is not None:
        check_series_type(self.parameter)
        if self.parameter.type_.datatype is not None:
            if not issubclass(self.parameter.type_.datatype, bool):
                raise StaticTypeError(msg)
    else:
        if not all(issubclass(p.type_, bool) for p in self.params):
            raise StaticTypeError(msg)
    return bool
...
def amml_structure_type(self):
    ...
    if not all(c.name in tab_typs for c in self.tab.columns):
        col = next(c for c in self.tab.columns if c.name not in tab_typs)
            ...
            for col in atoms.columns:
                ...
                if isinstance_m(col, ['Series']):
                    if col.name not in atoms_dtyps:
                        ...
                        raise_exception(col, StaticValueError, msg)
                    if not issubclass(col.type_.datatype, atoms_dtyps[col.name]):
...
def amml_calculator_type(self):
    ...
    assert self.parameters is None or issubclass(self.parameters.type_, typemap['Table'])
...
def amml_property_type(self):
    ...
    if not issubclass(self.struct.type_, typemap['AMMLStructure']):
        msg = f'Parameter \"{self.struct.ref.name}\" must be an AMML structure'
        raise_exception(self.struct, StaticTypeError, msg)
    if self.calc and not issubclass(self.calc.type_, typemap['AMMLCalculator']):
    ...
    if self.algo is not None:
        if not issubclass(self.algo.type_, typemap['AMMLAlgorithm']):
...
def amml_constraint_type(self):
    ...
    if self.fixed is not None:
        if not (issubclass(self.fixed.type_, typemap['Series'])
           and issubclass(self.fixed.type_.datatype, bool)):
...
def chem_species_type(self):
    ...
    if self.composition and not issubclass(self.composition.type_, str):
