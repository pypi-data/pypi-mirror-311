...
class Session:
    ...
    def __init__(self, lpad, uuid=None, grammar_str=None, grammar_path=None,
                 ...
                 on_demand=False, async_run=False, textx_debug=False, config_dir=None,
        ...
        self.textx_debug = textx_debug
        self.detect_duplicates = detect_duplicates
        self.logger.info('duplicate detection enabled: %s', self.detect_duplicates)
        self.tx_kwargs = {'auto_init_attributes': False, 'debug': textx_debug}
        if uuid is not None:
            ...
            for wfl in wflows:
                check_compatibility(wfl['metadata']['grammar_str'],
                                    match['metadata'].get('data_schema_version', None))
    ...
    def get_model_history(self, uuid):
        ...
        for fwk in self.get_model_nodes(uuid):
            iso_time = get_iso_datetime(fwk['updated_on'])
            for line in fwk['spec']['_source_code']:
                line = line.replace('\n', '\n' + ' '*37)
                lines.append(' '.join((f'{fwk["state"]:10}', iso_time, line)))
        return '\n'.join(lines)
    ...
    def process_models(self, model_str=None, model_path=None):
        ...
        for index, (model, uuid, strn, path, vary) in enumerate(zip_):
            if strn or path or (model is None and uuid is not None):
                args = self._get_src_model(uuid, strn, path)
    ...
    def _expand_reuse(self, model_str):
        ...
        for obj in reuse_refs:
            ...
            for node in wfl['nodes']:
                version = node['spec'].get('_grammar_version')
                if version not in versions['grammar']:
                    msg = (f"Source-model grammar has version {version} but the "
    ...
    def _parse_vary(self, model_str):
        ...
        if textx_isinstance(model, meta['Program']):
            ...
            if vary_statements:
                fws = self.get_model_nodes(self.uuid)
    ...
    def _process_model(self, uuid, tx_get_model, model_src, source_code):
        ...
        if not textx_isinstance(model, self.metamodel['Program']):
            model = None
        elif self.wfe and self.on_demand:
            new_nodes = set(self.wfe.nodes_torun+model.fw_ids_torun)
            self.wfe.nodes_torun = list(new_nodes)
