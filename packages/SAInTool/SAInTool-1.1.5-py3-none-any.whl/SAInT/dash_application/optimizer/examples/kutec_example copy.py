...
#    print(f"load_from_json: {path}")
#    if not os.path.exists(path):
#        raise RuntimeError(f"{path} does not exist!")
#    with open(path, 'r') as file:
#        return json.load(file)

#def get_definition(name: str) -> OrderedDict:
#    current_directory = os.path.dirname(os.path.abspath(__file__))
#    return load_from_json(f"{current_directory}/{name}.json")
...
def get_quality(chlor: float,
    ...
    if verbose is True:
        chlor_str = f"Cl: {chlor:.2f} ({chlor_ok})"
        kalium_str = f"K: {kalium:.2f} ({kalium_ok})"
        sulfur_str = f"S: {sulfur:.2f} ({sulfur_ok})"
        print(f"{chlor_str}, {kalium_str}, {sulfur_str}")
    quality = 1.0 if chlor_ok and kalium_ok and sulfur_ok else 0.0
    return quality

class KutecExample(Example):
    def __init__(self):
        self.manipulatable_features_dict = manipulatable_features_dict #get_definition("manipulatable_features_dict")
        print("KUTEC MANIPULATABLE: ", self.manipulatable_features_dict)
        self.y_limits = {
