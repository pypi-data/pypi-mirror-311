...
class DataLoader():
    ...
    def load_data(self,
        ...
        def process_chunk(chunk, encode_features = None):
            ...
            # Encode binary features as 1 for the first mode and 0 otherwise
            #for col in binary_features:
            #    # Get the first mode (most frequent value, or lexicographically smallest if tied)
            #    mode = chunk[col].mode()[0]
            #    # Map the first mode to 1 and others to 0
            #    chunk[col] = chunk[col].apply(lambda x: 1 if x == mode else 0)

            # Detect pairs of opposite binary columns
            #if encode_features is not None:
            #    binary_features = [col for col in binary_features if col in encode_features]
            #opposites = {}
            #for i, col1 in enumerate(binary_features):
            #    for col2 in binary_features[i+1:]:
            #        if (chunk[col1].cat.codes + chunk[col2].cat.codes).eq(1).all():
            #            opposites[col1] = col2

            # Drop one column from opposite pairs
            #cols_to_drop = set(opposites.values())
