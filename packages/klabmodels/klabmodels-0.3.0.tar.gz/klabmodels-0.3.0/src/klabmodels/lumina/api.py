from .models import Chat, ChatMessage, Session, SOW
...
def create_sow(qualification: str, level: str, sessions: List[Dict]=None):
   try:
      session_list = []
      if sessions: session_list = [Session(**s) for s in sessions]
      sow = SOW(qualification=qualification, level=level, sessions=session_list)
      sow.save()
      return sow
   except Exception as e:
...
def read_sow(sow_pk: str):
   try:
      sow = SOW.find(SOW.pk==sow_pk).first()
...
def read_sow_by_qualification(qualification: str, level:str=None, session:str=None):
   sow_list = SOW.find(SOW.qualification==qualification).all()
   if sow_list:
      if level:
         sow_list = [s for s in sow_list if s.level==level]
      if session:
         sow_list = [s for s in sow_list if s.session==session]
      return sow_list
