...
def read_unv(
    ...
    if measurement_3d:
        id2_str_lst = [f"{data_channel} {dim}  {data_descriptor}" for dim in ["X", "Y", "Z"]]
    else:
        id2_str_lst = [f"{data_channel}  {data_descriptor}"]

    data = np.array([entry["data"] for entry in data_entries if entry["id2"] in id2_str_lst])

    if data.size == 0:
        print(f"Warning: No data found for id {id2_str_lst}")
