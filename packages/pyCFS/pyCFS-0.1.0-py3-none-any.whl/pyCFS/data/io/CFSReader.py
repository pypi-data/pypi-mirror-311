...
class CFSReader:
    ...
    def ResultInfo(self) -> List[CFSResultInfo]:
        ...
        result_info_list = []
        quantities = self.ResultQuantities
        for quantity in quantities:
            regions = self.get_result_regions(quantity)
            for reg in regions:
                result_info_list.append(self.get_result_info(quantity=quantity, region=reg))
        return result_info_list

    def get_result_info(self, quantity: str, region: str) -> CFSResultInfo:
        """
        ...
        is_complex = self.check_data_complex(quantity=quantity, region=region, restype=restype)
    ...
    def get_multi_step_data(
        ...
        if quantities is None:
            ...
            for q in quantities:
                if q not in self.ResultQuantities:
                    raise ValueError(
                        f"Quantity {q} not available in the result data. Available quantities: {self.ResultQuantities}"
                    )
        for quantity in quantities:
    ...
    def ResultQuantities(self) -> List[str]:
        ...
        -------
        List[str]
            List of result quantities for the active multi-step.
        ...
        """
        return self.get_result_quantities()
    ...
    @_catch_key_error
    def get_dim_names(self, quantity, multi_step_id: Optional[int] = None) -> List[str]:
        if multi_step_id is not None:
            self.set_multi_step(multi_step_id)

        with h5py.File(self._filename, driver=self._h5driver) as f:
            dim_names_array = np.array(
                f[f"Results/Mesh/MultiStep_{self._multiStepID}/ResultDescription/{quantity}/DOFNames"]
            ).flatten()
    ...
    def get_data_step(
        ...
        quantity : str
            Name of the quantity.
        result_def : cfs_result_definition, optional
            Result definition type. The default is cfs_result_definition.MESH.
        multi_step_id : int, optional
            MultiStepID to read result data from. The default is None.
    ...
    def get_history_data(self, multi_step_id: Optional[int] = None) -> CFSResultData:
        """
        Get data over all steps for a specified quantity and region. (AI-generated)
        ...
        for quantity in quantities:
            regions = self.get_result_regions(quantity=quantity, is_history=True)
            for region in regions:
                data.append(self.get_history_data_array(quantity=quantity, region=region))

        return CFSResultData(data=data)
    ...
    def get_history_data_array(self, quantity: str, region: str, multi_step_id: Optional[int] = None) -> CFSResultArray:
        ...
        with h5py.File(self._filename, driver=self._h5driver) as f:
            ...
            data_names = list(f[h5_data_path].keys())
            if "Imag" in data_names:
                data_array = f[f"{h5_data_path}/Real"][:] + f[f"{h5_data_path}/Imag"][:] * 1j
                is_complex = True
            else:
                data_array = f[f"{h5_data_path}/Real"][:]
                is_complex = False
