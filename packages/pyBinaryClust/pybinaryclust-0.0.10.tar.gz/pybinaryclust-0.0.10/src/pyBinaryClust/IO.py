...
def examine_data(input_file = None, transform = 'arcsinh', cofactor = 5, xdim = 7, ydim = 7):
...
def print_parameters(input_file = None):
    if input_file is None:
        return 'Please provide a FCS file.'
    
    meta, raw_data = fcsparser.parse(input_file, reformat_meta = True)
    to_return = [x for x in raw_data.columns.tolist() if '_' in x]
