...
from ngcbpc.command.args_validation import check_add_args_columns, check_valid_columns
from ngcbpc.command.clicommand import CLICommand
from ngcbpc.errors import BadRequestException, InvalidArgumentError, NgcException
from ngcbpc.util.utils import get_columns_help, has_org_role
...
class ConflictingLabelError(NgcException):
    ...
    def __str__(self):
        ...
        lines = [
            f"Label {self.label!r} already exists on the instance.",
            "",
...
def _wrap_bad_request_exception():
    try:
        yield
...
class InstanceCommand(ForgeCommand):  # noqa: D101
    ...
    @CLICommand.arguments(
        "--interface",
        metavar="<subnet:physical>",
        help=(
            "Specify subnet id and is physical <true|false>. Only one is attached over physical interface."
            "First one is attached over physical interface if not specified. Multiple interface arguments are allowed."
        ),
        type=str,
        action="append",
        required=True,
    ...
    def create(self, args):
        ...
        for inb in args.infiniband_interface or []:
            try:
                ...
                raise InvalidArgumentError(f"infiniband interface: {inb}") from None
        with _wrap_bad_request_exception():
    ...
    @CLICommand.arguments("--remove-label", help="Remove a label. (Can be specified multiple times.)", action="append")
    @CLICommand.arguments(
    ...
    def update(self, args):
        ...
        if args.label or args.remove_label:
            ...
            labels = self.api.info(org_name, team_name, args.instance).get("labels") or {}
            ...
            for label_key in parsed_labels.keys():
                if not args.overwrite_conflicting_labels and label_key in labels:
                    raise ConflictingLabelError(label=label_key)
    ...
    def remove(self, args):
        ...
        org_name = args.org or self.config.org_name
        team_name = args.team or self.config.team_name
        resp = self.api.remove(org_name, team_name, args.instance)
