...
from ngcbpc import environ
import ngcbpc.api.authentication
...
from ngcbpc.data.api.OrgListResponse import OrgListResponse
from ngcbpc.data.api.TeamListResponse import TeamListResponse
from ngcbpc.data.api.UserResponse import UserResponse
from ngcbpc.data.api.UserRoleDefinitionsResponse import UserRoleDefinitionsResponse
...
from ngcbpc.logger import set_log_level
from ngcbpc.util.db_util import NGC_DB
from ngcbpc.util.file_utils import get_cli_config_dir, get_cli_token_file
...
class Configuration:
    ...
    def _reset_authentication_tokens_cache():
        ngcbpc.api.authentication.Authentication._token_cache = ExpiringCache(
            max_timeout=ngcbpc.api.authentication.TOKEN_EXPIRATION_TIME
        )
    ...
    def get_team_list(self):  # noqa: D102
        ...
        if self.app_key or (self.starfleet_kas_session_key and self.starfleet_kas_email):
            if self.app_key and self.app_key.startswith(SCOPED_KEY_PREFIX):
                ...
                if sak_key_details.user is not None and sak_key_details.user.roles is not None:
    ...
    def _set_command_map(self):
        ...
        if self.app_key or (self.starfleet_kas_session_key and self.starfleet_kas_email):
            ...
            try:
                ...
                hidden_roles_defs = UserRoleDefinitionsResponse(hidden_roles_response)
                _actions = []
                ...
                public_roles_defs = UserRoleDefinitionsResponse(public_roles_response)
                user_role_choices = []
