...
from ngcbpc.data.search.SearchParams import SearchParams
from ngcbpc.data.search.SearchResponseResultResource import SearchResponseResultResource
from ngcbpc.errors import AccessDeniedException
...
DEFAULT_SEARCH_ORDER = [SearchParamOrderBy({"field": "score", "value": "ASC"})]
...
class SearchAPI:  # noqa: D101
    ...
    def _run_search_query(
        self, resource_type, resource_matcher, org, team, subscriber, filter_list=None
    ) -> List[SearchResponseResultResource]:
    ...
    def _build_common_filter_fields(ace_name=None, user_client_id=None, name=None, status=None):
        filter_list = []
        if ace_name:
            filter_list.append(SearchParamFilter({"field": "aceName", "value": ace_name}))
        if user_client_id:
            filter_list.append(SearchParamFilter({"field": "createdBy", "value": user_client_id}))
        if name:
            filter_list.append(SearchParamFilter({"field": "name", "value": name}))
        if status:
            for x in status:
                filter_list.append(SearchParamFilter({"field": "status", "value": x}))
        return filter_list
