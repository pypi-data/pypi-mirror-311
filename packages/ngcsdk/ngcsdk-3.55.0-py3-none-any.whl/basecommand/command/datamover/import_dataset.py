...
from basecommand.command.data import DataCommand
...
class DataImportSubCommand(DataCommand):  # noqa: D101
    CMD_NAME = "import"
    HELP = "Import data from an external storage system as a dataset in an ACE"
    DESC = "Dataset Import Commands"
    ...
    import_finish_str = "Finish a dataset import."
    import_info_str = "Status of the import job."
    import_list_str = "List all dataset import jobs"
    import_start_str = "Start import of a dataset from an object store into the ACE."
    ...
    def list(self, args):  # noqa: D102
        ...
        check_add_args_columns(args.column, DataImportSubCommand.columns_default)
        self.printer.print_jobs_list(
            filter_paginated_jobs(jobs, BcpJobType.IMPORT, BcpResourceType.DATASET), columns=args.column
        )
    ...
    @CLICommand.command(help=import_start_str, description=import_start_str)
    def start(self, args):  # noqa: D102
        ...
        dm_job = self.data_mover_api.create_job(
            origin=origin,
            destination=DataImportSubCommand.destination,
            org_name=org_name,
            team_name=team_name,
            generate_manifest=NGC_CLI_DM_MANIFEST_ENABLE,
        )
        dm_job.bcp_job = BcpJob(
            destination_resource_type=BcpResourceType.DATASET,
            tmp_resource_type=BcpResourceType.RESULTSET,
        )
        try:
    ...
    def info(self, args):  # noqa: D102
        ...
        if (
            ...
            and job.bcp_job.tmp_resource_type != BcpResourceType.RESULTSET
        ):
            raise ResourceNotFoundException(f"Dataset import job {args.jobid} could not be found")
    ...
    @CLICommand.arguments(
        ...
        metavar="<id>",
        help=(
            "Dataset to import metadata from. Used during ACE to ACE copies, typically after having run a dataset "
    ...
    @CLICommand.arguments(
        "--name",
        metavar="<name>",
        help="Dataset Name. If used together with --from-dataset, --name takes precedence.",
        type=str,
        action=SingleUseAction,
        required=False,
    )
    @CLICommand.arguments(
        "--desc",
        metavar="<desc>",
        help="Dataset Description. If used together with --from-dataset, --desc takes precedence.",
        type=str,
        action=SingleUseAction,
        required=False,
    )
    @CLICommand.arguments("jobid", metavar="<job_id>", help="Job ID.", type=str)
    @CLICommand.command(help=import_finish_str, description=import_finish_str)
    def finish(self, args):  # noqa: D102
        self.config.validate_configuration(csv_allowed=True)
        ace_name = args.ace or self.config.ace_name
        org_name = args.org or self.config.org_name
        team_name = args.team or self.config.team_name
        job_detail = self.data_mover_api.get_job(
            job_id=args.jobid, org_name=org_name, team_name=team_name, bcp_job=True
        )
        if job_detail.state not in FINAL_JOB_STATES:
            print(f"The job {args.jobid} has not yet finished")
            return
        ...
        finish_dataset_copy(
            ...
            ace_name=ace_name,
            dm_job=job_detail,
            original_dataset_id=args.from_dataset,
            name=args.name,
            desc=args.desc,
