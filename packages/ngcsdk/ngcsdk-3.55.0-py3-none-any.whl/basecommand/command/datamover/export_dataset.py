...
# TODO: Azure - remove next 3 lines once we're go for production
...
class DataExportSubCommand(DataCommand):  # noqa: D101
    CMD_NAME = "export"
    HELP = "Export a dataset from the ACE to an external storage system"
    DESC = "Dataset Export Commands"
    ...
    export_list_str = "List all dataset export jobs."
    export_run_str = "Export a dataset from the ACE into an object store."
    export_info_str = "Status of the dataset export job."
    ...
    def list(self, args):  # noqa: D102
        ...
        check_add_args_columns(args.column, DataExportSubCommand.columns_default)
        self.printer.print_jobs_list(
            filter_paginated_jobs(jobs, BcpJobType.EXPORT, BcpResourceType.DATASET), columns=args.column
        )
    ...
    def run(self, args):  # noqa: D102
        ...
        dm_job = self.data_mover_api.create_job(
            origin=DataExportSubCommand.origin,
        ...
        dm_job.bcp_job = BcpJob(
            origin_resource_type=BcpResourceType.DATASET,
            origin_resource_id=args.dataset,
        )
        try:
    ...
    def info(self, args):  # noqa: D102
        ...
        if job.bcp_job.origin_resource_type != BcpResourceType.DATASET:
            raise ResourceNotFoundException(f"Dataset export job {args.jobid} could not be found")
