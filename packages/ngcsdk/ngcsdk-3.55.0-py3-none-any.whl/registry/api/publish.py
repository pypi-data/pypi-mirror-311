...
class PublishAPI:  # noqa: D101
    ...
    def publish(
        ...
        if metadata_only:
            ...
            if resp.requestStatus and resp.requestStatus.statusCode == "ACCEPTED":
                ...
                ).format(target, COMMAND_NAME_MAP[artifact_type], resp.requestStatus.workflowId)
                if self.publish_printer.format_type != "json":
                    self.publish_printer.print_ok(_message)
                else:
                    self.publish_printer.print_json(
                        {
                            "workflowId": resp.requestStatus.workflowId,
                            "ngcCommand": "ngc registry {} publish --status {}".format(
                                COMMAND_NAME_MAP[artifact_type], resp.requestStatus.workflowId
                            ),
                        }
                    )
                return
        self.publish_printer.print_ok("Successfully published {}".format(target))
    ...
    def _publish_artifact_request(self, publish_request, artifact_type, org=None, team=None):
        ...
        if artifact_type == "collections":
            ...
            self._update_visibility_request(publish_request, artifact_type, org, team)
            return None
