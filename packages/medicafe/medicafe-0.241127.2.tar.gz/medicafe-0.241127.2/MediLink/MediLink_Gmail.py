...
def inspect_token(access_token):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Check for invalid token and prompt user to delete token.json
            if response.status_code == 400 and "invalid_token" in response.text:
                user_response = input("Access token is invalid. Would you like to delete token.json to force a refresh? (y/n): ")
                if user_response.lower() == 'y':
                    delete_token_file()
                    log("token.json has been deleted. Please re-authenticate.")
                else:
                    log("User chose not to delete token.json.")
