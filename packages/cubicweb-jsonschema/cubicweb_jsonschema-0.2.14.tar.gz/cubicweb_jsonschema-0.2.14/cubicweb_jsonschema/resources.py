    ...
    def __getitem__(self, value):
        ...
        for attrname in ("eid", self.cls.cw_rest_attr_info()[0]):
            resource = EntityResource(
                self.request, self.cls, attrname, value, self
            )
            try:
                rset = resource.rset
            except HTTPNotFound:
...
class _RelationshipProxyResource:
...
class EntityResource:
    ...
    def rset(self):
        ...
        if self.cls is None:
            return req.execute(
                "Any X WHERE X eid %(x)s", {"x": int(self.__name__)}
            )
        ...
        if self.attrname == "eid":
            ...
            rset = req.execute(st.as_string(), {"x": str(self.__name__)})
        return rset


class RelatedEntitiesResource:
        ...
        if sortterms:
            ...
            for term in sortterms.split(","):
                ...
                rel = nodes.make_relation(
                    mainvar, term, (mdvar,), nodes.VariableRef
                )
