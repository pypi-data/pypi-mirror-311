...
def _headerlist_to_string(headerlist):
    ...
    Content-Type: application/json
    """
    return "\n".join("{}: {}".format(name, value) for name, value in headerlist)
...
class WebTestReponseOutputChecker(doctest.OutputChecker):
    ...
    def output_difference(self, example, got, optionflags):
        ...
        if w_headers != g_headers:
            diff.append(
                "Expected headers: {}".format(_headerlist_to_string(w_headers))
            )
            diff.append(
                "Actual headers: {}".format(_headerlist_to_string(g_headers))
            )
        if w_body or g_body:
            if not w_body:
