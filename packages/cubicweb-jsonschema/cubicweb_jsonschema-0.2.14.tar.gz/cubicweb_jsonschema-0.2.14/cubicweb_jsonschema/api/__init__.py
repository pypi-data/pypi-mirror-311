...
def describes(func):
    ...
    def wrapper(context, request):
        response = func(context, request)
        response.headers["Link"] = link_header(
            request.resource_path(context), "describes", type="application/json"
        )
        return response
...
def json_problem(
    type=None, title=None, status=400, detail=None, instance=None, **problem
):
