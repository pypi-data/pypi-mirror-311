...
except ModuleNotFoundError:  # TODO remove with CubicWeb 3 support
...
class JslDocumentBuilder(Component):
    ...
    def fields_from_yams(
        self, etype, filter_in=None, filter_out=None, schema_role=None
    ):
            ...
            for value in filter_in:
                try:
                    ...
                    if value.startswith("reverse_"):
                        rtype_start_index = len("reverse_")
                        rtype = value[rtype_start_index:]
                        role = "object"
                    else:
                        rtype = value
                        role = "subject"
        ...
        if filter_out:
            rtype_roles = [
                rtype_role
                for rtype_role in rtype_roles
                if rtype_role[0] not in filter_out
            ]
        return OrderedDict(
            (rtype, self.generate_field(etype, rtype, role, schema_role))
    ...
    def generate_field(
        self, etype, rtype, role, schema_role, target_types=None, **kwargs
    ):
