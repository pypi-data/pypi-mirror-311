    ...
    *   'related': relation definition would not appear in entity's JSON
    ...
    def _init(self, sschema, rschema, oschema, role):
        ...
        if (
            ...
            rdef = rschema.relation_definition(sschema, oschema)
    ...
    def relations_by_section(self, entity, section, permission):
        ...
        rdefs = eschema.relation_definitions(include_final=True)
        for rschema, targetschemas, role in rdefs:
            ...
            for tschema in targetschemas:
                if tschema.type == "Password" and relpermission == "read":
                    continue
                if section not in self.etype_get(
                    eschema, rschema, role, tschema
                ):
                    continue
                rdef = rschema.role_relation_definition(eschema, tschema, role)
                if rschema.final:
                    if not rdef.has_perm(
                        cnx, permission, eid=eid, creating=eid is None
                    ):
                        continue
                else:
