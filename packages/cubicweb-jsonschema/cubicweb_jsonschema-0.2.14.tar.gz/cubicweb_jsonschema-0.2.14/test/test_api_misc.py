...
class EntitiesTC(BaseTC):
    ...
    def test_get_related(self):
        with self.admin_access.cnx() as cnx:
            ...
            author = cnx.create_entity(
                "Author", name="bob", reverse_author=book
            )
            jschema = (
                self.vreg["adapters"]
                .select(
    ...
    def test_add_related(self):
        ...
        self.assertEqual(
            res.location, "https://localhost:80/author/{}/".format(author_eid)
        )
    ...
    def test_add_related_object(self):
        ...
        self.assertEqual(
            res.location, "https://localhost:80/book/{}/".format(book_eid)
        )
        jsonschema.validate(entity, jschema)
    ...
    def test_validationerror_additionalproperties(self):
        ...
        self.login()
        res = self.webapp.post_json(
            "/author/", data, status=422, headers={"Accept": "application/json"}
        )
        ...
        self.assertEqual(
            res.json_body,
            {
                ...
                "detail": "unexpected properties: born",
            },
        )
    ...
    def test_validationerror_nosource(self):
        ...
        with self.admin_access.cnx() as cnx:
            gid = cnx.find("CWGroup", name="users")[0][0]
        data = {
            "login": "bob",
            "upassword": "pass",
            "in_group": [str(gid)],
    ...
    def test_validationerror_integrityerror(self):
        ...
        self.login()
        res = self.webapp.post_json(
            "/cwuser/", data, status=422, headers={"Accept": "application/json"}
        )
        self.assertEqual(res.content_type, "application/problem+json")
        self.assertEqual(
            res.json_body,
            {
                ...
                "invalid-params": [
                    {
                        "name": "login",
                        "reason": "login is part of "
                        "violated unicity constraint",
                    }
                ],
            },
        )
    ...
    def test_exception(self):
        ...
        with patch(
            "cubicweb.req.RequestSessionBase.create_entity", side_effect=error
        ):
            res = self.webapp.post_json(
                "/cwuser/",
                data,
            ...
            self.assertEqual(
                res.json_body,
                {
                    ...
                    "detail": "invalid literal for int() with base 10: 'foo'",
                },
            )
