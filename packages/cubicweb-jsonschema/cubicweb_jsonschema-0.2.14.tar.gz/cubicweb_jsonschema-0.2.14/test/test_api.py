    ...
    class ApiBaseTC(PyramidCWTest):
        ...
        def setup_database(self):
            with self.admin_access.cnx() as cnx:
                for topic_name in ("fishing", "gardening", "sword fish"):
...
def load_tests(loader, tests, ignore):
    ...
    try:
        ...
        warnings.warn(
            '"ajv" is not available, no validation of JSON Schema '
            "will be performed"
        )
        has_ajv = False

    temporary_file = partial(
    ...
    for fpath in glob(path.join(here, "api", "*.rst")):
        tests.addTests(docfilesuite(fpath))
    tests.addTests(docfilesuite(path.join(here, "hypermedia-walkthrough.rst")))
