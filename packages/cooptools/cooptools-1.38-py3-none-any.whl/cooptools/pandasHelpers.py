...
def import_data_from_csv_with_specified_columns(column_type_definition: Dict,
    ...
    # Return
    df = clean_a_dataframe(df, column_type_definition=column_type_definition, allow_partial_columnset=allow_partial, fill_missing=fill_missing)

    if echo_rows:
        pretty_print_dataframe(df, max_rows=echo_rows)
    return df
