...
class RabbitMQHandler:
    ...
    def setup_queues(self, queue_mappings: Dict[str, str], dlx_exchange: str = 'dlx_exchange'):
        ...
        for primary_queue, dlq in queue_mappings.items():
            try:
                self.channel.queue_declare(queue=dlq, durable=True, arguments={
                ...
                if e.reply_code == 406:
                    ...
                    self.channel = self.connection.channel()  # Reopen the channel after it was closed
                else:
                    raise e
            self.channel.queue_bind(exchange=dlx_exchange, queue=dlq, routing_key=dlq)

        # Declare primary queues with DLX
        for primary_queue, dlq in queue_mappings.items():
            try:
                self.channel.queue_declare(queue=primary_queue, durable=True, arguments={
        ...
        if retry_count >= self.retry_limit:
            ...
            try:
                ...
                if is_malformed:
                    ...
                    return
                callback(channel, method, properties, parsed_message)
                if channel.is_open:
                    channel.basic_ack(delivery_tag=method.delivery_tag)
            except Exception as e:
