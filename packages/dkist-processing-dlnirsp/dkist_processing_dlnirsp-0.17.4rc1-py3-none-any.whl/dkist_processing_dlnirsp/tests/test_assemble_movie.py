...
def test_assemble_movie(
    assemble_movie_task, dither_mode_on, is_polarimetric, requires_shrinking, mocker
):
    """
...
def test_tags_for_images(assemble_movie_task, dither_mode_on):
    ...
    expected_X_tile_values = (
        sum([[i for _ in range(num_Y_tiles)] for i in range(num_X_tiles)] * num_mosaics, [])
        * num_dither
    )
    expected_Y_tile_values = list(range(num_Y_tiles)) * num_X_tiles * num_mosaics * num_dither
    ...
    for n in range(task.num_images):
        assert sorted(task.tags_for_image_n(n)) == sorted(
            [
                ...
                DlnirspTag.tile_X_num(expected_X_tile_values[n]),
                DlnirspTag.tile_Y_num(expected_Y_tile_values[n]),
            ]
        )
