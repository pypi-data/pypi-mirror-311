...
def test_compute_average_gain(
    ...
    assert len(arrays) == 1
    avg_array = arrays[0]
    assert avg_array.shape == task._rectified_array_shape
...
def test_compute_characteristic_spectra(solar_task_with_full_corr, groups_by_slitbeam):
    ...
    assert char_spec.shape == task._rectified_array_shape
...
def test_normalize_gain(
    solar_task_with_no_data,
    array_grouped_by_slitbeam,
    groups_by_slitbeam,
    write_drifted_group_ids_to_task,
):
    """
    Given: A SolarCalibration task with a final gain image and a dark corrected raw gain image
    When: Normalizing the final gain image
    Then: The relative slitbeam signals are preserved from the raw gain image
    """
    task = solar_task_with_no_data

    write_drifted_group_ids_to_task(task)

    overall_raw_scale = 6.28  # tau because it's cool

    # The median of each slitbeam will be it's number times the overall scale
    dark_corrected_gain = array_grouped_by_slitbeam * overall_raw_scale
    task.write(
        data=dark_corrected_gain,
        tags=[DlnirspTag.intermediate(), DlnirspTag.frame(), DlnirspTag.task("SC_DARK_ONLY")],
        encoder=fits_array_encoder,
    )

    # Mimic a totally normalized final gain with some non-raw value
    flat_final_gain = np.ones_like(dark_corrected_gain) * 74.123

    normalized_gain = task.normalize_gain(flat_final_gain)

    for slitbeam, groups in groups_by_slitbeam.items():
        slitbeam_data_list = []
        for group in groups:
            group_data = task.group_id_get_data(data=normalized_gain, group_id=group)
            slitbeam_data_list.append(group_data.flatten())

        expected_slitbeam_median = slitbeam * overall_raw_scale
        np.testing.assert_allclose(
            np.nanmedian(np.hstack(slitbeam_data_list)), expected_slitbeam_median
        )
