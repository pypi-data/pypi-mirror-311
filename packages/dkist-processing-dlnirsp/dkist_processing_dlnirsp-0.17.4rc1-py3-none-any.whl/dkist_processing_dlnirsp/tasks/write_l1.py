...
class DlnirspWriteL1Frame(WriteL1Frame):
    ...
    def add_dataset_headers(
        ...
        header, tile_as_temporal_axis, num_extra_axes, axis_index = self._add_temporal_axis(
        ...
        if self.constants.num_dither_steps > 1:
            ...
            header[f"CNAME{axis_index}"] = "time"
        ...
        if self.constants.correct_for_polarization:
            ...
            header[f"CNAME{axis_index}"] = "polarization state"
        ...
        if (
            ...
            and self.constants.num_spatial_steps_X * self.constants.num_spatial_steps_Y > 1
        ):
            # TODO: MINDEX{12} keys are wrong right now. We need clarity/info from the DL team to make them correct
            num_mosaic_axes = 1

            header["MAXIS1"] = self.constants.num_spatial_steps_X
            header["MINDEX1"] = header["DLCSTPX"]

            if self.constants.num_spatial_steps_Y > 1:
                num_mosaic_axes += 1
                header["MAXIS2"] = self.constants.num_spatial_steps_Y
                header["MINDEX2"] = header["DLCSTPY"]

            header["MAXIS"] = num_mosaic_axes
    ...
    def _add_temporal_axis(
        ...
        2. `num_mosaic_repeats == 1` and EXACTLY ONE of `num_spatial_steps_X` or `num_spatia_steps_Y` > 1. In this case
        ...
        num_X_tiles = self.constants.num_spatial_steps_X
        num_Y_tiles = self.constants.num_spatial_steps_Y
