...
class GroupIdMixin:
    ...
    def group_id_get_idx(
        ...
        """
        group_id_array = (
            self.group_id_drifted_id_array if not rectified else self._rectified_group_id_array
        )
    ...
    def group_id_get_data_and_idx(
        ...
        *,
        ...
        if data_shape == self._unrectified_array_shape:
            rectified = False
        elif data_shape == self._rectified_array_shape:
            rectified = True
        else:
            raise ValueError(
                f"Input data shape ({data_shape}) doesn't match either the un-rectified "
                f"({self._unrectified_array_shape}) or rectified ({self._rectified_array_shape}) group-id array shape"
            )
    ...
    def group_id_convert_idx_to_2d_slice(
        ...
        spectral_min, spectral_max = self._ensure_uniform_spectral_size(
    ...
    def _ensure_uniform_spectral_size(
    ...
    def group_id_slitbeam_group_dict(self) -> dict[int, list[int]]:
        ...
        current_slitbeam = 0
        current_spectral_center = self._get_spectral_center_of_both_beams(even_group_id=0)
        for group_id in range(0, num_groups, 2):
            spectral_center = self._get_spectral_center_of_both_beams(even_group_id=group_id)
    ...
    @cached_property
    def _rectified_group_id_array(self) -> np.ndarray:
        """
    ...
    @property
    def _unrectified_array_shape(self) -> tuple[int, int]:
        """Return the shape of un-rectified data."""
        return self.group_id_drifted_id_array.shape

    @property
    def _rectified_array_shape(self) -> tuple[int, int]:
        """Return the shape of rectified data."""
        return self._rectified_group_id_array.shape

    def _get_spectral_center_of_both_beams(self, even_group_id: int) -> float:
    ...
    def group_id_rectified_illuminated_idx(self) -> np.ndarray:
        ...
        return ~np.isnan(self._rectified_group_id_array)
