...
class InstrumentPolarizationCalibration(
    ...
    def run(self) -> None:
        ...
        3. Downsample full frame by binning each group and keeping track of the two separate beams

        4. Send each beam's data to `dkist-processing-pac` for demodulation calculation

        4. Resample demodulation matrices from both beams to the full-frame size
        ...
        with self.apm_processing_step("Correct and extract polcal data"):
            for cs_step in range(self.constants.num_cs_steps):
                for modstate in range(1, self.constants.num_modstates + 1):
                    ...
                    for beam in [1, 2]:
                        global_binned_data = self._bin_global_data(data, beam)
                        ...
                        local_binned_data = self._bin_local_data(data, beam)
    ...
    def _bin_global_data(self, data: np.ndarray, beam: Literal[1, 2]) -> np.ndarray:
    ...
    def _bin_local_data(self, data: np.ndarray, beam: Literal[1, 2]) -> np.ndarray:
