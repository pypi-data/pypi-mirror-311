...
from websockets import ConnectionClosed, connect, protocol
...
class SCP:
    def __init__(self, options: SCPOptions = SCPOptions()):
        ...
        self._connectionState: protocol.State = protocol.State.CLOSED
        self._onStateChange: Optional[Callable[[protocol.State], None]] = None
        self._onError: Optional[Callable[[str], None]] = None
        self._session: Optional[SCPSession] = None
        self._readyFuture = asyncio.Future[Self]()
        self.requests: Dict[str, TransactionNotificationHandlers] = {}

    def onError(self, callback: Callable[[str], None]):
        self._onError = callback

    def onStateChange(self, callback: Callable[[protocol.State], None]):
        self._onStateChange = callback

    def _updateState(self, state: protocol.State):
        self._connectionState = state
        if self._onStateChange is not None:
            self._onStateChange(state)

    def isConnected(self):
        return self._connectionState == protocol.State.OPEN
    
    def getEndpoint(self):
        return self._endpoint

    def reset(self, options: Optional[SCPOptions] = None):
        if self._connectionState != protocol.State.CLOSED:
        ...
        self._readyFuture = asyncio.Future[Self]()
        self.requests = {}
        self._updateState(protocol.State.CLOSED)
        return self
    ...
    async def _connect(self, url: str, userKey: Key, knownTrustedKey: Optional[str] | Optional[bytes]):
        ...
        self._updateState(protocol.State.CONNECTING)
        trustedKey = base64.b64decode(knownTrustedKeyStr)
        ...
        try:
            ...
            serverHello = await asyncio.wait_for(self._socket.recv(decode=False), timeout=5)
            ...
            serverIdentity = await asyncio.wait_for(self._socket.recv(decode=False), timeout=5)
            ...
            if self._onError is not None:
                if isinstance(e, ConnectionClosed) and e.rcvd is not None:
                    ...
                    self._onError("Connect: Connection closed")
            
        self._updateState(protocol.State.OPEN)
    ...
    async def _onMessage(self):
        if self._socket is None:
            return
        try:
            while self._connectionState == protocol.State.OPEN:
