...
def stage_handle_files(conf_json, origin_dir, target_dir):
    # copy data dir
    # from dll_origin_dir to dll_target_dir
    ...
    # copy web
    LOG_D(f"copying `web`")
    copy_dir(src=f"{origin_dir}/web", dst=f"{target_dir}/web")
    ...
    if not conf_is_wegame(conf_json):
    ...
    if not conf_is_steam(conf_json):
...
def stage_save_conf(conf_json, target_dir, need_encrypt): # 加密配置数据，保存在指定目录
...
def cmd_handle_build_store(args):
    ...
    conf_json = stage_get_conf(args.params)

    if args.origin != None and len(args.origin) != 0:
        stage_handle_files(conf_json, args.origin, args.target)
    ...
    stage_save_conf(conf_json, args.target, args.encrypt)
...
def cmd_regist(subparsers):
    ...
    parser.add_argument('-p', '--params', type=str, default=None, help='store params, E.g: offcial-pc-10000-dev')
    parser.add_argument('-o', '--origin', type=str, default=None, help='origin full path')
    parser.add_argument('-t', '--target', type=str, default=None, help='target full path')
