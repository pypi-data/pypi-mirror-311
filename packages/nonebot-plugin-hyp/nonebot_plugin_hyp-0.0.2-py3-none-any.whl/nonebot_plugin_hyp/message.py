...
class Message:
    @staticmethod
    async def send_hyp_msg(data, data_a, online) -> str:
        """构建/hyp回复消息"""
        error = data.get("e")
        if error:
            return error
    ...
    @staticmethod
    async def send_apikey_msg(hyp_apikey, ant_apikey):
        msg = (
            f"当前API信息如下:\n"
    ...
    @staticmethod
    async def send_bw_msg(data, data_a, rank) -> str:
        """构建/bw回复消息"""
        error = data.get("e")
        if error:
            return error
    ...
    @staticmethod
    async def send_sw_msg(data, data_a, rank) -> str:
        """构建/sw回复消息"""
        error = data.get("e")
        if error:
            return error
