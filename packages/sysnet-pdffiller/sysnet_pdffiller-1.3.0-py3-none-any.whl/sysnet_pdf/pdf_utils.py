...
from sysnet_pyutils.utils import Singleton, who_am_i

from init import PDF_OUTPUT_DIR, PDF_TEMP_FILE, PDF_OWNER_PASSWORD, \
...
class PdfFactory(object, metaclass=Singleton):
    """
    ...
    def fill_pdf_form(self, template_filename, data=None, out_filename=PDF_TEMP_FILE, templates=None):
        ...
        try:
            ...
            write_fillable_pdf(str(os.path.join(PDF_TEMPLATES_DIR, template_filename)), out_filename, data)
            return out_filename
        except KeyError as e:
            LOG.logger.error('{}: {}'.format(__name__, str(e)))
            return None
