...
def init_mongodb_connections(database_pool):
    """Init mongo connections

    Args:
        database_pool (_type_): _description_
    """
    for alias, database in database_pool:
        connect(
            db=database["db"],
            alias=alias,
            host=database["host"],
            maxPoolSize=database.get("max_pool_size", 10),
            **database.get("kwargs", {}),
        )


# @contextmanager
# def connect_to_db(db: str, alias: str, count: int = 3) -> None:  # type: ignore
#     """
...
#     try:
#         # Code inside the `with` block can execute here
#         yield
#     finally:
#         # Ensure the connection is closed properly when the context exits
#         disconnect(alias)
...
#         metadata = getattr(self.mongo_collection, "_meta", {})
