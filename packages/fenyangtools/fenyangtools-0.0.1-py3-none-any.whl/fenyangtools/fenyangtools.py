...
if csdir=="":
    ...
    csdir =csdir +csdir.split ("pinzhongtools")[0 ][-1 ]#
...
def get_directory_path(directory_path):
    ...
    global csdir
    # 检查目录是否存在 A.txt 文件
    if not (os.path.isfile(os.path.join(directory_path, '地市清单.xlsx'))):
        # 解压 A.zip 到指定目录
...
def aaa(methon):
	all_list = pd.read_excel(peizhidir+"所有分过的表格.xls", header=0, sheet_name=0,converters={"报告编码": str})
	fenbiao= pd.read_excel(peizhidir+"任务分配.xls", header=0, sheet_name=0)
	qingdan= pd.read_excel(peizhidir+"地市清单.xlsx", header=0, sheet_name=0)
	all_now = pd.read_excel(peizhidir+"报告审核任务分配表.xls", header=0, sheet_name=0,converters={"报告编码": str})
	#now = datetime.now()
...
if __name__ == '__main__':
	...
	peizhidir=str(setting_cfg["settingdir"])+csdir.split("fenyangtools")[0][-1]
