...
class AI_Stepper:
    ...
    def _validate_and_notify(
        ...
        for output_name, output_schema in expected_outputs.items():
            if output_name not in parsed_output:
                error_msg = (
                    f"Step '{step_name}' - Missing required output '{output_name}'.\n"
                    f"Available outputs: {list(parsed_output.keys())}"
                )
            ...
            try:
                validate(parsed_output[output_name], output_schema)
            except ValidationError as e:
                error_msg = (
                    f"Step '{step_name}' - Validation error for output '{output_name}':\n"
                    f"{str(e)}"
                )
                print(f"[bold red]{error_msg}[/bold red]")
                attempts_history.append((error_msg, cleaned_content))
                ...
                callback(CallBack(
                    sender="LLM",
                    object="error",
                    message=f"<span style='color: red;'>{error_msg}</span>",
        ...
        callback(CallBack(
            sender="LLM",
            object="result",
            message=f"<span style='color: green;'>Successfully validated outputs for step '{step_name}'</span>",
            created=int(datetime.utcnow().timestamp()),
            step_name=step_name
        ))
    ...
    def _execute_llm_query(
        ...
        try:
            ...
            if len(expected_outputs) == 1:
                ...
                if isinstance(parsed_output, dict) and output_name in parsed_output:
