    ...
    with patch('ai_stepper.ai_stepper.completion', return_value=mock_response):
        try:
            stepper.run(
                ...
                {
                    "query": "How to optimize a sorting algorithm?"
                },
                test_callback
            )
        except Exception:
            pass  # We expect an error due to the mock response chain
...
def test_llm_retry_mechanism(stepper, chain_of_thoughts_yaml):
    ...
    def test_callback(callback_data: CallBack):
        ...
        if (callback_data.object == "step" and
            "failed" in callback_data.message and
            callback_data.step_name == "direct_answer"):  # Only count errors from our step
            error_count += 1
    ...
    # We should get one error callback since the retry mechanism handles intermediate failures
    assert error_count == 1, f"Expected 1 error callback, got {error_count}"
