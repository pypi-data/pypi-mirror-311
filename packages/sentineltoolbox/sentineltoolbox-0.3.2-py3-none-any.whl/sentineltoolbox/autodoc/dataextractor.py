...
from sentineltoolbox.autodoc.constants import AUTODOC_CONF
...
class DataExtractor:
    ...
    def title(self, key: str) -> str:
        return PRODUCT_DESCRIPTION.get(key, key)

    def has_flag_attrs(self, obj: DataTree[Any]) -> bool:
        """
        Test if variable is a mask/flag
    ...
    def contains_flags(self, item: DataTree[Any]) -> bool:
        try:
    ...
    def has_flags(self, obj: DataTree[Any], recursive: bool = True) -> bool:
        try:
    ...
    def has_attrs(self, obj: DataTree[Any], ignore_list: Iterable[str] | None = None, recursive: bool = True) -> bool:
    ...
    def flags(self, root: DataTree[Any], path: str, ellipsis: Ellipsis | None = None) -> list[DataTreeNode]:
        return list(self.iter_flags(root, path, ellipsis=ellipsis))

    def has_data(self, obj: DataTree[Any]) -> bool:
    ...
    def all_paths(self, obj: DataTree[Any]) -> list[str]:
        paths = []
    ...
    def attributes_stac_discovery(
        ...
        if exclude_attrs is None:
            exclude_attrs = []
        attrs = {k: v for k, v in obj.attrs.items() if k not in ["stac_discovery", "other_metadata"]}
        if "stac_discovery" in obj.attrs:
            attrs.update(obj.attrs["stac_discovery"])
        return {k: v for k, v in attrs.items() if k not in exclude_attrs}

    def attributes_stac_extensions(self, obj: DataTree[Any], exclude_attrs: list[str] | None = None) -> list[Any]:
        attrs = self.attributes_stac_discovery(obj)
        return attrs.get("stac_extensions", [])

    def attributes_properties(self, obj: DataTree[Any], exclude_attrs: list[str] | None = None) -> dict[Hashable, Any]:
        if exclude_attrs is None:
            exclude_attrs = []
    ...
    def attributes(self, obj: DataTree[Any], exclude_attrs: list[str] | None = None) -> dict[Hashable, Any]:
        if exclude_attrs is None:
            exclude_attrs = []
        return {k: v for k, v in obj.attrs.items() if k not in exclude_attrs}

    def attribute_desc(self, name: str) -> str:
        return TERM_DESCRIPTION.get(name, name)
    ...
    def product_type(self, root: DataTree[Any]) -> str | None:
        return AttributeHandler(root).get_attr("product:type", default=None)

    def is_coordinate(self, root: DataTree[Any], path: str) -> bool:
    ...
    def name(self, root: DataTree[Any], path: str, ellipsis: Ellipsis | None = None) -> str:
        if ellipsis:
    ...
    def long_name(self, obj: DataTree[Any], ellipsis: Ellipsis | None = None) -> str:
    ...
    def legacy_field(self, root: DataTree[Any], obj: DataTree[Any], ellipsis: Ellipsis | None = None) -> str:
    ...
    def description(self, root: DataTree[Any], path: str, ellipsis: Ellipsis | None = None, obj: Any = None) -> str:
        if ellipsis:
            ellipsis_path = self.path(path, ellipsis)
            if ellipsis_path in ellipsis.repr_name:
                return ellipsis.repr_name[ellipsis_path]
    ...
    def documentation(self, root: DataTree[Any], path: str = "", ellipsis: Ellipsis | None = None) -> str:
        if ellipsis:
            ellipsis_path = self.path(path, ellipsis)
    ...
    def product_property_documentation(self, key: str, value: str | None = None) -> str:
        ...
        if isinstance(value, dict) and "units" in value:
            extracted = f'unit in {value["units"]}'
