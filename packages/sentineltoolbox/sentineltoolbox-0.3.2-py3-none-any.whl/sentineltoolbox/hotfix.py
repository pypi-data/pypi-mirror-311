...
HotfixType_L: TypeAlias = Literal["wrapper", "path", "value", "datatree", "alias"]
HotfixPathInput: TypeAlias = dict[str, tuple[MetadataType_L, str | None]]
HotfixValueInput: TypeAlias = dict[MetadataType_L, dict[str, Any]]
...
class Hotfix:
    ...
    def is_applicable(self, attrs: T_Attributes, product_type: str = "") -> bool:
...
class HotfixPath(Hotfix):
    def __init__(
        ...
        super().__init__(data=final_data, hotfix_type="path", is_applicable_func=is_applicable_func)
...
class HotfixValue(Hotfix):
    def __init__(
        ...
        """
        :param data:  example: {"stac_properties": {"platform": lambda value, **kwargs: str(value).lower()}}
        """
        super().__init__(data=data, hotfix_type="value", is_applicable_func=is_applicable_func)
...
class HotfixWrapper(Hotfix):
    def __init__(
        ...
        """
        super().__init__(data=data, hotfix_type="wrapper", is_applicable_func=is_applicable_func)
...
class HotfixDataTree(Hotfix):
    def __init__(
        ...
        :param data: {"stac_properties": {"created": ConverterDateTime()}}
        """
        super().__init__(data=data, hotfix_type="datatree", is_applicable_func=is_applicable_func)


class HotfixManager:
    def __init__(self, hotfix_list: list[Hotfix]):
        self.hotfix_list = hotfix_list
    ...
    def _fixes(self, htype: HotfixType_L, attrs: T_Attributes | None = None, product_type: str = "") -> list[Hotfix]:
        ...
        if attrs is None:
            attrs = {}
        for hotfix in self.hotfix.get(htype, []):
