...
def convert_semantics(fmt: str, old_semantic: str, **kwargs: Any) -> list[str]:
    ...
    if "semantic" in kwargs:
        ...
        if old_semantic in MAPPING_LEGACY_SEMANTIC_TO_DPR:
            new_semantic = MAPPING_LEGACY_SEMANTIC_TO_DPR[old_semantic]
            if isinstance(new_semantic, str):
                return [new_semantic]
            elif isinstance(new_semantic, list):
                return new_semantic
            else:
                raise DataSemanticConversionError(
                    f"Invalid value for legacy {old_semantic!r}.\n"
                    "Please specify correct semantic with semantic='XXXXX'",
                )
        else:
            raise DataSemanticConversionError(
...
def _extract_data_from_product_filename(filename: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if fmt.startswith("product/eopf"):
        ...
        semantic = convert_semantic(fmt, old_semantic, **kwargs)  # eg MSIL1C
    ...
    def to_string(
        ...
        **kwargs: Any,
        ...
        if extension is None:
            extension = PRODUCT_FORMATS.get(self.semantic, {}).get("ext", ".zarr")
