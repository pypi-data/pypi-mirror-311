...
from sentineltoolbox.resources.data import LIST_DPR_ADFS_FROM_MERGED_LEGACY
...
LEGACY_FILES_PATTERNS = ["*.SEN3", "*.SAFE", "*.SEN3.zip", "*.SAFE.zip", "*.SEN3.tgz", "*.SAFE.tgz"]
    ...
    **kwargs : Any
    ...
    for upath_output_and_parent, upath_inputs in out_in_dict.items():
        ...
        if len(upath_inputs) == 1:
            ...
            if semantic in LIST_DPR_ADFS_FROM_MERGED_LEGACY:
...
def _dump_datatree(data: dict[Any, Any] | DataTree[Any], upath_output: PathFsspec) -> None:
