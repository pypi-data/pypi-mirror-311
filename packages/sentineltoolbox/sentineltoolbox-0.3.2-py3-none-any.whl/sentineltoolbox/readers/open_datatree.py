...
from sentineltoolbox.resources.data import DPR_SEMANTIC_WITH_SAT_PRODUCTS
...
def open_eop_datatree(
    ...
    **kwargs: Any,
    ...
    tree_root = datatree.DataTree.from_dict({"/": xr.open_dataset(filename_or_obj, **kwargs)})
...
def convert_sentinel_legacy_data(
    ...
    **kwargs: Any,
    ...
    is_s3_product = product_type_pattern_str.startswith("product/s3-legacy") or (
        product_type in DPR_SEMANTIC_WITH_SAT_PRODUCTS and product_type.startswith("S03")
    )
    ...
    if is_s3_product:
        ...
        if isinstance(converted_data, datatree.DataTree):
            xdt = converted_data
        else:
...
def extract_metadata(xdt: DataTree[Any], upath: PathFsspec, **kwargs: Any) -> dict[Hashable, Any]:
    """
...
def open_datatree(
    ...
    **kwargs: Any,
    ...
    if (
        ...
        return convert_sentinel_legacy_data(upath, product_type_pattern=product_type_pattern, **kwargs)
    # it is a JSON file, convert it to datatree
    elif is_json(url):
        logger.info(f"open and convert 'json' file {url!r} to datatree")
        return convert_dict_to_datatree(open_json(upath, credentials=credentials, **kwargs))
    # it is a zarr file, open it normally
        ...
        if kwargs["engine"] == "zarr":
            ...
            return datatree.open_datatree(**kwargs)
