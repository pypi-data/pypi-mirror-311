...
def Glob(array_dt,cd="CHN",ty="CR"):
    ...
    if  len(array_dt.shape) == 2 and ty =="pre":
        ...
        plt.colorbar(ax.images[0], ax=ax, orientation='vertical',shrink=0.5)
        plt.savefig(outpath)
    else :
...
def GlobLonLatPlus(array_dt,Lon,Lat,cd="CHN",ty="CR",name="temp",markcolor="red",outpath1=None):   ###  x_coords2 维度 
    if cd == "g":
        env = [-179.617020, 179.632979,-85.098871,85.051128] 
    elif cd == "US":
        env = [-132.0, -47.0, 0, 57.0]
