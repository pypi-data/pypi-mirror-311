...
from docknet.schema.system import SystemState
...
def open_login_page(
    ...
    callback_uri = os.path.join(
        ...
        config.settings.DOCKNET_API_PATH,
...
def create_token(
    ...
    if token_purpose is not None:
        if token_purpose in docknet_token_purposes:
...
def login_callback(
    ...
    response = RedirectResponse(
        os.path.join(
            schema,
            config.settings.get_redirect_uri(),
            f"{config.settings.DOCKNET_WEBAPP_PATH}/",
