...
@cli.command()
def install_docknet() -> None:
    from docknet.managers.components import install_components
...
def start_dev_server(host: str = "0.0.0.0", port: int = 8082) -> None:
    ...
    uvicorn.run("docknet.api:app", host=host, port=port, log_level="info", reload=True)
