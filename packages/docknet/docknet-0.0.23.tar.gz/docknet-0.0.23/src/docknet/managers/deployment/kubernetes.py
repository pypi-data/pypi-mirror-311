...
from docknet.managers.deployment.kube_utils import (
...
from docknet.schema import Job, JobInput, ResourceAction, Service, ServiceInput
from docknet.schema.deployment import DeploymentType, ServiceUpdate
...
class KubernetesDeploymentPlatform:
    ...
    def get_service_metadata(self, project_id: str, service_id: str) -> Service:
        try:
            ...
            # Make sure that the service belongs to the same docknet namespace as the core-backend. Also, double check that this service really belongs to the project (even though the serviceId should be unique)
            if (
