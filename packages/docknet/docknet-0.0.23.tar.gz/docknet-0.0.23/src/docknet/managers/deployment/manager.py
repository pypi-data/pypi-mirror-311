...
from docknet.schema.shared import ResourceActionExecution
from docknet.utils.auth_utils import parse_userid_from_resource_name
from docknet.utils.id_utils import generate_short_uuid
...
class DeploymentManager(DeploymentOperations):
    ...
    def list_services(
        ...
        # Add services to DB that were not added via the docknet API
            ...
            access_actions.append(
                ResourceAction(
                    ...
                    instructions=[
                        {
                            "type": "new-tab",
                            "url": f"{settings.DOCKNET_BASE_URL}/projects/{project_id}/services/{service_id}/access/{endpoint}",
                        }
                    ],
                )
            )
