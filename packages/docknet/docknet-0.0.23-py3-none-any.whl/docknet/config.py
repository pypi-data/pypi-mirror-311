...
class Settings(BaseSettings):
    ...
    DOCKNET_HOST: Optional[str] = None
    # Set the base url prefix
    DOCKNET_BASE_URL: str = ""
    ...
    # TODO: Maybe only introduce (rename?) a flag indicating whether an external identity provider based on OIDC is used for authentication or whether the default "docknet password flow" will be used
    ...
    def get_redirect_uri(self, omit_host: bool = False) -> str:
        ...
        if not omit_host and not self.DOCKNET_HOST:
            logger.critical(
                "The DOCKNET_HOST configuration is missing and OIDC_AUTH_ENABLED."
            )
            return ""

        if omit_host:
            host = ""
            base_url = "" if not self.DOCKNET_BASE_URL else self.DOCKNET_BASE_URL
        else:
            assert self.DOCKNET_HOST
            host = self.DOCKNET_HOST
            base_url = (
                "" if not self.DOCKNET_BASE_URL else self.DOCKNET_BASE_URL.lstrip("/")
            )
