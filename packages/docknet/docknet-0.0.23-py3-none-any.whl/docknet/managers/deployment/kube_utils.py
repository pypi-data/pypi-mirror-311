...
from docknet.schema import Job, Service
...
from docknet.schema.exceptions import ResourceNotFoundError, ServerBaseError
from docknet.utils.utils import remove_none_values_from_dict
...
def get_pod(
    ...
    Args:
        service_id (str): If deployed via Docknet, corresponds to the deployment id of the pod.
...
def build_kube_service_config(
    ...
    if service.endpoints:
        for endpoint in service.endpoints:
            ...
            # An endpoint can be in the form of one of the examples ["8080", "9001/webapp/ui", "9002b"]. See the Docknet endpoint docs <LINK> for more information.
...
def build_project_network_policy_spec(
    ...
    network_policy = V1NetworkPolicy(
        ...
        spec=V1NetworkPolicySpec(
            ingress=[
                V1NetworkPolicyIngressRule(
                    # Allow traffic from other pods of the same project and the Docknet backend container
                    _from=[
