...
class Esp8266Gateway(GatewayBase, threading.Thread):
    ...
    def __init__(self, subscriber_list, connection_mode='client',
        ...
        if connection_mode == 'server':
            ...
            # wait for ip address message before connecting
            if not wait_for_ip_address:
                # we are not waiting
                if not ip_address:
    ...
    def additional_banyan_messages(self, topic, payload):
        if payload['command'] == 'ip_address':
            if not self.connection_socket:
                try:
                    self.sock.connect((payload['address'], self.ip_port))
                except (ConnectionRefusedError, OSError, KeyError):
                    payload = {'report': 'panic'}
...
def esp8266_gateway():
    ...
    parser.add_argument("-w", dest="wait_for_ip_address", default="False",
                        help="Wait ESP IP address message.")
    ...
    wait_for_ip_address = args.wait_for_ip_address.lower()
    if wait_for_ip_address == 'false':
        wait_for_ip_address = False
    else:
