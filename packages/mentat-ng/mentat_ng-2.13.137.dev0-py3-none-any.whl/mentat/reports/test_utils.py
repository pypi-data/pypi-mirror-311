...
import mentat.const
from mentat.datatype.sqldb import FilterModel, GroupModel, NetworkModel, SettingsReportingModel
...
class TestMentatReportsUtils(unittest.TestCase):
    ...
    def setUp(self):
        ...
        FilterModel(group = group, name = 'FLT2', source_based = True, type = 'basic', filter = 'Category == "Recon.Scanning"', description = 'DESC2', enabled = True)
    ...
    def test_01_generate_cache_keys(self):
        ...
        self.assertEqual(
            ...
            }, '191.113.144.194', False),
            'anomaly-traffic+++191.113.144.194'
        )
        self.assertEqual(
            self.stcache._generate_cache_key({  # pylint: disable=locally-disabled,protected-access
                'Source': [{'IP4': ['195.113.144.104']}],
                '_Mentat': {'EventClass': 'vulnerable-implementation',
                            'EventSubclass': ['cvr:73']}
            }, '195.113.144.104', False),
        ...
        self.assertEqual(
            self.stcache._generate_cache_key({  # pylint: disable=locally-disabled,protected-access
                ...
                '_Mentat': {'TargetClass': 'anomaly-traffic-target',
                            'TargetSubclass': ['123']}
            }, '195.119.144.104', True),
    ...
    def test_02_no_thr_cache(self):
        ...
        self.assertFalse(
            self.ntcache.event_is_thresholded(self.ideas_obj[0], '192.168.1.1', ttltime, False)
        )
        self.ntcache.set_threshold(self.ideas_obj[0], '192.168.1.1', thresholdtime, relapsetime, ttltime, False)
        self.assertFalse(
            self.ntcache.event_is_thresholded(self.ideas_obj[0], '192.168.1.1', ttltime, False)
        )
        self.ntcache.threshold_event(self.ideas_obj[0], '192.168.1.1', 'Test', 'low', datetime.datetime.utcnow(), False)
        self.assertFalse(
            self.ntcache.event_is_thresholded(self.ideas_obj[0], '192.168.1.1', ttltime, False)
        )
    ...
    def test_03_storage_thr_cache(self):
        ...
        self.stcache.set_threshold(self.ideas_obj[0], '192.168.0.2-192.168.0.5', thrtime, reltime, ttltime, False)
        self.assertTrue(
            self.stcache.event_is_thresholded(self.ideas_obj[0], '192.168.0.2-192.168.0.5', ttltime - datetime.timedelta(seconds = 50), False)
        )
        self.assertFalse(
            self.stcache.event_is_thresholded(self.ideas_obj[0], '192.168.0.0/25', ttltime - datetime.timedelta(seconds = 50), False)
        )
        ...
        self.stcache.threshold_event(self.ideas_obj[0], '192.168.0.2-192.168.0.5', 'test@domain.org', 'low', ttltime - datetime.timedelta(seconds = 50), False)
        self.stcache.threshold_event(self.ideas_obj[0], '192.168.0.0/25', 'test@domain.org', 'low', ttltime - datetime.timedelta(seconds = 50), False)
