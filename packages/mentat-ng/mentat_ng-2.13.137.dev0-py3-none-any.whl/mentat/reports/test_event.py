...
class TestMentatReportsEvent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        FilterModel(group = group, name = 'FLT1', source_based = True, type = 'basic', filter = 'Node.Name == "org.example.kippo_honey"', description = 'DESC1', enabled = True)
        FilterModel(group = group, name = 'FLT2', source_based = True, type = 'basic', filter = 'Source.IP4 IN [10.0.0.0/24]', description = 'DESC2', enabled = True)
        FilterModel(group = group, name = 'FLT3', source_based = True, type = 'basic', filter = 'Source.IP4 IN [10.0.1.0/28]', description = 'DESC3', enabled = True)
    ...
    def test_03_filter_events(self):
        ...
        self.assertEqual(list(sorted(aggr_result.keys())), ['recon-scanning'])
    ...
    def test_04_fetch_severity_events(self):
        ...
        self.assertEqual(list(map(lambda x: x['ID'], events)), ['msg02'])
        ...
        self.assertEqual(list(map(lambda x: x['ID'], events)), ['msg01'])
    ...
    def test_07_render_report_extra(self):
        ...
        self.assertIn('https://csirt.cesnet.cz/cs/services/eventclass/fraud-phishing', report_txt_phishing_en)
        ...
        self.assertIn('https://csirt.cesnet.cz/cs/services/eventclass/fraud-phishing', report_txt_phishing_cz)
        ...
        self.assertIn('[1] The machine performed some type of active scanning.', report_txt_scanning_en)
        self.assertIn('https://csirt.cesnet.cz/cs/services/eventclass/recon-scanning', report_txt_scanning_en)
        ...
        self.assertIn('[1] Stroj se pokoušel o nějakou formu aktivního skenování.', report_txt_scanning_cz)
        self.assertIn('https://csirt.cesnet.cz/cs/services/eventclass/recon-scanning', report_txt_scanning_cz)
    ...
    def test_08_render_report_target(self):
        ...
        self.assertIn('https://csirt.cesnet.cz/cs/services/eventclass/recon-scanning-target', report_txt)
    ...
    def _generate_mock_report(self, abuse_group, severity, rtype, events=None, is_target=False):
        ...
        report.statistics = mentat.stats.idea.truncate_evaluations(
            mentat.stats.idea.evaluate_events(events, is_target)
        )
        ...
        for obj in events:
            for src in (jpath_values(obj, 'Source.IP4') + jpath_values(obj, 'Source.IP6')):
                events_aggr[src] = [obj]
        report.structured_data = self.reporter.prepare_structured_data(events_aggr, events_aggr, self.reporting_settings, is_target)
        return report
