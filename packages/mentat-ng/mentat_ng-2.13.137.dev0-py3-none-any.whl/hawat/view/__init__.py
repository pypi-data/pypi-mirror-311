...
class ItemActionView(RenderableView):  # pylint: disable=locally-disabled,abstract-method
    ...
    def inform_about_filter_change(self, _filter, chlog):
        ...
        def _send_email(to, recipient_type=None, locale=None):
            ...
            with force_locale(locale):
                ...
                email_body = flask.render_template(
                    'filters/email_filter_change.txt',
        ...
        try:
            ...
            if maintainer_emails_override:
                flask.current_app.logger.info(
                    "Attempting to send information about the change of filter '{}' to overriden maintainers '{}'.".format(
                        _filter.name, maintainer_emails_override
                    )
                )
                ...
                # Inform maintainers.
                flask.current_app.logger.info(
                    "Attempting to send information about the change of filter '{}' to maintainers '{}'.".format(
                        _filter.name, maintainers
                    )
                )
        ...
        # Inform group managers.
        flask.current_app.logger.info(
            "Attempting to send information about the change of filter '{}' to managers of group '{}'.".format(
                _filter.name, _filter.group.name
            )
        )
        try:
    ...
    def inform_admins(cls, account, form_data):
        ...
        with force_locale(mail_locale):
            email_headers = {
                ...
                'to': flask.current_app.config['EMAIL_ADMINS']
            }
            email_body = flask.render_template(
