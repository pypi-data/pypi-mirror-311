...
from mentat.datatype.sqldb import EventClassModel, FilterModel, GroupModel, ItemChangeLogModel
...
def add_event_classes_rules(event_classes, rules):
    ...
    if source_classes and target_classes:
        rules.append('(_Mentat.EventClass IN ["{}"] OR _Mentat.TargetClass IN ["{}"])'
                     .format('","'.join(source_classes), '","'.join(target_classes)))
    elif source_classes:
...
def process_rule(item):
    ...
    if item.type == REPORTING_FILTER_BASIC:
        ...
        if item.protocols:
            ...
            rules.append('(Source.Proto IN ["{protocols}"] OR Target.Proto IN ["{protocols}"])'.format(protocols=protocols))
...
class ListView(HTMLMixin, SQLAlchemyMixin, ItemListView):
    ...
    def build_query(query, model, form_args):
        ...
        if 'validity' in form_args and form_args['validity']:
            if form_args['validity'] == 'valid':
                query = query.filter(and_(
                    or_(FilterModel.valid_to.is_(None), not_(FilterModel.is_expired)),
                    or_(FilterModel.valid_from.is_(None), not_(FilterModel.will_be_valid))
                ))
        ...
        if 'filtering' in form_args and form_args['filtering']:
        ...
        if 'hits' in form_args and form_args['hits']:
