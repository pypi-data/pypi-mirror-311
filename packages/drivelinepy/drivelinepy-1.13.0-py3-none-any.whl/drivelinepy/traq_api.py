...
class TRAQAPI(BaseAPIWrapper):
    ...
    def get_smart_report_data(self, traq_id, start_date, end_date, training_track):
        ...
        if training_track not in self.VALID_TRAINING_TRACKS:
            raise ValueError("Invalid training_track. Possible options: " + ", ".join(map(str, self.VALID_TRAINING_TRACKS)) + ".")
        else:
            valid_training_track = True
    ...
    def post_smart_report_data(self, traq_id, report_date, training_track, report_data):
        ...
        if not all([
            ...
            training_track in self.VALID_TRAINING_TRACKS
        ]):
            return None
        ...
        #check report date
        if not self.validate_date_yyyy_mm_dd(report_date):
            raise ValueError("Invalid report_date. Must be in YYYY-MM-DD format.")
