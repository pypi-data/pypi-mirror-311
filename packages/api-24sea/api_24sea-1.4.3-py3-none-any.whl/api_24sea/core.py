...
class API:
    ...
    def __init__(self):
        ...
        self._auth: Optional[req.auth.HTTPBasicAuth] = None
    ...
    def authenticate(
        ...
        self._auth = req.auth.HTTPBasicAuth(self._username, self._password)
        ...
        try:
            ...
            if r_profile.ok or self._metrics_overview is not None:
                self._authenticated = True
            logging.info(
                f"\033[32;1m{username} has access to https://api.24sea.eu/.\033[0m"
            )
    ...
    def get_data(
        ...
        # fmt: on
        data_frames = []
        if query.metrics == ["all"] or query.metrics == "all":
            grouped_metrics = pd.DataFrame(columns=["metric"], data=["all"])
            grouped_metrics = pd.DataFrame(
                {
                    "site": [
                        site
                        for site, _ in self._selected_metrics.groupby(
                            ["site", "location"]
                        ).groups.keys()
                    ],
                    "location": [
                        location
                        for _, location in self._selected_metrics.groupby(
                            ["site", "location"]
                        ).groups.keys()
                    ],
                    "metric": ["all"]
                    * len(
                        self._selected_metrics.groupby(
                            ["site", "location"]
                        ).groups
                    ),
                }
            ...
            grouped_metrics = self._selected_metrics.groupby(
                ["site", "location"]
            )
        ...
        if as_dict:
            if as_star_schema:
                ...
                return to_star_schema(
                    data,
                    self.selected_metrics(data).reset_index(names=["metric"]),
                    as_dict=True,
                )
        ...
        if as_star_schema:
            ...
            return to_star_schema(
                data, self.selected_metrics(data).reset_index(names=["metric"])
            )
        return data
...
def to_category_value(
    ...
    if keep_stat_in_metric_name:
        categorized["stat_short_hand"] = categorized["statistic"] \
                                         + "_" \
                                         + categorized["short_hand"]
...
def to_star_schema(
    ...
    if "timestamp" not in data.columns:
        raise ValueError("`data` must include a 'timestamp' column or "
                            "indices convertible to timestamps.")
    ...
    fact_pivot_metrics = (
        fact_metrics.pivot_table(
            index=["timestamp", "location", "data_group"],
            columns="metric_id",
            values="value",
            aggfunc="first",
        )
