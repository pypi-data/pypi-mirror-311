...
class DataSignals:
    ...
    def __init__(self, pandasdata: pd.DataFrame):
        ...
        _auth : req.auth.HTTPBasicAuth, optional
    ...
    def _lazy_authenticate(self) -> Any:
    ...
    def authenticate(
        ...
        self._auth = req.auth.HTTPBasicAuth(username, password)
        if self.__api._authenticated:
            self._authenticated = True
            return None
        self._authenticated = self.__api.authenticate(
    ...
    def as_dict(
        ...
        if self._obj.index.name == "timestamp":
            if "timestamp" in self._obj.columns:
                # The timestamp column is already in the DataFrame
                logging.warning(
                    "\033[33mThe timestamp column is already in "
                    "the DataFrame. Dropping the index."
                )
                self._obj.reset_index(drop=True, inplace=True)
            else:
                self._obj.reset_index(inplace=True)
        ...
        for (s_, l_), group in groups:
            ...
            if (
                os.getenv("24SEA_API_USERNAME") is None
                and os.getenv("24SEA_API_PASSWORD") is None
            ):
                try:
                    __dict[s_][l_].datasignals.__api = C.API()
                    __dict[s_][l_].datasignals.__api._username = self._username
                    __dict[s_][l_].datasignals.__api._password = self._password
                    __dict[s_][l_].datasignals.__api._auth = self._auth
                    __dict[s_][
                        l_
                    ].datasignals.__api._authenticated = self._authenticated
                    __dict[s_][l_].datasignals._username = self._username
                    __dict[s_][l_].datasignals._password = self._password
                    __dict[s_][l_].datasignals._auth = self._auth
                    __dict[s_][
                        l_
                    ].datasignals._authenticated = self._authenticated

                    __dict[s_][
                        l_
                    ].datasignals.__api.metrics_overview = (
                        self.__api.metrics_overview
                    )
