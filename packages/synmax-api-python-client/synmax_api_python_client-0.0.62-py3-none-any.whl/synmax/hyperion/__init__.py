...
from .hyperion_client import HyperionApiClient, ApiPayload  # noqa
...
def add_daily(
    df: pd.DataFrame,
    date_column="date",
    monthly_columns=["gas_monthly", "oil_monthly", "water_monthly"],
    ...
    for index, column in enumerate(monthly_columns):
        if column in df.columns:
            df[daily_columns[index]] = df.apply(
                monthly_to_daily,
                axis=1,
                args=(
                    column,
...
def make_fips():
    ...
    fips_df["name"] = fips_df.apply(
        lambda x: x["name"].replace(" County", "").replace(" Parish", ""), axis=1
    )
