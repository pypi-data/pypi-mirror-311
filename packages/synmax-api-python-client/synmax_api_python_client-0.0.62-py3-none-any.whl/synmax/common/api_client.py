...
class ApiClient(ApiClientBase):
    ...
    def post(
        ...
        with tqdm(
            desc=f"Querying API {url} pages", total=1, dynamic_ncols=True, miniters=0
        ) as progress_bar:
            while (
                not got_first_page
...
#everything below this line is dead, unused code  (async might be used in the future)
    def post_v1(
        ...
        total_pages = (
            total_pages + 1 if total_count % pagination["page_size"] > 0 else 0
        )
        ...
        LOGGER.info(
            "Total data size: %s, total pages to scan: %s", total_count, total_pages
        )
...
class ApiClientAsync(ApiClientBase):
        ...
        async def fetch_from_api(page_number):
            async with semaphore:
                ...
                async with session.post(
                    url, data=_data, timeout=_api_timeout, verify_ssl=False
                ) as async_resp:
    ...
    def post(
        ...
        with tqdm(
            ...
            if total_pages > 1:
                connector = aiohttp.TCPConnector(
                    limit=PARALLEL_REQUESTS, limit_per_host=PARALLEL_REQUESTS
                )
