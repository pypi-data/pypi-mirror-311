...
def create_pos_tagger_model(
    ...
    word_tag_concatanated = tf.keras.layers.Concatenate()(
        [word_embedded, tag_rnn_output]
    )
    ...
    left_context_pos_input = tf.keras.layers.Input(
        shape=(sentence_max_len, pos_vocab_size + 1)
    )
    ...
    right_context_word_td_tags_concatenated = tf.keras.layers.Concatenate()(
        [right_context_word_embedded, right_context_td_tag_rnn_output]
    )
    ...
    current_left_right_concat = tf.keras.layers.Concatenate()(
        [word_tag_concatanated, lc_output, rc_output]
    )
    ...
    pos_label_output = tf.keras.layers.Dense(
        pos_vocab_size + 1, activation="sigmoid"
    )(fc_layer_two)
...
def process_single_word_input(
    ...
    if t == 0:  # there's no left context yet
        ...
        left_context_pos_label_vectors = np.zeros(
            (sentence_max_len, pos_vector_size)
        )
    else:
        ...
        for left_context_pos_label_ in left_context_pos_labels:
            left_context_one_hot_pos_label = tf.keras.utils.to_categorical(
                left_context_pos_label_, num_classes=pos_vector_size
            )
            left_context_pos_label_vectors.append(
                left_context_one_hot_pos_label
            )
        ...
        left_context_pos_label_vectors = np.array(
            left_context_pos_label_vectors_ + left_context_pos_label_vectors
        )
    # ===============================================
    # Right Context
    if t == (
    ...
    left_context_tags = (
        left_context_tags[-sentence_max_len:]
        .reshape(1, sentence_max_len, tag_max_len)
        .astype(int)
    )
    left_context_pos_label_vectors = (
        left_context_pos_label_vectors[-sentence_max_len:]
        .reshape(1, sentence_max_len, pos_vector_size)
        .astype(int)
    )
    ...
    right_context_tags = (
        right_context_tags[:sentence_max_len]
        .reshape(1, sentence_max_len, tag_max_len)
        .astype(int)
    )
