...
class StemmerAnalyzer:
    ...
    def predict(self, sentence: str, batch_size: int = 64) -> List[str]:
        ...
        for token in tokens:
            ...
            candidate_analyzes = (
                self.candidate_generator.get_analysis_candidates(token)
            )
        ...
        ambig_levels = np.array(
            [len(analyses) for analyses in sentence[1]], dtype=np.int32
        )
        ...
        if len(tokens) <= batch_size:
            ...
            probs_of_sentence = tf.constant(
                0, shape=(0, NUM_MAX_ANALYSIS), dtype=tf.float32
            )
            for idx in range(0, len(tokens), batch_size):
                low = idx
                high = low + batch_size
                probs_ = self.model([X_[low:high] for X_ in X])
                probs_of_sentence = tf.concat(
                    (probs_of_sentence, probs_), axis=0
                )
        ...
        ambig_levels_tiled = tf.tile(
            tf.expand_dims(ambig_levels, axis=1), [1, NUM_MAX_ANALYSIS]
        )
