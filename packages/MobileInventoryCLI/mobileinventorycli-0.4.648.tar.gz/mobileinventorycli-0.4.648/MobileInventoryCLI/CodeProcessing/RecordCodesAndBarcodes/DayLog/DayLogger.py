		...
		with Session(ENGINE) as session:
			...
			for tag in tags:
				...
				for num,log in enumerate(search):
					...
					msg=f'''{num}/{num+1} of {ct} -{log}
Next Holiday:{next_holi[1]}
Next Holiday Date:{next_holi[0]}
{log.DayLogDate} Is Holiday:{log.DayLogDate in holidays.USA(years=log.DayLogDate.year)}
Holiday Name:{holidays.USA(years=log.DayLogDate.year).get(log.DayLogDate.strftime("%m/%d/%Y"))}
TAG/INDEX/Count of TTL: {tag} - {num}/{num+1} of {ct}
Name:{log.Name}
Barcode:{log.Barcode}
Code:{log.Code}
DOE:{log.DayLogDate}'''
