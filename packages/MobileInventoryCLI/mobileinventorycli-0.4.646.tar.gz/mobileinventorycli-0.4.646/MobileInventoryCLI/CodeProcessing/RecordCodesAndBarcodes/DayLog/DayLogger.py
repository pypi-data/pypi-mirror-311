		...
		with Session(ENGINE) as session:
			...
			for tag in tags:
				...
				for num,log in enumerate(search):
					msg=f'''{tag} - {num}/{num+1} of {ct} -{log}'''
					print(msg)
					nxt=Prompt.__init2__(None,func=FormBuilderMkText,ptext=f"{log.Name}|{log.Barcode}|{log.Code}|{log.DayLogDate} -> Next?",helpText="hit enter",data="boolean")
					if nxt in [None,]:
						return
					elif nxt in ['d']:
						pass
