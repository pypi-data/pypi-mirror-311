...
class FivetranAdapter(DuckDBAdapter):
    ...
    def __init__(self, config, mp_context: SpawnContext) -> None:
        ...
        config.credentials.path = os.path.join(config.target_path, f"{self.database}.local.db")
        ...
        # table = self.catalog.load_table("pg_public.test_model_2")
        # print(f" test_model_2 ..... {table.metadata_location}")
        # raise ValueError()
    ...
    def _check_s3_location(self, relation):
    ...
    def create_table_as(self, temporary: bool, relation: BaseRelation, sql: str):
        ...
        location_exists = self._check_s3_location(relation)
    ...
    def _delete_iceberg_table(self, relation: BaseRelation, metadata_location: str):
        ...
        objects_to_delete = self.s3_client.list_objects_v2(Bucket=bucket, Prefix=prefix)
        if 'Contents' in objects_to_delete:
            delete_keys = [{'Key': obj['Key']} for obj in objects_to_delete['Contents'] if f"{identifier}/" in obj['Key']]
            self.s3_client.delete_objects(Bucket=bucket, Delete={'Objects': delete_keys})
