...
class FivetranAdapter(DuckDBAdapter):
    ...
    def __init__(self, config, mp_context: SpawnContext) -> None:
        ...
        if secrets.type.lower() == 's3':
            ...
            self.file_io = PyArrowFileIO(properties={
                pyiceberg.io.S3_ACCESS_KEY_ID: secrets.secret_kwargs['key_id'],
                pyiceberg.io.S3_SECRET_ACCESS_KEY: secrets.secret_kwargs['secret'],
                pyiceberg.io.S3_REGION: secrets.secret_kwargs['region']})
        else:
    ...
    def create_table_as(self, temporary: bool, relation: BaseRelation, sql: str):
        ...
        dbg_print(f"{location_exists} | {location_identifier}")
        ...
        self._create_db_view_from_iceberg(table_identifier, table.metadata_location)
    ...
    def _create_db_view_from_iceberg(self, identifier: str, location: str):
    ...
    @available
    def handle_source(self, relation: BaseRelation, fetcher=_get_table_location) -> Optional[BaseRelation]:
        schema = relation.schema
        table = relation.table
        location = fetcher(self, schema, table)
        dbg_print(f"HANDLE SOURCE: {schema} {table} {location}")

        # source_def = context.get_rendered(
        #     "sources",
        ...
        if location:
            ...
            self._drop_db_view_if_exists(identifier)
            self._create_db_view_from_iceberg(identifier, location)
    ...
    def _delete_iceberg_table(self, relation: BaseRelation, metadata_location: str):
        dbg_print(f" DELETE ICEBERG TABLE: {metadata_location}")
        bucket = relation.database
