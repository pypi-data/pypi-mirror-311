        ...
        if self.rest:
            ...
            try:
                with self.engine.connect() as conn:
                    ...
                    # 执行查询
                    # TODO: 应放在ysdb_dialect里实现以便解耦，但需要解决sql解析/传参问题
                    if self.engine.name == "rdb":
                        rdbClient = RdbClient()
                        rdbClient.connect(self.db_url_obj.host, self.db_url_obj.port)
                        ...
                        for data_type in [0, 3]:
                            # e.g. [1,2,3,...]
                            point_ids = rdbClient.getPointIdAll(group=data_type)
                            int_point_infos = rdbClient.getPointInfos(group=data_type, idList=point_ids)
                            ...
                            # 结束时间-1以实现左闭右开
                            hisQueries = [HisQuery(data_type, point_id, b'', int(start_time.timestamp()), 0, int(end_time.timestamp()-1), 0, 0, 0) for point_id in point_ids]
                            retHisDatas = rdbClient.readPointHisDatas(hisQueries)
                            # 从ysdb返回的数据组织为二维数组
                            values = [[item.nVal if data_type == 3 else item.fVal for item in row] for row in retHisDatas]  # FIXME: consider bool types
    ...
    def _build_engine_meta(self) -> MetaData:
        ...
        for column_info in self.repo_meta['columns']:
            ...
            if column_info['type'] == 'string':
                ...
                self.logger.debug(f"Ignored unknown type column {column_info['name']}")
