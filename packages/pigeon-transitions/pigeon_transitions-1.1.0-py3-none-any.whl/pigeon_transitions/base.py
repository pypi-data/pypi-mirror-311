...
class BaseMachine(Machine):
    ...
    def _remap_state(self, state, remap):
        ...
        * Add any on_enter callbacks of the remapped state to the after callbacks of
        the transition.
        * Allow passing a dict as the value in the remap dict which specifies the
        destination, and adds specified conditions, and callbacks to the transition."""
        old_remap = {
            orig: new if isinstance(new, str) else new["dest"]
            for orig, new in remap.items()
        }
        remapped_transitions = super()._remap_state(state, old_remap)
        for orig, new in old_remap.items():
            for transition in remapped_transitions:
                if isinstance(remap[orig], dict):
                    for key, val in remap[orig].items():
                        if key != "dest":
                            transition[key] += listify(val)
                if transition["dest"] == new:
                    transition["before"] += self.states[orig].on_enter
