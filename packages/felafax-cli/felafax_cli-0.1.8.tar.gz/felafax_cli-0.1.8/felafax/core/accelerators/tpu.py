...
class TPUProvider(AcceleratorProvider):
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        while len(self.config.backup_zones) < 2:
            ...
            # TODO: check this only when you have more REGIONS. Right now, goes into infinite loop as it's only 2 regions
            # if (
            #     backup_zone != self.config.zone
            #     and backup_zone not in self.config.backup_zones
            # ):
            #     self.config.backup_zones.append(backup_zone)
    ...
    async def _run_docker_container(self) -> None:
        ...
        logger.info(
            f"Setting up docker container '{container_name}' from image {self.config.docker_image}"
        )

        # Stop and remove existing container if it exists
        await self.run_command(
            [f"sudo docker stop {container_name} 2>/dev/null || true"]
        )
        ...
        result = await self.run_command(
            [f"sudo docker pull {self.config.docker_image}"]
        )
        ...
        if self.config.docker_env:
            ...
            for key, value in self.config.docker_env.items():
                docker_run_cmd += f'-e {key}="{value}" '

        if self.config.attach_disk:
            logger.debug("Mounting persistent disk volume in container")
            docker_run_cmd += "-v /mnt/persistent-disk:/mnt/persistent-disk "

        docker_run_cmd += f"{self.config.docker_image}"

        # Run the container
        logger.info("Starting docker container")
        result = await self.run_command([docker_run_cmd])
