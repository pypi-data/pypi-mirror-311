...
from ..common import require_auth, get_server_uri, get_user_id
import json
...
def list_models():
    ...
    try:
        ...
        typer.echo("\nAvailable Models:")
        typer.echo("-" * 80)
        typer.echo(f"{'Model ID':12} | {'Base Model':20} | {'Status':10} | {'Created At'}")
        typer.echo("-" * 80)
        
        for model in models:
            created_at = model['created_at'].split('T')[0]
            typer.echo(f"{model['model_id']:12} | {model['base_model']:20} | {model['status']:10} | {created_at}")
    ...
    try:
        ...
        while True:
            try:
                ...
                # Send chat request with full message history
                try:
                    response = httpx.post(
