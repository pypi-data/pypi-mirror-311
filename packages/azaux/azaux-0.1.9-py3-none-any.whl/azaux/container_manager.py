...
from azaux.storage_resource import get_endpoint, StorageResource, StorageResourceType
...
def get_service_client(
    account: str, creds: str | AzureNamedKeyCredential, **kwargs: Unpack[ServiceKwargs]
):
    return BlobServiceClient(
        get_endpoint(account, StorageResourceType.blob), credential=creds, **kwargs
    )
    ...
    def client(self):
        ...
        if not self._client_cache:
            service = get_service_client(
                self.account, self.api_key, **self.kwargs
            )
