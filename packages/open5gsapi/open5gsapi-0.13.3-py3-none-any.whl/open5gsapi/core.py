...
class Open5GS:
    ...
    def _check_ue_interfaces(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            # Test the first port (8080) since auto-ue-api.py will always try this first
            url = f"http://10.10.0.132:8080/get_ue_ip"
            try:
                response = requests.get(url, timeout=1)
                if response.status_code == 200:
                    return True, None  # Found at least one running API endpoint
            except RequestException:
                # If first port isn't responding, it means auto-ue-api.py didn't start any Flask apps
                # This only happens when it found no TUN interfaces
                return False, "No TUN interfaces found"
