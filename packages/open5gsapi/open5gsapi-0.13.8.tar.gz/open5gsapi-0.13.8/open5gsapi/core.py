...
class Open5GS:
    ...
    def _check_ue_interfaces(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            # First try to get API response
            url = f"http://10.10.0.132:8080/get_ue_ip"
            try:
                response = requests.get(url)
                if response.status_code == 200:
                    return True, None
            except requests.RequestException:
                # API not responding yet, check if TUN interfaces exist
                result = subprocess.run(
                ...
                try:
                    ...
                    if num_tun_interfaces > 0:
                        # TUN interfaces exist, wait for API to come up
                        return False, "Waiting for UE API endpoints to start..."
                    else:
                        # No TUN interfaces found - real error case
                        return False, "UE API endpoints not accessible"
                except ValueError:
                    # Couldn't get TUN interface count
                    return False, "UE API endpoints not accessible"
