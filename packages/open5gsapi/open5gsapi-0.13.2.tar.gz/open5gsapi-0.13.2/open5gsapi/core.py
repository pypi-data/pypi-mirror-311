...
class Open5GS:
    ...
    def _check_ue_interfaces(self) -> Tuple[bool, Optional[str]]:
        """Check if UE interfaces are properly set up"""
        try:
            # First check how many TUN interfaces exist
            result = subprocess.run(
                ["docker", "exec", "ue", "python3", "-c", "import netifaces; print(len([i for i in netifaces.interfaces() if i.startswith('uesimtun')]))"],
                capture_output=True,
                text=True
            ...
            if not result.stdout.strip():
                logger.error("Empty output when checking TUN interfaces")
                return False, "Failed to check TUN interfaces - no output"
                
            try:
                num_tun_interfaces = int(result.stdout.strip())
            except ValueError:
                logger.error(f"Invalid output when checking TUN interfaces: '{result.stdout}'")
                return False, "Failed to check TUN interfaces - invalid output"
            ...
            for i in range(num_tun_interfaces):
                url = f"http://10.10.0.132:{base_port + i}/get_ue_ip"
                try:
                    response = requests.get(url, timeout=1)
                    if response.status_code == 200:
                        return True, None
