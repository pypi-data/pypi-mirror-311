...
class Open5GS:
    ...
    #     try:
    #         base_port = 8080
    #         ports_to_check = range(base_port, base_port + 3) 
    ...
    def _check_ue_interfaces(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            base_url = f"http://10.10.0.132:8080/get_ue_ip"
            try:
                response = requests.get(base_url, timeout=1)
                if response.status_code == 200:
                    logger.info("UE API endpoint responding successfully")
                    return True, None  # Success case - found working API
            except RequestException:
                pass

            # If base port fails, check if we can see TUN interfaces
            result = subprocess.run(
                ["docker", "exec", "ue", "python3", "-c", 
                "import netifaces; print(len([i for i in netifaces.interfaces() if i.startswith('uesimtun')]))"],
                capture_output=True,
                text=True
            )
            
            if not result.stdout.strip():
                return False, "Failed to check TUN interfaces - no output"
                
            num_interfaces = int(result.stdout.strip())
            if num_interfaces == 0:
                return False, "No TUN interfaces found"
            else:
                # We found TUN interfaces but API isn't responding yet - might still be starting up
                return False, "UE API endpoints starting up..."
                
        except Exception as e:
            logger.error(f"Error checking UE interfaces: {str(e)}")
            return False, f"Error checking UE interfaces: {str(e)}"
...
#---------------------------------------------------------
    ...
    #         else:
    #             data_to_send = data
    #             self.metrics.record_data_sent(data, current_time)
    ...
    #                     return data
    #                 else:
    #                     self.metrics.record_data_received(response.content, receive_time, content_type)
    #                     return data
...
#---------------------------------------------------------
    ...
    def run_background_nodes(self):
        ...
        try:
            ...
            subprocess.Popen(ue_command, shell=True)
            subprocess.Popen(upf_command, shell=True)
            logger.info("Scripts started successfully")
