...
class Logger:
    ...
    :param log_path: Optional[str]: Path to save log file (default: None)
    """
    ...
    def _get_formatter():
        ...
        class CustomFormatter(logging.Formatter):
            def format(self, record):
                ...
                for frame in caller_stack:
                    module = None
                    try:
                        module = inspect.getmodule(frame[2])
                    except KeyError:
                        module = inspect.getmodule(frame[1])
                    if module and not module.__file__.startswith(
