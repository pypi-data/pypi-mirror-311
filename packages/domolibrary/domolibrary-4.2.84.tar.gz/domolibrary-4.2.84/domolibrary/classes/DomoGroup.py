...
# %% auto 0
__all__ = ['DomoGroup', 'DomoGroups', 'Group_UpdateError']
...
# %% ../../nbs/classes/50_DomoGroup.ipynb 31
@dataclass
class DomoGroups:
...
async def search_by_name(
    ...
    if only_allow_one:
        return next((dg for dg in domo_groups if dg.name.lower() == group_name.lower()))

    return [dg for dg in domo_groups if dg.name.lower() == group_name.lower()]
...
async def create_from_name(
    ...
    if is_include_manage_groups_role:
        await domo_group.Membership.add_owner_manage_all_groups_role(debug_api = debug_api, session = session)

    return domo_group

# %% ../../nbs/classes/50_DomoGroup.ipynb 45
class Group_UpdateError(dmde.ClassError):
...
async def upsert(
    ...
    try:
        domo_group = await self.search_by_name(
            auth=self.auth, group_name=group_name, only_allow_one=True, session=session
        )
