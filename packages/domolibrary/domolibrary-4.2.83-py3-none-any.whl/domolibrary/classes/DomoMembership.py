...
class UpdateMembership(de.ClassError):
...
@dataclass
class Membership:
    auth: dmda.DomoAuh = field(repr=False)
...
async def set_owners(
    ...
    for oe in owner_entities:
        ...
        if (
            isinstance(oe.entity, dmdg.DomoGroup)
            and oe.entity.is_system
        ):
            self._add_owner(oe)
            continue
...
@patch_to(GroupMembership)
async def add_owner_manage_all_groups_role(self: GroupMembership):
    import domolibrary.classes.DomoGroup as dmg
