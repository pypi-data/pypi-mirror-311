...
# %% ../../nbs/classes/50_DomoStream.ipynb 7
@dataclass
...
# %% ../../nbs/classes/50_DomoStream.ipynb 9
@dataclass
class StreamConfig:
    stream_category: str
    parent_stream : DomoStream
    ...
    def process_sql(self):
        ...
        self.parent_stream.configuration_query = self.value
        
        for table in parse_one(self.value).find_all(exp.Table):
            
            self.parent_stream.configuration_tables.append(table.name.lower())  
            self.parent_stream.configuration_tables = sorted(list(set(self.parent_stream.configuration_tables)))
            
        return self.parent_stream.configuration_tables
    ...
    def from_json(cls, 
                  obj : dict ,
                  data_provider_type : str,
                 parent_stream : DomoStream = None):
                
                       
        config_name = obj['name']
        
        mapping_enum = StreamConfig_Mappings.search( data_provider_type.lower().replace('-', '_'))
...
# %% ../../nbs/classes/50_DomoStream.ipynb 10
@dataclass
class DomoStream:
