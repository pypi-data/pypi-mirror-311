...
class _Builder():
    devices: list[AUTD3]
    async def open_async(self: _Builder, link: LinkBuilder[L]) -> Controller[L]: ...
    ...
    def with_fallback_parallel_threshold(self: _Builder, fallback_parallel_threshold: int) -> _Builder: ...
    def with_fallback_timeout(self: _Builder, fallback_timeout: Duration) -> _Builder: ...
    ...
    @property
    def fallback_parallel_threshold(self: _Builder) -> int: ...
    @property
    def fallback_timeout(self: _Builder) -> Duration: ...
    @property
...
class _GroupGuard(Generic[K]):
    ...
    _keymap: dict[K, int]
    _k: int
    async def send_async(self: _GroupGuard[K], ) -> None: ...
...
class Controller(Geometry, Generic[L]):
    ...
    _link: L
    _disposed: bool
    async def _open_impl_async(self: Controller[L], link_builder: LinkBuilder[L], timeout: Duration | None = None) -> Controller[L]: ...
    async def firmware_version_async(self: Controller[L], ) -> list[FirmwareInfo]: ...
    async def close_async(self: Controller[L], ) -> None: ...
    async def fpga_state_async(self: Controller[L], ) -> list[FPGAState | None]: ...
    async def send_async(self: Controller[L], d: Datagram | tuple[Datagram, Datagram]) -> None: ...
    def __init__(self: Controller[L], geometry: GeometryPtr, runtime: RuntimePtr, handle: HandlePtr, ptr: ControllerPtr, link: L) -> None: ...
