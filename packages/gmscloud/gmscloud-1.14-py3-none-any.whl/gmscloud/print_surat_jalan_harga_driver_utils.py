...
def print_surat_jalan_harga_driver(doc_id, selected_printer, company_code):
    ...
    for result in json_obj['barang']:
        total = int(result['SURAT_JALAN_BARANG_HARGA']) * int(result['SURAT_JALAN_BARANG_QUANTITY'])
        grandtotal += total
        table.rows.append([result['barangSuratJalan']['MASTER_BARANG_NAMA'], format(int(result['SURAT_JALAN_BARANG_HARGA']),',d'), result['SURAT_JALAN_BARANG_QUANTITY'],format(int(total),',d')])
    table.columns.alignment = BeautifulTable.ALIGN_RIGHT

    barang_ttbk = BeautifulTable()
    barang_ttbk.columns.width = 40
    barang_ttbk.columns.header = ["                 Nama Barang                ", "             QTY            "]
    for result in json_obj['barang']:
        barang_ttbk.rows.append([result['barangSuratJalan']['MASTER_BARANG_NAMA'], ""])
    ...
    judul_ttbk = BeautifulTable()
    judul_ttbk.columns.width = 80
    judul_ttbk.columns.header = ["TTBK SURAT JALAN\nTANDA TERIMA TABUNG KOSONG"]
    judul_ttbk.rows.append(["                                                                      "])
    judul_ttbk.set_style(BeautifulTable.STYLE_NONE)
    judul_ttbk.columns.alignment = BeautifulTable.ALIGN_CENTER
    total = BeautifulTable()
    total.columns.width = 80
    total.rows.append(["", "", "Total (Rp)", format(int(grandtotal), ',d')])
    total.rows.append(["                     ", "               ", "             ", "                "])
