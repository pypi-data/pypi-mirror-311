...
def print_faktur_group(doc_id, selected_printer, company_code):
    ...
    for sj in json_obj.get('suratJalanData', []):
        for barang in sj.get('suratjalanbarang', []):
            ...
            harga_barang = int(barang.get('SURAT_JALAN_BARANG_HARGA', 0))
            quantity = int(barang.get('SURAT_JALAN_BARANG_QUANTITY', 0))
            total_harga = int(barang.get('SURAT_JALAN_BARANG_HARGA_TOTAL', 0))
    ...
    for (nama_barang, harga_barang), values in grouped_data.items():
        barang_table.rows.append([
            ...
            format(harga_barang, ',d'),  # Harga barang
            values['quantity'],  # Total quantity
            format(values['total_price'], ',d')  # Total harga
        ])
    ...
    ttd.rows.append(["\n", "\n", "\n"])
    ttd.rows.append(["                         ", "                         ", "                         "])
    ttd.rows.append([ "", "", user.get('USER_NAMA', '') ])
    ...
    transaksi.rows.append(["","","Total (Rp)", format(int(json_obj['dataTransaksi']['FAKTUR_TRANSAKSI_TOTAL']), ',d')])
    transaksi.rows.append(["", "", "Pajak (Rp)", format(int(json_obj['dataTransaksi']['FAKTUR_TRANSAKSI_PAJAK_RUPIAH']), ',d')])
    transaksi.rows.append(["", "", "Grand (Rp)", format(int(json_obj['dataTransaksi']['FAKTUR_TRANSAKSI_GRAND_TOTAL']), ',d')])
