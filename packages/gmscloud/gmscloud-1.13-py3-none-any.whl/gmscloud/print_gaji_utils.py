...
def print_gaji(doc_id, selected_printer, company_code):
    ...
    if json_obj['data']['GAJI_BONUS'] == "0":
        print('tidak ada')
    else:
        sj.rows.append(["Bonus", format(int(json_obj['data']['GAJI_BONUS']),',d')])

    if json_obj['data']['GAJI_PREMI_PENGANTARAN_RUPIAH'] == "0":
        print('tidak ada')
    else :
        sj.rows.append(["Premi Pengantaran", format(int(json_obj['data']['GAJI_PREMI_PENGANTARAN_RUPIAH']),',d')])

    if json_obj['data']['GAJI_PREMI_PRODUKSI_RUPIAH'] == "0":
        print('tidak ada')
    else :
...
# Menyusun header
    ...
    judul.columns.header = ["SLIP GAJI"]
    judul.rows.append([f"                          Bulan : {json_obj['data']['GAJI_BULAN']} ; Tahun : {json_obj['data']['GAJI_TAHUN']}                          "])
    ...
    with open(f'{doc_id}.txt', 'w') as f:
        ...
        f.write("\n")
        f.write(f"{json_obj['perusahaan']['PERUSAHAAN_TELP']}")
        f.write("\n")
        f.write("\n")
        f.write(str(judul))
        f.write("\n")
        f.write(f"Nama : {json_obj['data']['karyawan']['MASTER_KARYAWAN_NAMA']}")
        f.write("\n")
        f.write("\n")
        f.write(str(sj))
        f.write("\n")
