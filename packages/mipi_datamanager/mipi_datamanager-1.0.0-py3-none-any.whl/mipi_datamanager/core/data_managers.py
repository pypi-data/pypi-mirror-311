...
from mipi_datamanager.core.common import _maybe_rename_values
...
class _DMConfig:
    def __init__(self,
                 ...
                 store_all_dfs: bool,
                 store_base_df: bool,
                 default_format_func_dict: dict | FormatDict,
                 insert_table_type: str,
                 insert_table_name: str,
                 dialect: str):

        self.jinja_repo_source = jinja_repo_source
        self.store_all_dfs = store_all_dfs
        self.store_base_df = store_base_df
        self.default_format_func_dict = _maybe_convert_to_format_dict(default_format_func_dict)
        self.insert_table_type = insert_table_type
    ...
    def from_setup(cls,
        ...
        _user_setup = _DMConfig._load_user_setup(setup_path)
        _jinja_repo_source = override_jinja_repo_source if override_jinja_repo_source is not None else getattr(
            _user_setup, 'set_jinja_repo', None)
        _insert_table_type = override_insert_table_type if override_insert_table_type is not None else getattr(
            _user_setup, 'set_insert_table_type', None)
        _insert_table_name = override_insert_table_name if override_insert_table_name is not None else getattr(
            _user_setup, 'set_insert_table_name', None)
        _store_all_dfs = override_store_all_dfs if override_store_all_dfs is not None else getattr(_user_setup,
                                                                                                 'set_store_all_dfs',
                                                                                                 None)
        _store_base_df = override_store_base_df if override_store_base_df is not None else getattr(_user_setup,
                                                                                                   'set_store_base_df',
                                                                                                   None)
        _dialect = override_dialect if override_dialect is not None else getattr(_user_setup, 'set_dialect', None)

        if override_default_format_func_dict:
            _dict = override_dialect
            if extend_default_format_func_dict:
                raise ValueError(
                    "Invalid parameter combination. Can not use override and extend default_format_func_dict together.")
        else:
            if _user_setup.set_default_format_func_dict:
                _dict = _maybe_convert_to_format_dict(_user_setup.set_default_format_func_dict)
            else:
                _dict = FormatDict({})
            if extend_default_format_func_dict:
                _extend_dict = _maybe_convert_to_format_dict(extend_default_format_func_dict)
            else:
                _extend_dict = FormatDict({})

            _dict.update(_extend_dict)

        return cls(_jinja_repo_source, _store_all_dfs, _store_base_df, _dict, _insert_table_type,
                   _insert_table_name, _dialect)

    @staticmethod
    def _load_user_setup(setup_file_path: str | None = None):
        """
        Loads the user setup from mipi_setup.py. Allows an optional path to the setup file with highest priority.

        Args:
            setup_file_path (str, optional): Absolute path to mipi_setup.py. Highest priority.

        Raises:
            FileNotFoundError: If mipi_setup.py is not found in any of the searched directories.

        Returns:
            module: Imported mipi_setup module.
        """
        import importlib.util
        import os
        from pathlib import Path
        ...
        # 1. If setup_file_path is provided, add its directory as the first priority
        if setup_file_path is not None:
            setup_dir = os.path.dirname(os.path.abspath(setup_file_path))
            possible_paths.append(setup_dir)

        # 2. Current working directory (takes precedence over environment variable)
        possible_paths.append(os.getcwd())

        # 3. Directory of this file (where the package module is located)
        package_dir = os.path.dirname(os.path.abspath(__file__))
        possible_paths.append(package_dir)

        # 4. Path specified by an environment variable
        env_path = os.environ.get('MIPI_SETUP_PATH')
        if env_path:
            possible_paths.append(env_path)

        # 5. Virtual environment directory (where the package is installed)
        ...
        raise FileNotFoundError("No file found named mipi_setup.py")
...
class DataManager:
    ...
    def __init__(self, frame: meta._Frame, setup: _DMConfig):
    ...
    def from_jinja_repo(cls, inner_path: str,
        ...
        """
        _setup = _DMConfig(jinja_repo_source, store_all_dfs, store_base_df, default_format_func_dict,
                           insert_table_type, insert_table_name, dialect)
        config = _get_config_from_master(inner_path, _setup.jinja_repo_source)

        if not config["meta"]["population"]:
            raise ConfigError(f'expected population status is True script got: {config["meta"]['population']}')

        con = _setup.jinja_repo_source.conn_dict[config['meta']['connection']]
        df, sql = _get_df_and_sql_from_jinja_repo(_setup.jinja_repo_source, inner_path, con, jinja_parameters_dict)
    ...
    def from_jinja_repo_preset(cls, inner_path: str,
        ...
        """
        _setup = _DMConfig.from_setup(override_jinja_repo_source, override_store_all_dfs, override_store_base_df,
                                      override_default_format_func_dict, extend_default_format_func_dict,
                                      override_insert_table_type, override_insert_table_name, override_dialect,
                                      setup_path)
        config = _get_config_from_master(inner_path, _setup.jinja_repo_source)
    ...
    def from_dataframe_preset(cls, df: pd.DataFrame, frame_name: str = None,
        ...
        _setup = _DMConfig.from_setup(override_jinja_repo_source, override_store_all_dfs, override_store_base_df,
                                      ...
                                      extend_default_format_func_dict, override_insert_table_type,
                                      override_insert_table_name, override_dialect, setup_path)
    ...
    @classmethod
    def from_sql_preset(cls, script_path: str, connection: connection.Odbc, format_parameters_list: list = None,
                        frame_name: str = None,
                        override_jinja_repo_source: JinjaRepo = None,
                        override_store_all_dfs: bool = None, override_store_base_df: bool = None,
                        override_dialect: str = None,
    ...
    @classmethod
    def from_jinja_preset(cls, script_path: str, connection: connection.Odbc,
                          jinja_parameters_dict: dict = None,
                          frame_name: str = None,
                          override_jinja_repo_source: JinjaRepo = None,
                          override_store_all_dfs: bool = None, override_store_base_df: bool = None,
    ...
    @classmethod
    def from_excel(cls, excel_path: str, excel_sheet: str | int | None = None,
                   frame_name: str = None,
                   jinja_repo_source: JinjaRepo = None,
                   store_all_dfs: bool = False, store_base_df: bool = False,
                   dialect: str = "mssql",
    ...
    @classmethod
    def from_excel_preset(cls, excel_path: str, excel_sheet: str | int | None = None, frame_name: str = None,
                          override_jinja_repo_source: JinjaRepo = None,
                          override_store_all_dfs: bool = None, override_store_base_df: bool = None,
                          override_dialect: str = None,
                          override_insert_table_type=None,
    ...
    def join_from_jinja_repo(self, inner_path: str, how: JoinLiterals = "left", frame_name=None,
        ...
        config = _get_config_from_master(inner_path, self.setup.jinja_repo_source)  # TODO replace with JinjaRepo.pull
        ...
        con = self.setup.jinja_repo_source.conn_dict[config['meta']['connection']]
    ...
    def join_from_excel(self, excel_path: str, on: str = None, how: JoinLiterals = "left",
    ...
    def join_from_format_sql(self, script_path: str, connection: connection.Odbc, on: str | tuple = None,
        ...
        sql = self.resolve_dimension_format_sql_file(script_path, _on, format_parameters_list,
    ...
    def join_from_jinja(self, script_path: str, connection: connection.Odbc, on: str | tuple = None,
        ...
        sql = self.resolve_dimension_jinja_file(script_path, _on, jinja_parameters_dict=jinja_parameters_dict,
                                                rename_columns_dict=_rename_dict)
        df = query.execute_sql_string(sql, connection)
    ...
    def clone(self, base_name: str = None,
        ...
        _setup = _DMConfig(_jinja_repo_source, _store_all_dfs, _store_base_df, new_format_dict,
                           _insert_table_type, _insert_table_name,
                           _dialect)
    ...
    def generate_inserts(self, key: str, frame: str | int = None,
    ...
    def _get_temp_table_records2(self, key, frame=None, rename_columns_dict: dict | None = None):
        ...
        warnings.warn("build_temp_table = False will be depricated in a future version", DeprecationWarning,
                      stacklevel=2)
        df = frame.df_query if frame is not None else self.trgt
    ...
    def resolve_dimension_jinja_repo_template(self, inner_path: str, key: str | list | tuple,
        ...
        """
        jenv = JinjaRepo(self.setup.jinja_repo_source.root_dir)
