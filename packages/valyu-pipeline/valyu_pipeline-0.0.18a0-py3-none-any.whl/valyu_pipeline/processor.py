...
class PDFProcessor:
    ...
    def __init__(
        ...
        if self.output not in ['parquet', 'json', 'xml']:
            raise ValueError("Output format must be either 'parquet', 'json' or 'xml'")
    ...
    def start_job(self, folder_path: str) -> Dict[str, Dict[str, Any]]:
        """
    ...
    def _download_results(self, presigned_urls: Dict[str, Dict[str, str]], output_dir: str = "results") -> None:
        ...
        for pdf_name, urls in presigned_urls.items():
            ...
            if urls['text_url']:
                ...
                print(f"✓ Downloaded text for {pdf_name}")
            ...
            if urls['image_url']:
                ...
                with open(image_path, 'wb') as f:
                    f.write(response.content)
                print(f"✓ Downloaded images for {pdf_name}")
            ...
            if urls['markdown_url']:
                ...
                with open(markdown_path, 'wb') as f:
                    f.write(response.content)
                print(f"✓ Downloaded markdown for {pdf_name}")
