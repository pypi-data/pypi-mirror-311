...
def sort(key_arrs, lo, hi, data):  # pragma: no cover
    ...
    (stackSize, runBase, runLen, tmpLength, tmp, tmp_data, minGallop) = init_sort_start(
        key_arrs, data
    )
...
def countRunAndMakeAscending(key_arrs, lo, hi, data):  # pragma: no cover
    ...
    if getitem_arr_tup(key_arrs, runHi) < getitem_arr_tup(key_arrs, lo):  # Descending
        runHi += 1
        while runHi < hi and getitem_arr_tup(key_arrs, runHi) < getitem_arr_tup(
            key_arrs, runHi - 1
        ):
            runHi += 1
...
def init_sort_start(key_arrs, data):  # pragma: no cover
    ...
    tmpLength = (
        arr_len >> 1
        if arr_len < 2 * INITIAL_TMP_STORAGE_LENGTH
    ...
    stackLen = (
        5
        if arr_len < 120
        else (10 if arr_len < 1542 else (19 if arr_len < 119151 else 40))
    )
...
def mergeCollapse(
    ...
    while stackSize > 1:
        ...
        if (n >= 1 and runLen[n - 1] <= runLen[n] + runLen[n + 1]) or (
            n >= 2 and runLen[n - 2] <= runLen[n] + runLen[n - 1]
        ):
...
def mergeAt(
    ...
    len2 = gallopLeft(
        getitem_arr_tup(key_arrs, base1 + len1 - 1), key_arrs, base2, len2, len2 - 1
    )
    assert len2 >= 0
    if len2 == 0:
        return stackSize, tmpLength, tmp, tmp_data, minGallop
    ...
    if len1 <= len2:
        tmpLength, tmp, tmp_data = ensureCapacity(
            tmpLength, tmp, tmp_data, key_arrs, data, len1
        )
        minGallop = mergeLo(
        ...
        tmpLength, tmp, tmp_data = ensureCapacity(
            tmpLength, tmp, tmp_data, key_arrs, data, len2
        )
        minGallop = mergeHi(
            key_arrs, data, tmp, tmp_data, minGallop, base1, len1, base2, len2
        )
...
def mergeLo(
    ...
    len1, len2, cursor1, cursor2, dest, minGallop = mergeLo_inner(
        key_arrs, data, tmp_data, len1, len2, tmp, cursor1, cursor2, dest, minGallop
    )
    # XXX *****************
    ...
    while True:
        ...
        while True:
            ...
            count1 = len1 - gallopRight(
                getitem_arr_tup(tmp, cursor2), arr, base1, len1, len1 - 1
            )
            if count1 != 0:
                dest -= count1
                cursor1 -= count1
            ...
            count2 = len2 - gallopLeft(
                getitem_arr_tup(arr, cursor1), tmp, 0, len2, len2 - 1
            )
            if count2 != 0:
                dest -= count2
                cursor2 -= count2
