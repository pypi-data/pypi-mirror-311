...
class TextDetector(object):
    def __init__(self, args, logger=None):
        ...
        pre_process_list = [
            {
                "DetResizeForTest": { #FOR CNN, DOESNT HAVE THAT MUCH OF AN IMPACT (resizes so that it can be divided by 32)
    ...
    def predict(self, img):
        ...
        if img is None:
            return None, 0
        img = np.expand_dims(img, axis=0) # expands by a little for conv layers (to be divided by 32)
