...
class OCRDetector:
    def __init__(self, config_path=None, ocr_type=None):
        if config_path==None:
    ...
    def get_config_file_path(self):
        return str(resources.files("deteasypaddleocr") / "config.json")
    
    def get_model_path(self, what_ocr):
        base_path = str(resources.files("deteasypaddleocr"))
        model_path = os.path.join(base_path, "models", what_ocr)
        return model_path
    ...
    def config_loader(self, config_path, ocr_type=None):
        ...
        self.model_path = self.get_model_path(self.ocr_type)
    ...
    def download_model_from_azure(self, model_path):
        ...
        if self.ocr_type=="easyocr":
            ...
            model_download_path = model.download(target_dir=os.path.dirname(model_path), exist_ok=True)
        return model_download_path 

    def initialize_model(self):
        if self.ocr_type == "easyocr":
            from deteasypaddleocr.custom_easyocr.easyocr_custom_lib import Reader
            return Reader(['en'], model_storage_directory=self.model_path, gpu=True)
        elif self.ocr_type == "paddleocr":
            from deteasypaddleocr.custom_paddleocr.paddleocr_custom_lib import PaddleOCR
...
def process_image(image_path, config_path=None, target_width=None, target_height=None, ocr_type=None): # if 'ocr_type' not specified = easyocr by default (in config)
