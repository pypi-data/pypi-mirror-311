...
class Settings:
    """
    Dataclass for holding ClientStore environment variables with defaults
    """

    verify_ssl: bool = True
    log_level: int = logging.INFO
    settings_path: Path = Path("e360.ini")
    ...
    def __post_init__(self) -> None:
        ...
        if not (self.is_api_gateway_mode or self.is_oidc_mode):
            raise ValueError(
                f"Settings not valid for either Api Gateway or OIDC mode. Please review and try again. {self}"
            )

    @property
    def is_api_gateway_mode(self) -> bool:
        return bool(
            self.api_gateway_url and self.api_gateway_key and not self.use_oidc_mode
        )

    @property
    def is_oidc_mode(self) -> bool:
        return bool(
            self.oidc_endpoint_url
            and self.oidc_client_id
            and self.oidc_client_secret
            and self.service_directory_url
            and self.use_oidc_mode
        )
