...
class InferenceDatabaseRecords(InferenceRecords):
    ...
    rows: List[Any]
    records_schema: Optional[List[Dict[str, Any]]]
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(self.rows, columns=column_names)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        """
        return {
            "result":[dict(zip(self._get_column_names(), row)) for row in self.rows],
        }
...
class InferenceService(BasePydanticInfo):
    ...
    def get_inference_records(
        ...
        if isinstance(connection_config, LoggerConnectionDatabaseConfig):
            ...
            return InferenceDatabaseRecords(rows = rows, records_schema=json.loads(records_schema_str))
