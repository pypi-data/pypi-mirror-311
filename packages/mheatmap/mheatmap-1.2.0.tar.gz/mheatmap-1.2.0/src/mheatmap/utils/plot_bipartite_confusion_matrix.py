...
def plot_bipartite_confusion_matrix(reordered_cm, reordered_labels, epsilon=1):
    ...
    node_sizes = {
        ...
        **{f'Pred_{i}': 2000 * (s/np.max(pred_sizes)) for i, s in enumerate(pred_sizes)}
    }
    
    # Configure plot layout
    plt.figure(figsize=(15, 10))
    plt.title("Confusion Matrix Bipartite Graph", fontsize=16, pad=20, fontweight='bold')
    plt.axis('off')
    ...
    edge_labels = {edges[i]: f'{edge_percentages[i]:.1f}%' 
