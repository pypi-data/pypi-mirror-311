...
def copermute_from_bipermute(
    B_sizes: list[int],
    B_subrows: np.ndarray,
    B_subcols: np.ndarray,
    p_Asub: np.ndarray
    ...
    bi_marker = np.concatenate([
        np.ones(nr_Bsub, dtype=int),
        2 * np.ones(nc_Bsub, dtype=int)
    ])
    ...
    p_Brows = np.concatenate([
        pr_Bsub,
        np.setdiff1d(np.arange(0, nr_B), pr_Bsub)
    ])
    
    p_Bcols = np.concatenate([
        pc_Bsub,
        np.setdiff1d(np.arange(0, nc_B), pc_Bsub)
    ])
