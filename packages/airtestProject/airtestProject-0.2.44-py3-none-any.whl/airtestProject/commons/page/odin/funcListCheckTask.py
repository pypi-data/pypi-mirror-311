...
class FuncListCheckPage:
    ...
    def open_list(self, list_pos, next_pos, fun_name="air", ocrPlus=True):
        ...
        def _open_list():
            if operate(fun_name).wait_element_appear(list_pos, ocrPlus=ocrPlus):
                if operate(fun_name).wait_next_element(list_pos, next_pos, ocrPlus=ocrPlus) is False:
                    log.error('打开列表失败')
            else:
...
class FuncListCheckTask(TaskCaseTemplate, FuncListCheckPage):
    ...
    def scenes_jump(self):
        ...
        super().check_scenes("斗罗场景", "WaitingBar")
        super().open_list("Scene", "场景跳转")
        super().check_scenes("TestShader", "WaitingBar")
        super().open_list("Scene", "场景跳转")
        super().check_scenes("20003场景 (流式加载)", "WaitingBar")
        super().open_list("Scene", "场景跳转")
        super().check_scenes("斗罗场景 (流式加载)", "WaitingBar")
    ...
    def sound_setting_view(self):
        super().open_list("Func", "功能列表")
        super().open_ui_from_ui_list("soudsetingview", "MusicSettingInterface", "FuncListSwipe", "创建怪物", "多主键配置")
