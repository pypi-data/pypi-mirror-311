...
class myAirTest(OperateABC):
    ...
    def click(self, pos, times=1, rgb=False, translucent=False, ocr_plus=False, ocr_match_type=False, **kwargs):
        try:
            ...
            value = self.check_value(pos,  rgb=rgb, translucent=translucent, ocrPlus=ocr_plus,
    ...
    def _click_nolog(self, pos, times=1, ocrPlus=False, ocr_match_type=False, rgb=False, translucent=False, **kwargs):
        try:
            log.info(f"click_nolog  pos: {pos}")
            kwargs.pop('focus', None)
            kwargs.pop('sleep_interval', None)
            value = self.check_value(pos, ocrPlus, rgb=rgb, translucent=translucent, ocr_match_type=ocr_match_type)
    ...
    def swipe(self, value, v2=None, vector_direction=None, ocrPlus=False, ocr_match_type=False,
        ...
        value = self.check_value(value, ocrPlus=ocrPlus, ocr_match_type=ocr_match_type, rgb=rgb, translucent=translucent)
        if v2 is not None:
            v2 = self.check_value(v2, ocrPlus=ocrPlus, rgb=rgb, translucent=translucent, ocr_match_type=ocr_match_type)
    ...
    def swipe_plus(self, value, v2=None, down_event_sleep=None, vector_direction=None, ocrPlus=False, rgb=False,
        ...
        value = self.check_value(value, ocrPlus=ocrPlus, rgb=rgb, translucent=translucent, ocr_match_type=ocr_match_type)
        if isinstance(value, (myTemplate, OcrTemplate)):
            ...
            pos1 = value
        if v2:
            v2 = self.check_value(v2, ocrPlus=ocrPlus, ocr_match_type=ocr_match_type, rgb=rgb, translucent=translucent)
    ...
    def wait(self, pos, timeout=None, interval=0.5, ocrPlus=False, intervalfunc=None, rgb=False, translucent=False,
             ...
             **kwargs):
        try:
            pos = self.check_value(pos, ocrPlus=ocrPlus, ocr_match_type=ocr_match_type, rgb=rgb, translucent=translucent)
            return air.wait(pos, timeout, interval, intervalfunc)
        except Exception as e:
            log.warn(f"failed to input {pos} on the target device")
            return False

    def _wait_nolog(self, pos, timeout=None, interval=0.5, ocrPlus=False, intervalfunc=None, rgb=False,
                   translucent=False, ocr_match_type=False, **kwargs):
        try:
            pos = self.check_value(pos, ocrPlus=ocrPlus, ocr_match_type=ocr_match_type,rgb=rgb, translucent=translucent)
    ...
    def exists(self, pos, ocrPlus=False, ocr_match_type=False, rgb=False, translucent=False):
        try:
            pos = self.check_value(pos, ocrPlus, rgb=rgb, translucent=translucent, ocr_match_type=ocr_match_type)
    ...
    def check_value(self, val, ocrPlus=False, ocr_match_type=False, threshold=0.7, translucent=False, rgb=False):
        """
        ...
        while not fight_stop_even.is_set():
            if fight_time_out > 0:
                ...
                for check_pos in check_pos_list:
                    result = self._wait_nolog(check_pos, timeout=check_stop_time)
