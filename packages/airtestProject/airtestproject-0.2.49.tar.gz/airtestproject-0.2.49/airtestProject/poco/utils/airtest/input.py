...
from airtestProject.airtest.core.api import device as current_device
from airtestProject.airtest.core.api import touch, swipe, double_click, device
from airtestProject.airtest.core.android import Android
from airtestProject.airtest.core.helper import device_platform, logwrap
from airtestProject.poco.sdk.interfaces.input import InputInterface
...
class AirtestInput(InputInterface):
    ...
    def swipe_along(self, x1, y1, coordinates_list, duration=2.0):
        if duration <= 0:
            raise ValueError("Operation duration cannot be less equal 0. Please provide a positive number.")
        pos = self.get_target_pos(x1, y1)
        steps = int(duration * 40) + 1
        coordinates_list.insert(0, pos)
        if isinstance(device(), Android):
            device().swipe_along(coordinates_list, duration=duration, steps=steps)
            return True
        else:
            raise TypeError("Currently, it does not support long-distance sliding on devices other than Android")
