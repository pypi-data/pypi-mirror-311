...
from airtestProject.airtest.core.cv import Template, loop_find, try_log_screen, loop_find_nolog
...
from airtestProject.airtest.utils.compat import script_log_dir
from airtestProject.airtest.utils.snippet import parse_device_uri
from airtestProject.airtest.core.helper import (G, delay_after_operation, import_device_cls,
                                                logwrap, set_logdir, using, log)
# Assertions
from airtestProject.airtest.core.assertions import (assert_exists, assert_not_exists, assert_equal,
...
def touch_nolog(v, times=1, **kwargs):
    """
    Perform the touch action on the device screen
    ...
    """
    if isinstance(v, Template):
        pos = loop_find_nolog(v, timeout=ST.FIND_TIMEOUT)
    else:
        pos = v
    for _ in range(times):
        # If pos is a relative coordinate, return the converted click coordinates.
...
def wait(v, timeout=None, interval=0.5, intervalfunc=None):
    ...
    pos = loop_find_nolog(v, timeout=timeout, interval=interval, intervalfunc=intervalfunc)
    return pos


def wait_nolog(v, timeout=None, interval=0.5, intervalfunc=None):
    """
    Wait to match the Template on the device screen
