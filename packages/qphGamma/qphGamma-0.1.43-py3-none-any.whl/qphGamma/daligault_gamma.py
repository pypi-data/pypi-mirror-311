...
class DaligaultGamma:
    ...
    def get_gamma(self):
        ...
        #! Use E, where the Ef/mu is **NOT** set to 0 eV
        fd_occ = logistic.sf(self.E, self.mu, self.Te)

        E_mu = self.E - self.mu  # eV
        # E_gamma = np.array([E_mu, gamma_list]).T  # eV and 1/fs
        # E_dos = np.array([E_mu, fg_dos.dos]).T
        # E_occ = np.array([E_mu, fd_occ]).T

        # return E_gamma, E_dos, E_occ
        return E_mu, gamma_list, fg_dos.dos, fd_occ
