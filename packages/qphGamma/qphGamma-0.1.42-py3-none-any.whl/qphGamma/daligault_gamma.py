...
def get_gamma_omega_p(q_kF_list, k_kF_list, n, theta, theta_cutoff):
    # * Assign values independent of k and q
    kF = get_kF(n)
    mu = get_mu(n, theta, theta_cutoff)
...
class DaligaultGamma:
    ...
    def __init__(
        ...
        self.mu = get_mu(self.n, self.theta, self.theta_cutoff) * J_to_eV
        print(f"mu={self.mu}")
        print(f"V={self.V}")
    ...
    def get_gamma(self):
        ...
        k_kF_list = (np.sqrt(2 * m_e * self.E / J_to_eV) / hbar_Js) / get_kF(self.n)

        gamma_list = (
            np.array(
                get_gamma_omega_p(
                    self.q_kF_list, k_kF_list, self.n, self.theta, self.theta_cutoff
                )
            )
            * get_omega_p(self.n)
            * 1e-15
        ...
        #! This DOS is not set so that Ef=0 eV --> this is done later
        fg_dos = FermiGasDOS(self.E, self.V)

        mu = get_mu(self.n, self.theta, self.theta_cutoff) * J_to_eV  # eV
        E_mu_list = self.E - mu  # eV

        #! Use E-mu for energy, so mu=0 eV
        fd_occ = logistic.sf(E_mu_list, 0, self.Te)

        E_gamma = np.array([E_mu_list, gamma_list]).T  # eV and 1/fs
        E_dos = np.array([E_mu_list, fg_dos.dos]).T
        E_occ = np.array([E_mu_list, fd_occ]).T

        return E_gamma, E_dos, E_occ
