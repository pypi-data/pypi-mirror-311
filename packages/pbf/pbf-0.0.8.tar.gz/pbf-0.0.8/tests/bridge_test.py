...
class BridgeTest(unittest.TestCase):
    def test(self):
        ...
        layerHeight = 30 * units.um

        recoaterLevel = 20 * layerHeight
        totalHeight = 30 * layerHeight
        ...
        part = pbf.implicitIntersection([pbf.implicitTransformation(cylinder, transformation),
                                         pbf.implicitHalfspace([0.0, 0.0, recoaterLevel - layerHeight],
                                                               [0.0, 0.0, 1.0])])

        # Setup problem
        filebase = "outputs/pbftests/bridge"
        grid = pbf.createMesh(domainMin, domainMax, elementSize * 2 ** trefinement, layerHeight=layerHeight,
                              zfactor=0.5)

        setup = pbf.ProcessSimulation(grid)
        materials = {"powder": pbf.makePowder(material), "structure": material, "baseplate": material,
                     "air": pbf.makeAir()}

        setup.setMaterials(materials)
        ...
        tsetup.addPostprocessor(pbf.materialVtuOutput(filebase, interval=2))
        tsetup.addPostprocessor(
            pbf.thermalVtuOutput(filebase, interval=2, functions=[(refinementFunction, "RefinementLevel")]))
        ...
        expectedMaterialNCellsList = [186264, 111735, 111805, 111854, 111882, 111889, 111959, 112029, 112078, 112106,
        ...
        self.assertAlmostEqual(temperature([0.0, 2.0, recoaterLevel]), 730.0888019468413, delta=1e-6)
