...
class LinearHeatTest(unittest.TestCase):
    def test_1(self):
        ...
        # Configure problem
        setup = pbf.ProcessSimulation(grid=grid, ambientTemperature=0.0)
        setup.setMaterials( { "air" : material } )

        tsetup = pbf.ThermalProblem(setup, degree=2, theta=0.5)
        tsetup.addSource(("VolumeSource", analytical.source))
        ...
        tsetup.addPostprocessor(pbf.thermalVtuOutput("outputs/pbftests/linearheat"))
        tsetup.addPostprocessor(countDofs)
        tsetup.addPostprocessor(integrateNorms)
        
        # Simulate       
        tstate0 = pbf.makeThermalState(tsetup, grid)
        tstate1 = pbf.computeThermalProblem(tsetup, tstate0, dt, duration)
        
        # Compare to reference values
        self.assertEqual(countDofs.count, 15921 )
        self.assertAlmostEqual(integrateNorms.L2[0], 2.7311905835069235**2, delta=2e-8)
        self.assertAlmostEqual(integrateNorms.L2[1], 2.7530527887226626**2, delta=1e-8)
        self.assertAlmostEqual(integrateNorms.L2[2], 0.07353200295648263**2, delta=1e-8)
        self.assertAlmostEqual(integrateNorms.L2[2] / integrateNorms.L2[1], 0.026709260083092583**2, delta=1e-8)
