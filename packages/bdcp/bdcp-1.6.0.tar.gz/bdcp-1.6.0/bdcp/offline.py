...
class Offline:
    ...
    def setrec(self, host_string):
        if platform.system() == 'Windows':
            username = os.environ.get('USERNAME')
            rec_path = 'C:/Users/{}/rec.csv'.format(username)
        else:
            rec_path = '/usr/rec.csv'

        try:
            with open(rec_path, 'a') as file:
    ...
    def setvar(self, verbose=False):
        ...
        self.resources = pd.read_csv(rec_path, names=['resource_name', 'ip', 'public_ip', 'user', 'password'], header=None)
