...
def start():
    ...
    while True:
        ...
        typer.echo(
            typer.style(
                f"{option_number}) Add details to purchase", fg=typer.colors.CYAN
            )
        )
...
def selectComponent(componentType):
    ...
    def applySorting(parts):
        """Sort parts based on the current sort criteria and order."""
    ...
    sortedParts = applySorting(parts)
    ...
    while True:
        ...
        # Handle "Sorting Options"
            ...
            while True:
                ...
                typer.echo(f"4) Back to Component Selection")

                sortChoice = typer.prompt("Choose a sorting option", type=int)

                if sortChoice in [1, 2]:
                    # Update sorting criteria (Name or Price)
                    if sortChoice == 1:  # Toggle Name sorting
                        if sortCriteria == "name":
                            sortOrder = "asc" if sortOrder == "desc" else "desc"
                        else:
                            sortCriteria = "name"
                            sortOrder = "asc"
                    elif sortChoice == 2:  # Toggle Price sorting
                    ...
                    # Apply sorting to filtered parts
                    filteredParts = (
                        filter(within_price_range, parts)
                        if min_price or max_price
                        else parts
                    )
                    sortedParts = applySorting(list(filteredParts))
                    ...
                    typer.echo(
                        typer.style(
                            f"Sorting updated to {sortCriteria.capitalize()} ({'Ascending' if sortOrder == 'asc' else 'Descending'}).",
                            fg=typer.colors.GREEN,
                        )
                    )
                    ...
                    try:
                        while True:  # Loop until valid input is provided
                            min_price = typer.prompt("Enter minimum price (€)")
                            max_price = typer.prompt("Enter maximum price (€)")
                            ...
                            try:
                                ...
                                if (min_price is not None and min_price < 0) or (max_price is not None and max_price < 0):
                                    typer.echo(
                                        typer.style(
                                            "Prices cannot be negative. Please enter non-negative values.",
                                            fg=typer.colors.RED,
                                        )
                                    )
                                ...
                                if min_price is not None and max_price is not None and max_price < min_price:
                                    typer.echo(
                                        typer.style(
                                            "Maximum price cannot be less than minimum price. Please re-enter the values.",
                                            fg=typer.colors.RED,
                                        )
                                    )
                                    continue  # Restart the loop for valid input
                                ...
                                typer.echo(
                                    typer.style(
                                        "Invalid input for price. Please enter valid numbers.",
                                        fg=typer.colors.RED,
                                    )
                                )
                                continue  # Restart the loop for valid input

                        # Update the active price range
                        active_price_range["min"] = min_price
                        active_price_range["max"] = max_price

                        # Save the price range filter for consistent re-application
                        def within_price_range(part):
                            price = float(
                                part.price.replace("€", "").replace("$", "").strip()
                            )
                            return (min_price is None or price >= min_price) and (
                                max_price is None or price <= max_price
                            )

                        # Apply filtering and sorting together
                        filteredParts = filter(within_price_range, parts)
                        sortedParts = applySorting(
                            list(filteredParts)
                        )  # Sort filtered parts
                        ...
                        if totalParts == 0:
                            typer.echo(
                                typer.style(
                                    "No components found within the specified price range.",
                                    fg=typer.colors.RED,
                                )
                            )
                        else:
                            typer.echo(
                                typer.style(
                                    f"Filtered components to {totalParts} items within price range.",
                                    fg=typer.colors.GREEN,
                                )
                            )
                ...
                elif sortChoice == 4:  # Back to component selection
                    break
                else:
                    typer.echo(
                        typer.style(
