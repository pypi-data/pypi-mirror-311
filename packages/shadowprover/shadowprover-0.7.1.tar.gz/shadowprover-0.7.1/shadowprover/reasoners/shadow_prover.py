...
from reasoners.instantiate_universals import get_instantiations_of_universal
from reasoners.shadow_repr import fol_reify, shadow
...
from reasoners.proof import BaseProof, CompoundProof
...
def shadow_prover_internal(
    ...
    base = set(inputs)
    while len(base) <= PARAMS["max_depth"]:
        if depth == 0:
            pass
        ...
        if new_answers and not detect_conflicts(answers, new_answers):
            _, _proof, reified_answers = fol_prove(
                    [fol_reify(b) for b in base], fol_reify(output), find_answer, verbose, max_answers=max_answers
                )
