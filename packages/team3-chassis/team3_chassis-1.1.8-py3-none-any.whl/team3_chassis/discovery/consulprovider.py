...
class ConsulProvider:
    ...
    @classmethod
    async def create(cls, consul_host, service_port, service_name, service_id, logger):
        self = ConsulProvider()
    ...
    def set_ip(self):
        ip = self.get_adapter_ip("eth0")  # this is the default interface in docker
        if ip is None:
            ip = "127.0.0.1"
        self.ip = ip
    ...
    def create_resolver(self):
        ...
        self.consul_resolver.nameservers = [self.consul_host]
    ...
    def register_consul_service(self):
        self.logger.debug(f"Registering {self.service_name} service ({self.service_id})")
        ...
        self.logger.info(f"Registered {self.service_name} service ({self.service_id})")
