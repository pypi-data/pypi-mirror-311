...
from ..debugging import assert_less
...
class QuantizedStatefulRecurrent(QuantizedLayer, StatefulRecurrent):
    ...
    def call(self, inputs):
        ...
        for i in range(timesteps):
            ...
            # and thus their FixedPoint representation is all fractional part and this will also be
            # True for updated_state. So the higher frac_bits are on the state (input_proj being
            # 16-bits, frac_bits it at most 15).
            assert_less(input_step.frac_bits, updated_real.frac_bits), f"Input resolution is too \
                high {input_step.frac_bits} must be below {updated_real.frac_bits}."
            input_step, input_step_add_shift = input_step.align(updated_real, self.buffer_bitwidth)
            self.input_add_shift(input_step_add_shift)
