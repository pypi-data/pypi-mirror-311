...
class ImageNetDataset(Dataset):
    ...
    def _get_annotation(self, paths: List, base_uri: str, usage: str, work_dir: str):
        ...
        for file in annotation_file_list:
            ...
            if self.standard_task:
                ...
                bcelogger.info(f"Find the config_for_sdk_encapsulation.yaml file in {file}, is standard task")
                ...
                label_data = None
            elif os.path.exists(os.path.join(os.path.dirname(file), "labels.txt")):
                for idx in range(len(raw_data)):
                ...
                bcelogger.info(f"Find the labels.txt file in {file}")
                label_data = open(os.path.join(os.path.dirname(file), "labels.txt"), "r").read().strip("\n").split("\n")
            else:
                raise FileNotFoundError(f"Can't find the labels.txt or config_for_sdk_encapsulation.yaml file \
    ...
    def _concat_annotation(self, raw_data_list: List):
        ...
        if len(raw_data_list) >= 1:
            if self.standard_task:
                raw_data_imagenet = self._standard_imagenet_data_raw_concat(raw_data_list)
            else:
    ...
    def _standard_imagenet_data_raw_concat(self, raw_data: List[List]):
        raw_data_imagenet = []
