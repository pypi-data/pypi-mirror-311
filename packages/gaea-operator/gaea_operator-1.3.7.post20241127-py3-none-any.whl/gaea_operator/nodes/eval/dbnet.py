...
from gaea_operator.dataset import PPOCRDataset
...
from gaea_operator.config import DBNetConfig
...
def dbnet_eval(args):
    ...
    # 3. 合并分片数据集
    coco_dataset = PPOCRDataset(windmill_client=windmill_client,
    ...
    coco_dataset.concat_dataset(dataset_name=args.dataset_name,
                                ...
                                usage=PPOCRDataset.usages[1])

    # 4. 生成评估配置文件
    DBNetConfig(windmill_client=windmill_client, tracker_client=tracker_client).write_eval_config(
