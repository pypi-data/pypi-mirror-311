...
from gaea_operator.dataset import CityscapesDataset
...
def ocrnet_eval(args):
    ...
    # 3. 合并分片数据集
    cityscapes_dataset = CityscapesDataset(windmill_client=windmill_client, work_dir=tracker_client.work_dir)
    cityscapes_dataset.concat_dataset(dataset_name=args.dataset_name,
                                      output_dir=args.output_dataset_uri,
                                      usage=CityscapesDataset.usages[1],
                                      save_label=True)

    # 4. 生成评估配置文件
    OCRNetConfig(windmill_client=windmill_client, tracker_client=tracker_client).write_eval_config(
