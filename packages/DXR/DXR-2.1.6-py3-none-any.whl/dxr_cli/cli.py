...
async def login(login_data: LoginData):
    ...
    if login_data.token:
        if login_data.token == user_data.get("token"):
            return {"status": "success", "permission": user_data["permission"], "token": user_data["token"], "whitelist": whitelist_json}
...
def start_server():
    ...
    server_process = subprocess.Popen(
        ["python", "-c", 
         "from dxr_cli.cli import app; import uvicorn; uvicorn.run(app, host='0.0.0.0', port=9002, log_config={'version': 1, 'formatters': {'default': {'format': '%(asctime)s - %(levelname)s - %(message)s', 'datefmt': '%Y-%m-%d %H:%M:%S'}}, 'handlers': {'default': {'class': 'logging.FileHandler', 'filename': '/tmp/fastapi_server.log', 'formatter': 'default'}}, 'root': {'level': 'INFO', 'handlers': ['default']}})"
        ],
...
SERVER_LOG_FILE = "/tmp/fastapi_server.log"
...
def load_whitelist():
    try:
        with open('/tmp/whitelist.json', 'r') as f:
            return set(json.load(f))  # 将加载的列表转换为集合
    except FileNotFoundError:
        return set()  # 返回空集合而不是空列表

def save_whitelist(whitelist):
    with open('/tmp/whitelist.json', 'w') as f:
        json.dump(list(whitelist), f)  # 将集合转换回列表再保存
