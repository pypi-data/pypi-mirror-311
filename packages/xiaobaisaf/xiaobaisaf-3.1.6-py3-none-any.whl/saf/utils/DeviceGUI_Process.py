...
class WinGUI(Tk):
    ...
    def __tk_check_button_run_console_button(self, parent):
        ...
        cb = Checkbutton(parent, text="console", variable=self.check_status, state="disabled", )
...
class Controller:
    ...
    def init(self, ui):
        ...
        self.ui.after(500, self._monitor_connect_device_list_thread)
    ...
    def connect_device(self, _):
        ...
        if self.ui.tk_button_connect_button['text'] == '连  接':
            ...
            try:
                ...
                self.ui.after(100, self._monitor_device_screen_thread)
                # Thread(target=self._monitor_device_package_activity_thread, daemon=True).start()
                self.ui.after(1000, self._monitor_device_package_activity_thread)
    ...
    def start_stop_appium_service(self, _):
        ...
        if self.ui.tk_button_appium_status_button['text'] == '启动Appium服务':
            ...
            self._monitor_appium_service_thread()
        else:
            try:
    ...
    def run_code_text_script(self, _):
        ...
        if self.ui.tk_button_run_code_button['text'] == '运行脚本':
            ...
            if self.current_script_path:
                ...
                self._monitor_start_stop_script_thread()
            else:
...
def _monitor_device_event_thread(queue: mp.Queue):
    ''' 监控设备发生的事件 '''
    print('监控设备发生的事件...')
    self = queue.get()
    print(repr(self))
    ...
    while device_name == device.serial:
        ...
        if "BTN_TOUCH" in line and "DOWN" in line:
            ...
            # 计算和上一个动作之间的时间为等待时间
            if self.output_type == 'appium':
                self.ui.tk_text_output_code_text.insert(END, f'sleep({round(down_time - up_time, 1)})\n')
            elif self.output_type == 'source':
                sj = f"""\r\n\t\t{{\
                    \r\n\t\t\t'action': 'wait',\
                    \r\n\t\t\t'xpath': None,\
                    \r\n\t\t\t'bounds': None,\
                    \r\n\t\t\t'hold_time': {round(down_time - up_time, 1)}\
                    \r\n\t\t}},\
                    \r\n\t]\
                    \r\n}}\n"""
                ...
                self.ui.tk_text_output_code_text.insert(END, f'{SLEEP} {int(down_time - up_time)} {SLEEP_END}\n')
        ...
        if "BTN_TOUCH" in line and "UP" in line:
            ...
            action['STATUS'][1] = 'UP'
            action['HOLD_TIME'] = round(up_time - down_time, 1)
            ...
            dx, dy = self.convert_position(action['DOWN_POSITION_X'], action['DOWN_POSITION_Y'], x_page_max,
                                           y_page_max)
            ...
            if action['STATUS'] == ['DOWN', 'UP']:
                if up_index == down_index and abs(ux - dx) ** 2 < 1000 and abs(uy - dy) ** 2 < 1000:
                    ...
                    if self.output_type == 'source':
                        ...
                        sj = 'adb shell input swipe {} {} {} {} {}'.format(dx, dy, ux, uy,
                                                                           int(action["HOLD_TIME"] * 1000))
...
if __name__ == '__main__':
    ...
    queue = mp.Queue()
    event_process = mp.Process(target=_monitor_device_event_thread, args=(queue,))
    ui_process = mp.Process(target=main, args=(queue,))
    ui_process.start()
    event_process.start()
    event_process.join()
    ui_process.join()