...
def get_client(
    ...
    if client_type == "http":
        ...
        try:
            ...
            internal_logger.warning("HUD_TAGS is not set properly: {}".format(e))
            raise HudClientException("HUD_TAGS is not set properly: {}".format(e))
        ...
        if not (
            ...
            and all(isinstance(k, str) and isinstance(v, str) for k, v in tags.items())
        ):
            internal_logger.warning("HUD_TAGS Should be of type Dict[str, str]")
            raise HudClientException("HUD_TAGS Should be of type Dict[str, str]")
        if any("." in key for key in tags.keys()):
            internal_logger.warning("HUD_TAGS keys contain a dot - Invalid")
            raise HudClientException("HUD_TAGS keys contain a dot - Invalid")
        if is_async:
            return AsyncHttpClient(host, key, service, tags)
        return SyncHttpClient(host, key, service, tags)
