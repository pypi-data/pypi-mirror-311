...
class Agent:
    def __init__(
        ...
        shared_memory_size: int = config.agent_shared_memory_size,
    ):
        self.key = key
        ...
        self.perf_monitor = PerformanceMonitor("agent", self.pod_cpu_limit)
    ...
    async def handle_exit(self) -> None:
        internal_logger.info("Received termination signal, stopping agent")
        self.stop_tasks()
    ...
    async def _log_runtime(self) -> None:
        ...
        internal_logger.info("Agent Runtime data", data=runtime.to_json_data())
    ...
    @suppress_exceptions_sync(default_return_factory=lambda: None)
    def _check_agent_disabled(self) -> None:
        if not get_hud_running_mode():
            internal_logger.critical("HUD is disabled, stopping agent")
            self.stop_tasks()
    ...
    @suppress_exceptions_sync(default_return_factory=lambda: None)
    def _check_existence_of_multiple_agents(self) -> None:
        for ps in psutil.process_iter():
            try:
                if "hud_sdk.agent" in ps.cmdline() and ps.pid != os.getpid():
                    internal_logger.warning(
                        "Multiple agents detected. Another agent found with PID: {}".format(
                            ps.pid
                        )
                    )
    ...
    async def cleanup(self) -> None:
        internal_logger.info("Cleaning up agent")
        if self._memory:
    ...
    async def run(self) -> None:
        with contextlib.ExitStack() as stack:
            ...
            # Used to communicate port with parent process. TODO: Use a better method
            print(manager.address[1], flush=True)

            if config.testing_output_directory:
                with open(
                    os.path.join(config.testing_output_directory, "agent_pid.txt"), "w"
                ) as f:
                    print(os.getpid(), file=f)

                with open(
                    os.path.join(config.testing_output_directory, "agent_pid_written"),
                    "w",
                ) as f:
                    pass
            ...
            try:
                self.client = get_client(
                    self.key, self.service, self.tags, is_async=True
                )
                if self.client.is_async:
            ...
            self.task_manager.register_periodic_task(
                self._check_manager, config.agent_manager_check_interval
            )
            self.task_manager.register_periodic_task(
                self.queue_processor,
                config.agent_queue_process_interval,
                callback=self.process_queue_until_empty,
            )

            self._check_existence_of_multiple_agents()

            loop = asyncio.get_event_loop()
            task_manager = self.task_manager
            loop.call_later(
                config.agent_process_registry_warmup_period,
                lambda: task_manager.register_periodic_task(
                    self._process_housekeeping,
                    config.agent_process_registry_update_interval,
                ),
            )
            ...
            self.task_manager.register_periodic_task(
                self._check_agent_disabled, config.agent_disabled_check_interval
            )
            self.task_manager.register_periodic_task(
...
def exception_handler(loop: asyncio.AbstractEventLoop, context: Dict[str, Any]) -> None:
    internal_logger.error(
        "Exception in agent loop", data={"message": context["message"]}
    )
