...
class FormItem(ttk.Frame):
    ...
    def __init__(
        ...
        **kwargs
        ...
        frame_kwargs = {k: v for k, v in kwargs.items() 
                    if k not in ['mode', 'filetypes', 'multiple_buttons']}
    ...
    @classmethod
    def radio(cls, master, label: str, options: List[str], **kwargs) -> 'FormItem':
        """创建单选框组"""
        item = cls(master, label, **kwargs)
        radio = Radio(item, options=options)
