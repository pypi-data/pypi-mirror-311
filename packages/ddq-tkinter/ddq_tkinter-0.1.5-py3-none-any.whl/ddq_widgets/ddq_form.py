...
class Form(ttk.Frame):
    ...
    def _create_form_item(self, item_type: str, name: str, label: str, **kwargs) -> FormItem:
        """统一的表单项创建方法"""
        print(f"Creating form item: {item_type}, {name}, {label}")  # 添加日志
        print(f"Current grid frame: {self.grid_frame}")  # 添加日志
        print(f"kwargs: {kwargs}")  # 添加日志
        # 获取对应的 FormItem 类方法
        create_method = getattr(FormItem, item_type)
        
        # 创建表单项
        item = create_method(self.grid_frame, label, **kwargs)
        print(f"Item created: {item}")  # 添加日志
    ...
    def _bind_item_events(self, name: str, item: FormItem):
        """统一的事件绑定处理"""
        print(f"Binding events for item: {name}")  # 添加日志
        if isinstance(item, Form):
            print(f"Item {name} is a Form")  # 添加日志
            item._change_callback = lambda values: self._notify_change()
        elif hasattr(item, 'var'):
            print(f"Item {name} has var")  # 添加日志
            item.var.trace_add('write', lambda *args: self._notify_change())
        elif hasattr(item, 'vars'):
            print(f"Item {name} has vars")  # 添加日志
            for var in item.vars:
                var.trace_add('write', lambda *args: self._notify_change())
        elif isinstance(item.widget, tk.Text):
            print(f"Item {name} is Text")  # 添加日志
            item.widget.bind('<KeyRelease>', lambda e: self._notify_change())
    ...
    def _notify_change(self):
        """通知表单变化"""
        print(f"Form._notify_change called, has callback: {self._change_callback is not None}")  # 添加日志
        if self._change_callback and not self._initializing:
            values = self.get_values()
            print(f"Form values: {values}")  # 添加日志
    ...
    def section(self, title: str = "", columns: int = 1) -> 'Form':
        ...
        sub_form._change_callback = lambda values: self._notify_change()  # 关键修改：子表单变化时通知父表单
