...
async def _deserialize_list_connection_status(
    http_response: HTTPResponse, config: Config
) -> ListConnectionStatusOutput:
    if http_response.status != 200 and http_response.status >= 300:
        raise await _deserialize_error_list_connection_status(http_response, config)
    ...
    if (_results := output.get("results")) is not None:
        kwargs["results"] = _deserialize_connection_status_results(_results, config)

    return ListConnectionStatusOutput(**kwargs)


async def _deserialize_error_list_connection_status(
    http_response: HTTPResponse, config: Config
) -> ApiError:
    code, message, parsed_body = await parse_rest_json_error_info(http_response)
...
def _deserialize_connection_availability(
    output: DocumentValue, config: Config
) -> list[ConnectionAvailabilityEntry]:
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [_deserialize_connection_availability_entry(e, config) for e in output]


def _deserialize_connection_availability_entry(
    output: DocumentValue, config: Config
) -> ConnectionAvailabilityEntry:
    if not isinstance(output, dict):
        raise ServiceError(f"Expected dict, found {type(output)}")
    ...
    if (_availability := output.get("availability")) is not None:
        kwargs["availability"] = expect_type(int, _availability)

    if (_status := output.get("status")) is not None:
        kwargs["status"] = expect_type(str, _status)

    return ConnectionAvailabilityEntry(**kwargs)


def _deserialize_connection_last_status(
    output: DocumentValue, config: Config
) -> ConnectionLastStatus:
    if not isinstance(output, dict):
        raise ServiceError(f"Expected dict, found {type(output)}")
    ...
    return ConnectionLastStatus(**kwargs)


def _deserialize_connection_status_result(
    output: DocumentValue, config: Config
) -> ConnectionStatusResult:
    if not isinstance(output, dict):
        raise ServiceError(f"Expected dict, found {type(output)}")
    ...
    if (_group := output.get("group")) is not None:
        kwargs["group"] = expect_type(str, _group)

    if (_subgroup := output.get("subgroup")) is not None:
        kwargs["subgroup"] = expect_type(str, _subgroup)

    if (_connection_type := output.get("connectionType")) is not None:
        kwargs["connection_type"] = expect_type(str, _connection_type)

    if (_connection_id := output.get("connectionId")) is not None:
        kwargs["connection_id"] = expect_type(str, _connection_id)

    if (_connection_name := output.get("connectionName")) is not None:
        kwargs["connection_name"] = expect_type(str, _connection_name)

    if (_last_status := output.get("lastStatus")) is not None:
        kwargs["last_status"] = _deserialize_connection_last_status(
            _last_status, config
        )

    if (_availability := output.get("availability")) is not None:
        kwargs["availability"] = _deserialize_connection_availability(
            _availability, config
        )

    return ConnectionStatusResult(**kwargs)


def _deserialize_connection_status_results(
    output: DocumentValue, config: Config
) -> list[ConnectionStatusResult]:
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [_deserialize_connection_status_result(e, config) for e in output]
...
def _deserialize_resource_object(
    output: DocumentValue, config: Config
) -> ResourceObject:
    if not isinstance(output, dict):
        raise ServiceError(f"Expected dict, found {type(output)}")
...
def _deserialize_scan_execution_record(
    ...
    if (_resource := output.get("resource")) is not None:
        kwargs["resource"] = _deserialize_resource_object(_resource, config)
