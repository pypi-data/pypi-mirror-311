...
from .models import (
    ...
    TagFilter,
    TagResourceUnion,
    TagResourceUnionFilter,
    TagResourceUnionResourceIds,
    TagResourcesFilter,
    TomSelectFilter,
    UntagResourcesInput,
    UpdateAssetInput,
...
async def _serialize_list_connection_status(
    input: ListConnectionStatusInput, config: Config
) -> HTTPRequest:
    ...
    if input.filter is not None:
        result["filter"] = _serialize_list_connection_filter(input.filter, config)
...
async def _serialize_untag_resources(
    input: UntagResourcesInput, config: Config
) -> HTTPRequest:
    path = "/resource/tags"
    query: str = f""
...
def _serialize_list_connection_filter(
    input: ListConnectionFilter, config: Config
) -> dict[str, DocumentValue]:
    result: dict[str, DocumentValue] = {}
...
def _serialize_tag_resources_filter(
    input: TagResourcesFilter, config: Config
) -> dict[str, DocumentValue]:
    result: dict[str, DocumentValue] = {}
...
def _serialize_tag_resource_union(
    input: TagResourceUnion, config: Config
) -> dict[str, DocumentValue]:
    match input:
        case TagResourceUnionFilter():
            return {"filter": _serialize_tag_resources_filter(input.value, config)}

        case TagResourceUnionResourceIds():
            return {"resourceIds": input.value}
