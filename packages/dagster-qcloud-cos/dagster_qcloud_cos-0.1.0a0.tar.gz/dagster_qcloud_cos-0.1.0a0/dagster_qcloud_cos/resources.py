...
- **BucketId** (str):
    ...
    SecretId: str = Field(default=EnvVar("COS_SECRET_ID"), description="用户的 SecretId")
    SecretKey: str = Field(default=EnvVar("COS_SECRET_KEY"), description="用户的 SecretKey")
    Token: Optional[str] = Field(default=EnvVar("COS_TOKEN"), description='如果使用临时密钥需要填入 Token。')

    Region: Optional[str] = Field(default=EnvVar("COS_REGION"), description='用户存储桶归属 Region')
    Endpoint: Optional[str] = Field(default=EnvVar("COS_ENDPOINT"), description='可设置为用户的 Endpoint 或者全局加速域名')
    Domain: Optional[str] = Field(default=EnvVar("COS_DOMAIN"), description='用户自定义 CDN 域名')
    BucketId: Optional[str] = Field(default=EnvVar("COS_BUCKET_ID"), description='用户存储桶 BucketId')

    @classmethod
    def _is_dagster_maintained(cls) -> bool:
        return False

    def create_resource(self, context: InitResourceContext) -> CosS3Client:
        """
        返回一个 `CosS3Client` 实例。
        :param context:
        :return:
        ...
        return CosS3Client(CosConfig(**config_dict))
