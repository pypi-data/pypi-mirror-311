...
from s1_cns_cli.cli.registry import CodeTypeSubParser, BASELINE_FILE, MissingRequiredFlags, MissingDependencies, \
    LogColors, HttpMethod, PUBLISH_ISSUES_API, InvalidInput, SUPPORTED_GIT_PROVIDERS, OUTPUT_FORMAT, \
    SEVERITY_MAP, ScanNotEnabledException
from s1_cns_cli.cli.utils import read_json_file, write_json_to_file, get_version, \
    ...
    get_output_file_and_format, get_sarif_payload, get_actionable_and_non_actionable
...
def secret_post_evaluation(args, issues, secret_config_data, global_config_data):
    ...
    if len(filtered_findings) > 0:
        ...
        send_result_to_sentinelone(args, sorted_findings, global_config_data)
