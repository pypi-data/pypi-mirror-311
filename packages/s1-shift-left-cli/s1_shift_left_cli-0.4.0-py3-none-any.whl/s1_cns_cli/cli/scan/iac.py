...
from s1_cns_cli.cli.registry import CodeTypeSubParser, HttpMethod, TIME_FORMAT, SUPPORTED_FRAMEWORKS, \
    IacFramework, GlobalConfig, BASELINE_FILE, LogColors, MissingDependencies, \
    DOWNLOAD_IAC_CACHE_API, PUBLISH_ISSUES_API, InvalidInput, MissingRequiredFlags, SUPPORTED_GIT_PROVIDERS, \
    OUTPUT_FORMAT, SEVERITY_MAP, RegoException, ScanNotEnabledException, DEFAULT_IAC_CACHE_UPDATE_FREQUENCY
from s1_cns_cli.cli.utils import make_request, read_json_file, write_json_to_file, wrap_text, \
    ...
    read_from_file, get_priority, get_output_file_and_format, get_sarif_payload, send_exception_to_sentry, \
    get_actionable_and_non_actionable
    ...
    for node in graph_nodes:
        ...
        if len(plugin_data["connections"]) > 0:
            ...
            cur_node = generate_sub_graph(plugin_data["connections"], cur_node, graph_nodes_by_id, grouped_links_by_source,
                                      grouped_links_by_target)
        if evaluate_rego(cur_node, plugin_data["rego"], cache_directory):
...
def evaluate_rego(input_json, rego, cache_directory):
    ...
    try:
        ...
        if result.stderr:
            ...
            raise RegoException(err_msg)
        if "isVulnerable:true" in result.stdout.decode():
            return True
        return False
    except RegoException as e:
...
def validate_iac_command(args):
    if args.publish_result and (args.branch == "" or args.repo_url == "" or args.repo_full_name == ""):
        raise MissingRequiredFlags(
            "Please provide mandatory flags (--branch, --repo-url, --repo-full-name) to publish results to SentinelOne CNS.")
...
def iac_post_evaluation(args, findings, cache_directory):
    ...
    if len(filtered_findings) > 0:
        ...
        exit_code = len(actionable_findings) > 0
        ...
        send_issues_to_sentinelone(args, sorted_findings, global_config_data)
...
def check_if_skip(plugin_id_to_skip, node, framework, iac_config_data):
    ...
    if framework == IacFramework.TERRAFORM:
        if "tags" in node and "s1-cns-skip" in node["tags"]:
            if type(node["tags"]["s1-cns-skip"]) == str and node["tags"]["s1-cns-skip"] == plugin_id_to_skip:
                LOGGER.debug(f"Skipping {plugin_id_to_skip} as per resource tags.")
                return True
            elif type(node["tags"]["s1-cns-skip"]) == list and plugin_id_to_skip in node["tags"]["s1-cns-skip"]:
                LOGGER.debug(f"Skipping {plugin_id_to_skip} as per resource tags.")
                return True

    elif framework == IacFramework.CLOUDFORMATION:
        if "Tags" in node and "s1-cns-skip" in node["Tags"] and plugin_id_to_skip in node["Tags"]["s1-cns-skip"]:
            return True

    elif framework == IacFramework.KUBERNETES or framework == IacFramework.HELM:
        if "metadata" in node and "labels" in node["metadata"] and "s1-cns-skip" in node["metadata"]["labels"] \
                and plugin_id_to_skip in node["metadata"]["labels"]["s1-cns-skip"]:
            return True
