...
from botrun_flow_lang.utils.llm_utils import get_api_key, get_base_url
        ...
        for agent_idx, agent_prompt in enumerate(agents):
            ...
            if agent_idx > 0:
                input_message = AGENT_TEMPLATE.replace(
                    "{context}", last_output
                ).replace("{rules}", agents[agent_idx].system_prompt)
                ...
                ).replace("{rules}", agents[0].system_prompt)
                messages_for_llm.append({"role": "user", "content": input_message})
