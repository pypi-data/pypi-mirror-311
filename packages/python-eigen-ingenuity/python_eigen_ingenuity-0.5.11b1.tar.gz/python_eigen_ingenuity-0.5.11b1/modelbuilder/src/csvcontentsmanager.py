...
class CSVContentsManager:
    ...
    def get_property_values_dict(self, row_number, column_numbers, allow_blank_property_list, default_key='', prefix='', blank_override=''):
        this_row = self.data_list[row_number]
    ...
    def get_property_values_list(self, row_number, column_numbers, default_key='', override_key=''):
        ...
        split_values = [this_row[column].split('::') for column in column_numbers]
        split_keys = [(split_values[count][0]+':').split(':') for count, column in enumerate(column_numbers)]
        this_list = [Neo4jProperty((override_key or self.mapped_keys[column] or default_key), this_row[column], self.formats[column]) if len(split_values[count]) == 1 else Neo4jProperty(split_keys[count][0] or default_key, split_values[count][1], split_keys[count][1] or self.formats[column]) for count, column in enumerate(column_numbers) if this_row[column]]
        return this_list
