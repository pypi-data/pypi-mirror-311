...
def saveconfig_old(data={}, title = 'title', fig=None, fold=None, close=False, savedat=True,basepath=r'G:\QuLabData\skzhao\data'):
    today=date.today()
    ...
    if fig is not None:
        fig.savefig(basepath / title_new)
    if close:
        plt.close()
    return basepath / title_new

def saveconfig(task_id, title = 'title', fig=None, fold=None, show=True, savedat=True,basepath=r'G:\QuLabData\skzhao\data',cover=False):
    from datetime import date
    from pathlib import Path
    import os
    today=date.today()
    ...
    # title_new = title
    # while basepath / (title_new+'.png') in files:
    #     a +=1
    #     title_new = title+f'_{a}'
#     np.savez(basepath / title_new, **data)
    if fig is not None:
