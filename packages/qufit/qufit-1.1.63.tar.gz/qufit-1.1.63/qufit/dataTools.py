...
def choose_min_or_max(data):
    l_min = np.max(np.abs(np.min(data)-data[data!=np.max(data)]))
    l_max = np.max(np.abs(np.max(data)-data[data!=np.min(data)]))
    if l_min >= l_max:
...
def P_depart(c0,c1,radio):
    x1,y1 = c0[0],c0[1]   
    circ.extend(self.readout(*qubits_read,index=[i for i in range(len(qubits_read))]))
    circ.extend([(('Delay', 1111e-9), qubit) for qubit in qubits_read])
    circ.extend(self.readout(*qubits_read,index=[i+len(qubits_read) for i in range(len(qubits_read))]))
    circ.extend([(('Delay', 1111e-9), qubit) for qubit in qubits_read])
    circ.extend(self.readout(*(qubits_read[:3]),index=[i+2*len(qubits_read) for i in range(len(qubits_read[:3]))],jpalst=tuple([])))
    x2,y2 = c1[0],c1[1]
