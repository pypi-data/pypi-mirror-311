...
class AdamZ(Optimizer):
    def __init__(
        ...
        *,
        ...
        # Additional checks for AdamZ-specific parameters
        if not 0.0 < overshoot_factor < 1.0:
            raise ValueError(f"Invalid overshoot_factor: {overshoot_factor}")
        if not 1.0 < stagnation_factor:
            raise ValueError(f"Invalid stagnation_factor: {stagnation_factor}")
        if not 0.0 <= stagnation_threshold:
            raise ValueError(f"Invalid stagnation_threshold: {stagnation_threshold}")
        if not patience > 0:
            raise ValueError(f"Invalid patience value: {patience}")
        if not stagnation_period > 0:
            raise ValueError(f"Invalid stagnation_period value: {stagnation_period}")
        if not 0.0 < min_lr <= max_lr:
            raise ValueError(f"Invalid learning rate bounds: min_lr={min_lr}, max_lr={max_lr}")
        ...
        # Initialize AdamZ-specific attributes
...
AdamZ.__doc__ = (
    ...
    .. math::
       \begin{aligned}
            ...
            &\hspace{13mm}      \text{overshoot factor}, \text{stagnation factor},               \\
            &\hspace{13mm}      \text{stagnation threshold}, \text{patience},                    \\
            &\hspace{13mm}      \text{stagnation period}, \text{min lr}, \text{max lr}          \\
            ...
            &\hspace{5mm}\textbf{apply AdamZ-specific adjustments}                               \\
            &\hspace{10mm}\textit{apply overshoot and stagnation handling as per AdamZ}          \\
            &\rule{110mm}{0.4pt}                                                          \\[-1.ex]
            &\bf{return} \:  \theta_t                                                     \\[-1.ex]
            &\rule{110mm}{0.4pt}                                                          \\[-1.ex]
       \end{aligned}

    For further details regarding the algorithm we refer to `AdamZ: An Enhanced Optimization Method for Neural Network Training`_.
    ...
    Args:
        ...
        amsgrad (bool, optional): whether to use the AMSGrad variant of this
            ...
            (default: False)
        overshoot_factor (float, optional): factor for handling overshooting (default: 0.5)
        stagnation_factor (float, optional): factor for handling stagnation (default: 1.2)
        stagnation_threshold (float, optional): threshold for detecting stagnation (default: 0.2)
        patience (int, optional): number of iterations to wait before applying adjustments (default: 100)
        stagnation_period (int, optional): period to consider for stagnation detection (default: 10)
        max_norm (float, optional): maximum norm for gradient clipping (default: 1.0)
        min_lr (float, optional): minimum learning rate for adjustments (default: 1e-7)
        max_lr (float, optional): maximum learning rate for adjustments (default: 1.0)
        maximize (bool, optional): maximize the objective, instead of minimizing (default: False)
        foreach (bool, optional): whether to use foreach implementation of optimizer (default: None)
        capturable (bool, optional): whether this instance is safe to capture in a CUDA graph (default: False)
        differentiable (bool, optional): whether this instance is to be used with torch.autograd.functional (default: False)
        fused (bool, optional): whether to use fused kernels (default: False)
    """)



def _single_tensor_adamz(
    params: List[Tensor],
    grads: List[Tensor],
    exp_avgs: List[Tensor],
    exp_avg_sqs: List[Tensor],
...
@_disable_dynamo_if_unsupported(single_tensor_fn=_single_tensor_adamz)
def adamz(
    params: List[Tensor],
    grads: List[Tensor],
    exp_avgs: List[Tensor],
    ...
    *,
    ...
    if fused and not torch.jit.is_scripting():
        ...
        func = _multi_tensor_adamz
    else:
        func = _single_tensor_adamz
