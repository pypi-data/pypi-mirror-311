...
def detach_audio(input_video, output_audio, output_video_no_audio):
    ...
    try:
        ...
        audio_stream = ffmpeg.output(audio_stream, output_audio, an=None).overwrite_output()
        
        # Create the video without audio and save it to output_video_no_audio
        video_stream = ffmpeg.input(input_video)
        video_stream = ffmpeg.output(video_stream, output_video_no_audio, **{'c:v': 'copy'}, an=None).overwrite_output()

        # Run both ffmpeg commands (audio and video extraction)
