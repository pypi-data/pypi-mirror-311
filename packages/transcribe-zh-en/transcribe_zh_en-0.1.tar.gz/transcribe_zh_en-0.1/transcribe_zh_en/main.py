...
def detach_subtitles(input_video, output_subtitles):
    """
    Extract subtitles from the video if they exist.
    """
...
def detach_audio(input_video, output_video_no_audio):
    """
    Remove audio from the video by copying the video stream without re-encoding.
    """
    try:
        # Extract the video stream and remove audio
        stream = ffmpeg.input(input_video)
        stream = ffmpeg.output(stream, output_video_no_audio, **{'c:v': 'copy'}, an=None).overwrite_output()
        ...
        print(f"Audio detached to {output_video_no_audio}")
    except Exception as e:
...
def process_video(video_path, processed_dir, detach_subtitles_flag, detach_audio_flag):
    """
    Process a single video: copy it, detach subtitles, and/or detach audio based on flags.
    """
    ...
    if detach_audio_flag:
        detach_audio(video_path, output_video_no_audio)
...
def main():
    ...
    parser.add_argument(
        '--detach-audio', 
        action='store_true', 
        help='If set, the tool will remove audio from the video files.'
    )
