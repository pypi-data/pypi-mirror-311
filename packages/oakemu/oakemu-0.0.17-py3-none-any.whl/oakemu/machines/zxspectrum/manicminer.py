...
class ManicMiner(Game):
    ...
    def __getstate__(self):
        state = {
            "_manic_miner": bytes(DotNetStateSerializer.SaveState(self._manic_miner)),
        }
        return state

    def __setstate__(self, state):
        self._manic_miner = DotNetManicMiner()
        DotNetStateSerializer.LoadState[DotNetManicMiner](self._manic_miner, state["_manic_miner"])
