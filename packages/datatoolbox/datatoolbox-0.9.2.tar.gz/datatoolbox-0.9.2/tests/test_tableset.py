...
df = dt.Datatable(
    data, meta=metaDict, columns=[2010, 2012], index=["ARG", "DEU", "FRA", "GBR"]
)
df2 = dt.Datatable(
    data2, meta=metaDict2, columns=[2009, 2012], index=["ARG", "DEU", "FRA", "GBR"]
)
...
def test_convert():
    ...
    tableSet = tableSet.convert(newUnit="mm")


def test_from_list():
    ts1 = dt.TableSet()
    ts1.add([df, df2])
    ts2 = dt.TableSet.from_list([df, df2])

    assert ts1 == ts2


def test_compact_excel():
    ts = dt.TableSet.from_list([df, df2])
    writer = pd.ExcelWriter("test.xlsx")
    ts.to_compact_excel(writer)
    writer.close()

    data = dt.data_structures.read_compact_excel("test.xlsx")

    assert ts.keys() == data.keys()


def test_fromat_conversion():
    ts = dt.TableSet.from_list([df, df2])
    ts.to_compact_long_format()


def test_country_tables():
    ts = dt.TableSet.from_list([df, df2])
    country_tables = ts.create_country_dataframes()
    assert list(country_tables.keys()) == ["ARG", "DEU", "FRA", "GBR"]
