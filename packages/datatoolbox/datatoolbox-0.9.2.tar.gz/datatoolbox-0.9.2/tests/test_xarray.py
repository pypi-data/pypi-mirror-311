...
def test_to_xarray_interface():
    ...
    dimSize, dimList = dt.core.get_dimension_extend(
        tableSet, dimensions=["region", "time", "pathway"]
    )
...
def test_to_xdset_interface():
    ...
    assert (
        xData[list(xData.data_vars)[0]].attrs["unit"]
        == tableSet[list(tableSet.keys())[0]].meta["unit"]
    )
...
def test_quantified_xdataset_from_tableset():
    ...
    assert ds.sizes["pathway"] == 1
    assert ds.sizes["region"] == 4
    assert list(ds.keys()) == ["Numbers|Ones", "Numbers|Fives"]
    assert hasattr(
        ds["Numbers|Ones"], "pint"
    )  # check that dataset is quantified with unit
