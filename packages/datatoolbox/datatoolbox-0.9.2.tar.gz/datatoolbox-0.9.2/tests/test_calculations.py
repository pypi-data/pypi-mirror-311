...
df3 = dt.Datatable(
    [[50, 50], [40, 40]], columns=[2000, 2010], index=["DEU", "IND"], meta=metaDict2
)
...
def test_raddition_dfs_conv():
    ...
    obs = df1 + df3
    exp = dt.Datatable(
        [[10.05, 20.05, np.nan], [40.04, 40.04, np.nan]],
        columns=[2000, 2010, 2020],
        index=["DEU", "IND"],
        meta=metaDict,
    )
...
def test_laddition_dfs_conv():
    obs = df3 + df1
    exp = dt.Datatable(
        [[10050.0, 20050, np.nan], [40040, 40040, np.nan]],
        columns=[2000, 2010, 2020],
        index=["DEU", "IND"],
        meta=metaDict2,
    )
...
def test_rsub_unit():
    # from util_for_testing import df3

    df_res = df1 - 1 * dt.core.unit_registry.ur("Mt CO2")

    assert df_res.loc["DEU", 2000] == 9
...
def test_rsubstraction_dfs_conv():
    ...
    obs = df1 - df3
    exp = dt.Datatable(
        [[9.95, 19.95, np.nan], [39.96, 39.96, np.nan]],
        columns=[2000, 2010, 2020],
        index=["DEU", "IND"],
        meta=metaDict,
    )
...
def test_lsubstraction_dfs_conv():
    obs = df3 - df1
    exp = dt.Datatable(
        [[-9950.0, -19950.0, np.nan], [-39960.0, -39960.0, np.nan]],
        columns=[2000, 2010, 2020],
        index=["DEU", "IND"],
        meta=metaDict2,
    )
