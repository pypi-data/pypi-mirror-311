...
for string, newString in replacementDict.items():
    ...
    inventory.loc[mask, 'entity'] = inventory.loc[mask, 'entity'].apply(
        lambda x: x.replace(string, newString)
    )
#%%
entityList = [
...
for entity in entityList:
    ...
    inventory.loc[mask, 'category'] = inventory.loc[mask, 'category'] + inventory.loc[
        mask, 'entity'
    ].apply(lambda x: x.replace(entity, ''))
