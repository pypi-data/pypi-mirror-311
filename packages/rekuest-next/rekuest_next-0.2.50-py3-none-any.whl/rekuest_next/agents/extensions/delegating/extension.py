...
async def on_init(writer):
    
    await writer.writeline(f"johannes.roos")
    await writer.writeline(f"password")

    str = await writer.readline()
...
class CLIExtension(BaseModel):
    ...
    async def astart(self, instance_id: str):
            ...
            while True:
                ...
                try:
                    ...
                    if self.on_process_stdout:
                        await self.on_process_stdout(line.decode())
        ...
        self._stdout_task = asyncio.create_task(enqueue_output(self._proc.stdout, self._stdout_queue))
        self._stderr_task = asyncio.create_task(enqueue_output(self._proc.stderr, self._stdout_queue))
