<template>
  <div class="w-full h-auto flex flex-col gap-2">
    ...
    <ConfirmationModal
      ...
      cancel-button="No"
      confirm-button="Yes"
      @modal-confirm="onDeleteService(activeServiceIndex!, isNewService)"
      @modal-close="closeModal"
    />
      ...
      <div v-if="!isLoadingServices" class="handling-step-content">
        ...
        >
          ...
          >
            ...
            <div
              class="handling-step-content-element-el flex justify-between items-center w-full px-[0.5rem] relative"
            >
              <div v-if="service.comment" class="hover-wrap contents">
        ...
        >
          ...
          >
            ...
            <div
              class="handling-step-content-element-el flex justify-between items-center w-full px-[0.5rem] gap-1"
            >
              <img
                ...
                alt="comment"
                class="comment-button cursor-pointer p-[0.75rem] rounded-lg"
                @click="openModal('comment', index, true)"
              />
              <img
                v-else
                width="44"
              ...
              <Button
                ...
                @click="onDeleteService(index, true)"
              >
                <img
                  width="20"
                  height="20"
    ...
    <div class="handling-step bg-white w-full border border-transparent rounded-md">
      ...
      >
        ...
        <div class="handling-step-content-none-header">There are no quotes yet</div>
      </div>
      <div
...
const openModal = (modalName: 'comment' | 'delete', index: number, isNew = false) => {
  if (isNew) {
...
const onSelectNewService = (value: any, serviceId: number) => {
  newServices.value![serviceId] = { ...value };
};

const onDeleteService = async (serviceId: number, isNew = false) => {
  closeModal();
  if (isNew) {
    deleteNewService(serviceId);
    return;
  }
