...
def process_representer(dumper, data):
    ...
    for section_name in ["PrologProcedure", "MetadataProcedure", "DataProcedure", "EpilogProcedure"]:
        process_section = output_data.get(section_name).replace("\r\n", "\n")
        output_data[section_name] = MultiLineString(process_section)
    
    return dumper.represent_mapping(
        '!TM1py.ProcessObject', output_data)
...
def dump_process(process: Process) -> str:
    ...
    return yaml.dump(process, default_flow_style=False, allow_unicode=True, sort_keys=False)
