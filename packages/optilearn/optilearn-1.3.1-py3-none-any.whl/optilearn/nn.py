...
class Sequential:
  ...
  def add(self,layers):
    ...
    if type(layers) == Dense:
      ...
      try:
        ...
        if self.bit == 64
          self.weights.append(layers.weights.astype(np.float64))
          #if layers.biases != None:
          self.biases.append(layers.biases.astype(np.float64))
        elif self.bit == 32:
          self.weights.append(layers.weights.astype(np.float32))
          #if layers.biases != None:
          self.biases.append(layers.biases.astype(np.float32))
        elif self.bit == 16:
          self.weights.append(layers.weights.astype(np.float16))
          #if layers.biases != None:
          self.biases.append(layers.biases.astype(np.float16))
        else:
          self.weights.append(layers.weights.astype(np.float128))
          #if layers.biases != None:
          self.biases.append(layers.biases.astype(np.float128))
      except:
      ...
      if len(self.n_nurones)>2:
        if self.activation[-1] in ('sigmoid','tanh','softmax'):
          #print(self.activation)
          if self.bit == 64:
            self.weights.append(np.random.uniform(low=-0.1,high=0.1,size=(self.n_nurones[-2],self.n_nurones[-1]))).astype(np.float64)
            self.biases.append(np.zeros(self.n_nurones[-1])).astype(np.float64)
            
          elif self.bit == 32:
            self.weights.append(np.random.uniform(low=-0.1,high=0.1,size=(self.n_nurones[-2],self.n_nurones[-1]))).astype(np.float32)
            self.biases.append(np.zeros(self.n_nurones[-1])).astype(np.float32)
            
          elif self.bit == 16:
            self.weights.append(np.random.uniform(low=-0.1,high=0.1,size=(self.n_nurones[-2],self.n_nurones[-1]))).astype(np.float16)
            self.biases.append(np.zeros(self.n_nurones[-1])).astype(np.float16)
            
          else:
            self.weights.append(np.random.uniform(low=-0.1,high=0.1,size=(self.n_nurones[-2],self.n_nurones[-1]))).astype(np.float128)
            self.biases.append(np.zeros(self.n_nurones[-1])).astype(np.float128)
            
        else:
          shape=(self.n_nurones[-2],self.n_nurones[-1])
          if self.bit == 64:
            self.weights.append(np.random.normal(loc=0.0, scale=np.sqrt(2 / shape[0]), size=shape)).astype(np.float64)
            self.biases.append(np.zeros(self.n_nurones[-1])).astype(np.float64)
          elif self.bit == 32:
            self.weights.append(np.random.normal(loc=0.0, scale=np.sqrt(2 / shape[0]), size=shape)).astype(np.float32)
            self.biases.append(np.zeros(self.n_nurones[-1])).astype(np.float32) 
          elif self.bit == 16:
            self.weights.append(np.random.normal(loc=0.0, scale=np.sqrt(2 / shape[0]), size=shape)).astype(np.float16)
            self.biases.append(np.zeros(self.n_nurones[-1])).astype(np.float16)
          else:
            self.weights.append(np.random.normal(loc=0.0, scale=np.sqrt(2 / shape[0]), size=shape)).astype(np.float128)
            self.biases.append(np.zeros(self.n_nurones[-1])).astype(np.float128)
  ...
  def forward(self,input_data,return_all_outputs=False):
    ...
    for i in range(len(weights)):
      output = np.dot(input1, weights[i]) + biases[i]
