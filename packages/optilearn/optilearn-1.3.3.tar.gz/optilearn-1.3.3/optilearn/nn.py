...
class Sequential:
  ...
  def add(self,layers):
    ...
    if len(self.layers) ==1:
      if self.bit == 64:
        self.weights[0] = self.weights[0].astype(np.float64)
        self.biases[0] =  self.biases[0].astype(np.float64)
      elif self.bit == 32:
        self.weights[0] = self.weights[0].astype(np.float32)
        self.biases[0] =  self.biases[0].astype(np.float32)
      elif self.bit == 16:
        self.weights[0] = self.weights[0].astype(np.float16)
        self.biases[0] =  self.biases[0].astype(np.float16)
      else:
        self.weights[0] = self.weights[0].astype(np.float128)
        self.biases[0] =  self.biases[0].astype(np.float128)
