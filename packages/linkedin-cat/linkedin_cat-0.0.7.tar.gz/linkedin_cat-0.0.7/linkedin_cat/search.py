...
class LinkedinSearch(LinkedinMessage):
    ...
    def parse_linkedin_results(self,html_text):
        ...
        profiles = soup.find_all('li', class_='jeunwPGJmLFbispnuBzeitiGewwbttjxdzgQ')
        ...
        for profile in profiles:
            ...
            company_tag = profile.find('div', class_='LcxqaAQEXzuaApsdzTmCwdbKdNUirxSyIw')
            profile_data['title'] = company_tag.get_text(strip=True) if company_tag else None

            location_tag = profile.find('div', class_='OjplEaJZzhBoCIdDVFiKyoWGQpUCwEAFYc')
            profile_data['location'] = location_tag.get_text(strip=True) if location_tag else None


            title_tag = profile.find('p', class_='ZQUlYMqwzbIsTsMXCWoUuytEjFVuiDuAcGCabE')
            profile_data['introduction'] = title_tag.get_text() if title_tag else None

            # 查找具有特定class的<span>标签
            link_tag = profile.find('span', class_='esEJqtxgjdXKAqBxVfqmIrqraLFktgBA').find('a')
    ...
    def search_linkedin_profile(self,url,save_folder='./linkedin',thread_pool=True):
        ...
        try:
            print(Fore.GREEN + "Opening LinkedIn URL" + Style.RESET_ALL)
            ...
            if not profile_data:
                ...
                print(Fore.GREEN + "Saving profile data to JSON" + Style.RESET_ALL)
