...
#-------------------------------------------------pympress installation -----------------------------
    def setup_pympress(self) -> bool:
        ...
        try:
            # First try in virtual environment
            venv_paths = [
                Path.home() / 'my_python/bin/pympress',  # Virtual env path
                Path.home() / '.local/bin/pympress',     # User installation
                Path('/usr/local/bin/pympress'),         # System installation
                Path('/usr/bin/pympress')                # Alternative system path
            ]
            ...
            if not pympress_path:
                ...
                try:
                    subprocess.run(
                        [sys.executable, '-m', 'pip', 'install', '--user', 'pympress'],
                        check=True,
                        capture_output=True,
                        text=True
                    )
