...
#-------------------------------------------------------------------------------------
    def setup_python_paths(self):
        """Setup Python paths for imports and packages"""
        try:
            import site
            import sys
            from pathlib import Path
            ...
            # Get virtual environment paths if active
            ...
            if hasattr(sys, 'real_prefix') or (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix):
                ...
                if venv_site.exists() and str(venv_site) not in sys.path:
                    ...
                    paths_added.append(str(venv_site))
                    self.write(f"✓ Added venv site-packages: {venv_site}\n", "green")
            ...
            if user_site.exists() and str(user_site) not in sys.path:
                ...
                self.write(f"✓ Added user site-packages: {user_site}\n", "green")

            # Add BSG-IDE specific paths
            bsg_paths = [
                Path.home() / '.local/lib/bsg-ide',
                user_site / 'bsg_ide',
                Path('/usr/local/lib/bsg-ide'),
            ]
            ...
            for path in bsg_paths:
                if path.exists() and str(path) not in sys.path:
                    ...
                    self.write(f"✓ Added BSG-IDE path: {path}\n", "green")

            # Now modify venv_paths in setup_pympress to use these paths
            self.venv_paths = [
            ...
            if paths_added:
                ...
                new_pythonpath = os.pathsep.join(paths_added + ([current_pythonpath] if current_pythonpath else []))
                os.environ['PYTHONPATH'] = new_pythonpath

            return True

        except Exception as e:
            self.write(f"✗ Error setting up Python paths: {str(e)}\n", "red")
            return False
