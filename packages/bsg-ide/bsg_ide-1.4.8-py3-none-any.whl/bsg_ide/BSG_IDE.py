...
#-------------------------------------------------------Capture Screen ---------------------------------------
 ...
 #----------------------------------------------------------------------------
    ...
    def create_toolbar(self) -> None:
        """Create main editor toolbar with presentation features and capture controls"""
        ...
        # Add separator
        ttk.Separator(self.toolbar, orient='vertical').pack(side='left', padx=10, pady=5, fill='y')

        # Screen capture controls
        capture_frame = ctk.CTkFrame(self.toolbar, fg_color="transparent")
        capture_frame.pack(side='left', padx=5)

        # Initialize capture settings as class attributes
        self.capture_mode = tk.StringVar(value="single")
        self.frame_count = tk.IntVar(value=10)
        self.frame_delay = tk.DoubleVar(value=0.5)

        # Create header
        mode_label = ctk.CTkLabel(capture_frame, text="Screen Capture:")
        mode_label.pack(side='left', padx=5)
        self.create_tooltip(mode_label, "Choose capture mode and settings")

        # Create mode buttons
        mode_frame = ctk.CTkFrame(capture_frame, fg_color="transparent")
