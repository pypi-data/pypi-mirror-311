...
% Support for frame-specific colors
\newcommand{\fc}[2][black,white]{\colorlet{frame-bg}{#1}\colorlet{frame-fg}{#2}}
\newcommand{\setframecolors}[2]{\colorlet{frame-bg}{#1}\colorlet{frame-fg}{#2}}
\AtBeginEnvironment{frame}{\pagecolor{frame-bg}\color{frame-fg}}
...
\usetikzlibrary{fadings}
    ...
    % Frame color commands and text effects
    \usetikzlibrary{shadows.blur}
    \newcommand{\fc}[2][black,white]{\colorlet{frame-bg}{#1}\colorlet{frame-fg}{#2}}
    \newcommand{\setframecolors}[2]{\colorlet{frame-bg}{#1}\colorlet{frame-fg}{#2}}
    \AtBeginEnvironment{frame}{\pagecolor{frame-bg}\color{frame-fg}}

    % Enhanced text effects with colors
    \newcommand{\shadowtext}[2][2pt]{%
        \begin{tikzpicture}[baseline]
            \node[blur shadow={shadow blur steps=5,shadow xshift=0pt,shadow yshift=-#1,
                  shadow opacity=0.75}, text=frame-fg] {#2};
        \end{tikzpicture}%
    }
    ...
    \newcommand{\glowtext}[2][myblue]{%
        \begin{tikzpicture}[baseline]
            \node[circle, inner sep=1pt,
                  blur shadow={shadow blur steps=10,shadow xshift=0pt,
                  shadow yshift=0pt,shadow blur radius=5pt,
                  shadow opacity=0.5,shadow fill=#1},
                  text=frame-fg] {#2};
        \end{tikzpicture}%
    }

    % Basic highlighting commands with color support
    ...
    \newcommand{\hlkey}[1]{\colorbox{myblue!20}{\textcolor{frame-fg}{\textbf{#1}}}}
    \newcommand{\hlnote}[1]{\colorbox{mygreen!20}{\textcolor{frame-fg}{\textbf{#1}}}}
