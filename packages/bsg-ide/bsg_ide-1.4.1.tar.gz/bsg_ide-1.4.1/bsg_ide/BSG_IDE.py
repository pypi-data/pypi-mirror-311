...
class BeamerSlideEditor(ctk.CTk):
    ...
    """
        ...
        # Add early in initialization
        self.setup_python_paths()
        # Initialize UI components
        self.create_widgets()
        # Check pympress installation
        self.setup_pympress()
...
#-------------------------------------------------------------------------------------
    def setup_python_paths(self):
        """Setup Python paths and virtual environment properly"""
        try:
            import site
            from pathlib import Path

            # Store paths that are successfully added
            paths_added = []

            # Get virtual environment paths
            venv_path = Path(sys.prefix)
            venv_lib = venv_path / 'lib' / f'python{sys.version_info.major}.{sys.version_info.minor}'
            venv_site_packages = venv_lib / 'site-packages'

            # Add virtual environment paths
            if venv_site_packages.exists():
                if str(venv_site_packages) not in sys.path:
                    sys.path.insert(0, str(venv_site_packages))
                    paths_added.append(str(venv_site_packages))
                    print(f"✓ Added venv site-packages: {venv_site_packages}")

                # Also add venv binary path to system PATH
                venv_bin = venv_path / ('Scripts' if sys.platform == 'win32' else 'bin')
                if venv_bin.exists():
                    os.environ['PATH'] = f"{venv_bin}{os.pathsep}{os.environ.get('PATH', '')}"
                    print(f"✓ Added venv bin directory to PATH: {venv_bin}")

            # Add user site-packages
            user_site = Path(site.getusersitepackages())
            if user_site.exists() and str(user_site) not in sys.path:
                sys.path.insert(0, str(user_site))
                paths_added.append(str(user_site))
                print(f"✓ Added user site-packages: {user_site}")

            # Add BSG-IDE specific paths
            bsg_paths = [
                Path.home() / '.local/lib/bsg-ide',
                Path.home() / '.local/share/bsg-ide',
                user_site / 'bsg_ide',
                venv_site_packages / 'bsg_ide'
            ]

            for path in bsg_paths:
                if path.exists() and str(path) not in sys.path:
                    sys.path.insert(0, str(path))
                    paths_added.append(str(path))
                    print(f"✓ Added BSG-IDE path: {path}")

            # Update PYTHONPATH environment variable
            if paths_added:
                pythonpath = os.environ.get('PYTHONPATH', '')
                new_pythonpath = os.pathsep.join(paths_added + ([pythonpath] if pythonpath else []))
                os.environ['PYTHONPATH'] = new_pythonpath
                print(f"✓ Updated PYTHONPATH: {new_pythonpath}")

            # Try to fix package installation issues
            self.fix_package_installations()

            return True

        except Exception as e:
            print(f"✗ Error setting up Python paths: {str(e)}")
            traceback.print_exc()
            return False
    def fix_package_installations(self):
        ...
        try:
            ...
            # Try to install problematic packages with specific error handling
            packages = [
                ('PyMuPDF', '1.23.7'),
                ('python-magic', '0.4.27')
            ]
            ...
            for package, version in packages:
                try:
                    ...
                    subprocess.run(
                        [str(pip_path), 'install', '--no-cache-dir', f'{package}=={version}'],
                        check=True,
                        capture_output=True,
                        text=True
                    )
                    ...
                    if package == 'python-magic' and sys.platform == 'linux':
                        print("Installing libmagic1...")
                        try:
                            subprocess.run(['sudo', 'apt-get', 'install', '-y', 'libmagic1'],
                                        check=True)
                            # Try installing python-magic again
                            subprocess.run(
                                [str(pip_path), 'install', '--no-cache-dir', f'{package}=={version}'],
                                check=True
                            )
                            print(f"✓ Successfully installed {package} after installing libmagic1")
                        except:
                            print("✗ Failed to install libmagic1")

            return True

        except Exception as e:
            print(f"✗ Error fixing package installations: {str(e)}")
            return False

    def setup_pympress(self) -> bool:
        """Verify and setup pympress with proper path handling"""
        try:
            # Ensure Python paths are set up
            if not hasattr(self, 'venv_paths'):
                self.setup_python_paths()

            self.write("Checking pympress installation...\n")
            pympress_path = None
            ...
            for path in self.venv_paths:
                if path.exists():
                    pympress_path = path
                    self.write(f"✓ Found pympress at: {path}\n", "green")
                    break

            if not pympress_path:
                # Try to get path using which command
                try:
                    # Use updated PYTHONPATH when running which
                    env = os.environ.copy()
                    result = subprocess.run(
                        ['which', 'pympress'],
                        capture_output=True,
                    ...
                    if result.stdout:
                        pympress_path = Path(result.stdout.strip())
                        self.write(f"✓ Found pympress using which: {pympress_path}\n", "green")
                except Exception as e:
                    self.write(f"Warning: Could not locate pympress: {e}\n", "yellow")

            if not pympress_path:
                # Try installing if not found
                self.write("pympress not found. Installing...\n")
                try:
                    # Install using the virtual environment's pip if available
                    pip_cmd = [sys.executable, '-m', 'pip', 'install', '--user', 'pympress']
                    result = subprocess.run(
                        pip_cmd,
                        check=True,
                        capture_output=True,
                        text=True,
                        env=os.environ  # Use updated environment
                    )
                    ...
                    if result.stdout:
                        pympress_path = Path(result.stdout.strip())
                        self.write(f"✓ Installed pympress at: {pympress_path}\n", "green")
                    else:
                        raise FileNotFoundError("pympress not found after installation")
                except Exception as e:
                    self.write(f"✗ Error installing pympress: {str(e)}\n", "red")
                    return False

            # Store the path for later use
            self.pympress_path = pympress_path
            return True

        except Exception as e:
            self.write(f"✗ Error setting up pympress: {str(e)}\n", "red")
            return False


    def verify_pdf_dependencies(self):
        """Verify PDF-related dependencies"""
        try:
            # Check for PyMuPDF
            import fitz

            # Check for system PDF viewer
            if sys.platform.startswith('linux'):
                viewers = ['xdg-open', 'evince', 'okular', 'atril', 'firefox']
                has_viewer = any(shutil.which(viewer) for viewer in viewers)
                if not has_viewer:
                    self.write_to_terminal(
                        "Warning: No PDF viewer found. Please install evince, okular, or another PDF viewer.",
                        "yellow"
                    )
            ...
            try:
                self.write_to_terminal("Installing PDF dependencies...")
                subprocess.check_call([
                    sys.executable, "-m", "pip", "install", "--user",
                    "PyMuPDF==1.23.7"
                ])
                self.write_to_terminal("✓ PDF dependencies installed", "green")
                return True

            except Exception as e:
                self.write_to_terminal(
                    f"Error installing PDF dependencies: {str(e)}\n" +
                    "Please install manually: pip install PyMuPDF==1.23.7",
                    "red"
                )
                return False
#-------------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------------------------------------------------
    def setup_output_redirection(self):
        """Set up permanent output redirection to terminal"""
        if hasattr(self, 'terminal'):
            # Keep reference to original stdout/stderr
            self.original_stdout = sys.stdout
            self.original_stderr = sys.stderr

            # Create redirectors for standard output and errors
...
#------------------------------------------------------------------------------
    ...
    def write(self, text: str, color: str = "white") -> None:
        ...
        try:
            ...
            # If terminal write fails, try original stdout as fallback
            if hasattr(self, 'original_stdout'):
                print(f"Terminal write error: {str(e)}", file=self.original_stdout)


    def run_pdflatex(self, tex_file: str) -> bool:
        """Run pdflatex with output to built-in terminal"""
        try:
            ...
            if return_code == 0:
                self.write("✓ pdflatex completed successfully\n", "green")
            else:
                self.write("✗ pdflatex failed\n", "red")

            return return_code == 0

        except Exception as e:
            self.write(f"✗ Process error: {str(e)}\n", "red")
            return False
    ...
    def preview_pdf(self) -> None:
        ...
        if not os.path.exists(pdf_file):
            ...
            if not os.path.exists(pdf_file):
                messagebox.showerror("Error", "Could not generate PDF file.")
                return
        ...
        try:
            ...
            if sys.platform.startswith('win'):
                ...
                # Try multiple PDF viewers on Linux
                viewers = ['xdg-open', 'evince', 'okular', 'atril', 'firefox']
                for viewer in viewers:
                    try:
                        subprocess.run([viewer, pdf_file], check=True)
                        break
    ...
    def create_default_logo(self):
        ...
        if not hasattr(self, 'logo_image'):
            try:
                ...
                draw.polygon([(10, 40), (25, 10), (40, 40)], fill='red')

                # Save to resources directory
                resources_dir = Path.home() / '.local' / 'share' / 'bsg-ide' / 'resources'
                logo_path = resources_dir / 'airis4d_logo.png'
                img.save(logo_path)

                # Create icon version
                icon_path = resources_dir / 'bsg-ide.png'
                img.save(icon_path)
    ...
    def present_with_notes(self):
        ...
        try:
            ...
            if not os.path.exists(abs_pdf_path):
                self.write("PDF not found. Generating...\n")
                self.generate_pdf()
            ...
            self.write("Checking pympress installation...\n")
            if not hasattr(self, 'pympress_path') and not self.setup_pympress():
                messagebox.showerror(
                    "Error",
            ...
            # Launch presentation with pympress using absolute paths
            self.write("Launching pympress presentation viewer...\n")
            try:
                # Use stored path and add to PATH temporarily
                env = os.environ.copy()
                if hasattr(self, 'pympress_path'):
                    pympress_dir = str(self.pympress_path.parent)
                    if 'PATH' in env:
                        env['PATH'] = f"{pympress_dir}:{env['PATH']}"
                    else:
                        env['PATH'] = pympress_dir

                # Launch pympress with updated environment
                result = subprocess.run(
                    [str(self.pympress_path), abs_pdf_path],
                    env=env,
                    capture_output=True,
                    text=True
                )
                ...
                if result.returncode == 0:
                    ...
                    if result.stderr:
                        self.write(f"Error from pympress: {result.stderr}\n", "red")
                    raise Exception("pympress failed to start")
                ...
                self.write(f"pympress '{abs_pdf_path}'\n")

                # Show PATH for debugging
                self.write("\nCurrent PATH:\n")
                self.write(env.get('PATH', 'PATH not set') + "\n")

        except Exception as e:
            self.write(f"✗ Error launching presentation: {str(e)}\n", "red")
