...
#-------------------------------------------------------Capture Screen ---------------------------------------
    ...
    def capture_screen(self) -> None:
        """Screen capture with animation support and proper transparency"""
        try:
            import pyautogui
            ...
            options = self.create_capture_options()
            if not options:  # User cancelled
                return
            ...
            class TransparentCapture:
                def __init__(self, parent, options):
                    ...
                    self.overlay.attributes('-type', 'splash')  # Linux-specific for transparency
                    ...
                    self.overlay.geometry(f"{total_width}x{total_height}+0+0")
                    ...
                    self.overlay.bind('<Escape>', lambda e: self.cleanup())
                ...
                def capture_frame(self, bbox):
                    """Capture a single frame"""
                    self.overlay.withdraw()
                    self.overlay.update()
                    time.sleep(0.1)  # Let the screen update
                ...
                def end_selection(self, event):
                    ...
                    try:
                        if self.options['mode'] == 'animation':
                            ...
                            # Capture frames
                            for i in range(self.options['frames']):
                                progress_bar['value'] = (i + 1) / self.options['frames'] * 100
                                progress_label['text'] = f"Capturing frame {i+1}/{self.options['frames']}"
                                progress.update()

                                frame = self.capture_frame(bbox)
                            ...
                            self.frames[0].save(
                                ...
                                duration=int(self.options['delay'] * 1000),
                                loop=0
                            )
                        else:
                        ...
                        # Update media entry
                        self.parent.media_entry.delete(0, 'end')
                        self.parent.media_entry.insert(0, f"\\file media_files/{filename}")

                        messagebox.showinfo(
                            "Success",
                            f"{'Animation' if self.options['mode'] == 'animation' else 'Screenshot'} saved as:\n{filename}"
                        )
            ...
            # Create capture instance and start
            capture = TransparentCapture(self, options)
    ...
    def create_capture_options(self):
        """Create dialog for capture options"""
        dialog = ctk.CTkToplevel(self)
        dialog.title("Screen Capture Options")
        ...
        # Capture mode
        ctk.CTkLabel(dialog, text="Capture Mode:").pack(pady=5)
        ...
        anim_frame = ctk.CTkFrame(dialog)
        anim_frame.pack(pady=10, padx=10, fill='x')

        frames_var = tk.IntVar(value=10)
        delay_var = tk.DoubleVar(value=0.5)

        def update_anim_settings(*args):
            if mode_var.get() == "animation":
                anim_frame.pack(pady=10, padx=10, fill='x')
            else:
                anim_frame.pack_forget()

        mode_var.trace('w', update_anim_settings)

        # Animation settings
        ctk.CTkLabel(anim_frame, text="Number of frames:").pack()
        ctk.CTkEntry(anim_frame, textvariable=frames_var).pack()

        ctk.CTkLabel(anim_frame, text="Delay between frames (seconds):").pack()
        ctk.CTkEntry(anim_frame, textvariable=delay_var).pack()

        def confirm():
            options.update({
                'mode': mode_var.get(),
                'frames': frames_var.get(),
                'delay': delay_var.get()
            })
            dialog.destroy()
