...
def table(matrix, head=None, left=None, width=10, sep="  ", uni=None):
...
class Sets:
    ...
    def __init__(self, x, y=None): # Sets method
        ...
        self.xo = None
        self.yo = None
        self.lists() # ensure lists of real 1D arrays
...
def table(matrix, head=None, left=None, width=10, csv=False, uni=None):
    """
    ...
    csv : bool, default False
        Flag to render table as a csv.
    uni : bool, default None
        Flag to use Unicode characters. If None, use Config.uni.
    """

    def fixed_width_string(x, w=6):
        """
    ...
    # Check that width is between 6 and 2 less than the terminal width.
    cols, _ = term_size()
    width = min(max(width, 6), cols - 2)
    ...
    if len(head) > 0:
        ...
        if not csv:
            row_str = ""
            if len(left) > 0:
                row_str += horz_bar*width + left_sep
    ...
    for n_row, vals in enumerate(matrix):
        row_str = ""
        if len(left) > n_row:
            row_str += fixed_width_string(left[n_row], width) + left_sep
        elif len(left) > 0:
            row_str += horz_bar*width + left_sep
...
def colors():
    ...
    print(white, end="")
    for i in range(8):
        print(white + "  " + BKGND + f"{i}m{i:^4}", end="")
    print(black)
    for i in range(8, 16):
        print(black + "  " + BKGND + f"{i}m{i:^4}", end="")
    ...
    for l in range(6):
        ...
        for r in range(6):
            ...
            for c in range(6):
                i = 16 + 6*l + c + r*36
                print(reset + "  " + BKGND + f"{i}m{i:^6}", end="")

    # Grays
    print(white + "\n\n", end="")
    for i in range(232, 244):
        print(white + " " + BKGND + f"{i}m{i:^3}", end="")
    print(black + "\n", end="")
    for i in range(244, 256):
        print(black + " " + BKGND + f"{i}m{i:^3}", end="")
    print(RESET)
