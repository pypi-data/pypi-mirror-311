...
if __name__ == "__main__":
    ...
    db = DynamicDatabase(host='localhost', user='root', password='actowiz', database='practice')

    # List of names and ages (dynamic data)
    names = ["Ramesh", "Suresh"]
    ages = [30, 25]

    # Metadata with primary key and auto-increment
    user_data = {
        ('id', 'INT', None, 'primary_key', 'auto_increment'): [],
        ('name', 'VARCHAR', 200, None): names,
        ('age', 'INT', None, None): ages
    }
    ...
    # # Update user age
    # Custom update query
    custom_query = "UPDATE users SET age = 50 WHERE age = 35 AND name = 'Ramesh';"

    # Execute the custom query directly
    db.update('users', custom_query=custom_query)

    print("After updating Ramesh's age:")
    users = db.read('users')
    print(users)
    #
    # # Delete a user
    # db.delete('users', {'name': 'Suresh'})
    # print("After deleting Suresh:")
    # users = db.read('users')
    # print(users)
