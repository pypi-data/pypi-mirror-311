...
def find_files_parameters(v, files):
    ...
    if isinstance(v, dict):
        if "sub_configs" in v:
            res = {}
            for lp, sub_config_path in v["sub_configs"].items():
                global_basename = os.path.basename(sub_config_path)
                with open(sub_config_path) as sub_config_file:
                    sub_config = json.load(sub_config_file)
                find_files_parameters(sub_config, files)
                with tempfile.NamedTemporaryFile('w', delete=False) as fp:
                    json.dump(sub_config, fp, indent=1)
                    fp.close()

                res[lp]=("${TMP_DIR}/%s" % global_basename)
                files[global_basename] = (global_basename, open(fp.name, 'rb'))
            v["sub_configs"] = res
...
parser_launch.add_argument('-c_trans', '--cpus_trans', type=int,
                           help='number of cpus for translation task - if not provided, '
                                'will be obtained from pool config')
parser_launch.add_argument('-g_trans', '--gpus_trans', type=int,
                           help='number of gpus for translation task - if not provided, '
                                'will be obtained from pool config')
parser_launch.add_argument('-g_score', '--gpus_score', type=int,
                           help='number of gpus for scoring task - if not provided, '
                                'will be obtained from pool config')
...
def _parse_local_filename(argument, files):
    ...
    if os.path.isdir(argument):
        argument = find_files_parameters(argument, files)
        return argument
...
def process_request(s_list, cmd, subcmd, is_json, args, auth=None):
    ...
    if cmd == "service" and subcmd == "list":
        ...
        if cmd == "task" and subcmd == "launch":
            ...
            content["ngpus_trans"] = args.gpus_trans
            content["ngpus_score"] = args.gpus_score
            ...
            if 'lps' in args and args.lps:
                content["lps"] = args.lps
