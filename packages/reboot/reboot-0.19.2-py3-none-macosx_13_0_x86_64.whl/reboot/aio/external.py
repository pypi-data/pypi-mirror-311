    ...
    def __init__(
        ...
        *,
        ...
        if gateway is not None:
            if url is not None:
                raise ValueError(
                    ...
                    "'url' or 'gateway', not both"
                )

            url = f"{'http' if secure_channel is None or not secure_channel else 'https'}://{gateway}"
            secure_channel = None
        ...
        if url is not None:
            ...
            if scheme == '' and secure_channel is None:
                raise ValueError(
                    "ExternalContext expects a URL including an explicit "
                    "'http' or 'https' or you must pass 'secure_channel'"
                )
            ...
            if scheme != '' and secure_channel is not None:
                raise ValueError(
                    "ExternalContext should be constructed by either passing "
                    "'url' that includes an explicit 'http' or 'https' or by "
                    "passing 'secure_channel', not both"
                )

            channel_manager = _ChannelManager(
                StaticResolver(address if address != '' else path),
                secure_channel
                if secure_channel is not None else scheme == 'https',
            )
            ...
            raise ValueError(
                "ExternalContext construction with 'secure_channel' is "
                "invalid when also passing 'channel_manager'"
            )
...
async def retry_context_unless_pods_have_failed(
    *,
    ...
    """
    context = ExternalContext(name=name, url=url, gateway=gateway)
