...
from reboot.settings import ENVVAR_REBOOT_NODEJS_EXTENSIONLESS
...
class NodejsRebootProtocPlugin(TypescriptRebootProtocPlugin):
    ...
    def _nodejs_states(
        ...
        for state in states:
            result.append(
                NodejsState(
                    proto=state.proto,
                    services=self._nodejs_services(file, state.services),
                )
            )
        return result
    ...
    def _nodejs_clients(
        ...
        for client in clients:
            result.append(
                NodejsClient(
                    proto=client.proto,
                    services=self._nodejs_services(file, client.services),
                    state=client.state,
                )
            )
        return result
    ...
    def add_language_dependent_data(self, file: BaseFile) -> BaseFile:
        ...
        nodejs_file: BaseFile = NodejsFile(
            ...
            states=self._nodejs_states(file, file.states),
            clients=self._nodejs_clients(file, file.clients),
            ...
            nodejs_extensionless=os.environ.get(
                ENVVAR_REBOOT_NODEJS_EXTENSIONLESS, "false"
            ).lower() == "true",
        )
