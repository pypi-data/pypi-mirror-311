...
from reboot.log import log_at_most_once_per
from typing import (
    AsyncGenerator,
    AsyncIterator,
    Awaitable,
        ...
        *,
        ...
        page = await self._sidecar_client.colocated_range(
            ...
            start=(
                ...
                StateRef.from_id(SORTED_MAP_ENTRY_TYPE_NAME, StateId(start))
            ),
            end=(
                None if end is None else
                StateRef.from_id(SORTED_MAP_ENTRY_TYPE_NAME, StateId(end))
            ),
            limit=limit,
            transaction=transaction,
        )
