...
class StoreOnceAction(argparse.Action):
    ...
    def _error(self) -> str:
        return f"found more than one instance of --{self.dest.replace('_', '-')}=VALUE"
...
class StoreOnceIncludingEnvvarsAction(StoreOnceAction):
    ...
    def _error(self) -> str:
        return (
            super()._error() + " (possibly via environment variable" +
...
def add_common_channel_args(subcommand: SubcommandParser):
    ...
    # formed?
    # TODO: We should replace these with a single --application-url parameter or
    # such.
    subcommand.add_argument(
        '--gateway-address',
        type=str,
        help="the gateway address of the Reboot application",
        required=True,
    )
    subcommand.add_argument(
