...
def test_valid_collection_transfer(
    ...
    with httpx.Client() as from_client, httpx.Client() as to_client:
        ...
        panoramax_cli.status.wait_for_upload_sets(
            panoramax_with_token, to_client, [panoramax_cli.model.UploadSet(id=usid)]
        )
        ...
        u = httpx.get(
            f"{panoramax_with_token_for_elie.url}/api/upload_sets/{usid}"
        ).raise_for_status()

        assert u.json()["metadata"] == {
            "original:collection_id": load_data.upload_sets[0]
            .associated_collections[0]
            .id,
            "original:instance": panoramax_with_token.url,
        }

        # Note: we should not be able to access this upload set with elysee credentials
        ...
        cols_id = {
            c["id"] for c in existing_cols.json()["links"] if c["rel"] == "child"
        }
...
def test_valid_user_me_transfer(panoramax_with_token, load_data, user_credential):
    ...
    with httpx.Client() as c:
        ...
        us = [
            panoramax_cli.status.get_uploadset_files(
                panoramax_with_token, c, panoramax_cli.model.UploadSet(id=usid)
            )
            for usid in usIds
        ]
        assert len(us) == 2
