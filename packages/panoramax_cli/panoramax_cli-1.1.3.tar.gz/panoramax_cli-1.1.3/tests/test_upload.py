...
def test_path_to_upload_sets_basic(datafiles):
    ...
    for f in us.files:
        assert f.externalMetadata == None
...
def mock_api_post_collection_fail(respx_mock):
    respx_mock.post(MOCK_API_URL + "/api/upload_sets").mock(
...
def test_upload_set_creation_failure_incompatible(respx_mock, datafiles):
    ...
    respx_mock.get(MOCK_API_URL + "/api/configuration").respond(
...
def test_upload_with_invalid_file(respx_mock, datafiles):
    ...
    respx_mock.post(f"{MOCK_API_URL}/api/upload_sets/{usId}/files").mock(
        side_effect=file_upload_mocked
    )
...
def test_local_checks_reportfile(datafiles):
    ...
    upload.local_checks(picf, None, False, reportFile)
    ...
    with reportFile.open("r", encoding="utf-8") as f:
        ...
        assert len(res["pictures_exif_issues"]) == 0
        assert len(res["pictures_duplicates"]) == 0
