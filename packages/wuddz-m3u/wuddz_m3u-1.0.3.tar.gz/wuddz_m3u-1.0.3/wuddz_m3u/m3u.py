...
class M3U:
    ...
    def __convert(self, fn: str, d: str):
        ...
        :param d: String Output Directory Path.
        """
    ...
    def __mkdir(self, d: str=''):
        """
        Create Default Directories.
        :param d: Optional String Output Directory Path (Default = M3U On User's Desktop).
        """
        self.hd = path.join(path.expanduser('~'), 'Desktop', 'M3U')
        if d:self.hd = d
        self.cd = path.join(self.hd, 'Created')
        self.dd = path.join(self.hd, 'Downloads')
        self.pd = path.join(self.hd, 'Parsed')
        self.vf = path.join(self.hd, 'm3u_valid.txt')
        [mkdir(p) for p in [self.hd,self.cd,self.dd,self.pd] if not path.exists(p)]
    ...
    def __create(self, fa: list, dd: str, tt: str='', ss: int=1, ep: int=1, am: int=0):
        ...
        :param dd: String Output Directory Path.
    ...
    def __check(self, u: str, fd: str, vf: str, d: bool=False, to: int=5):
        """
        ...
        :param fd: String Output Directory Path.
        :param vf: String Output File To Save Valid Links To.
        :param d: Bool Specifying Download Playlist (Default=False)
        :param to: Optional Integer Specifying Connection Timeout (Default = 5 seconds)
        """
        um = u.split('/')
        hd = {
"Host": um[2],
...
"Accept-Encoding": "gzip, deflate"
        ...
        res = requests.get(u, headers=hd, timeout=to, verify=False, stream=d)
    ...
    def main(self, args: dict):
        ...
        if args.create:self.__create(fa, self.cd, tt=args.title, ss=args.season, ep=args.episode, am=args.amount)
        elif ml:
            c = 0
            with ThreadPoolExecutor(max_workers=args.threads) as exe:
                for m in ml:
                    while len(tl) > args.threads and not qt:
                        try:
                            tl.remove(next(as_completed(tl)))
                        except KeyboardInterrupt:qt = True
                    if qt:
                        clear_screen()
                        print(self.red + "\n[-]Closing Running Tasks Be Patient Fam...\n" + self.white)
                        break
                    try:
                        c += 1
                        if args.legit:tl.append(exe.submit(self.__check, m, self.dd, self.vf, d=args.download))
                        elif args.convert:tl.append(exe.submit(self.__convert, m, self.cd))
                        elif args.parse:
                            tl.append(exe.submit(self.__search, m, self.pd, t=args.title, g=args.group, h=args.hosts, c=c))
                    except KeyboardInterrupt:qt = True
        clear_screen()
        if self.vc != 0:
        ...
        else:print(self.red+"\n...Nothing There Fam, Do It Right..."+self.white)
...
"""
    ...
    parser.add_argument("-o", "--output", type=str, default='', help="Output Directory (e.g C:\\M3U | Default = 'M3U' Folder On User's Desktop.")
    ...
    parser.add_argument("-to", "--timeout", type=int, default=5, help="M3u Check/Download Connection Timeout (e.g 10 | Default = 5).")
