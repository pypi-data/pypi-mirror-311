...
class Connection(object):
    ...
    def make_request(self, method, bucket="", key="", data="",
        ...
        resp = make_request(self.host, self.port, self.access_key_id,
                            ...
                            query_args, headers, data, metadata, method=method, is_secure=self.is_secure,
                            domain_mode=self.domain_mode, timeout=timeout, ua_addon=self.ua_addon,
                            block_size=self.block_size, proxy_host=self.proxy_host, proxy_port=self.proxy_port,
    ...
    def generate_url(self, expires_in, method, bucket='', key='', headers=None,
        ...
        if response_headers:
            for k, v in list(response_headers.items()):
                ...
                extra_qp_encoded.append("%s=%s" % (k, parse.quote(v)))
        if extra_qp:
