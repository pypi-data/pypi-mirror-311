...
from ks3.utils import compute_md5, compute_encrypted_md5, BlockThreadPoolExecutor
...
class Key(object):
    ...
    def get_presigned_url(self, expires_in, headers=None, expires_in_absolute=False, version_id=None, domain=False):
        ...
        url = self.bucket.connection.generate_url(expires_in, 'PUT',
                                                  ...
                                                  headers=headers, expires_in_absolute=expires_in_absolute,
    ...
    def send_file(self, fp, method="PUT", headers=None, cb=None, num_cb=10,
        ...
        """
        return self._send_file_internal(fp, method=method, headers=headers, cb=cb, num_cb=num_cb,
        ...
        if self.bucket is not None:
            ...
            if self.bucket.connection.local_encrypt and self.size:
                ...
                try:
                    ...
                    if hasattr(e, 'strerror') and e.strerror == 'Broken pipe':
                        reason = 'Bucket:' + self.bucket.name + ', Key:' + self.name + ', Error:Broken pipe'
                        raise KS3ClientError(reason)
                    if hasattr(e, 'response') and e.response.status == 400:
                        if e.error_code == 'InvalidDigest':
                            raise KS3ClientError('The Content-MD5 you specified was invalid.')
                    else:
                        raise e
    ...
    def get_object_tagging(self, version_id=None, headers=None):
        ...
        body = response.read()
        if response.status == 200:
            objectTagging = ObjectTagging(status=response.status, reason=response.reason, headers=response.headers,
                                          raw_body=body)
