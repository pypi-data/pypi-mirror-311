...
class FindList:
    ...
    def __init__(self, path: Path, ci_list_path: Path, board: str, source: str, sct_job_path: Path = '') -> None:
        ...
        # self._data_frame = pd.DataFrame(columns=['_function_name','_py_path_list','_x86_path_list','_hw_path_list','_team_info'], index=[], dtype=object)

    def create_target(self, target_name: str) -> TargetElement:
        # print(self._data_frame)
        # self._data_frame = pd.DataFrame(self._data_frame, index = [target_name])
        # print(f"after T {self._data_frame}")
        return TargetElement(target_name)

    def find_case_name_in_py(self, target_element: TargetElement, sub_path: Path = "") -> None:
        find_case_files = os.listdir(self._path) if not sub_path else os.listdir(sub_path)
        ...
        for find_case_file in find_case_files:
            if '.py' not in find_case_file:
                continue
            case_list_path = os.path.join(self._path, find_case_file) if not sub_path else os.path.join(sub_path, find_case_file)
            ...
            if (not (search_board or search_fh)) and self._board == "thor":
                # print(f"continue target is {target_name}")
                continue
            function_name = ""
            team_info = ''
            pre_line = ''
            ...
            with open(case_list_path, "r") as f:
                for line in f:
                    ...
                    if matchObj:
                        # if 'test_FH_NR_With_UlPhy' in find_case_file:
    ...
    def _check_gate_file_vaildation(self, find_case_file: str) -> int:
        ...
        if 'check.yaml' in str(check_gate_list_file):
            ...
            check_gate_list_file = re.sub(r'post', 'check-nightly', check_gate_list_file)
            if _is_list_name_in_file(file_name, check_gate_list_file):
                return 1
    ...
    def check_case_vaildation(self, target_element: TargetElement) -> int:
        ...
        # print(f"function name is {target_element._function_name}")
        if not target_element._function_name:
        ...
        if self._source == 'py' and self._get_list_max_len(target_element._py_path_list) == 0:
            ...
            dict_data = {
                '_function_name': str(target_element._function_name),
                '_py_path_list': str(target_element._py_path_list),
                '_x86_path_list': str(target_element._x86_path_list),
                '_hw_path_list': str(target_element._hw_path_list),
                '_team_info': str(target_element._team_info),
            }

            self._data_frame = pd.DataFrame(dict_data, dtype=object, index=[0])
            # self._data_frame.set_index('name', inpalce=True)

            # self._data_frame.loc[[target_element._target_name],['_function_name']] = target_element._function_name
            # self._data_frame.loc[[target_element._target_name],['_py_path_list']] = str(target_element._py_path_list)
            # self._data_frame.loc[[target_element._target_name],['_x86_path_list']] = str(target_element._x86_path_list)
            # self._data_frame.loc[[target_element._target_name],['_hw_path_list']] = str(target_element._hw_path_list)
            # self._data_frame.loc[[target_element._target_name],['_team_info']] = target_element._team_info
            # self._data_frame.loc[target_element._target_name] = [
            #     # target_element._target_name,
            #     target_element._function_name,
            #     target_element._py_path_list,
            #     target_element._x86_path_list,
            #     target_element._hw_path_list,
            #     target_element._team_info,
            # ]

            # self._data_frame.append([
            #     target_element._target_name,
            #     target_element._function_name,
            #     target_element._py_path_list,
            #     target_element._x86_path_list,
            ...
            #     target_element._team_info,
            #     ],
            #     columns = ['_target_name','_function_name','_py_path_list','_x86_path_list','_hw_path_list','_team_info'])
            # print(f"df is {self._data_frame.values}")
        return result
