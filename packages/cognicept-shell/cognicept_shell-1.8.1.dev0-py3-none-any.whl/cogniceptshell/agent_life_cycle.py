...
class AgentLifeCycle:
    ...
    def display_version(object, args):
        ...
        for x in data['Container Name']:
            if x in object._docker_images:
                ...
                if len(image_data) > 1:
                    latest_version_data = object.get_version_tag_from_latest(image_data,object.ecr_client)
    ...
    def parse_compose(object, compose_agent):
        ...
        if compose_agent.get("ipc"):
            options["ipc_mode"] = compose_agent.get("ipc")
        if compose_agent.get("pid_mode"):
            options["pid_mode"] = compose_agent.get("pid")
