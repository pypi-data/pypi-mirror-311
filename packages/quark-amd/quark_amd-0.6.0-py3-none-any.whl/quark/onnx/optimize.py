...
from .quant_utils import (QUANT_OP_TYPES, DEQUANT_OP_TYPES, get_clip_min_max)
...
class Optimize(object):
    ...
    def fuse_layer_norm(self) -> None:
        ...
        def _get_opset_version(model: onnx.ModelProto) -> Any:
        ...
        opset_version = _get_opset_version(onnx_model.model)
        if opset_version < 17:
        ...
        for node in onnx_model.model.graph.node:
            if node.op_type == "Add":
                try:
                    add1_node = node
                    add1_inp_0 = add1_node.input[0]
                    add1_inp_1 = add1_node.input[1]
                    add1_inp_0_node = tensor_to_producer_dict[add1_inp_0]
                    if add1_inp_0_node.op_type == "Mul":
                        mul_node = add1_inp_0_node
                        mul_inp_0 = mul_node.input[0]
                        mul_inp_1 = mul_node.input[1]
                        mul_inp_0_node = tensor_to_producer_dict[mul_inp_0]
                        if mul_inp_0_node.op_type == "Div":
                            div_node = mul_inp_0_node
                            div_inp_0 = div_node.input[0]
                            div_inp_1 = div_node.input[1]
                            div_inp_0_node = tensor_to_producer_dict[div_inp_0]
                            div_inp_1_node = tensor_to_producer_dict[div_inp_1]
                            if div_inp_0_node.op_type == "Sub" and div_inp_1_node.op_type == "Sqrt":
                                sqrt_node = div_inp_1_node
                                sqrt_inp_0 = sqrt_node.input[0]
                                sqrt_inp_0_node = tensor_to_producer_dict[sqrt_inp_0]
                                if sqrt_inp_0_node.op_type == "Add":
                                    add2_node = sqrt_inp_0_node
                                    add2_inp_0 = add2_node.input[0]
                                    add2_inp_1 = add2_node.input[1]
                                    add2_inp_0_node = tensor_to_producer_dict[add2_inp_0]
                                    if add2_inp_0_node.op_type == "ReduceMean":
                                        red1_node = add2_inp_0_node
                                        red1_inp_0 = red1_node.input[0]
                                        red1_inp_0_node = tensor_to_producer_dict[red1_inp_0]
                                        if red1_inp_0_node.op_type == "Pow":
                                            pow_node = red1_inp_0_node
                                            pow_inp_0 = pow_node.input[0]
                                            pow_inp_1 = pow_node.input[1]
                                            pow_inp_0_node = tensor_to_producer_dict[pow_inp_0]
                                            if pow_inp_0_node.op_type == "Sub":
                                                sub_node = pow_inp_0_node
                                                sub_inp_0 = sub_node.input[0]
                                                sub_inp_1 = sub_node.input[1]
                                                sub_inp_0_node = tensor_to_producer_dict[sub_inp_0]
                                                sub_inp_1_node = tensor_to_producer_dict[sub_inp_1]
                                                if sub_inp_0_node.op_type and sub_inp_1_node.op_type == "ReduceMean":
                                                    red2_node = sub_inp_1_node
                                                    red2_inp_0 = red2_node.input[0]
                                                    red2_inp_0_node = tensor_to_producer_dict[red2_inp_0]
                                                    if red2_inp_0_node.op_type:
                                                        temp_node = red2_inp_0_node
                                                        logger.info(f"Found LayerNorm ops from {node.name}.")
                                                        nodes_to_remove_list = [
                                                            add1_node, mul_node, div_node, sqrt_node, add2_node,
                                                            red1_node, pow_node, sub_node, red2_node
                                                        ]
                                                        remove_nodes.extend(nodes_to_remove_list)
                                                        delete_init_name_list = [pow_inp_1, add2_inp_1]
                                                        for delete_init_name in delete_init_name_list:
                                                            delete_init = onnx_model.get_initializer(delete_init_name)
                                                            remove_inits.append(delete_init)
                                                        inp = temp_node.output[0]
                                                        out = add1_node.output[0]
                                                        layernorm_node = onnx.helper.make_node(
                                                            "LayerNormalization", [inp, mul_inp_1, add1_inp_1], [out],
                                                            node.name + "/layer_normalization")
                                                        onnx_model.add_node(layernorm_node)
                                                        logger.info(
                                                            f"Converted LayerNorm ops from {node.name} to LayerNormalization."
                                                        )
                except Exception as e:
                    logger.debug(
                        f"FuseLayerNorm is enabled, but {node.name} does not meet the matching rules:{e}, skipping this node"
                    )
        onnx_model.remove_nodes(remove_nodes)
        onnx_model.remove_initializers(remove_inits)
        onnx_model.clean_initializers()
