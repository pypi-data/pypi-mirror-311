...
class SimulateDPU(object):
    ...
    def __init__(self, model: onnx.ModelProto, should_quantize_node: Callable[[Any], bool],
    ...
    def insert_mul(self, node: onnx.NodeProto, scale: float) -> None:
        constant_node = onnx.helper.make_node('Constant',
                                              inputs=[],
        ...
        for node in self.model.graph.node:
            if node.op_type == 'LeakyRelu' and self.should_quantize_node(node):
                ...
                if alpha_attr:
                    ...
                    logger.info(f"Found Leaky ReLU node {node.name} with alpha={ori_alpha}. "
        ...
        for node in self.model.graph.node:
            if node.op_type == 'Sigmoid' and self.should_quantize_node(node):
                hard_sigmoid_node = onnx.helper.make_node('HardSigmoid',
                                                          ...
                                                          outputs=node.output,
                                                          name=node.name)
                hard_sigmoid_alpha = onnx.helper.make_attribute("alpha", 1. / 6.)
        ...
        for node in self.model.graph.node:
            if node.op_type in ['AveragePool', 'GlobalAveragePool'] and self.should_quantize_node(node):
                ...
                if shape_to_check and (kh * kw > 0):
                    ...
                    logger.info('Rescale {} {} with factor {} to simulate DPU behavior.'.format(
                        node.op_type, node.name, scale))
                else:
                    logger.warning('Do not support rescale {} {} to simulate DPU behavior.'.format(
    ...
    def convert_softmax_to_dpu_version(self) -> None:
        ...
        for node in nodes:
            ...
            if len(new_nodes):
                ...
                logger.info('Softmax {} to simulate DPU behavior under opset {}.'.format(node.name, opset_version))
            else:
                logger.warning('Softmax {} to simulate DPU behavior under opset {} failed.'.format(
                    node.name, opset_version))
    ...
    def convert_instance_norm_to_dpu_version(self) -> None:
        ...
        for node in self.model.graph.node:
            if node.op_type == 'InstanceNormalization' and self.should_quantize_node(node):
                ...
                logger.info('InstanceNormalization node {} to simulate DPU behavior by {}.'.format(
