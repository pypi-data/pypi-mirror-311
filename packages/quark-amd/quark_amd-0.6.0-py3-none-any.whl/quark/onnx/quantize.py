...
from .quantizers.matmul_nbits_quantizer import MatMulNBitsQuantizer, DefaultWeightOnlyQuantConfig, GPTQWeightOnlyQuantConfig
...
def check_fast_fintune_arguments(extra_options: Dict[str, Any], activation_type: Union[QuantType, VitisQuantType],
    ...
    fp_types = [VitisQuantType.QFloat16, VitisQuantType.QBFloat16]
    if activation_type in fp_types or weight_type in fp_types:
        raise ValueError("Fast finetune does not support float16 or bfloat16.")
    else:
        if not is_ort_version_below("1.19.0"):
...
def quantize_static(
    ...
    print_quantize_info(model_input, model_output, calibration_data_reader, calibration_data_path, quant_format,
                        ...
                        reduce_range, activation_type, weight_type, nodes_to_quantize, nodes_to_exclude, optimize_model,
                        use_external_data_format, calibrate_method, execution_providers, enable_npu_cnn,
                        enable_npu_transformer, specific_tensor_precision, debug_mode, convert_fp16_to_fp32,
                        convert_nchw_to_nhwc, include_cle, include_sq, include_fast_ft, extra_options)
    if print_summary:
    ...
    if enable_npu_cnn or enable_npu_transformer or quant_format is VitisQuantFormat.QDQ:
    ...
    if (fuse_instance_norm or fuse_l2_norm or fuse_layer_norm or convert_bn_to_conv
    ...
    if extra_options.get("UseMatMulNBits", False):
        ...
        algo_config: Union[DefaultWeightOnlyQuantConfig, GPTQWeightOnlyQuantConfig, None] = None
...
def quantize_dynamic(
    ...
    print_quantize_dynamic_info(model_input, model_output, op_types_to_quantize, per_channel, reduce_range, weight_type,
                                nodes_to_quantize, nodes_to_exclude, use_external_data_format, debug_mode,
                                extra_options)
