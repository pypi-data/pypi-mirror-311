...
from quark.torch.export.nn.modules.export_operator import AWQ_QUANT_DTYPES
...
def get_quant_config(
    quant_config: Dict[Any, Any], layer_type: Type[nn.Module], layer_name: str
) -> tuple[bool, Optional[Dict[Any, Any]], Optional[Dict[Any, Any]], Optional[Dict[Any, Any]], Optional[Dict[Any,
                                                                                                             Any]]]:

    if quant_config["quant_method"] == "fp8":
        if quant_config["ignored_layers"] is not None:
            for ignored_layer in quant_config["ignored_layers"]:
                if fnmatch.fnmatch(layer_name, ignored_layer):
                    return False, None, None, None, None

        weight_dic = {"dtype": "fp8_e4m3", "qscheme": "per_tensor", "dynamic": False}
        inp_dic = None
        out_dic = None
        if quant_config["activation_scheme"] == "dynamic":
            inp_dic = {"dtype": "fp8_e4m3", "qscheme": "per_tensor", "dynamic": True}
            out_dic = {"dtype": "fp8_e4m3", "qscheme": "per_tensor", "dynamic": True}
        elif quant_config["activation_scheme"] == "static":
            inp_dic = {"dtype": "fp8_e4m3", "qscheme": "per_tensor", "dynamic": False}
            out_dic = {"dtype": "fp8_e4m3", "qscheme": "per_tensor", "dynamic": False}

        return True, weight_dic, None, inp_dic, out_dic

    elif quant_config["quant_method"] == "awq":
        if quant_config["modules_to_not_convert"] is not None:
            for modules_to_not_convert in quant_config["modules_to_not_convert"]:
                if fnmatch.fnmatch(layer_name, modules_to_not_convert):
                    return False, None, None, None, None

        weight_dict: Dict[str, Any] = {}
        weight_dict["qscheme"] = "per_group"
        weight_dict["axis"] = 1
        weight_dict["group_size"] = quant_config["group_size"]
        weight_dict["round_method"] = None
        weight_dict["scale_type"] = "float"

        if quant_config["bits"] == 8:
            if not quant_config["zero_point"]:
                weight_dict["dtype"] = "int8"
                weight_dict["symmetric"] = True
            else:
                weight_dict["dtype"] = "uint8"
                weight_dict["symmetric"] = False
        else:
            if not quant_config["zero_point"]:
                weight_dict["dtype"] = "int4"
                weight_dict["symmetric"] = True
            else:
                weight_dict["dtype"] = "uint4"
                weight_dict["symmetric"] = False
        return True, weight_dict, None, None, None

    else:
        if "exclude" in quant_config and quant_config["exclude"] is not None:
            for exclude_layer in quant_config["exclude"]:
                if fnmatch.fnmatch(layer_name, exclude_layer):
                    return False, None, None, None, None

        if "layer_quant_config" in quant_config and quant_config["layer_quant_config"] is not None:
            if layer_name in quant_config["layer_quant_config"]:
                tmp_config = quant_config["layer_quant_config"][layer_name]

        if "layer_type_quant_config" in quant_config and quant_config['layer_type_quant_config'] is not None:
            if layer_type.__name__ in quant_config["layer_type_quant_config"]:
                tmp_config = quant_config["layer_type_quant_config"][layer_type.__name__]

        tmp_config = quant_config["global_quant_config"]
        return True, tmp_config["weight"], tmp_config["bias"], tmp_config["input_tensors"], tmp_config["output_tensors"]
...
class QuantConfigParser:
    ...
    @staticmethod
    def from_custom_config(custom_config_dict: Dict[str, Any], is_bias_quantized: bool) -> Config:
        """
        ...
        if custom_config_dict["quant_method"] == "fp8":
            ...
            output_tensors = None  # This one is assumed, we have no way from the saved config to know.
