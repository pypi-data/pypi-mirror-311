...
def set_cell(sheet, i, j, value, number_format='General', font=None, fill=None, border=None, alignment=None):
	...
	if isinstance(value, datetime.datetime):
		# 日期时间类需要设置格式默认值
		number_format	= 'yyyy-mm-dd hh:mm:ss' if number_format=='General' else number_format
...
def get_records_with_title(sheet, title_row=1):
	...
	for column in range(1, sheet.max_column+1):
		v	= sheet.cell(title_row, column).value
		# None用空白字符串表示
...
if __name__ == '__main__':
	...
	# book	= openpyxl.Workbook()
	# sheet	= book.active
	# set_cell(sheet, 1, 1, "321", border=cell_border())
	# set_cell(sheet, 2, 3, data["1"]['records'][3]['key1'], border=cell_border(sides='left, bottom'))
	# book.save('t.xlsx')
	# csv_save_as_xlsx('1.csv', overwrite=True)
	pass