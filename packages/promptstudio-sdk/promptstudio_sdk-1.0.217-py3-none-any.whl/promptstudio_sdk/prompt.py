...
class PromptManager(Base):
    ...
    async def windowmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            if platform_name == "claude" and prompt_details.get("is_session_enabled"):
                assistant_reply = response
            elif platform_name == "claude" and not prompt_details.get(
                "is_session_enabled"
            ):
            ...
            if self.is_logging:
                ...
                endpoint = f"/save_bypass_logs/{user_prompt_id}"
                payload = {
    ...
    async def fullmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            prompt_details["system_prompt"] = self.replace_placeholders(
                prompt_details["system_prompt"], variables
            )
    ...
    async def summarizedmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            prompt_details["is_session_enabled"] = interaction_request.get(
            ...
            if prompt_details["ai_platform"] == "claude":
                ...
                if (
                    prompt_details.get("is_session_enabled")
                    and prompt_details["ai_platform"] == "claude"
                ):
                    new_summarized_content = summary_response
                elif prompt_details[
                    "ai_platform"
                ] == "claude" and not prompt_details.get("is_session_enabled"):
            ...
            #     "response": {"response1": response1, "summary": new_summarized_content},
            #     "session_id": session_id,
            # }
            ...
            return {
                ...
                "data": {
                    ...
                    "response": {
                        "response1": response1,
                        "summary": new_summarized_content,
                    },
                    "session_id": session_id,
                },
            }
    ...
    async def old_summarizedmemory_save_log_ai_interaction_prompt(
        self,
        user_prompt_id: str,
        interaction_request: Dict,
    ) -> Dict:
        ...
        try:
            ...
            if (
                prompt_details["response_format"] == "object"
                and prompt_details["ai_platform"] != "gemini"
            ):
            ...
            if prompt_details["ai_platform"] == "claude":
                new_summarized_content = create_summary["content"]

                ai_response = {"type": "text", "text": response["content"][0]["text"]}
            ...
            elif prompt_details["ai_platform"] == "claude":
            ...
            if prompt_details.get("is_session_enabled") is True:
                ...
                return {
                    "data": {
                        ...
                        "session_id": None,  # Use the passed session_id
                    }
                }
        except Exception as e:
    ...
    async def _no_cache_direct_ai_request(
        ...
        try:
            ...
            platform_config = result.get("platformConfig", {})
            system_message = self.replace_placeholders(
                result["messages"]["systemMessage"], variables
            )
            ...
            if self.bypass:
                ...
                if not is_session_enabled:
                    return await self._no_cache_direct_ai_request(
                        prompt_id, user_message, variables, version
                    )
                if memory_type == "summaryMemory":
                    # raise ValueError(
                    #     "Memory type 'summaryMemory' is not supported. Please use 'summarizedMemory' instead."
                    # )
    ...
    async def _make_ai_platform_request(
        ...
        if prompt_details["system_prompt"]:
            prompt_details["system_prompt"] = self.clean_system_message(system_message)
