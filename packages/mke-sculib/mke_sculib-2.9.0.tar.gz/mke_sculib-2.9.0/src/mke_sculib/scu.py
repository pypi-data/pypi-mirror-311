...
class ScuEvent():
    ...
    def pprint(self, seperate_time=False):
        ...
        if seperate_time:
            ...
            return '[{timestamp}] event {level} | {codeName}: {path} {currentState} => {resultCode} ({resultMessage})'.format(path=path, **flatdict)
...
class DataStreamHandler():
    ...
    def log(self, *args, **kwargs):
        if hasattr(self.parent, 'log'):
            t = kwargs.pop('t', None)
            if t is None:                    
                t = datetime.datetime.utcnow().isoformat()[:-4].replace('T', ' ') + 'Zl'
...
class TrackHandler(object):
    ...
    def get_session_df(self, interval_ms=100, do_sanity_test=True):
        ...
        if do_sanity_test:
            ...
            t_start_df = df.index.first().to_datetime()
            t_end_df = df.index.last().to_datetime()
...
# ██████  ██ ███████ ██   ██     ███████    ██    ██   ██ ██   ██    ██        ██         ███████    ██     ██████  ██      
    ...
    def start(self, az_start=None, el_start=None, band_start=None, az_speed=None, el_speed=None, send_default_configs=True, interlock_acknowledge_on_start=True, **kwargs):
        ...
        if not 'all' in configs_scu:
            self.log('WARNING: logger config "all" is missing in SCO logger configurations', color=colors.WARNING)
...
# ███████  ██████  ██████      ███████  ██████   ██████   ██████  ███████ ██   ██     ██   ██ ██      ██ 
    ...
    def configs_create_minimal(self):
        ...
        chans = set(self.get_channel_list())
        configs_scu_dc = {d['name']: set(d['paths']) for d in self.logger_configs()}
        if 'all' in configs_scu_dc:
            assert configs_scu_dc['all'] == chans, f'config "all" differs from all available channels! N_scu = {len(configs_scu_dc["all"])} vs N_all = {len(chans)}'
        else:
            self.create_logger('all', chans)

        if 'full_configuration' in configs_scu_dc:
            assert configs_scu_dc['full_configuration'] == chans, f'config "full_configuration" differs from all available channels! N_scu = {len(configs_scu_dc["all"])} vs N_all = {len(chans)}'
        else:
...
#  ██████  ███████    ██        ██ ██   ████ ██      ███████  ██████  ██   ██ 
    ...
    def get_from_scu_influx_dt(self, dt_seconds, token=None, org = 'OHBDC', timeout=360_000, channels = None):
        tend = Time.now()
        tstart = tend - dt_seconds * u.s
        return self.get_from_scu_influx(tstart, tend, token=token, org = org, timeout=timeout, channels = channels)
