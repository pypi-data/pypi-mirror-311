...
class Trades:
    ...
    def limit(
        ...
        for i, row in trades.iterrows():
            ...
            for end in range(start + 1, row[EXIT]):
                if row[TYPE] == LONG:
                    take_return = bid[end] / ask[start]
                    stop_return = bid[end] / ask[start]

                else:
                    take_return = bid[start] / ask[end]
                    stop_return = bid[start] / ask[end]

                current_stop = (
                    (stop if constant_stop else stop[end])
                    if stop is not None else stop_return
                )
                current_take = (
                    (take if constant_take else take[end])
                    if take is not None else take_return
                )

                take_profit = take and (take_return > current_take)
                stop_loss = stop and (stop_return < current_stop)
                ...
                if dynamic and (
                    ((row[TYPE] == LONG) and (ask[end] < ask[start])) or
                    ((row[TYPE] == SHORT) and (bid[end] > bid[start]))
                ):
                    start = end
