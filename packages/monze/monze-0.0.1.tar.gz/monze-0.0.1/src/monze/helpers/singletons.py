...
class UserSettings(metaclass=UserSetingsMeta):
	...
	def init_props(self):  # PROPS pad en OD pad EN het od pad is te vinden in de props pickle
		try:
			...
			Mainroad.loglog(f'Kon geen settingsfile maken of lezen')
			sys.exit(f'Kon geen settingsfile maken of lezen')
		...
		if self._settings_path == '' \
			...
			sys.exit(f'Kon geen settingsfile maken of lezen op locatie: {self._settings_path}')
		try:
	...
	def set_props(self, d: dict):
		Pickles.write(self._settings_path, d)
		self.init_props()
