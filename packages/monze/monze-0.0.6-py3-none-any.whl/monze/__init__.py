...
while odpath is None:
	...
	if odpath is None or odpath == '':  # cancel
		# sys.exit_message('No Open OneDrive dir _BUTTERFLY given.')
		continue
...
app.config['FLASK_DEBUG'] = app.config['DEBUG'] = True
...
@app.post("/login")
def index_post():
	if not "alias" in request.form or not "password" in request.form:
		return redirect("/login")

	alias = Casting.str_(request.form["alias"], default="")
	password = Casting.str_(request.form["password"], default="")
	user = Mainroad.login(alias, password)
	if user is None:
		return redirect("/login")
...
def run():
	if not app.config['initialized']:
		...
		print("Served cold on http://127.0.0.1:5000/")
