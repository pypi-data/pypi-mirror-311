...
def get_qmin_qmax_for_qType(qType: int, reduce_range: bool = False, symmetric: bool = False) -> Any:
    ...
    if qType in ONNX_FP_QTYPES_LIST:
        raise NotImplementedError("This function is not implemented for (b)float 16 as not needed.")
...
def is_node_needs_annotated(model: onnx.ModelProto, node: onnx.NodeProto) -> bool:
    ...
    if (node.op_type in remove_qdq_op_type or is_clip_with_min_max(model, node, 0, 6)
            or is_clip_with_min_max(model, node, 0, 1)):
        return True
    return False
...
def compute_scale_zp_fp(rmin: np.ndarray[Any, Any],
    ...
    if symmetric:
        ...
        rmid = (rmax + rmin) / 2
        zero_point = np.array((0 - rmid / scale), dtype=scale.dtype)
...
def quantize_data_pof2s(data: np.ndarray[Any, Any],
    ...
    if qType in ONNX_FP_QTYPES_LIST:
        zero_point, scale = compute_scale_zp_fp(rmin, rmax, qType, symmetric=symmetric)
...
def print_quantize_info(model_input: str, model_output: str, calibration_data_reader: str,
                        ...
                        nodes_to_exclude: List[str], optimize_model: bool, use_external_data_format: bool,
                        calibrate_method: Union[Any, PowerOfTwoMethod,
                                                Int16Method], execution_providers: Union[List[str], None],
                        enable_npu_cnn: bool, enable_npu_transformer: bool, specific_tensor_precision: bool,
                        debug_mode: bool, convert_fp16_to_fp32: bool, convert_nchw_to_nhwc: bool, include_cle: bool,
                        include_sq: bool, include_fast_ft: bool, extra_options: Dict[str, Any]) -> None:
    """
...
def print_quantize_dynamic_info(model_input: Union[str, Path, onnx.ModelProto], model_output: Union[str, Path],
                                ...
                                weight_type: Union[Any, VitisQuantType], nodes_to_quantize: List[str],
                                nodes_to_exclude: List[str], use_external_data_format: bool, debug_mode: bool,
...
class ONNXQuantizedModel(object):
    ...
    def find_target_node_qdqs(self, target_node_name: str) -> Dict[str, Any]:
        """ Get the qdqs on all inputs and outputs of the target node.
        """
...
def print_quantized_info(quantized_model_path: str, debug_mode: bool) -> None:
    try:
        ...
        op_type_with_weights_bias = [
            "Conv", "ConvTranspose", "Gemm", "LayerNormalization", "EmbedLayerNormalization", "InstanceNormalization",
            "PRelu"
        ]
        quantized_data = []
        ...
        for node in onnx_model.model.graph.node:
            if len(node.input) >= 1:
                if node.input[0] in tensor_to_node_dict and tensor_to_node_dict[
                    ...
                    weights_init = None
                    bias_init = None
                    if weights_dq_node is not None and node.op_type in op_type_with_weights_bias:
                        weights_init = tensor_to_init_dict[weights_dq_node.input[2]]
                        weights_dq_data_type = weights_init.data_type
                    if bias_dq_node is not None and node.op_type in op_type_with_weights_bias:
