...
def compile_vai_custom_op_cpu(kernel_name: str, compile_dir: str, extra_cuda_cflags: List[str],
    ...
    try:
        ...
        if '-DTORCH_OP' in extra_cflags:
            sources_list.append(str(path / "src/torch_op_bfp.cc"))
        ...
        load(name=kernel_name,
             ...
             extra_include_paths=[str(path / "include")],
             verbose=False)
...
def compile_custom_op() -> None:
    ...
    extra_cflags.append(include_path_prefix + "/src/in/cpu")
    extra_cflags.append(include_path_prefix + "/src/qdq/cpu")
    extra_cflags.append(include_path_prefix + "/src/lstm/cpu")
