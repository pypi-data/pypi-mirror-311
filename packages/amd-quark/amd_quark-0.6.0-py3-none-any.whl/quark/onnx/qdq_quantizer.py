...
class VitisQDQQuantizer(VitisONNXQuantizer):
    ...
    def __init__(
        ...
        # We quantize Bias using Int32 by default except Float16 or BFloat16 quantization
...
class VitisExtendedQuantizer(VitisQDQQuantizer):
    ...
    def _add_fn_pair_for_weight(self,
                                weight_proto: TensorProto,
                                axis: Any = None,
                                zp_type: Any = None,
                                qdq_pair: Any = False) -> None:
        weight_name = weight_proto.name
        ...
        if axis is not None:
            if self.opset_version < 13:
                raise ValueError("Per-Channel support with QDQ format requires onnx opset version 13 or above.")
            q_weight_name, zp_name, scale_name = self.quantize_weight_per_channel(weight_name,
                                                                                  zp_type,
                                                                                  axis,
                                                                                  self.calibrate_method,
                                                                                  keep_float_weight=qdq_pair)
        else:
            q_weight_name, zp_name, scale_name = self.quantize_initializer(
                weight_proto,
                zp_type,
    ...
    def _add_fn_pair_for_activation(self, tensor_name: str, scale_name: str, zp_name: str) -> Any:
        if (self.dedicated_qdq_pair and tensor_name in self.tensor_to_its_receiving_nodes
                and len(self.tensor_to_its_receiving_nodes[tensor_name]) > 1):
            num_dedicated_qdq_pair = len(self.tensor_to_its_receiving_nodes[tensor_name])
            for i in range(num_dedicated_qdq_pair):
                ...
                if self.quant_format == VitisQuantFormat.FixNeuron:
                    fn_node_name_postfix = tensor_name + "_FixNeuron" + postfix
                    self._create_fn_nodes(
                        tensor_name,
                        tensor_name_dequant_output_postfix,
                        fn_node_name_postfix,
                        scale_name,
                        zp_name,
                    )
                elif self.quant_format == VitisQuantFormat.QDQ:
    ...
    def _quantize_normal_tensors(self) -> None:
        for tensor_name, tensor_info in self.tensors_to_quantize.copy().items():
            ...
            if not tensor_info.is_shared:
                zp_type = None
                qdq_pair = self.add_qdq_pair_to_weight
                if tensor_name in self.quantized_tensor_type:
                    zp_type = self.quantized_tensor_type[tensor_name]
                    if zp_type in [VitisQuantType.QBFloat16]:
                        qdq_pair = True
                # Quantize the input
                initializer = find_by_name(tensor_name, self.model.initializer())
                if initializer:
                    self._add_fn_pair_for_weight(initializer, tensor_info.axis, zp_type, qdq_pair)
                else:
                    used_scale, used_zp = self.find_quant_scale_zp(tensor_name)
                    data_found, scale_name, zp_name, _, _ = self._get_quantization_params(
    ...
    def _quantize_refine(self) -> None:
        ...
        if self.quant_format == VitisQuantFormat.QDQ and (not self.use_qdq_vitis_custom_ops):
...
class VitisBFPQuantizer(VitisQDQQuantizer):
    ...
    def __init__(self,
        ...
        Args:
            ...
            extra_options (Optional[Dict[str, Any]], optional): Additional options for quantization.
        """
        # For the self-defined quantization type, no need to pass the type to super class
        super().__init__(model, per_channel, reduce_range, mode, static, QuantType.QInt8, QuantType.QInt8,
                         tensors_range, nodes_to_quantize, nodes_to_exclude, op_types_to_quantize, calibrate_method,
    ...
    def _add_fn_pair_for_weight(self, weight_proto: TensorProto) -> None:
        ...
        self.model.replace_input_of_all_nodes(weight_name, quantized_weight_name)
        ...
        self._create_fn_nodes(weight_name, quantized_weight_name, add_dequant_suffix(weight_name), axis, convert_to)
