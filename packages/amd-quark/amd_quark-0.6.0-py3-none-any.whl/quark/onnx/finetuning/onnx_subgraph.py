...
TARGET_OPS = ['Conv', 'ConvTranspose', 'InstanceNormalization', 'VitisInstanceNormalization', 'Gemm', 'MatMul']
Q = ["QuantizeLinear", "VitisQuantizeLinear"]
DQ = ["DequantizeLinear", "VitisDequantizeLinear"]
BFPFN = ["BFPFixNeuron"]
ACT_OPS = ['Relu', 'PRelu', 'LeakyRelu', 'Tanh', 'Clip', 'Sigmoid', 'Softmax']
...
class Subgraph(object):
    ...
    def __init__(self, fmodel_path: str, qmodel_path: str, data_reader: Any, extra_options: Dict[str, Any]) -> None:
        ...
        optim_device = extra_options.get('FastFinetune', {}).get('OptimDevice', 'cpu').lower()
        self.ort_infer_device = extra_options.get('FastFinetune', {}).get('InferDevice', optim_device).lower()
    ...
    def check_node_with_weights_num(self, node: NodeProto, weights_check: int) -> bool:
        weight_count = 0
        for input_name in node.input:
            input_node = None
            for n in self.qmodel.graph.node:
                if input_name in n.output:
                    input_node = n
                    break

            if input_node and input_node.op_type == "DequantizeLinear":
                if any(init.name == input_node.input[0] for init in self.qmodel.graph.initializer):
                    weight_count += 1
                    if weight_count > weights_check:
                        return False
        return weight_count == weights_check
    ...
    def find_start(self, node: NodeProto) -> Any:
        try:
            ...
            if inp_0_node.op_type in DQ and inp_1_node.op_type in DQ:
                dq_node = inp_0_node
                dq_inp_0 = dq_node.input[0]
                dq_inp_0_node = self.q_tensor_to_producer[dq_inp_0]
                w_node = inp_1_node
                w_inp_0 = w_node.input[0]
                w_inp_0_node = self.q_tensor_to_producer[w_inp_0]
                # ensure weight of conv is initializer
                if dq_inp_0_node.op_type in Q and isinstance(w_inp_0_node, TensorProto):
                    return dq_inp_0_node.input[0]
            elif inp_0_node.op_type in BFPFN and inp_1_node.op_type in BFPFN:
                w_node = inp_1_node
                w_inp_0 = w_node.input[0]
                w_inp_0_node = self.q_tensor_to_producer[w_inp_0]
                # ensure weight of conv is initializer
                if isinstance(w_inp_0_node, TensorProto):
                    return inp_0_node.input[0]
            # support QDQ weights-only
            elif inp_0_node.op_type not in DQ and inp_1_node.op_type in DQ:
                w_node = inp_1_node
                w_inp_0 = w_node.input[0]
                w_inp_0_node = self.q_tensor_to_producer[w_inp_0]
                if isinstance(w_inp_0_node, TensorProto):
    ...
    def find_end(self, node: NodeProto) -> Tuple[Any, bool]:
        try:
            ...
            if out_0_node.op_type in Q:
            ...
            elif out_0_node.op_type in BFPFN:
                if not self.output_qdq:
                    return out_0, False
                else:
                    # Remove qdq after output
                ...
                if a_node.op_type in ACT_OPS:
                    ...
                    if not self.output_qdq:
                        ...
                        elif a_out_0_node.op_type in BFPFN:
    ...
    def get_subgraph_qmodel(self) -> Any:
        ...
        for node in self.qmodel.graph.node:
            if node.op_type in self.target_ops:
                if node.op_type == "MatMul" and not self.check_node_with_weights_num(node, 1):
                    continue
            ...
            for n in self.fmodel.graph.node:
                if n.name == k:
                    ...
                    if n.input[0] in self.f_tensor_to_producer:
                        ...
                        if isinstance(
                                parent,
                                NodeProto) and parent.op_type in DQ and parent.input[0] in self.f_tensor_to_producer:
    ...
    def extract_submodel_weight(
        ...
        for _, name in enumerate(self.subgraph_qmodel):
            ...
            for node in model.graph.node:
                if node.name == name:
                    weight_name = self.q_tensor_to_producer[node.input[1]].input[0]
                    if self.quantize_bias and len(node.input) == 3:
                        bias_name = self.q_tensor_to_producer[node.input[2]].input[0]
                    is_weight_init = False
                    for init in model.graph.initializer:
                        if init.name == weight_name:
                            weight = numpy_helper.to_array(init)
