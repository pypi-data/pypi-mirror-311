...
from quark.onnx.finetuning.create_torch.quant_base_fns import BFPQuantizer
from quark.onnx.finetuning.create_torch.quant_base_ops import AdaroundQuantizer
...
class ModelOptimizer:
    ...
    def _replace_quantizer(self, quant_module: torch.nn.Module) -> None:
        ...
        if isinstance(default_quantizer, BFPQuantizer):
            # BFPQuantizer does not support adaround
            raise NotImplementedError("Can't apply adaround for FixNeuron")
        elif not isinstance(default_quantizer, AdaroundQuantizer):
            # Create a new quantizer that has "alpha" parameter
            quantizer = AdaroundQuantizer(default_quantizer.scale, default_quantizer.zero_point,
                                          default_quantizer.min_q, default_quantizer.max_q, default_quantizer.ch_axis,
