...
def _create_fq_from_qspec(quantization_spec: Optional[QuantizationSpec]) -> FakeQuantizeBase:
...
def _get_fq_map(edge_or_node_to_group_id: Dict[EdgeOrNode, int],
                edge_or_node_to_qspec: Dict[EdgeOrNode, QuantizationSpec]) -> Dict[EdgeOrNode, FakeQuantizeBase]:
    obs_or_fq_map: Dict[EdgeOrNode, FakeQuantizeBase] = {}
    group_id_to_obs_or_fq: Dict[int, FakeQuantizeBase] = {}
    for edge_or_node, qspec in edge_or_node_to_qspec.items():
        group_id = edge_or_node_to_group_id[edge_or_node]
        if group_id not in group_id_to_obs_or_fq:
            group_id_to_obs_or_fq[group_id] = _create_fq_from_qspec(qspec)
...
def _insert_quantizer_for_quantized_module(model: GraphModule) -> None:
    ...
    for node in model.graph.nodes:
        if node.op != "call_module":
            continue
        if not isinstance(getattr(model, node.name), QUANT_CONV_MODULE):
            continue
        quantized_mod = getattr(model, node.name)
        ...
        if node.meta.get("weight_quantizer_quant_config", None) is None:
            ...
            quantized_mod._weight_quantizer = _create_fq_from_qspec(
        ...
        if quantized_mod.bias is not None or (isinstance(quantized_mod, QuantizedConvBatchNorm2d)
            ...
            if node.meta.get("bias_quantizer_quant_config", None) is None:
                ...
                quantized_mod._bias_quantizer = _create_fq_from_qspec(
...
def _instance_fkq_in_module(model: GraphModule) -> None:
...
def _insert_obs_or_fkq(model: GraphModule, edge_or_node_to_group_id: Dict[EdgeOrNode, int],
                       obs_or_fq_map: Dict[EdgeOrNode, FakeQuantizeBase]) -> None:
    '''
    ...
    for edge_or_node, group_id in edge_or_node_to_group_id.items():
        ...
        fake_quant = obs_or_fq_map[edge_or_node].to(model_device)
        if isinstance(edge_or_node, Node):
            _insert_obs_or_fq(edge_or_node, fake_quant, model)
        else:
            _insert_obs_or_fq(edge_or_node[0], fake_quant, model)
...
def insert_quantizer(model: GraphModule, ) -> GraphModule:
    ...
    obs_or_fq_map = _get_fq_map(edge_or_node_to_group_id, edge_or_node_to_qspec)  # type: ignore [arg-type]

    _insert_obs_or_fkq(model, edge_or_node_to_group_id, obs_or_fq_map)
    model: GraphModule = GraphModule(model, model.graph)
    '''
    step 2 init the Fakequantizer in QuantizedConvBatchNorm2d (the fakequantizer will not treated as Node)
    '''
    _instance_fkq_in_module(model)
    return model
