...
class ModelQuantizer:
    ...
    def _prepare_model(self, model: nn.Module) -> nn.Module:
        if self.config.quant_mode is QuantizationMode.eager_mode:
            ...
            return prepare_quant_model(model, self.config).eval()
            ...
            for module in tqdm(model.modules()):
                if isinstance(module, QuantMixin):
                    if module._weight_quantizer is not None and isinstance(module._weight_quantizer, ScaledFakeQuantize) \
                        ...
                        module.weight = module.get_quant_weight(module.weight)
                    if module._bias_quantizer is not None and isinstance(module._bias_quantizer, ScaledFakeQuantize) \
                        ...
                        module.bias = module.get_quant_bias(module.bias)
...
def from_float_and_dict(float_module: nn.Module, quant_info: Dict[str, Any], param_dict: Dict[str, torch.Tensor],
    ...
    if quant_info.get("output_quant", None) is not None:
        ...
        quant_params["output_scale"] = param_dict[layer_name + ".output_scale"]  # pragma: no cover
        quant_params["output_zero_point"] = param_dict[layer_name + ".output_zero_point"]  # pragma: no cover

    weight = None
    if quant_info.get("weight_quant", None) is not None:
        weight = QuantizationSpec.from_dict(quant_info["weight_quant"])
        quant_params["weight_scale"] = param_dict[layer_name + ".weight_scale"]
        quant_params["weight_zero_point"] = param_dict[layer_name + ".weight_zero_point"]
...
def load_params(model: Optional[nn.Module] = None,
                ...
                pth_path: str = "",
                quant_mode: QuantizationMode = QuantizationMode.eager_mode) -> nn.Module:
    """
    ...
    **Examples**:
    ...
    if quant_mode is QuantizationMode.eager_mode:
        ...
        if json_path == "" or safetensors_path == "":
            raise ValueError("Json_path and safetenros_path should not be empty if loading eager_mode quantized model")
        import json
        ...
        for name, module_info in get_name_and_info(model_dict["structure"]):
            ...
            if module_info["type"] in QUARK_QUANT_OPS:
                module = from_float_and_dict(float_module, module_info, params_dict, layer_name=name)
