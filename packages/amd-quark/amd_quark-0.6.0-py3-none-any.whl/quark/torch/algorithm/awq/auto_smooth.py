...
class AutoSmoothQuantProcessor(BaseAlgoProcessor):
    ...
    def __init__(self, model: nn.Module, quant_algo_config: Any,
    ...
    def _compute_best_scale(self,
        ...
        for i in range(n_grid):
            ...
            loss = (fp16_output - int_w_output).float().abs().pow(0.5).mean().item()  # NOTE: float prevents overflow
    ...
    def init_quant(self) -> Tuple[nn.ModuleList, Dict[str, Any], List[Any]]:
        ...
        modules, layer_kwargs, inputs = cache_model_inps(self.model, modules, [batched_samples])
