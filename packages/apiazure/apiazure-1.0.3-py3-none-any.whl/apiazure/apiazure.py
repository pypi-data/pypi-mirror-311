...
def create_headers(TOKEN=str) -> str:
    credenciales = f"{TOKEN}"
    credenciales_base64 = base64.b64encode(credenciales.encode('utf-8')).decode('utf-8')
...
def create_pr(API_URL=str, TOKEN=str, REPO_ID=str, sourceRefName=str, targetRefName=str, pr_title=str, headers=object|None, data=object|None, params=object|None) -> object:
    ...
    base_headers = {
        ...
        "Authorization": f"Basic {create_headers(TOKEN=TOKEN)}"
    }
...
def add_reviwer(API_URL=str, TOKEN=str, REPO_ID=str, pullRequestId=str, reviewerId=str, headers=object|None, data=object|None, params=object|None) -> object:
    """
    ...
    if data is not None and isinstance(data, dict):
        body.update(data)
    try:
        response = requests.put(url=f"{API_URL}/repositories/{REPO_ID}/pullrequests/{pullRequestId}/reviewers/{reviewerId}", headers=base_headers, params=base_parameters, data=json.dumps(body))
        if response.status_code == 200:
            json_response={
                "funcition_code":200,
...
def approve_pr(API_URL=str, TOKEN=str, REPO_ID=str, pullRequestId=str, reviewerId=str, headers=object|None, data=object|None, params=object|None) -> object:
    """
    ...
    if data is not None and isinstance(data, dict):
        body.update(data)
    try:
        response = requests.put(url=f"{API_URL}/repositories/{REPO_ID}/pullrequests/{pullRequestId}/reviewers/{reviewerId}", headers=base_headers, data=json.dumps(body), params=base_parameters)
        if response.status_code == 200:
            json_response={
                "funcition_code":200,
...
def complete_pr(API_URL=str, TOKEN=str, REPO_ID=str, pullRequestId=str, commitData=str, headers=object|None, data=object|None, params=object|None) -> object:
    """
    ...
    if data is not None and isinstance(data, dict):
        body.update(data)
    try:
        response = requests.post(url=f"{API_URL}/repositories/{REPO_ID}/pullrequests/{pullRequestId}", headers=base_headers, data=json.dumps(body), params=base_parameters)
        if response.status_code == 200:
            json_response={
                "funcition_code":200,
...
def get_repo_id(API_URL=str, TOKEN=str, repo_name=str, headers=object|None, params=object|None) -> object:
    """
    Retrieves the reviewer ID by team ID and user email.
    ...
    if headers is not None and isinstance(headers, dict):
        base_headers.update(headers)
    try:
        response = requests.get(url=f"{API_URL}/_apis/repositories", headers=base_headers, params=base_parameters)
        if response.status_code == 200:
            for repos in response:
                if repos['uniqueName'] == repo_name:
                    json_response={
                        "funcition_code":200,
                        "repoId": f"{repos['id']}",
                        "message":f"Obtencion de Id de usuario correcto"
