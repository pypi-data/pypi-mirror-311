...
class ClickZettaEngineSpec(BaseEngineSpec):
    ...
    def execute(  # pylint: disable=unused-argument
        ...
        try:
            ...
            logger.error("ClickZettaEngineSpec.execute: %s", ex)
    ...
    def execute_with_cursor(cls, cursor: Any, sql: str, query: Query) -> None:
        ...
        if debug_mode:
            logger.debug("Query %s: Running query: %s", cancel_query_id, sql)
        cls.execute(cursor, sql, cancel_query_id=cancel_query_id)
        if debug_mode:
            logger.debug("Query %s: Handling cursor", cancel_query_id)
