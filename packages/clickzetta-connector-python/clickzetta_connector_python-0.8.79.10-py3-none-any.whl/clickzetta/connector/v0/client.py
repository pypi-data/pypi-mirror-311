...
class Client(object):
        ...
        if "protocol" in query:
            ...
            if self.protocol == "http":
                if not port:
                    ...
                    self.service = (
                        HTTP_PROTOCOL_PREFIX + self.host + ":" + str(port)
                    )
            else:
                raise ValueError(
            ...
            if not port:
                ...
                self.service = (
                    HTTPS_PROTOCOL_PREFIX + self.host + ":" + str(port)
                )
      ...
      with self.lock:
        ...
        try:
            if not _globals["token_https_session_inited"] or not _globals["token_https_session"]:
                ...
                session.mount(self.protocol + "://",
                              HTTPAdapter(pool_connections=20, pool_maxsize=100, max_retries=retries))
                _globals["token_https_session"] = session
                _globals["token_https_session_inited"] = True
            api_response = _globals["token_https_session"].post(self.service + path, data=data, headers=HEADERS,
                                                                timeout=10)
    ...
    def set_config(self, sql: str) -> bool:
        str_lower = sql.lower()
        logger.info("setting config: %s", str_lower)
        ...
        if len(set_key_value) == 2 and set_key_value[0].strip() and set_key_value[1].strip():
            logger.info("setting config: '%s' = '%s'", set_key_value[0].strip(), set_key_value[1].strip())
    ...
    def pre_process_sql(self, sql) -> Optional[str]:
    ...
    def process_use_cmd(self, sql: str) -> bool:
        sql = re.sub(r"\s+", " ", sql)
        lower_sql = sql.lower()
        has_processed = True
            ...
            if not Client.verify_result_dict_finished(result, sql):
                ...
                if sleep_time_ms * 1.5 <= 30000:
                    ...
                    if logger.isEnabledFor(logging.DEBUG):
                        logging.debug(
                            "Get async sql job:{0} result pending, retry after {1} seconds, tried:{2}, time cost:{3}, next"
                            " sleep time:{4}".format(str(job_id.id), str(second), str(tried),
                                                     str(begin_time - datetime.now()),
                                                     str(sleep_time_ms))
                        )
                return False
            else:
