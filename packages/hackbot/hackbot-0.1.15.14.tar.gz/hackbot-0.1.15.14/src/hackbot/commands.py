...
def get_args() -> argparse.Namespace | int:
    ...
    # The --starter argument (which cannot be used with the --model argument) enables the starter plan, false means the pro plan
    run_parser.add_argument("--starter", action="store_true", help="Enable the starter plan")
    # The --model argument (which cannot be used with the --starter argument) requires the --starter argument to not be used, and specifies the model to use
    run_parser.add_argument(
        "--model",
        type=str,
        required=False,
    ...
    if args.starter and args.model:
        log.error(
            "❌ Error: The --starter and --model arguments cannot be used together, go to https://hackbot.co/pricing to upgrade your subscription to the pro plan!"
        )
        return 1
...
def hackbot_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        invocation_args = {
            ...
            "profile": "starter" if args.starter else "pro",
        }

        # Perform the analysis
        results = asyncio.run(cli_hack_target(invocation_args, args.address, args.port, args.api_key, args.source, args.output))
