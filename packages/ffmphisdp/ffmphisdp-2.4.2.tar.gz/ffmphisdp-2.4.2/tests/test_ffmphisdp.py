...
def fps_conversion_test_case(test_obj, filename, fps, expected_stop_frame, expected_frames, red_shift, green_shift):
    ...
    test_obj.assertEqual(
        len(expected_frames), expected_stop_frame, msg='incoherent test'
    )  # Check test_case is coherent
    frame_idx = 0
    while True:
        ret, frame = video_handle.read()
        if ret is False:  # indicate we reached the last frame or there was a reading error
            test_obj.assertEqual(frame_idx, expected_stop_frame, msg='stopped at wrong frame')
            break
        ...
        test_obj.assertTrue(
            ...
            msg=f'{frame_color[2]} != {expected_red} (fps: {fps}, Frame_idx {frame_idx}, Frame {frame_color}, color: red)',
        )
        test_obj.assertTrue(
            is_almost_same_color(frame_color[1], expected_green),
            msg=f'{frame_color[1]} != {expected_green} (fps: {fps}, Frame_idx {frame_idx}, Frame {frame_color}, color: green)',
        )
        frame_idx += 1


class TestffmphisDP(unittest.TestCase):
    """Tests for `ffmphisDP` package."""
    ...
    def test_video_reader_CFR(self):
        ...
        # Test ffmphisdp's ControlledFPSVideoCapture at various fps
        test_cases = [
        ...
        for fps, expected_stop_frame, expected_frames in test_cases:
            with self.subTest(str((fps, expected_stop_frame, expected_frames))):
                fps_conversion_test_case(
                    self, filename, fps, expected_stop_frame, expected_frames, red_shift, green_shift
                )

        # Cleanup
        os.remove(filename)

    def test_video_reader_VFR(self):
        ...
        for fps, expected_stop_frame, expected_frames in test_cases:
            with self.subTest((fps, expected_stop_frame, expected_frames)):
                fps_conversion_test_case(
                    self, filename, fps, expected_stop_frame, expected_frames, red_shift, green_shift
                )
            ...
            for frame_idx, expected_frames in test_cases:
                with self.subTest((frame_idx, expected_frames)):
                    ...
                    for expected_frame in expected_frames:
                        ...
                        self.assertTrue(
                            abs(int(frame_color[2]) - int(expected_red)) < 4,
                            msg=f'{frame_color[2]} != {expected_red} (fps= {fps}, Frame_idx {frame_idx}, Expected_frame: {expected_frame}, Frame {frame_color}, color: red)',
                        )
                        self.assertTrue(
                            abs(int(frame_color[1]) - int(expected_green)) < 4,
                            msg=f'{frame_color[1]} != {expected_green} (fps= {fps}, Frame_idx {frame_idx}, Expected_frame: {expected_frame}, Frame {frame_color}, color: green)',
                        )

        # Cleanup
        os.remove(filename)

    def test_video_reader_start_time(self):
        ...
        for fps, test_cases in fps_test_cases.items():
            ...
            for time_ms, expected_frames in test_cases:
                with self.subTest((time_ms, expected_frames)):
                    ...
                    for expected_frame in expected_frames:
                        ...
                        self.assertTrue(
                            is_almost_same_color(frame_color[2], expected_red),
                            msg=f'{frame_color[2]} != {expected_red} (fps= {fps}, Time_ms {time_ms}, Expected_frame: {expected_frame}, Frame {frame_color}, color: red)',
                        )
                        self.assertTrue(
                            is_almost_same_color(frame_color[1], expected_green),
                            msg=f'{frame_color[1]} != {expected_green} (fps= {fps}, Time_ms {time_ms}, Expected_frame: {expected_frame}, Frame {frame_color}, color: green)',
                        )
