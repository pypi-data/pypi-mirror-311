...
def get_device_list():
    subkey = 'SOFTWARE\\ZhiXin\\zhixin-ide'
    filePath = ''
    try:
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, subkey)
        if key:
            value = "InstallPath"
            filePath, type_no = winreg.QueryValueEx(key, value)
            winreg.CloseKey(key)
    except FileNotFoundError:
        click.echo("no find key：%s" % subkey)
        
    filePath = "{}{}".format(filePath, '/ARM/Flash') 
    file_names = []
    if os.path.isdir(filePath) and os.path.exists(filePath):   
        files = os.listdir(filePath)
        for file in files:
            if file.endswith(".svd"):
                file_name_with = os.path.basename(file)
                file_name_without = os.path.splitext(file_name_with)[0]
                file_names.append(file_name_without)
    return file_names
    ...
    [
        ...
        for option in [
            ...
            ConfigZhixinOption(
                ...
                type=click.Choice(["Cortex-M0+", "Cortex-M4"]),
            ...
            ConfigZhixinOption(
                ...
                type=click.Choice(["Z20K144M", "Z20K146M", "Z20K148M", 
                "Z20K144MS", "Z20K142MS",
                "Z20K114M", "Z20K116M", "Z20K118M",
                "Z20K114N", "Z20K116N", "Z20K118N",
                "Z20M1343L"] if len(get_device_list())==0 else get_device_list()),
            ),
            ConfigZhixinOption(
                group="generic",
                name="default_envs",
            ...
            ConfigZhixinOption(
                ...
                name="startup_file",
                description=(
                    "A location where startup.s file"
                ),
            ),
            ConfigZhixinOption(
                group="linker",
                name="linker_script",
                description=(
                    "A location where linker configuration file"
                ),
            ),
            ConfigZhixinOption(
                group="debugger",
                name="ddf_file",
                description=(
                    "A location where device description file"
                ),
            ),
            ConfigZhixinOption(
                group="debugger",
            ...
            ConfigZhixinOption(
                ...
                name="software_path",
                description=(
                    "A location where ZhiXin library"
                ),
            ...
            ConfigZhixinOption(
                ...
                name="include_dirs",
                description=(
                    "A default location for project header files." 
                    "You can use relative path variables '${PROJECT_PATH}'"
                ),
                multiple=True,
                sysenvvar="ZHIXIN_INCLUDE_DIRS",
