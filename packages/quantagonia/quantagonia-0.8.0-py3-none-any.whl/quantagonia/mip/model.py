...
class Model:
    ...
    def write(self, filename: str) -> None:
        ...
        """
        # When redirectOutput() is used, writing the model prints the model to stdout instead of the file.
        # This is a known issue in pyscipopt: https://github.com/scipopt/PySCIPOpt/issues/243.
        # For now, we capture the output and write it to a file.

        # Redirect stdout to a StringIO object
        captured_output = io.StringIO()
        sys.stdout = captured_output
        # Write the problem, which will now be captured
        self._model.writeProblem(filename, verbose=False)
        # Restore stdout
        sys.stdout = sys.__stdout__
        # Remove comment lines starting with '*' from the captured output
        filter_comments = [line for line in captured_output.getvalue().splitlines() if not line.startswith(("*", "\\"))]
    ...
    def _optimize(self) -> HybridSolverStatus:
        ...
        with tempfile.TemporaryDirectory() as tmp_path:
            ...
            try:
                ...
                res, _ = self._hybrid_solver.solve(tmp_problem)
            except ValueError as e:
