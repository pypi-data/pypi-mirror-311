...
def train_evaluate_split(model, X, y, test_size=0.2, describe='准确率', verbose=True, return_predict=False,
	...
	return train_evaluate(model, X_train, X_test, y_train, y_test, describe, verbose, return_predict, evaluate_fn, show_time)
...
def search_model_params(model_name, X_train, X_test, y_train, y_test, param_grid, result_num=5, iter_num=8,
	...
	if executor is None:
		...
		if 1 == n_proc:
			sub_n_proc = 1
			results = [_search_params(model_name, classifier, X_train, X_test, y_train, y_test, params, verbose) for params in param_grid]
		else:
			with ProcessPoolExecutor(n_proc) as e:
				futures = [e.submit(_search_params, model_name, classifier, X_train, X_test, y_train, y_test, params, verbose) for
				           params in param_grid]
				results = [f.result() for f in as_completed(futures)]
	else:
		futures = [executor.submit(_search_params, model_name, classifier, X_train, X_test, y_train, y_test, params, verbose) for
		           params in param_grid]
		results = [f.result() for f in as_completed(futures)]
	...
	for param, score in results[:result_num]:
		...
		_evaluate_params(model_name, classifier, X_train, X_test, y_train, y_test, param, iter_num, sub_n_proc, executor)
	return params
...
def search_model_params_split(model_name, X, y, param_grid, test_size=0.2, result_num=5, iter_num=8, random_state=42,
	...
	"""
	X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=random_state)
	return search_model_params(model_name, X_train, X_test, y_train, y_test, param_grid, result_num, iter_num, n_proc, executor, verbose)
...
def _search_params(model_name, classifier, X_train, X_test, y_train, y_test, params, verbose=False):
	if verbose:
		pid = os.getpid()
...
def _evaluate_params(model_name, classifier, X_train, X_test, y_train, y_test, params, iter_num, n_proc,
	...
	if executor is None:
		...
		if 1 == n_proc:
			results = [_search_params(model_name, classifier, X_train, X_test, y_train, y_test, params, verbose) for _ in range(iter_num)]
		else:
			with ProcessPoolExecutor(n_proc) as e:
				futures = [e.submit(
					_search_params, model_name, classifier, X_train, X_test, y_train, y_test, params, verbose) for _ in range(iter_num)
				]
				results = [f.result() for f in as_completed(futures)]
	else:
