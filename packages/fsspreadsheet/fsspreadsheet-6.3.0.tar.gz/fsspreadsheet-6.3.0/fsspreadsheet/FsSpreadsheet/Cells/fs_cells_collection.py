...
from ...fable_modules.fable_library.map_util import (get_item_from_dict, add_to_dict, remove_from_dict, add_to_set)
from ...fable_modules.fable_library.option import some
from ...fable_modules.fable_library.range import range_big_int
from ...fable_modules.fable_library.reflection import (TypeInfo, class_type)
from ...fable_modules.fable_library.seq import (map, iterate, is_empty, max, collect, delay, empty, singleton, min, min_by)
from ...fable_modules.fable_library.util import (ignore, IEnumerable_1, compare_primitives)
...
def FsCellsCollection__GetFirstAddress(this: FsCellsCollection) -> FsAddress:
    if True if is_empty(this._rowsCollection) else is_empty(this._rowsCollection.keys()):
        ...
        def _arrow117(__unit: None=None, this: Any=this) -> int:
            def projection(d: Any) -> int:
                ...
                return min(d.keys(), ObjectExpr114())
            ...
            d_1: Any = min_by(projection, this._rowsCollection.values(), ObjectExpr115())
            class ObjectExpr116:
                @property
                def Compare(self) -> Callable[[int, int], int]:
                    return compare_primitives

            return min(d_1.keys(), ObjectExpr116())

        return FsAddress__ctor_Z37302880(min(this._rowsCollection.keys(), ObjectExpr113()), _arrow117())
