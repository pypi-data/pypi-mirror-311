...
class DbProxy(object):
    ...
    async def close_connection_and_reset_status(self, dbinstance):
        if dbinstance.conn is not None:
            try:
                await dbinstance.conn.close()
            except Exception as close_error:
                LOG.error(f'Error closing connection: {close_error}')
                # 可以在这里添加更多的处理逻辑，如尝试重新关闭连接等
            finally:
                dbinstance.conn = None
                dbinstance.disconnected = True
        else:
            LOG.warning('Connection object is None, cannot close connection.')
    ...
    async def _execute_rdbms_result(self, dbinstance: _DbInstance, qry: query.Query, fetch_all: bool = False, fetch_one: bool = False, execution_options: dict = sqlalchemy.util.EMPTY_DICT, sql_params = None):
        ...
        try:
            ...
            await self.close_connection_and_reset_status(dbinstance)
            raise e
