...
def find_conditions(yaml_content: Union[Dict[str, Any], List[Any]], context: str = "") -> List[Dict[str, str]]:
    """
    ...
    if isinstance(yaml_content, dict):
        ...
        if "condition" in yaml_content:
            conditions.append({"context": context, "condition": yaml_content["condition"]})
        ...
        for index, item in enumerate(yaml_content):
            ...
            if isinstance(item, str) and item.strip().startswith("${{ if "):
                conditions.append({"context": f"{context} > item {index}", "condition": item.strip()})
            else:
