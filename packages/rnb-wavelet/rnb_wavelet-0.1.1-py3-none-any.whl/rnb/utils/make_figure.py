...
def plotfft(fr, po,log_scale='none', ax=None ):
...
def plotbeta(betas,ax=None):
    ...
    ax.set_facecolor('none')  # Set axis background color to transparent
    ax.plot(b[:-1], p, '.')  # Plot normalized histogram
    smoothed_p = savgol_filter(p, 10, 2)  # Smooth with a window size of 7 and polynomial order 3
