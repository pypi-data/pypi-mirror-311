...
class ParamsPool(ParamsPoolBase):
    ...
    def __init__(self, graph_id, object_id, ref=None, is_predict=False):
...
def save_params(graph_id, object_id, node_id, anchor, data, is_predict=False, ref=None):
    """保存图模块输出结果到文件中"""
...
def get_params(graph_id, object_id, node_id, anchor, is_pd=False, is_predict=False, ref=None):
    """获取图模块输出结果从文件中"""
    key = NODE_OUTPUT_KEY.format(graph_id=graph_id, node_id=node_id, anchor=anchor)
    if ref:
        folder_path = os.path.join(settings.RUN_MODEL_SAVE_PATH, graph_id, object_id, ref)
...
def delete_params(graph_id, object_id, is_predict=False, ref=None):
    """删除所有模块输出结果从文件中"""
    try:
...
def load_graph(graph_id=None, object_id=None, file_path=None, use_predict_model=False, ref=None):
    """获取图从pickle文件里"""
    if file_path:
        folder_path = os.path.dirname(file_path)
...
def has_graph(graph_id, object_id, is_predict=False, ref=None):
    """判断图模型是否存在"""
    model_name = settings.MODEL_SAVE_NAME.format(graph_id=graph_id, object_id=object_id)
    if ref:
        folder_path = os.path.join(settings.RUN_MODEL_SAVE_PATH, str(graph_id), object_id, ref)
...
def push_result(graph_id, object_id, data, is_predict=False, ref=None):
    """增加模块进行结果到文件中"""
...
def pop_result(graph_id, object_id, is_predict=False, ref=None):
    """从文件中获取模块运行结果"""
    key = MOD_RETURN_RESULT.format(graph_id=graph_id)
    if ref:
        folder_path = os.path.join(settings.RUN_MODEL_SAVE_PATH, str(graph_id), object_id, ref)
...
def clear_result(graph_id, object_id, is_predict=False, ref=None):
    """从文件中清空所有模块运行结果"""
    key = MOD_RETURN_RESULT.format(graph_id=graph_id)
    if ref:
        folder_path = os.path.join(settings.RUN_MODEL_SAVE_PATH, str(graph_id), object_id, ref)
...
def save_algo_model(graph_id, object_id, node_id, algo_mod_type, model, folder_path=None, ref=None):
    """保存深度学习模型图到文件里"""
    try:
        res = list()
        if not folder_path:
...
def load_algo_model(graph_id, object_id, node_id, algo_mod_type, model_name, folder_path=None, ref=None):
    """获取深度学习模型从文件里"""
    try:
        res = list()
        if not folder_path:
