...
class Client:
    def __init__(self, endpoint, username=None, password=None, is_secure=True):

        self.endpoint = f"ws{'s' if is_secure else ''}://{endpoint}/v1/websocket"
    ...
    async def receive_loop(self):
        while True:
            try:
                ...
                if header_id == NIL and header["kind"] == Kind.ERROR.value:
