...
from ADLStream2.data.stream import BaseStream


class RiverStream(BaseStream):
    """River stream wrapper

    This class acts as a wrapper for river streams, allowing to use ADLStream2
    ...
    ```python
        ...
        stream = ADLStream2.data.RiverStream(dataset)
        generator = ADLStream2.data.ClassificationStreamGenerator(stream)
