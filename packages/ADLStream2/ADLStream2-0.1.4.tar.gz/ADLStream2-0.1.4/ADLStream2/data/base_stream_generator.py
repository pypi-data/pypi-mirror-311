...
class BaseStreamGenerator(ABC):
    ...
    Arguments:
        stream (inherits [BaseStream]):
            Stream source to be feed to the ADLStream2 framework.
    ...
    def next(self, context: "ADLStreamContext") -> List[float]:
        ...
        Args:
            context (ADLStreamContext): ADLStream2 shared object. Used for logging.
    ...
    def run(self, context: "ADLStreamContext"):
        """The function that sends data to ADLStream2 framework

        It gets messages from the stream, preprocesses them and sends to the specific
        ADLStream2 context.

        Args:
            context (ADLStream2.ADLStreamContext): context where to send the stream data
        """
        self.stream.start()
