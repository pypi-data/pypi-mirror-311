...
def create_snow_incident(snow_config: dict, incident_url: str, snow_incident: dict, tag: dict) -> dict:
    """
    Create a new incident in ServiceNow

    :param dict snow_config: ServiceNow configuration as a dictionary
...
def create_incident(
    ...
    """
    response = None
    if iss.get("serviceNowId", "") == "" or iss["serviceNowId"] is None:
        snow_incident = {
            "description": iss["description"],
            "short_description": iss["title"],
            "assignment_group": snow_assignment_group,
            "due_date": iss["dueDate"],
            "comments": f"RegScale Issue #{iss['id']} - {config['domain']}/form/issues/{iss['id']}",
            "state": "New",
...
def get_or_create_snow_tag(snow_config: dict, tag_name: str) -> dict:
    """
...
def create_snow_tag(snow_config: dict, tag_name: str) -> Optional[dict]:
    """
    Create a new assignment group in ServiceNow or if one already exists,
    a 403 is returned from SNOW.

    :param dict snow_config: ServiceNow configuration dictionary
...
def get_snow_incidents(snow_config: dict, query: str = "", tag: Optional[dict] = None) -> List[dict]:
    """
    Get all incidents from ServiceNow
...
def get_service_now_work_notes(snow_config: dict, incidents: list) -> list:
    """
    Get all work notes from ServiceNow

    :param dict snow_config: ServiceNow's configuration dictionary
...
if __name__ == "__main__":
    sync_snow_and_regscale(
        ...
        snow_assignment_group="Testing-SNOW",
        snow_incident_type="Low",
