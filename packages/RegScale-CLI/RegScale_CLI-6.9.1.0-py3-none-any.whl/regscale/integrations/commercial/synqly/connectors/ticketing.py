...
class Ticketing(SynqlyModel):
    ...
    def validate_params(self, **kwargs) -> bool:
        ...
        if missing_params:
            ...
            for param, data in missing_params.items():
                self.logger.error(f"{param} ({data['type']}) - {data['description']}")  # type: ignore
            error_and_exit("Please provide the required parameters mentioned above.")
        return len(missing_params) == 0

    def run_sync(self, regscale_id: int, regscale_module: str, sync_attachments: bool = True, **kwargs) -> None:
        """
        ...
        try:
            ...
            self.logger.error("Error running background job: " + str(e))
            self._cleanup_handler()
            raise e
        finally:
            self._cleanup_handler()

    def fetch_integration_issues(self, **kwargs) -> list[Ticket]:
        """
        ...
        :rtype: list[Ticket]
        """
        query_filter = f"project[eq]{kwargs['project']}" if "project" in kwargs else None
    ...
    def update_regscale_issues(self, *args, **_) -> None:
        """
