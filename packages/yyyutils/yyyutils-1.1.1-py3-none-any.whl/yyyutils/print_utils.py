...
class PrintUtils:
    ...
    def set_structured_print(self, enable=True, max_str_len=100, max_level=None):
        """设置结构化打印的参数"""
    ...
    def print_structure(self, obj, level=0, prefix='', max_str_len=100, max_level=None, struct_torch=False,
                        struct_numpy=False, struct_pandas=False):
        """
        ...
        # 处理字典
            ...
            for i, (key, value) in enumerate(obj.items()):
                ...
                self.print_structure(value, level + 2, max_str_len=max_str_len, max_level=max_level,
                                     struct_torch=struct_torch, struct_numpy=struct_numpy, struct_pandas=struct_pandas)
        ...
        # 处理可迭代对象
            ...
            try:
                for i, item in enumerate(obj):
                    is_last = i == len(obj) - 1  # 可能会失败,因为有些迭代器不支持len()
                    branch = '└─' if is_last else '├─'
                    PrintUtils.original_print(f"{indent}  {branch} [{i}]")
                    self.print_structure(item, level + 2, max_str_len=max_str_len, max_level=max_level,
                                         struct_torch=struct_torch, struct_numpy=struct_numpy,
                                         struct_pandas=struct_pandas)
            except:
