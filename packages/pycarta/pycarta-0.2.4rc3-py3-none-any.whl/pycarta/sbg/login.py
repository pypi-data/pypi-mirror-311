...
# from pycarta import is_interactive
...
class SbgLoginManager:
    ...
    Note:
    - There are two types of "interactive":
      1. Fail-safe interactive: Prompt the user as a last resort.
      2. Explicit interactive: Ignore all other methods and ask the user for credentials.
    # TODO: Add an interactive switch that preempts all other login methods and asks the user for credentials.
    """
    ...
    def _api_login(self, api: str | sbg.Api) -> bool:
        ...
        if isinstance(api, str):
            try:
                ...
                logger.error(f"Failed to authenticate with provided API token: {e}")
                return False
        ...
        logger.error("Invalid API token provided for authentication.")
        return False
    ...
    def _carta_secret_login(self, profile:str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to retrieve SBG token for {profile!r}.")
            return False
        else:
            try:
                ...
                logger.error(f"Failed to authenticate with Seven Bridges using stored Carta secret.")
                return False
    ...
    def _sbg_profile_login(self, profile:str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to authenticate with SBG profile '{profile}': {e}")
            return False
    ...
    def _env_login(self) -> bool:
        ...
        if api_key:
            try:
                ...
                logger.error(f"Failed to authenticate with environment API key: {e}")
                return False
    ...
    def _interactive_login(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to authenticate with user-provided API token: {e}")
            return False
