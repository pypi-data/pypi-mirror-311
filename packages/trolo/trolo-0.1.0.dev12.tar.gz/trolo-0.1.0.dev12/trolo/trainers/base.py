...
class BaseTrainer(object):
    ...
    def execute_ddp(self, device: str):
        ...
        with open(tmp_config, 'w') as f:
            # Make all include paths absolute by resolving them relative to cfg_path
        ...
        with open(tmp_file, 'w', encoding='utf-8') as f:  # Explicitly specify UTF-8 encoding
            f.write(f'''
import os
from pathlib import Path
...
''')
        ...
        cmd = f'CUDA_VISIBLE_DEVICES={device} {python_executable} -m torch.distributed.run --nproc_per_node={num_gpus} --master_port 12321 {tmp_file}'
