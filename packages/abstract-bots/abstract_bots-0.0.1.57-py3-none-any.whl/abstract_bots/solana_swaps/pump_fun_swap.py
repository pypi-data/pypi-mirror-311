import time,requests,json,struct,os,base58,sys,json,base64,asyncio
...
from abstract_solcatcher import getEndpointUrl

from utils import *
from abstract_apis import get_headers,asyncPostRequest
...
def get_token_balance(payer,mint,commitment=None):
    response = makeLimitedCall("getTokenAccountBalance", makeParams(payer,mint=mint,commitment=commitment or 0))
...
def get_transaction(signature):
    response = asyncio.run(asyncPostRequest(url=getEndpointUrl("getTransaction"), data={"signature":signature,"maxSupportedTransactionVersion":0}))
    return response
...
def getKeys(coin_data,token_account,owner,buy=True):
        coin_data = get_pump_fun_data(str(token_account))
...
def buildTxn(mint, amount, slippage, token_account_pubkey,sol_in,token_account_instructions,buy=True):
    ...
    blockHash = asyncio.run(asyncPostRequest(url=getEndpointUrl("getLatestBlockhash"), data={"commitment":"processed"}))
...
def pump_fun_buy(mint_str: str, sol_in: float = 0.001, slippage: int = 25) -> bool:
    ...
    try:
        account_data = asyncio.run(asyncPostRequest(url=getEndpointUrl("getTokenAccountByOwner"), data={"account":owner,"mint":mint,"encoding":"base58"}))
...
def get_token_price(mint_str: str) -> float:
    try:
        ...
        return None
input(pump_fun_buy(mint_str="ANfeuKatBMBgAuAqDMXrqigxPUNqWuVa7dT84P1U4mdd",sol_in= 0.001, slippage= 25))
