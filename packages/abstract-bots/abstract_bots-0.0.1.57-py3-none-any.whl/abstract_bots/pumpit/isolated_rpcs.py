...
from abstract_solana.rate_limiter import RateLimiter
...
def rate_limit(method=None,status_code=False):
  return RateLimiter().get_rate_url(method=method,status_code=status_code)
def log_response(method=None,response=None,status_code=False):
  return RateLimiter().log_response(method=method,response=response,status_code=status_code)
...
def get_token_accounts_by_owner(owner,mint):
    return abstract_solana_rate_limited_call('get_token_accounts_by_owner', owner, TokenAccountOpts(mint))
