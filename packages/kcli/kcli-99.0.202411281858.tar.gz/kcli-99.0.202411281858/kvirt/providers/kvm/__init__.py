...
class Kvirt(object):
    ...
    def create(self, name, virttype=None, profile='kvirt', flavor=None, plan='kvirt', cpumodel='host-model',
        ...
        custom_emulator_aarch64 = custom_emulator is not None and custom_emulator.endswith('aarch64')
        aarch64 = custom_emulator_aarch64 or capabilities['arch'] == 'aarch64'
        aarch64_full = aarch64 and capabilities['kvm']
        as390x = capabilities['arch'] == 's390x'
        if aarch64:
            if custom_emulator_aarch64:
...
<backend type='emulator' version='2.0'>
        ...
        if uefi or uefi_legacy or uefi_firmware is not None or secureboot or aarch64:
            secure = 'yes' if secureboot else 'no'
            if uefi_legacy or uefi_firmware is not None:
                firmware = uefi_firmware or '/usr/share/OVMF/OVMF_CODE.secboot.fd'
