...
def warp_ndarray_rasterio(ndarray: np.ndarray,
                          in_gt: (float, float, float, float, float, float),
                          in_prj: str,
                          out_prj: str,
                          out_gt: (float, float, float, float, float, float) = None,
                          outRowsCols: (int, int) = None,
                          outUL: (float, float) = None,
                          out_res: (int, int) = None,
                          out_extent: (float, float, float, float) = None,
                          out_dtype: np.dtype = None,
                          rsp_alg: int = 0,
                          in_nodata: float = None,
                          out_nodata: float = None,
                          outExtent_within: bool = True
                          ):  # pragma: no cover
...
def warp_ndarray(ndarray: np.ndarray,
                 in_gt: (float, float, float, float, float, float),
                 in_prj: str = None,
                 out_prj: str = None,
                 out_dtype: gdal.GDT_Unknown = None,
                 out_gsd: (float, float) = (None, None),
                 out_bounds: (float, float, float, float) = None,
                 out_bounds_prj: str = None,
                 out_XYdims: (int, int) = (None, None),
                 rspAlg: str = 'near',
                 in_nodata: float = None,
                 out_nodata: float = None,
                 in_alpha: bool = False,
                 out_alpha: bool = False,
                 targetAlignedPixels: bool = False,
                 gcpList: list = None,
                 polynomialOrder: int = None,
                 options: str = None,
                 transformerOptions: list = None,
                 warpOptions: list = None,
                 CPUs: Optional[int] = 1,
                 warpMemoryLimit: Optional[int] = None,
                 progress: bool = True,
                 q: bool = False
                 ) -> (np.ndarray, tuple, str):
    r"""Warp the given numpy array with gdal.Warp and return the warped array, the GeoTransform, and the projection.
    ...
    :param out_XYdims:
    :param rspAlg:              Resampling method to use. Available methods are:
                                near, bilinear, cubic, cubicspline, lanczos, average, mode, max, min, med, q1, q2
    :param in_nodata:
    :param out_nodata:
    :param in_alpha:            Force the last band of a source image to be considered as a source alpha band.
    :param out_alpha:           Create an output alpha band to identify nodata (unset/transparent) pixels
    ...
    :param polynomialOrder:     order of polynomial GCP interpolation
    :param options:             additional GDAL options as string, e.g. '-nosrcalpha' or '-order'
    :param transformerOptions:  list of transformer options, e.g.  ['SRC_SRS=invalid']
    :param warpOptions:         list of warp options, e.g.  ['CUTLINE_ALL_TOUCHED=TRUE'],
                                find available options here: https://gdal.org/doxygen/structGDALWarpOptions.html
    :param CPUs:                number of CPUs to use (default: None, which means 'all CPUs available')
    :param warpMemoryLimit:     size of working buffer in bytes (default: 0)
    ...
    # TODO otherwise implement error_threshold=0.125
    ...
    if multiprocessing.current_process().name == 'MainProcess':
        cpus_str = str(CPUs) if CPUs and CPUs >= 1 else "ALL_CPUS"
    else:
        cpus_str = str(1)

    with gdal.config_options(
        dict(
            GDAL_NUM_THREADS=cpus_str,
            # GDAL_CACHEMAX=str(800)
        )
    ):
        ...
        res_ds = gdal.Warp(
            ...
            width=out_XYdims[0] or 0,
            height=out_XYdims[1] or 0,
            resampleAlg=rspAlg,
            srcNodata=in_nodata,
            ...
            callback_data=[0],
            errorThreshold=0.125,  # this is needed to get exactly the same output as the console version of gdal.Warp
        )
    ...
    # get outputs
    res_arr = gdal_array.DatasetReadAsArray(res_ds)
