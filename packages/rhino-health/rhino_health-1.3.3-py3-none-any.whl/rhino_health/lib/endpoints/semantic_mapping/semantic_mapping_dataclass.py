...
class SubcategoriesFilter(BaseModel):
    """
    ...
    or_list: Optional[List[str]] = None  # The items in the list are "or"ed together
    and_list: Optional[List[str]] = None  # The items in the list are "and"ed together


class VocabularyFilters(BaseModel):
    subcategories: Optional[SubcategoriesFilter] = None
    ...
    def from_dict(filters_dict):
        ...
        if filters_dict:
            if IndexFields.SUBCATEGORIES.value in filters_dict:
                vocabulary_filters.subcategories = SubcategoriesFilter(
                    **filters_dict["subcategories"]
                )
            if IndexFields.TERM_CLASS.value in filters_dict:
                vocabulary_filters.term_class = filters_dict[IndexFields.TERM_CLASS.value]
            return vocabulary_filters


class VocabularyFiltersList(BaseModel):
    filters: List[VocabularyFilters]
...
class VocabularySearch(BaseModel):
    search_term: str
...
class SemanticMappingDataclass(
    ...
    semantic_map_info: Optional[Dict[str, Any]] = {}
