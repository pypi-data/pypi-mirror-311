...
class HLSHandler(ContentHandler):
    ...
    def download(self, output_path: str = None, num_segments: int = 0, progress_callback: Optional[Callable] = None):
        sub_mapping = {}

        total_tasks = 1 + self.num_children() * (1 + num_segments)
        logger.debug(f"Number of files to download: {total_tasks}")
        if progress_callback:
            progress_callback(f"Number of files to download: {total_tasks}", total=total_tasks)
        ...
        for sub_playlist in self.sub_playlists():
            filename = self._download_sub_playlist(sub_playlist.uri, output_path, num_segments, progress_callback)
    ...
    def _download_sub_playlist(self, playlist_uri: str, output_path: str, num_segments: int = 0, progress_callback: Optional[Callable] = None):
        # extract the relative path from the URI    
        # TODO - handle absolute paths
        if playlist_uri.startswith("http"):
            raise NotImplementedError(f"The URI {playlist_uri} is an absolute URL, not a relative path")
        if playlist_uri.startswith("../"):
            raise NotImplementedError(f"The URI {playlist_uri} is a relative path, not an absolute path")
        ...
        if num_segments > 0:
            for i in range(0, num_segments):
                ...
                if segment_uri.startswith("http"):
                    raise NotImplementedError(f"The URI {segment_uri} is an absolute URL, not a relative path")
                if segment_uri.startswith("../"):
                    raise NotImplementedError(f"The URI {segment_uri} is a relative path, not an absolute path")
