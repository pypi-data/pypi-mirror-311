...
class KPP(SuperPage):
    def __init__(self, root, settings, operator, can):
        ...
        self.page_buttons = [self.ok_btn, self.abort_btn] + self.other_footer_btns
    ...
    def set_arriving_status(
        ...
        if text.lower() == "ожидается проезд авто":
            print("TT11. FROM SET ARRIVING STATUS")
    ...
    def draw_manual_pass_entry(
        ...
        self.root.bind('<Escape>',
                       lambda event: self.destroy_orup(reason="выход через кнопку Esc"))
    ...
    def destroyBlockImg(self, mode="total"):
        self.operator.ar_qdk.catch_orup_decline(
            car_number="", mode="закрытие модального окна в КПП")
    ...
    def place_car_detect_text(self, side_name):
        self.operator.place_car_detect_text(side_name)
    ...
    def operate_new_plate_recognition_trying(
        ...
        if self.blurDrawn:
            ...
            super().operate_new_plate_recognition_trying(
                current_try, max_tries, side)
