    ...
    async def proc_order__thread(self, order: dict, res: dict):
        try:
            ...
            if broadcast := order.pop("broadcast", None):
                ...
                ), None if order.pop("broadcast.self", False) else self.current_connection.id)
            if order.pop("connections", None):
                res["connections"] = {
                    conn.id: conn.properties
    ...
    async def proc_order__broadcast(self, order: dict, res: dict):
        try:
            ...
            with self.server:
                ...
                ), None if order.pop("self", False) else self.current_connection.id)
        except Exception as exc:
            self.exception_handle(order, res, exc)
            return 2
        return 0
...
class Connection(ConnectionWorker):
    ...
    async def read_iteration(self) -> None:
        ...
        await self.server.operator(
            self, bytes().join(f.payload for f in frames))
...
class ConnectionsThread(threading.Thread, ConnectionWorker):
    ...
    async def broadcast(self, payload: bytes, except_: str | None) -> None:
...
class Server(threading.Thread):
    ...
    async def broadcast(self, payload: bytes, except_: str | None = None) -> None:
