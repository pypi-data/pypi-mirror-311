...
def loadComponent(component, program: str = None):
    ...
    try:
        ...
        if downloadComponent(component) is True:
            ...
            return importComponent(component, classpath, package="components")
        raise NotSupported(f"Error: No Component {component!r} was Found: {e}") from e
...
def getComponent(component: str, program: str = None):
    try:
        ...
        logger.exception(f"Error loading component {component}: {err}", stack_info=True)
        raise ComponentError(f"Error loading component {component}: {err}") from err
