...
class HookService:
    ...
    def add_hook(self, hook: BaseHook):
        ...
        return hook.setup(app=self.app)
    ...
    def load_hooks(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        # Load Database Hooks
        self.logger.notice(":: Loading Hooks from Database.")
        try:
            loop.run_until_complete(
                self.load_db_hooks(loop)
            )
        except FlowTaskError as exc:
            print(exc)
            pass
        except Exception as exc:
            self.logger.error(
    ...
    async def load_db_hooks(self, loop: asyncio.AbstractEventLoop) -> None:
        """load_db_hooks.
