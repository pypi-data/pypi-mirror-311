...
class Strokes:
    ...
    def process(self, image: np.ndarray, angles: np.ndarray = None, points: np.ndarray = None) -> np.ndarray:
        ...
        angles[grad < 0.01] = np.random.random(angles.shape)[grad < 0.01] * 180
        ...
        for y in range(h):
            for x in range(w):
                ...
                dx, dy = X[y, x], Y[y, x]
                cv2.line(surf, (int(x + ln * dy), int(y + ln * dx)), (int(x - ln * dy), int(y - ln * dx)), (75, 75, 75),
                         1)
        return surf
