...
class Dataset(SendableMixin, ExtraFilesMixin):
    ...
    @property
    def uuid_override(self) -> str:
        """uuid for a "master" dataset in a dependency situation"""
        if self._uuid_override is None:
            return self.short_uuid
        return self._uuid_override

    @property
    def repofile(self) -> TrackedFile:
        """Returns the TrackedFile instance responsible for the repository"""
        fname = f"{self.name}-{self.uuid_override}-repo.py"
    ...
    def manifest_log(self) -> TrackedFile:
        ...
        fname = f"{self.name}-{self.uuid_override}.manifest"
        return TrackedFile(self.local_dir, self.remote_dir, fname)
    ...
    def set_downstream(self, dataset) -> None:
        ...
        dataset._uuid_override = self.uuid_override
    ...
    def set_upstream(self, dataset) -> None:
        ...
        self._mirror_dependency(dataset)
        self._uuid_override = dataset.uuid_override
        ...
        **run_args,
        ...
        for i, sub in enumerate(bash_cache):
            ...
            run_func = create_run_function(
                ...
                script_run=self.function is None,
                add_docstring=add_doc,  # add docstring for first function only
            )
            master_content.insert(0, run_func)
    ...
    def _is_finished(
        ...
        if self.run_cmd is not None and \
                self.run_cmd.returncode is not None and \
                self.run_cmd.is_finished and \
                not self.run_cmd.succeeded:
