...
class Database:
    ...
    def _read(self) -> dict:
        ...
        if check_version and oldversion < get_version():
            msg = (
            ...
            for breaking_version in breaking_changes:
                breaking_version = Version(breaking_version)
                if oldversion < breaking_version <= get_version():
                    backup_file = self.backup()

                    msg = (
                        f"{oldversion} cannot be upgraded to {get_version()}. "
    ...
    def _write(self, data: dict):
        ...
        tmp_path = self._path + random_string() + ".tmp"
        ...
        while not os.path.exists(tmp_path):
            ...
            if time.time() > t0 + timeout:
                raise RuntimeError(f"Database file {tmp_path} was never created.")
