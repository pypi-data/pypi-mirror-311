...
def run_autoswarm(task: str, model: str):
    ...
    try:
        console.print("[yellow]Initializing autoswarm configuration...[/yellow]")
        ...
        console.print(f"[yellow]Generating swarm for task: {task}[/yellow]")
        result = generate_swarm_config(task=task, model=model)
        
        if result:
            console.print("[green]✓ Swarm configuration generated successfully![/green]")
        else:
            raise SwarmCLIError("Failed to generate swarm configuration")
...
def main():
    try:
        ...
        formatter.print_panel(f"Error detected: {error} check your args")
        raise error
