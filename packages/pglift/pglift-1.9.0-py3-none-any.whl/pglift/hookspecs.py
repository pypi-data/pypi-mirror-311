...
MaybeFirstResult: TypeAlias = Optional[Literal[True]]
...
@hookspec
async def site_configure_install(
    settings: Settings, header: str, env: dict[str, Any]
) -> None:
...
async def deinit_postgresql(instance: system.PostgreSQLInstance) -> None:
    """De-initialize a PostgreSQL instance."""
    raise NotImplementedError
...
@hookspec(firstresult=True)
def configure_auth(instance: PostgreSQLInstance, manifest: interface.Instance) -> bool:
...
async def start_postgresql(
    instance: PostgreSQLInstance, foreground: bool, wait: bool
) -> FirstResult:
...
async def stop_postgresql(
    instance: PostgreSQLInstance, mode: str, wait: bool, deleting: bool
) -> FirstResult:
...
async def restart_postgresql(
    instance: PostgreSQLInstance, mode: str, wait: bool
) -> FirstResult:
...
@hookspec(firstresult=True)
async def reload_postgresql(instance: PostgreSQLInstance) -> FirstResult:
...
@hookspec(firstresult=True)
async def promote_postgresql(instance: PostgreSQLInstance) -> FirstResult:
...
@hookspec
def check_hba_is_editable(instance: PostgreSQLInstance) -> None:
    """Check if pg_hba.conf can be modified for specified PostgreSQL instance."""
    raise NotImplementedError
