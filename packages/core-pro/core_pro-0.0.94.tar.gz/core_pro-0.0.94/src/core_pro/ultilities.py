...
def format_df(
    ...
    :param frozen_rows: int
    :param position_title: position title
    :param position_df: position df
    :param num_col_format_df: # columns to format
    """
    # Get sheet_id
    sheet = Sheet(spreadsheet_id)
...
def make_sync_folder(folder_name: str) -> Path:
    dict_ = {
        ...
        'kevinx680': Path(f'/media/kevin/data_4t/{folder_name}'),
    }
...
def create_batch_index(total_rows: int, n_size: int) -> dict:
    ...
    return {i: batches[i] for i in range(len(batches))}
