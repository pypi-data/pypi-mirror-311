...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        right_axis = gr.Dropdown(
            choices=["X", "-X", "Y", "-Y", "Z", "-Z"],
            value="X",  # Matches the default right_axis="X" in pce
            label="Right Axis",
            info="Select the right direction"
        )
    ...
    pce = PointCloudEditor(up_axis="Y", right_axis="X", forward_axis="-Z")
    ...
    right_axis.change(fn=lambda x: gr.update(right_axis=x), inputs=right_axis, outputs=pce)
