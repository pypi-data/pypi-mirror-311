<script lang="ts">
	...
	export let coordinate_system = {
		...
		right: 'X' as CoordinateAxis, // Default from backend
	...
	function centerCamera() {
		...
		// Get the basis vectors from current coordinate system
		const basisVectors = {
			'X': new Vector3(1, 0, 0),
			'Y': new Vector3(0, 1, 0),
			'Z': new Vector3(0, 0, 1),
	...
	function updateCoordinateSystem() {
		...
		const rightVector = basisVectors[coordinate_system.right];
		const upVector = basisVectors[coordinate_system.up];
		const forwardVector = basisVectors[coordinate_system.forward];
		
		if (!rightVector || !upVector || !forwardVector) {
		...
		// Create projection matrix with potential left/right flip
		...
		// Check if we need to flip left/right
		const flipX = rightVector.x < 0 || rightVector.y < 0 || rightVector.z < 0;
		if (flipX) {
			// Flip the projection matrix horizontally
			projectionMatrix.makePerspective(right, left, bottom, top, near, far);
		} else {
			projectionMatrix.makePerspective(left, right, bottom, top, near, far);
		}
		...
		// Invert controls rotation direction when camera is flipped
		if (flipX) {
