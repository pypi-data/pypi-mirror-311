...
def format_color_name(color_name: str) -> str:
    """
    Format the color name by replacing parentheses with a hyphen.
    
    Args:
        color_name (str): The color name in the format "ColorName (Brand)".
    
    Returns:
        str: The formatted color name in the format "ColorName-Brand".
    """
    ...
    if '(' in color_name and ')' in color_name:
        try:
            ...
            brand_name = color_name.split(' (')[1].rstrip(')')
        except IndexError:
            base_name = color_name.split('(')[0]
            brand_name = color_name.split('(')[1].rstrip(')')
        
        return f"{base_name}-{brand_name}"
...
class ColorInfoResponse:
    def __init__(self, name: str, hex_: str, websafe: dict, rgb: tuple, brightness: int, shades: list, palette: BytesIO, image: BytesIO):
        self.name = format_color_name(name)
        self.hex_ = hex_
