...
if __name__ == '__main__':
    # key = bytes.fromhex(input('请输入 key: '))
    # print(len(key))
    # data = input('请输入要加密的字符串: ').encode('utf-8')
    # print(TEA.encrypt(data, key=key).hex())

    key = bytes.fromhex(input('请输入 key: '))
    print(len(key))

    data = bytes.fromhex(input('请输入解密: '))
    #
    print(TEA.decrypt(data,
