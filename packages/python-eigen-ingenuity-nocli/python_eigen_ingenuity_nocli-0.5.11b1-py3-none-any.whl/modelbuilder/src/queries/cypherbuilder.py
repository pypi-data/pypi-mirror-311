...
class Where(Query):
    ...
    def where_additional(self, filters: Properties, comparison_operator: str = "=", boolean_operator: str = ' AND '):
        filt = boolean_operator + Properties(filters).to_str(comparison_operator, boolean_operator)
        return ValidAfterWhere(self.query + filt)
...
class Node(Query):
    ...
    def node(self, labels: List[str] = None, ref_name: str = None, properties = None, properties_literal: str = ''):
        if not labels:
            labels_string = ''
    ...
    def node_batch(self, labels: List[str] = None, ref_name: str = None, properties: str = None):
        if not labels:
            labels_string = ''
        elif isinstance(labels, str):
            labels_string = labels
...
class Set(Query):
    ...
    def set_functional(self, properties: Properties, property_ref=''):
        return ValidAfterSet(self.query + ' SET ' + property.format_property_list(properties, "=", ",", property_ref, property_ref))

    def set_labels(self, ref_name: str = '', labels: List[str] = None):
        if not labels:
            labels_string = ''
        elif isinstance(labels, str):
