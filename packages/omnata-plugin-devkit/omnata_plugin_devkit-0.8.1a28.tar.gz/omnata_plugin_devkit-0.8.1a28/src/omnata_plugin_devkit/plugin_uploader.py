...
class PluginUploader:
  ...
  AS 'PARSE_JSON($${json_string}$$)::object';"""
    ...
    def upload_plugin(
        ...
        # if snowpark is not present or not pinned, we add it with a specific pinned version
        if len([w for w in other_packages if "snowflake-snowpark-python" in w]) == 0:
            # if not present at all, pin it to a specific version
            default_packages = default_packages + ["snowflake-snowpark-python==1.22.1"]
            print("snowflake-snowpark-python not present, adding to default packages")
        elif len([w for w in other_packages if w == "snowflake-snowpark-python"]) > 0:
            # if present, but not pinned, we replace it with the pinned version
            ...
            other_packages = [
                ...
                for w in other_packages
            ]
        else:
            found_version = [w for w in other_packages if "snowflake-snowpark-python" in w][0]
            print(f"{found_version} found already pinned, leaving alone")
            # if present and pinned, we leave it alone
            pass
