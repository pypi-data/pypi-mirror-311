...
class PluginUploader:
  ...
  AS 'PARSE_JSON($${json_string}$$)::object';"""
    ...
    def upload_plugin(
        ...
        if len([w for w in other_package_names if "snowflake-snowpark-python" in w]) == 0:
            ...
            snowpark_package = [w for w in other_packages if "snowflake-snowpark-python" in w.name][0]
            if len(snowpark_package.specs) == 0:
                ...
                other_package_names = [
                    ...
                    for w in other_package_names
                ]
            else:
                print(f"{snowpark_package.name} found already pinned ({snowpark_package.specs}), leaving alone")
