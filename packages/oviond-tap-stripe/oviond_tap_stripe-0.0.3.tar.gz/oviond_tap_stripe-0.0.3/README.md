...
```bash
pipx install git+https://github.com/ORG_NAME/tap-stripe.git@main
```
...
```
tap-stripe --about --format=markdown
```
-->
...
You can easily run `tap-stripe` by itself or in a pipeline using [Meltano](https://meltano.com/).
...
meltano run tap-stripe target-jsonl
```
