...
2. Creating AE and Diresa models from hyperparameters
  ...
  - build_diresa(input_shape, stack, stack_filters, latent_filters, kernel_size=(3, 3),
   ...
   kwargs are passed to all Con2D and Dense layers, e.g. they can be used for changing kernel_initializer or kernel_regularizer.
...
def build_ae(input_shape=(),
    ...
    :param kwargs: are passed to all Con2D and Dense layers, e.g. can be used for changing kernel_initializer, kernel_regularizer
...
#
def _residual_unit(x, filters, kernel_size, activation, name, **kwargs):
    ...
    return layers.Activation(activations.relu)(y)


def _encoder_block(y, block, filters, kernel_size, residual, activation, name, **kwargs):
    ...
    for unit in range(block):
        if residual:
            y = _residual_unit(y, filters, kernel_size, activation=activation,
                               name=name + "_Conv2D_" + str(unit), **kwargs)
        else:
...
def _decoder_block(y, block, filters, kernel_size, conv_transpose, residual, activation, up_first, name, **kwargs):
    if up_first:
...
def _encoder_model(input_shape=(),
    ...
    for block, filters in zip(stack, stack_filters):
        y = _encoder_block(y, block=block, filters=filters, kernel_size=kernel_size, residual=residual,
                           activation=activation, name='Enc_' + str(block_nr), **kwargs)
        block_nr += 1
...
def _decoder_model(input_shape=(),
    ...
    if len(dense_units) != 0:
        ...
        if len(stack) != 0:
            ...
            y = layers.Dense(_prod(conv_output_shape), name='Dec_' + str(block_nr) + '_Dense_0', **kwargs)(y)
            # Shape of encoder output after convolution
            y = layers.Reshape(conv_output_shape)(y)
        else:
            y = layers.Dense(input_shape[0], name='Dec_' + str(block_nr) + '_Dense_0', **kwargs)(y)
        block_nr -= 1
    ...
    for block, filters in zip(stack[::-1], stack_filters[::-1]):
        if len(dense_units) != 0 and block_nr == len(stack):
            ...
            y = _decoder_block(y, block=block - 1, filters=filters, conv_transpose=conv_transpose,
                               kernel_size=kernel_size,
                               residual=residual, activation=activation, up_first=up_first,
                               name='Dec_' + str(block_nr), **kwargs)
        else:
...
if __name__ == "__main__":
    ...
    hyper_params = [{"input_shape": [8, 8, 1], "stack": [1, ], "stack_filters": [32, ],
                    ...
                    {"input_shape": [8, 8, 1], "stack": [1, 2], "stack_filters": [32, 16],
                     "dense_units": [20, 10], "up_first": True, "residual": True},
                    {"input_shape": [8, 8, 1], "stack": [1, 1], "stack_filters": [32, 16],
                     "dense_units": [20, 10, 10], "dropout_rate": 0.25},
                    {"input_shape": [8, 8, 1], "stack": [1, 1], "stack_filters": [32, 16],
