...
def photon(wavelength,spectre,time,diam,point_nb,transmission=False):
        ...
        intens=rand.uniform(low = 0,high = max(flux),size = ph_nbr)
...
def detector_scale(detector_dim,photon_dict):
    ...
    for ph in range(len(photon_dict)):
        if int(photon_dict[ph][0]) < detector_dim/2 and int(photon_dict[ph][1]) < detector_dim/2 and int(photon_dict[ph][0]) >= -detector_dim/2 and int(photon_dict[ph][1]) >= -detector_dim/2 :
...
def Timeline(phase, exptime, noisetimeline, nreadoutscale, timelinestep, wmap):
    
    timeline = np.zeros(shape = exptime, dtype = object)
    for t in range(exptime):
        timeline[t] = []
    m_noise = max(noisetimeline[1])
    for t in range(exptime):
            ...
            for ph in range(int(wmap * len(phase))):
                if int(phase[ph][0][0]/1e6) == t:
                    ...
                    if int(phase[ph][0][0]/1e6) == int(phase[ph][0][-1]/1e6):
                        ...
                        inxb = np.int32(np.linspace(0, len(phase[ph][0][inx:])-1, len(phase[ph][0][inx:])))
                        ntime[inxa] += phase[ph][1][inxa]
                        if int(phase[ph][0][-1]/1e6) < exptime:
                            phase.append([inxb + (phase[ph][0][-1]/1e6)*1e6, phase[ph][1][inxb]])

        if (ntime == compare).all():
            peaks = []
        else:
            peaks, _ = find_peaks(ntime,  prominence=10, height=m_noise)
            timeline[t] = [peaks, ntime[peaks]]
