...
def PhaseNoise(photon, scale, baseline = None):
    ...
    if len(photon[0]) > 0:
        for k in range(len(photon[0])):
            if baseline == None:
                sig[0].append(rand.normal(loc = 0,scale = scale, size = len(photon[0][k] )) + photon[0][k])
            else:
                sig[0].append(rand.normal(loc = baseline,scale = scale, size = len(photon[0][k] )) + photon[0][k])
...
def PhotonPhase(photondetect, pxnbr, conversion, nphase, decay, exptime, baseline, baselinepix, nreadoutscale, k, l):
    phase = []
    if len(photondetect) > 0:
        phaseconv = PhaseConversion(photondetect, conversion, nphase, k, l)
...
def PhaseConversion(photondetect, conversion, nphase, k, l):

    convtab = ConversionTab(photondetect,conversion, k, l)
