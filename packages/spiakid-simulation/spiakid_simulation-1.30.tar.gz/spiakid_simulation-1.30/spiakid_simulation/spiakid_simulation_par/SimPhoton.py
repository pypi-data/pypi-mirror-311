...
class Simulation():
        ...
        try: DATA['Phase']
            ...
            for i in range(len(pix)):
                ...
                args.append(np.array([k,l,baseline, decay, templatetime, trigerinx, pointnb, nperseg, nreadoutscale, baselinepix[k,l],
                             wv, nbwv, calibtype, conversion, save_type, nphase, decay, timelinestep, self.wmap[k,l], self.photons[k,l] ], dtype = object))
            ...
            print('res')
            for r in res:
                h5file['Filter/'+str(r[0])+'_'+str(r[1])] = r[2][0]
                for i in range(len(r[3][1])):
                   
                    h5file['Calib/'+str(r[3][1][i])+'/'+str(r[0])+'_'+str(r[1])] = r[3][0][str(r[3][1][i])]
                
                for i in range(len(r[4])):
                    print(len())
                    h5file['Photons/'+str(i)+'/'+str(r[0])+'_'+str(r[1])] = r[4][i]
    ...
    def ParFun(args):
        ...
        [
            ...
            wv, nbwv, calibtype, conversion, save_type, nphase, decay, timelinestep, wmap, photons
        ] = args
        ...
        pixcalib = Calib(wv, nbwv, calibtype, conversion, save_type, pixfilter[1],nphase, decay, timelinestep, nreadoutscale, baselinepix, wmap, k, l)
     
        # print('Phase', flush = True)
        pixphase = PhotonPhase(photons, pxnbr, conversion, nphase, decay, exptime, baseline, baselinepix, nreadoutscale, k, l)
        # print('Timeline', flush = True)
        pixpeaks =  Timeline(pixphase, exptime, pixfilter[1], nreadoutscale, timelinestep, wmap)
    ...
    class Star():
            ...
            def __init__(self, psf,i):
                ...
                self.posx  = rand.uniform(low = -pxnbr/2, high= 0)
                self.posy  = rand.uniform(low = -pxnbr/2, high= pxnbr/2)
