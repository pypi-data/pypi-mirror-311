...
def PhaseNoiseCalib(photon, scale, baseline):
    ...
    sig = [photon[1], photon[0] + baseline + rand.normal(loc = 0,scale = scale, size = len(photon[0]))]
    return(sig)
...
def Calib(*args):
    # [
    #     wv, nbwv, pxnbr, calibtype, conversion, save_type, noisetimeline,
    #     nphase, decay, timelinestep, nreadoutscale, baselinepix, wmap, h5file
    ...
    #         CalibCompute(photondetectcalib[i], tab, nphase, decay, timelinestep, nreadoutscale, baselinepix, pxnbr, wmap, noisetimeline, h5file, wvcalib[i])
    # print('Calib done', flush = True)

    [
        wv, nbwv, calibtype, conversion, save_type, noisetimeline,
        nphase, decay, timelinestep, nreadoutscale, baselinepix, wmap, k, l
        ] = args
    calib = {}
    wvcalib = np.linspace(wv[0], wv[-1], nbwv)
    ...
    if save_type == 'photon_list':
        for i in range(len(wvcalib)):
                calib[str(wvcalib[i])] = CalibCompute(photondetectcalib[i], conv, nphase, decay, timelinestep, nreadoutscale, baselinepix, wmap, noisetimeline, wvcalib[i])
...
def CalibCompute(*args):
    phcalib, tab, nphase, decay, timelinestep, nreadoutscale, baselinepix, wmap, noisetimeline, wvcalib = args
    ...
    peaks, _ = find_peaks(nexpphasecalib[1], prominence=10, height=max(noisetimeline[1]))
    nbpeaks = int((len(peaks)) * wmap)
    peaks = peaks[:nbpeaks]
    return(nexpphasecalib[1][peaks])
