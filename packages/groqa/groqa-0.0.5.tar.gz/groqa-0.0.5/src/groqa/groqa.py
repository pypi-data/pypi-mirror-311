...
class GroqaError(Exception):
    def __init__(self, status_code: int, detail: str):
        self.status_code = status_code
        self.detail = detail
        super().__init__(f"GROQA ERROR : {status_code} - {detail}")

class GroqaSync:
    def __init__(self, api_key: str, base_url: str = "https://v1.groqadev.com"):
    ...
    def _make_request(self, endpoint: str, params: Dict[str, Any]) -> httpx.Response:
        ...
        if response.status_code == 401:
            raise GroqaError(401, "Invalid GROQA API Key")
                ...
                try:
                    if line.startswith("data:"):
                        ...
                        if isinstance(data, dict) and "type" in data:
                            if data["type"] == "error":
                                raise GroqaError(data.get("status_code", 500), data.get("detail", "Unknown error"))
                            else:
                                yield data
    ...
    def _handle_error_response(self, response: httpx.Response):
        try:
            ...
            if 'OPENAI Error:' in detail:
                raise GroqaError(status_code, detail.split('GROQA Error:', 1)[1].strip())
            else:
                raise GroqaError(status_code, f"Unexpected error: {detail}")
        except json.JSONDecodeError:
            raise GroqaError(response.status_code, f"Unexpected error: {response.text}")
            ...
            if line:
                try:
                    if line.startswith("data:"):
                        ...
                        if isinstance(data, dict) and "type" in data:
                            if data["type"] == "error":
                                raise GroqaError(data.get("status_code", 500),
