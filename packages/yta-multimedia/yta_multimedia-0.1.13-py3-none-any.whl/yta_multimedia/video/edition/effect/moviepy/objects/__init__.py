from yta_multimedia.video.edition.effect.moviepy.objects import prepare_background_clip
...
from moviepy.Clip import Clip, CompositeVideoClip
...
class MoviepyWith:
    ...
    def apply_over_video(video: Clip, background_video: Clip, with_position: Union[MoviepyArgument, None] = None, resized: Union[MoviepyArgument, None] = None, rotated: Union[MoviepyArgument, None] = None):
        ...
        background_video = prepare_background_clip(background_video, video)
...
class MoviepyWithPrecalculated:
    ...
    def apply(video: Clip, with_position_list: Union[list, None] = None, resized_list: Union[list, None] = None, rotated_list: Union[list, None] = None):
        ...
        # Prepare data for pre-calculations
        frames_number = video.fps * video.duration
        frame_duration = video.duration / frames_number

        # Validate provided lists
        if resized_list is not None and len(resized_list) != frames_number:
            raise Exception(f'The provided "resized_list" contains {str(len(resized_list))} elements and there are "{str(frames_number)} frames.')
        if with_position_list is not None and len(with_position_list) != frames_number:
            raise Exception(f'The provided "with_position_list" contains {str(len(resized_list))} elements and there are "{str(frames_number)} frames.')
        if rotated_list is not None and len(rotated_list) != frames_number:
            raise Exception(f'The provided "rotated_list" contains {str(len(resized_list))} elements and there are "{str(frames_number)} frames.')

        # I add frame_duration * 0.1 to make sure it fits the
        # next index
        if resized_list is not None:
            video = video.resized(lambda t: resized_list[int((t + frame_duration * 0.1) // frame_duration)])
        if with_position_list:
...
# TODO: This 'validate_position' is used in Coordinate class
