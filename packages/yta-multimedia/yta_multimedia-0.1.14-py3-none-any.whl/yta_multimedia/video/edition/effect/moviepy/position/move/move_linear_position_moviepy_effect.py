...
class MoveLinearPositionMoviepyEffect(BasePositionMoviepyEffect):
    ...
    @classmethod
    def apply(cls, video: Union[str, VideoFileClip, VideoClip, CompositeVideoClip, ImageClip, ColorClip], initial_position: Union[Position, tuple] = Position.OUT_LEFT, final_position: Union[Position, VideoPosition] = Position.CENTER):
        """
        ...
        image of 1920x1080.
        """
        return cls.apply_over_video(video, BasePositionMoviepyEffect.get_black_background_clip(video.duration), initial_position, final_position)

    @classmethod
    def apply_over_video(cls, video: Union[str, VideoFileClip, VideoClip, CompositeVideoClip, ImageClip, ColorClip], background_video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip], initial_position: Union[Position, VideoPosition] = Position.OUT_LEFT, final_position: Union[Position, VideoPosition] = Position.CENTER):
        """
        TODO: Write well
        ...
        effect = video.with_position(lambda t: TFunctionSetPosition.linear(t, duration, initial_position, final_position, RateFunction.linear)).with_start(0).with_duration(duration)
