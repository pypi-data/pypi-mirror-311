...
# TODO: This is not only for video so I should have
...
class ResizeMode(Enum):
    ...
    """
    The video is just placed over a black background that
    has the desired dimensions. This will work exactly as
    FIT_ONE_DIMENSION if the video provided is larger than
    the expected size.
    """

def resize_video(video: Clip, size: tuple, resize_mode: ResizeMode = ResizeMode.RESIZE_KEEPING_ASPECT_RATIO):
    """
    ...
    if not resize_mode:
        ...
        resize_mode = ResizeMode.to_enum(resize_mode)
    ...
    if resize_mode == ResizeMode.RESIZE:
        ...
        top_left, bottom_right = get_cropping_points_to_keep_aspect_ratio(video.size, size)
        # Crop the video to fit the desired aspect ratio
        # TODO: Maybe avoid this if nothing to crop
        video = video.with_effects([Crop(video, width = bottom_right[0] - top_left[0], height = bottom_right[1] - top_left[1], x_center = video.width / 2, y_center = video.height / 2)])
