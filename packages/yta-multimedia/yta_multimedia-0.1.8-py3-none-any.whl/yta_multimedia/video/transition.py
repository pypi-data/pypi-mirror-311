...
class TransitionMethod:
    ...
    def slide(video1: VideoClip, video2: VideoClip, **kwargs):
        ...
        # TODO: This below is now done in 'to_moviepy' but I 
        # keep the code for the next commit
        # We need to make sure both clips have the same size so,
        # by now, I force it to be in the middle of a black
        # background to fit the other video's size
        # if video1.size > video2.size:
        #     video2 = CompositeVideoClip([
        #         ClipGenerator.get_default_background_video(video1.size, video2.duration, False),
        #         video2.with_position(('center', 'center'))
        #     ])
        # elif video2.size > video1.size:
        #     video1 = CompositeVideoClip([
        #         ClipGenerator.get_default_background_video(video2.size, video1.duration, False),
        #         video1.with_position(('center', 'center'))
        #     ])

        # Transition from last frame of video1 to first of video2
        t1_arg = MoviepyArgument(VideoPosition(0, 0), VideoPosition(-video1.w, 0), TFunctionSetPosition.linear, RateFunction.linear)
        transition_clip_1 = MoviepyWith.apply(video1, with_position = t1_arg)

        t1_position = transition_clip_1.pos(t = 0)
        t2_arg = MoviepyArgument(VideoPosition(video2.w, 0), VideoPosition(t1_position[0], t1_position[1]), TFunctionSetPosition.linear, RateFunction.linear)
