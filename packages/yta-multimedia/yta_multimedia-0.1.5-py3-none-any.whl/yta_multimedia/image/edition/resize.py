...
def resize_image(image: Union[str, Any], size, output_filename: Union[str, None] = None):
    ...
    # We need to adjust the image to the AR of the given 'size'
    new_ar = size[0] / size[1]
    image_width, image_height = image.size
    new_image_size = adjust_to_aspect_ratio(image_width, image_height, new_ar)

    # We crop a region of the image (centered) that fits that AR
    left = (image_width - new_image_size[0]) / 2  # it was // 2
    top = (image_height - new_image_size[1]) / 2  # it was // 2
    right = left + new_image_size[0]
    bottom = top + new_image_size[1]
...
# TODO: Are these methods interesting or even used (?) I think they
# can be reached with libraries .resize methods...
...
def resize_image_file_without_scaling(image_filename: str, width: int = 1920, height: int = 1080):
    """
