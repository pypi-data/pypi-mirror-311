...
class MoviepyArgument:
    ...
    """
    def __init__(self, start, end, t_function: type, rate_func: RateFunction = RateFunction.linear):
        self.start = start
        self.end = end
        self.t_function = t_function
        self.rate_func = rate_func
...
class MoviepyWith:
    ...
    def apply_over_video(video: Clip, background_video: Clip, with_position: Union[MoviepyArgument, None] = None, resized: Union[MoviepyArgument, None] = None, rotated: Union[MoviepyArgument, None] = None):
        ...
        if resized:
            ...
            # We build this array to make 'with_position' dependant and
            # to be able to position it properly based on the size that
            # the video has in each frame. If we don't do that, the 
            # position will be using the first position only
        ...
        if with_position:
            ...
            for i, t in enumerate(ts):
                ...
                initial_position = (with_position.start[0] * resizes[i], with_position.start[1] * resizes[i])
                final_position = (with_position.end[0] * resizes[i], with_position.end[1] * resizes[1])

                # Adapt the positions to the background
            ...
            # positions = [with_position.t_function(t, video.duration, (with_position.start[0] * resizes[i], with_position.start[1] * resizes[i]), (with_position.end[0] * resizes[i], with_position.end[1] * resizes[i])) for i, t in enumerate(ts)]
            # else:
            #     positions = [with_position.t_function(t, video.duration, with_position.start, with_position.end, with_position.rate_func) for t in ts]
        # else:
        #     # TODO: Maybe just don't do 'with_position' as the only one 
        #     # which is dependant is the 'with_position' from the 'resized'
        #     positions = [video.pos for _ in ts]

        # TODO: Where did 'rotated' go (?)
        rotations = None
        if rotated:
            rotations = [rotated.t_function(t, video.duration, rotated.start, rotated.end, rotated.rate_func) for t in ts]

        video = video.resized(lambda t: resizes[int(t // frame_duration)])
        if positions:
            video = video.with_position(lambda t: positions[int(t // frame_duration)])
        if rotated:
            video = video.rotated(lambda t: rotations[int(t // frame_duration)])
        
        return video

# TODO: Maybe relocate these 2 methods from below
def validate_position(position: Union[Position, VideoPosition]):
    """
