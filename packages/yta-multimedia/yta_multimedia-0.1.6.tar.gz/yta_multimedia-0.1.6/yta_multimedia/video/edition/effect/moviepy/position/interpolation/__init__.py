...
class GraphicRateFunction:
    ...
    def __init__(self, points: list[Coordinate, Position, VideoPosition, tuple], interpolation_formula: Callable):
        if not PythonValidator.is_list(points) or len(points) < 4 or any(not PythonValidator.is_instance(point, [Coordinate, Position, VideoPosition, tuple]) for point in points):
...
class InterpolatedPair:
    ...
    def __init__(self, coordinate_a: Union[Coordinate, Position, VideoPosition], coordinate_b: Union[Coordinate, Position, VideoPosition], interpolation_method: InterpolationMethod = InterpolationMethod.LINEAR):
        if not PythonValidator.is_instance(coordinate_a, [Coordinate, Position, VideoPosition]) or not PythonValidator.is_instance(coordinate_b, [Coordinate, Position, VideoPosition]):
            raise Exception(f'The provided "coordinate_a" or "coordinate_b" is not a valid Coordinate, Position or VideoPosition.')
        ...
        if PythonValidator.is_instance(coordinate_a, Position):
            coordinate_a = coordinate_a.as_video_position().coordinate
        elif PythonValidator.is_instance(coordinate_a, VideoPosition):
            coordinate_a = coordinate_a.coordinate

        if PythonValidator.is_instance(coordinate_b, Position):
            coordinate_b = coordinate_b.as_video_position().coordinate
        elif PythonValidator.is_instance(coordinate_b, VideoPosition):
            coordinate_b = coordinate_b.coordinate
...
class GraphicInterpolation:
    ...
    def get_coord_from_x(self, x: float, is_normalized: bool = False):
        """
    ...
    def get_coord_from_d(self, d: float):
        """
        Obtain the coordinate corresponding to the provided 
    ...
    def plot(self, do_denormalize_values: bool = False):
        ...
        graph = np.array([self.get_coord_from_d(d) for d in np.linspace(0, 1, len(self.coordinates) * 100)])
        points = np.array([coordinate.as_tuple() for coordinate in self.coordinates])
        ...
        if do_denormalize_values:
            ...
            graph = np.array([(Math.denormalize(graphi[0], -10000, 10000), Math.denormalize(graphi[1], -10000, 10000)) for graphi in graph])
            points = np.array([(Math.denormalize(pointi[0], -10000, 10000), Math.denormalize(pointi[1], -10000, 10000)) for pointi in points])
