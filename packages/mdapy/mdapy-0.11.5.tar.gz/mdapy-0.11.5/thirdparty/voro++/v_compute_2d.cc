...
void voro_compute_2d<c_class_2d>::find_voronoi_cell(double x,double y,int ci,int cj,int ij,particle_record_2d &w,double &mrs) {
    ...
    // and store it in (fx,fy). We use this to compute an index (di,dj) of
    // which subregion the particle is within.
    unsigned int m1,m2;
    con.frac_pos(x,y,ci,cj,fx,fy);
    di=int(fx*xsp*wl_fgrid_2d);dj=int(fy*ysp*wl_fgrid_2d);

    // The indices (di,dj) tell us which worklist to use, to test the blocks in
    // the optimal order. But we only store worklists for the eighth of the
    ...
    // for these cases, by reflecting high values of di, dj. For these cases, a
    // mask is constructed in m1 and m2 which is used to flip the worklist
    // information when it is loaded.
    if(di>=wl_hgrid_2d) {
        mxs=boxx-fx;
...
bool voro_compute_2d<c_class_2d>::compute_cell(v_cell_2d &c,int ij,int s,int ci,int cj) {
    ...
    for(l=0;l<s;l++) {
        ...
        x1=p[ij][ps*l]-x;
        y1=p[ij][ps*l+1]-y;
        rs=con.r_scale(x1*x1+y1*y1,ij,l,r_rad);
    ...
    do {
        ...
        // which case we skip this block and move on. Otherwise, it computes
        // the maximum distance to the block and returns it in crs.
        if(compute_min_max_radius(di,dj,fx,fy,gxs,gys,crs,mrs,r_mul)) continue;
        ...
        if(co[ij]>0) {
            ...
            if(!con.r_ctest(crs,mrs,r_mul)) {
                ...
                do {
                    ...
                    y1=p[ij][ps*l+1]-y2;
                    rs=x1*x1+y1*y1;
                    if(con.r_scale_check(rs,mrs,ij,l,r_rad)&&!c.nplane(x1,y1,rs,id[ij][l])) return false;
                    l++;
                } while (l<co[ij]);
            }
        }
    ...
    while(qu_s!=qu_e) {
        ...
        if(ei>i) {
            if(ej>j) {
                if(corner_test(c,xlo,ylo,xhi,yhi,r_mul,r_val)) continue;
            } else if(ej<j) {
                if(corner_test(c,xlo,yhi,xhi,ylo,r_mul,r_val)) continue;
            } else {
                if(edge_x_test(c,xlo,ylo,yhi,r_mul,r_val)) continue;
            }
        } else if(ei<i) {
            if(ej>j) {
                if(corner_test(c,xhi,ylo,xlo,yhi,r_mul,r_val)) continue;
            } else if(ej<j) {
                if(corner_test(c,xhi,yhi,xlo,ylo,r_mul,r_val)) continue;
            } else {
                if(edge_x_test(c,xhi,ylo,yhi,r_mul,r_val)) continue;
            }
        } else {
            if(ej>j) {
                if(edge_y_test(c,xlo,ylo,xhi,r_mul,r_val)) continue;
            } else if(ej<j) {
                if(edge_y_test(c,xlo,yhi,xhi,r_mul,r_val)) continue;
...
template<class c_class_2d>
template<class v_cell_2d>
inline bool voro_compute_2d<c_class_2d>::corner_test(v_cell_2d &c,double xl,double yl,double xh,double yh,double &r_mul,double &r_val) {
    con.r_prime(xl*xl+yl*yl,r_mul,r_val);
    if(c.plane_intersects_guess(xl,yh,con.r_cutoff(xl*xl+yl*yh,r_val))) return false;
//    if(c.plane_intersects(xl,yl,con.r_cutoff(xl*xl+yl*yl,r_val))) return false;  XXX not needed?
    if(c.plane_intersects(xh,yl,con.r_cutoff(xl*xh+yl*yl,r_val))) return false;
    return true;
}
...
template<class c_class_2d>
template<class v_cell_2d>
inline bool voro_compute_2d<c_class_2d>::edge_x_test(v_cell_2d &c,double xl,double y0,double y1,double &r_mul,double &r_val) {
    con.r_prime(xl*xl,r_mul,r_val);
    if(c.plane_intersects_guess(xl,y0,con.r_cutoff(xl*xl,r_val))) return false;
    if(c.plane_intersects(xl,y1,con.r_cutoff(xl*xl,r_val))) return false;
    return true;
}
...
template<class c_class_2d>
template<class v_cell_2d>
inline bool voro_compute_2d<c_class_2d>::edge_y_test(v_cell_2d &c,double x0,double yl,double x1,double &r_mul,double &r_val) {
    con.r_prime(yl*yl,r_mul,r_val);
    if(c.plane_intersects_guess(x0,yl,con.r_cutoff(yl*yl,r_val))) return false;
    if(c.plane_intersects(x1,yl,con.r_cutoff(yl*yl,r_val))) return false;
    return true;
}
 ...
 * within mrs.*/
template<class c_class_2d>
bool voro_compute_2d<c_class_2d>::compute_min_max_radius(int di,int dj,double fx,double fy,double gxs,double gys,double &crs,double mrs,double &r_mul) {
    double xlo,ylo;
    if(di>0) {
        xlo=di*boxx-fx;
        crs=xlo*xlo;
