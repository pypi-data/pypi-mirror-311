...
voronoicell_base_3d::~voronoicell_base_3d() {
...
void voronoicell_base_3d::check_memory_for_copy(vc_class &vc,voronoicell_base_3d* vb) {
...
void voronoicell_base_3d::copy(voronoicell_base_3d* vb) {
    int i,j;
    p=vb->p;up=0;
 ...
 * extending memory allocation if necessary.
 * \param[in] c the class to copy. */
void voronoicell_neighbor_3d::operator=(voronoicell_3d &c) {
...
/** Copies the information from another voronoicell_neighbor_3d class into this
 * class, extending memory allocation if necessary.
 * \param[in] c the class to copy. */
void voronoicell_neighbor_3d::operator=(voronoicell_neighbor_3d &c) {
    voronoicell_base_3d *vb=((voronoicell_base_3d*) &c);
    check_memory_for_copy(*this,vb);copy(vb);
    int i,j;
    for(i=0;i<c.current_vertex_order;i++) {
...
template<class vc_class>
void voronoicell_base_3d::add_memory(vc_class &vc,int i) {
    int s=(i<<1)+1;
    if(mem[i]==0) {
...
template<class vc_class>
void voronoicell_base_3d::add_memory_vorder(vc_class &vc) {
 ...
 * fatal error. */
void voronoicell_base_3d::add_memory_ds() {
...
void voronoicell_base_3d::add_memory_ds2() {
 ...
 * allocation exceeds the absolute maximum set in max_delete2_size, then the
 * routine causes a fatal error. */
void voronoicell_base_3d::add_memory_xse() {
...
void voronoicell_base_3d::check_relations() {
    int i,j;
...
void voronoicell_base_3d::check_duplicates() {
    int i,j,k;
...
void voronoicell_base_3d::construct_relations() {
    int i,j,k,l;
 ...
 * routine from .
 * \param[in,out] up */
inline bool voronoicell_base_3d::search_for_outside_edge(int &up) {
...
inline void voronoicell_base_3d::add_to_stack(int sc2,int lp) {
...
inline bool voronoicell_base_3d::search_downward(unsigned int &lw,int &lp,int &ls,int &us,double &l,double &u) {
    int vs;
...
bool voronoicell_base_3d::definite_min(int &lp,int &us,double &l,double &u,unsigned int &lw) {
    int tp=up,ts,qp=0;
    unsigned int qw;
    double q;
    ...
    *ds=qp;
    ...
    while(spp<stackp) {
        ...
        for(ts=0;ts<nu[tp];ts++) {
            ...
            // This point is a better minimum. Reset markers and return true.
            if(q<u) {
                flip(up);
                up=tp;
                us=ts;
...
template<class vc_class>
bool voronoicell_base_3d::nplane(vc_class &vc,double x,double y,double z,double rsq,int p_id) {
    ...
    while(xse+k<stackp3) {
        ...
        for(ls=0;ls<nu[lp];ls++) {
            ...
            if(uw==0) {
                ...
                // This is a possible facet starting from a vertex on the
                // cutting plane
    ...
    // Add the points in the auxiliary delete stack, and reset their back
    // pointers
    for(dsp=ds2;dsp<stackp2;dsp++) {
        j=*dsp;
        ed[j][nu[j]<<1]=j;
...
template<class vc_class>
bool voronoicell_base_3d::create_facet(vc_class &vc,int lp,int ls,double l,int us,double u,int p_id) {
    int i,j,k,qp,qs,iqs,cp,cs,rp,*edp,*edd;
    unsigned int lw,qw;
    bool new_double_edge=false,double_edge=false;
    double q,r;

    // We're about to add the first point of the new facet. In either routine,
    // we have to add a point, so first check there's space for it.
    if(p==current_vertices) add_memory_vertices(vc);

    if(lp==-1) {

        // We want to be strict about reaching the conclusion that the cell is
        // entirely within the cutting plane. It's not enough to find a vertex
        // that has edges which are all inside or on the plane. If the vertex
        // has neighbors that are also on the plane, we should check those too.
        ...
        if(lw!=0) {
            ...
            // The first edge is either inside the cutting space, or lies
            // within the cutting plane. Test the edges sequentially until we
            // find one that is outside.
            unsigned int rw=lw;
            do {
                i++;

                // If we reached the last edge with no luck then all of the
                // vertices are inside or on the plane, so the cell is
                // completely deleted
            ...
            // We found an edge outside the cutting space. Keep moving through
            // these edges until we find one that's inside or on the plane.
            while(j<nu[up]) {
                lp=ed[up][j];
                lw=m_testx(lp,l);
                if(lw!=0) break;
            ...
            // Compute the number of edges for the new vertex. In general it
            // will be the number of outside edges found, plus two. But we need
            // to recognize the special case when all but one edge is outside,
            // and the remaining one is on the plane. For that case we
            // have to reduce the edge count by one to prevent doubling up.
            if(j==nu[up]&&i==1&&rw==1) {
                nu[p]=nu[up];
                double_edge=true;
            } else nu[p]=j-i+2;
            ...
            // Add memory for the new vertex if needed, and initialize
            ...
            // Copy the edges of the original vertex into the new one. Delete
            // the edges of the original vertex, and update the relational
            // table.
            us=cycle_down(i,up);
            while(i<j) {
                qp=ed[up][i];
            ...
            // In this case, the zeroth edge is outside the cutting plane.
            // Begin by searching backwards from the last edge until we find an
            // edge which isn't outside.
            i=nu[up]-1;
            lp=ed[up][i];
            lw=m_testx(lp,l);
            while(lw==0) {
                ...
                // If i reaches zero, then we have a point in the plane all of
                // whose edges are outside the cutting space, so we just exit
                if(i==0) return false;
                lp=ed[up][i];
                lw=m_testx(lp,l);
            }
            ...
            // will be the number of outside edges found, plus two. But we need
            // to recognize the special case when all but one edge is outside,
            // and the remaining one is on the plane. For that case we have to
            // reduce the edge count by one to prevent doubling up.
            if(i==j&&qw==1) {
                double_edge=true;
                nu[p]=nu[up];
            } else {
            ...
            // Add memory to store the vertex if it doesn't exist already
            k=1;
            while(nu[p]>=current_vertex_order) add_memory_vorder(vc);
            if(mec[nu[p]]==mem[nu[p]]) add_memory(vc,nu[p]);
        ...
        *(stackp2++)=up;
        ...
        // Look at the edges on either side of the group that was detected.
        // We're going to commence facet computation by moving along one of
        // them. We are going to end up coming back along the other one.
        cs=k;
        qp=up;q=u;
        i=ed[up][us];
        us=ed[up][nu[up]+us];
        ...
        // The search algorithm found an intersected edge between the points lp
        // and up. Create a new vertex between them which lies on the cutting
        // plane. Since u and l differ by at least the tolerance, this division
        // should never fail.
        if(stackp==stacke) add_memory_ds();
        *(stackp++)=up;
        r=u/(u-l);l=1-r;
    ...
    // When the code reaches here, we have initialized the first point, and we
    // have a direction for moving it to construct the rest of the facet
    cp=p;rp=p;p++;
    while(qp!=up||qs!=us) {
        ...
        if(lw==2) {
            ...
            // The point is still in the cutting space. Just add it to the
            // delete stack and keep moving.
            ...
            *(stackp++)=qp;
            ...
            // The point is outside of the cutting space, so we've found an
            // intersected edge. Introduce a regular point at the point of
            // intersection. Connect it to the point we just tested. Also
            // connect it to the previous new point in the facet we're
            // constructing.
            if(p==current_vertices) add_memory_vertices(vc);
            r=q/(q-l);l=1-r;
            ...
            *ed[p]=cp;
            ...
            // We've found a point which is on the cutting plane. We're going
            // to introduce a new point right here, but first we need to figure
            // out the number of edges it has.
            if(p==current_vertices) add_memory_vertices(vc);

            // If the previous vertex detected a double edge, our new vertex
            // will have one less edge.
            k=double_edge?0:1;
            qs=ed[qp][nu[qp]+qs];
            qp=lp;
            iqs=qs;

            // Start testing the edges of the current point until we find one
            // which isn't outside the cutting space
            do {
                k++;
                qs=cycle_up(qs,qp);
                lp=ed[qp][qs];
            ...
            // Now we need to find out whether this marginal vertex we are on
            // has been visited before, because if that's the case, we need to
            // add vertices to the existing new vertex, rather than creating a
            // fresh one. We also need to figure out whether we're in a case
            // where we might be creating a duplicate edge.
            j=-ed[qp][nu[qp]<<1];
             if(qp==up&&qs==us) {

                // If we're heading into the final part of the new facet, then
                // we never worry about the duplicate edge calculation.
                new_double_edge=false;
                if(j>0) k+=nu[j];
            } else {
                if(j>0) {

                    // This vertex was visited before, so count those vertices
                    // to the ones we already have.
                    k+=nu[j];

                    // The only time when we might make a duplicate edge is if
                    // the point we're going to move to next is also a marginal
                    // point, so test for that first.
                    if(lw==1) {

                        // Now see whether this marginal point has been visited
                        // before.
                        i=-ed[lp][nu[lp]<<1];
                        if(i>0) {

                            // Now see if the last edge of that other marginal
                            // point actually ends up here.
                            if(ed[i][nu[i]-1]==j) {
                            ...
                            // That marginal point hasn't been visited before,
                            // so we probably don't have to worry about
                            // duplicate edges, except in the case when that's
                            // the way into the end of the facet, because that
                            // way always creates an edge.
                    ...
                    // The vertex hasn't been visited before, but let's see if
                    // it's marginal
                    if(lw==1) {

                        // If it is, we need to check for the case that it's a
                        // small branch, and that we're heading right back to
                        // where we came from
                        i=-ed[lp][nu[lp]<<1];
                        if(i==cp) {
                            new_double_edge=true;
            ...
            // k now holds the number of edges of the new vertex we are
            // forming. Add memory for it if it doesn't exist already.
            while(k>=current_vertex_order) add_memory_vorder(vc);
            if(mec[k]==mem[k]) add_memory(vc,k);

            // Now create a new vertex with order k, or augment the existing
            // one
            if(j>0) {

                // If we're augmenting a vertex but we don't actually need any
                // more edges, just skip this routine to avoid memory confusion
                if(nu[j]!=k) {

                    // Allocate memory and copy the edges of the previous
                    // instance into it
                    vc.n_set_aux1(k);
                    ...
                    // Remove the previous instance with fewer vertices from
            ...
            // Unless the previous case was a double edge, connect the first
            // available edge of the new vertex to the last one in the facet
            if(!double_edge) {
                ed[j][i]=cp;
                ed[j][nu[j]+i]=cs;
                vc.n_set(j,i,p_id);
            ...
            // Copy in the edges of the underlying vertex, and do one less if
            // this was a double edge
            qs=iqs;
            ...
            // Update the double_edge flag, to pass it to the next instance of
            // this routine
 ...
 *         was successful. */
template<class vc_class>
inline bool voronoicell_base_3d::collapse_order2(vc_class &vc) {
    ...
    while(mec[2]>0) {
        ...
        // If j doesn't already join k, join them together. Otherwise delete
        // the connection to the current vertex from j and k.
 ...
 *         successful. */
template<class vc_class>
bool voronoicell_base_3d::collapse_order1(vc_class &vc) {
    int i,j,k;
    while(mec[1]>0) {
        up=0;
#if VOROPP_VERBOSE >=1
...
template<class vc_class>
bool voronoicell_base_3d::delete_connection(vc_class &vc,int j,int k,bool hand) {
...
bool voronoicell_base_3d::failsafe_find(int &lp,int &ls,int &us,double &l,double &u) {
...
double voronoicell_base_3d::volume() {
...
void voronoicell_base_3d::face_areas(std::vector<double> &v) {
    double area;
    v.clear();
    int i,j,k,l,m,n;
    double ux,uy,uz,vx,vy,vz,wx,wy,wz;
...
double voronoicell_base_3d::surface_area() {
    double area=0;
    int i,j,k,l,m,n;
    double ux,uy,uz,vx,vy,vz,wx,wy,wz;
    for(i=1;i<p;i++) for(j=0;j<nu[i];j++) {
...
void voronoicell_base_3d::centroid(double &cx,double &cy,double &cz) {
...
double voronoicell_base_3d::total_edge_distance() {
    int i,j,k;
 ...
 * \param[in] (x,y,z) a displacement vector to be added to the cell's position.
 * \param[in] fp a file handle to write to. */
void voronoicell_base_3d::draw_gnuplot(double x,double y,double z,FILE *fp) {
    int i,j,k,l,m;
    for(i=1;i<p;i++) for(j=0;j<nu[i];j++) {
        k=ed[i][j];
        if(k>=0) {
 ...
 * \param[out] v the vector to store the results in. */
void voronoicell_base_3d::normals(std::vector<double> &v) {
    int i,j,k;
    v.clear();
    for(i=1;i<p;i++) for(j=0;j<nu[i];j++) {
        k=ed[i][j];
...
inline void voronoicell_base_3d::normals_search(std::vector<double> &v,int i,int j,int k) {
    ed[i][j]=-1-k;
...
int voronoicell_base_3d::number_of_faces() {
    int i,j,k,l,m,s=0;
    for(i=1;i<p;i++) for(j=0;j<nu[i];j++) {
        k=ed[i][j];
        if(k>=0) {
 ...
 * \param[out] v the vector to store the results in. */
void voronoicell_base_3d::vertex_orders(std::vector<int> &v) {
    v.resize(p);
...
void voronoicell_base_3d::output_vertex_orders(FILE *fp) {
    if(p>0) {
        fprintf(fp,"%d",*nu);
 ...
 *                    coordinate system. */
void voronoicell_base_3d::vertices(double x,double y,double z,std::vector<double> &v) {
    v.resize(3*p);
    double *ptsp=pts;
 ...
 * \param[out] v the vector to store the results in. */
void voronoicell_base_3d::face_vertices(std::vector<int> &v) {
 ...
 * \param[out] v the vector to store the results in. */
void voronoicell_base_3d::face_orders(std::vector<int> &v) {
    int i,j,k,l,m,q;
    v.clear();
    for(i=1;i<p;i++) for(j=0;j<nu[i];j++) {
        k=ed[i][j];
 ...
 * \param[out] v the vector to store the results in. */
void voronoicell_base_3d::face_freq_table(std::vector<int> &v) {
    int i,j,k,l,m,q;
    v.clear();
    for(i=1;i<p;i++) for(j=0;j<nu[i];j++) {
        k=ed[i][j];
...
int voronoicell_base_3d::number_of_edges() {
    int edges=0,*nup=nu;
...
void voronoicell_base_3d::output_custom(const char *format,int i,double x,double y,double z,double r,FILE *fp) {
 ...
 * \param[in] (ymin,ymax) the minimum and maximum y coordinates.
 * \param[in] (zmin,zmax) the minimum and maximum z coordinates. */
void voronoicell_neighbor_3d::init(double xmin,double xmax,double ymin,double ymax,double zmin,double zmax) {
 ...
 *              vertices are initialized at (-l,0,0), (l,0,0), (0,-l,0),
 *              (0,l,0), (0,0,-l), and (0,0,l). */
void voronoicell_neighbor_3d::init_octahedron(double l) {
 ...
 * \param (x2,y2,z2) a position vector for the third vertex.
 * \param (x3,y3,z3) a position vector for the fourth vertex. */
void voronoicell_neighbor_3d::init_tetrahedron(double x0,double y0,double z0,double x1,double y1,double z1,double x2,double y2,double z2,double x3,double y3,double z3) {
 ...
 * consistent. */
void voronoicell_neighbor_3d::check_facets() {
    int i,j,k,l,m,q;
    for(i=1;i<p;i++) for(j=0;j<nu[i];j++) {
        k=ed[i][j];
        if(k>=0) {
...
voronoicell_neighbor_3d::~voronoicell_neighbor_3d() {
...
void voronoicell_neighbor_3d::neighbors(std::vector<int> &v) {
    v.clear();
    int i,j,k,l,m;
    for(i=1;i<p;i++) for(j=0;j<nu[i];j++) {
        k=ed[i][j];
...
void voronoicell_neighbor_3d::print_edges_neighbors(int i) {
    if(nu[i]>0) {
        int j=0;
        printf("     (");
...
template bool voronoicell_base_3d::nplane(voronoicell_3d&,double,double,double,double,int);
template bool voronoicell_base_3d::nplane(voronoicell_neighbor_3d&,double,double,double,double,int);
template void voronoicell_base_3d::check_memory_for_copy(voronoicell_3d&,voronoicell_base_3d*);
template void voronoicell_base_3d::check_memory_for_copy(voronoicell_neighbor_3d&,voronoicell_base_3d*);
