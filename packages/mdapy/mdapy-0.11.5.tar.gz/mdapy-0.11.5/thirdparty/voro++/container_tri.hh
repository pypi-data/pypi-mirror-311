...
/** \file container_tri.hh
 * \brief Header file for the container_triclinic_base and related classes. */

#ifndef VOROPP_CONTAINER_TRI_HH
#define VOROPP_CONTAINER_TRI_HH
...
namespace voro
{
     ...
     * (bxy,by,0), and (bxz,byz,bz) that represent a parallelepiped. Internally,
     * the class stores particles in the box 0<x<bx, 0<y<by, 0<z<bz, and constructs
     * periodic images of particles that displaced by the three periodicity vectors
     * when they are necessary for the computation. The internal memory structure
     * for this class is significantly different from the container_base class in
     * order to handle the dynamic construction of these periodic images.
     *
    ...
    public:
         ...
         * block structure. */
        int ey;
        /** The lower z index (inclusive) of the primary domain within the
         * block structure. */
        int ez;
        /** The upper y index (exclusive) of the primary domain within the
         * block structure. */
        int wy;
        /** The upper z index (exclusive) of the primary domain within the
         * block structure. */
        int wz;
        /** The total size of the block structure (including images) in the y
         * direction. */
        int oy;
        /** The total size of the block structure (including images) in the z
         * direction. */
        int oz;
        ...
        int *mem;
        /** An array holding information about periodic image construction at a
         * given location. */
        char *img;
        /** The initial amount of memory to allocate for particles for each
         * block. */
        const int init_mem;
         ...
         * cell is initialized to be the pre-computed unit Voronoi cell based
         * on planes formed by periodic images of the particle.
         ...
         *                     the voro_compute coordinate system.
         * \param[out] disp a block displacement used internally by the
         *                  find_voronoi_cell routine (but not needed in this
         *                  instance.) */
         ...
         *                 find_voronoi_cell and compute_cell routines (but not
    ...
    protected:
         ...
         * primary domain and shifting them. If the given block is aligned with
         * the primary domain in the z-direction, the routine calls the simpler
         * create_side_image routine where the image block may comprise of
         * particles from up to two primary blocks. Otherwise is calls the more
         * complex create_vertical_image where the image block may comprise of
         * particles from up to four primary blocks.
         * \param[in] (di,dj,dk) the coordinates of the image block to create.
         */
     ...
     *
     * This class is an extension of the container_triclinic_base that has routines
    ...
    public:
         ...
         * position, z position) are searched for. If the
