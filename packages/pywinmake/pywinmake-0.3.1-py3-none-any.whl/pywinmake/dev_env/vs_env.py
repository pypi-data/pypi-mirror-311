...
class VSEnv:
    def __init__(self, arch="x64", version=None):
        ...
        if self.vs_version == "17":
            self.toolset_default = "v143"
        elif self.vs_version == "16":
            self.toolset_default = "v142"
        else:
            self.toolset_default = "v144"
        ...
        self.vars = self.__get_vs_env(self.arch, self.sdk_version)
        ...
        log.info(f"Using VS {self.vs_version} w/sdk {self.sdk_version} "
                 f"(latest: {self.latest_sdk_version}) w/toolset {self.toolset_default}")

        self.validated = True

    @staticmethod
    def get_ms_build_args(arch="x64", config_str="Release", use_env=True, toolset=""):
    ...
    def __get_cmake_generator(self, vs_version):
        ...
        if vs_version == "17":
            ...
            log.critical(f"Unsupported Visual Studio version {vs_version}")
            return None
