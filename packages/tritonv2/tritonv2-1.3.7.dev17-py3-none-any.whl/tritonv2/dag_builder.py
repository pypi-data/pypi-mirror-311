...
class PkgDAGBuilder:
    ...
    def _build_backend_bls(self):
        """
            get backend bls model name, etc
        # """
        nodes = copy.deepcopy(self.nodes)
        for name, node in nodes.items():
            for item in node.sub_models:
                self.nodes[item[0]] = self._build_node(
                    os.path.join(self.model_repo, item[0], self.extra_models[item[0]], PBTXT_NAME), item[0])
    ...
    def _build_in_out_connects(self, name_version_in_out: list, ensemble_node: Node):
        ...
        for n in name_version_in_out:
            ...
            if model_name not in self.nodes:
                self.nodes[model_name] = self._build_node(
                    os.path.join(self.model_repo, model_name, self.extra_models[model_name], PBTXT_NAME), model_name)
            node = self.nodes[model_name]
