...
from PySide6.QtCore import QFile, QTimer, QThread, QObject, Signal, Slot
from PySide6.QtGui import QPalette, QColor, QImage, QPainter
from resources.app import Ui_mainWindow
...
ALOHA_ROBOT_PATH = 'aloha_data_collection/configs/robot/aloha.yaml'
ALOHA_TASK_PATH = 'aloha_data_collection/configs/tasks.yaml'
...
def set_image(widget, image):
    ...
    widget.image = QImage(
        image.data, image.shape[1], image.shape[0], QImage.Format.Format_RGB888).rgbSwapped()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        robot_cfg = init_hydra_config(
            config_path=ALOHA_ROBOT_PATH, overrides=None)
        ...
        self.camera_widgets = [self.ui.openGLWidget_camera_0, self.ui.openGLWidget_camera_1,
                               self.ui.openGLWidget_camera_2, self.ui.openGLWidget_camera_3]

        for widget in self.camera_widgets:
            widget.set_image = lambda image, widget=widget: set_image(
                widget, image)
            widget.paintEvent = lambda event, widget=widget: paintEvent(
                widget, event)
    ...
    def populate_task_combobox(self):
        ...
        if self.tasks_config and "tasks" in self.tasks_config:
            ...
            print("No tasks found in the configuration file.")
    ...
    def get_task_parameters(self, task_name):
        ...
        if not self.tasks_config or "tasks" not in self.tasks_config:
            print("Task configuration not found or tasks not defined.")
            return None
    ...
    def start_recording(self):
        ...
        if not task_config:
            print("Unable to find configuration for the selected task.")
            return
        ...
        self.set_logs(
            f'Starting recording with {DATA_ROOT=}, {self.selected_task=}.')
        ...
        while timestamp < control_time_s:
            ...
            if display_cameras:
                ...
                for i, key in enumerate(image_keys[:4]):  # Limit to 4 cameras
                    ...
                    self.worker.image_update.emit(
                        i, rgb_image)  # Emit image signal
    ...
    def log_control_info(self, robot: Robot, dt_s, episode_index=None, frame_index=None, fps=None):
        log_items = []
    ...
    def record(
        ...
        self.control_loop(robot=robot, events=self.events, teleoperate=enable_teleoperation,
                          control_time_s=warmup_time_s, display_cameras=display_cameras, fps=fps)
        ...
        while True:
            ...
            if not self.events["stop_recording"] and (
                (episode_index < num_episodes -
                 1) or self.events["rerecord_episode"]
            ):
