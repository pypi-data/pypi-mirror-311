...
class AcceptsPluginsGroup(cloup.Group):
    ...
    def discover_plugins(self, ctx):
        ...
        candidate_plugins = discover_plugins()
...
@cache
def discover_plugins(cache_buster=False) -> dict[str, click.Command]:
    candidate_plugins = {}
    plugin_folder = CONFIG.get("path", section="plugins")
...
def list_plugins_by_file(cache_buster=False):
    plugins = discover_plugins(cache_buster=cache_buster)
