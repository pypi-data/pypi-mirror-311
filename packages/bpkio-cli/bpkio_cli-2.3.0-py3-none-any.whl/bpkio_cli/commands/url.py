...
def get_handler(
    ...
    # if the url is a valid path to a local file, we create an HTTP server. Check with os
    if os.path.isfile(url) and os.path.exists(url):
...
url.add_section(
    cloup.Section("Content Commands", [check, info, read, poll, monitor, play, download, profile])
)
