...
class HttpRequestData:
    """
    Structured result of an HTTP request parsing process.

    This class encapsulates the components of an HTTP request after parsing,
    providing structured access to the URL, method, headers, and request data.

    Attributes:
        url (str): The reconstructed URL from the HTTP request, including scheme, host, and path.
        method (str): The HTTP method used in the request (e.g., GET, POST, PUT).
        headers (dict): A dictionary of parsed HTTP headers.
        request_data (list): Parsed request body or query parameters as a list.
    """
    url: str
    method: str
    headers: dict
...
class HttpRequestParser():
    ...
    def parse_http_request(self, http_request: Union[str, TextIO, List[str]]) -> tuple:
        """
        Parses an HTTP request and extracts key components including URL, method, headers, and request data.

        :param http_request:
            The HTTP request input. Can be one of the following:
            - A string containing the path to a file with HTTP request data.
            - A file handler (TextIO) pointing to the request data.
            - A list of strings representing the HTTP request lines.

        :raises FileNotFoundError: If the specified file does not exist.
        :raises IsADirectoryError: If the provided path points to a directory instead of a file.
        :raises ValueError: If the request lacks essential components, such as the Host header.
        :raises Exception: For other unexpected errors during parsing.

        :return:
            An instance of `HttpRequestData` containing:
            - `url` (str): The reconstructed URL based on the Host header and path.
            - `method` (str): The HTTP method (e.g., GET, POST).
            - `headers` (dict): Parsed HTTP headers.
            - `request_data` (list): Parsed request body or query parameters.
    ...
    def _load_and_validate_http_request(self, http_request: Union[str, TextIO]) -> str:
        ...
        This method loads the HTTP request from a file if the input is a file path.
        If the input is a string, it returns the string as the HTTP request. It raises
        errors if the input is not a valid type or if the file cannot be found.

        :param http_request: A full HTTP request string or a path to a request file.
        :type http_request: Union[str, TextIO]

        :return: The loaded HTTP request string.
        :rtype: str

        :raises TypeError: If the input is neither a string nor a file handler.
        :raises FileNotFoundError: If the specified file does not exist.
        :raises IsADirectoryError: If the specified path is a directory.

        :note: If the input is a file handler, it reads from the file directly.
        """
