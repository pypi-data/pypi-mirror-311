...
class DeadLetterJob(core.Gs2Model):
    dead_letter_job_id: str = None
    name: str = None
    user_id: str = None
    script_id: str = None
    args: str = None
    result: List[JobResultBody] = None
    created_at: int = None
    updated_at: int = None

    def with_dead_letter_job_id(self, dead_letter_job_id: str) -> DeadLetterJob:
        self.dead_letter_job_id = dead_letter_job_id
        return self

    def with_name(self, name: str) -> DeadLetterJob:
        self.name = name
        return self

    def with_user_id(self, user_id: str) -> DeadLetterJob:
        self.user_id = user_id
        return self

    def with_script_id(self, script_id: str) -> DeadLetterJob:
        self.script_id = script_id
        return self

    def with_args(self, args: str) -> DeadLetterJob:
        self.args = args
        return self

    def with_result(self, result: List[JobResultBody]) -> DeadLetterJob:
        self.result = result
        return self

    def with_created_at(self, created_at: int) -> DeadLetterJob:
        self.created_at = created_at
        return self

    def with_updated_at(self, updated_at: int) -> DeadLetterJob:
    ...
    def create_grn(
        ...
        return 'grn:gs2:{region}:{ownerId}:queue:{namespaceName}:user:{userId}:dead:{deadLetterJobName}'.format(
            region=region,
            ownerId=owner_id,
            namespaceName=namespace_name,
            userId=user_id,
            deadLetterJobName=dead_letter_job_name,
        )
    ...
    def get_region_from_grn(
        ...
        match = re.search('grn:gs2:(?P<region>.+):(?P<ownerId>.+):queue:(?P<namespaceName>.+):user:(?P<userId>.+):dead:(?P<deadLetterJobName>.+)', grn)
        if match is None:
            return None
    ...
    @classmethod
    def get_dead_letter_job_name_from_grn(
        cls,
        grn: str,
    ) -> Optional[str]:
        match = re.search('grn:gs2:(?P<region>.+):(?P<ownerId>.+):queue:(?P<namespaceName>.+):user:(?P<userId>.+):dead:(?P<deadLetterJobName>.+)', grn)
        if match is None:
            return None
        return match.group('dead_letter_job_name')
    ...
    def from_dict(
        ...
        return DeadLetterJob()\
            .with_dead_letter_job_id(data.get('deadLetterJobId'))\
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "deadLetterJobId": self.dead_letter_job_id,
            "name": self.name,
            "userId": self.user_id,
            "scriptId": self.script_id,
            "args": self.args,
