...
from mtmai.db.db import get_engine
...
from mtmai.mtlibs.mq.mq import MtTaskMQ
...
class AgentContext:
    def __init__(
        self, *, thread_id: str = None, user_id: str = None, db_engine: Engine
    ):
    ...
    async def init_mq(self):
        if self._mq is None:
            self._mq = MtTaskMQ(thread_id=self.thread_id)
            await self._mq.init(self.thread_id)
...
mtmai_context: AgentContext = LazyProxy(get_mtmai_context, enable_cache=False)
