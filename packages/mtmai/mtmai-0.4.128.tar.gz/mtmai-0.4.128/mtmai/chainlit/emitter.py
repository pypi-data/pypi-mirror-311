...
from mtmai.chainlit.element import Element, ElementDict, File
from mtmai.chainlit.logger import logger
from mtmai.chainlit.message import Message
from mtmai.chainlit.session import BaseSession, WebsocketSession
...
class BaseChainlitEmitter:
    ...
    async def send_form(
        self, form: ThreadForm, timeout=300, raise_on_timeout=False
    ) -> Optional[Dict[str, Any]]:
        pass
    ...
    # async def send_list_item(self, list_response: ListItemResponse):
    #     """发送列表查询结果"""
    #     pass
...
class ChainlitEmitter(BaseChainlitEmitter):
    ...
    async def send_form(self, form: ThreadForm, timeout=300, raise_on_timeout=False):
        try:
            call_fn_res = await self.emit_call(
                "ask_form", jsonable_encoder(form), timeout
            )  # type: Dict

            await self.clear("clear_ask_form")
            return call_fn_res
        except TimeoutError as e:
            await self.send_timeout("call_fn_timeout")
    ...
    # 新功能 开始 ###########################################################################################
    # def send_list_item(self, list_response: ListItemResponse):
    #     return self.emit("list_item", jsonable_encoder(list_response))
