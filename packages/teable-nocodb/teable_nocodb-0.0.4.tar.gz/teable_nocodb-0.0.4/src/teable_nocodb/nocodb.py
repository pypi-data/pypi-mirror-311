...
class Table:
    ...
    def get_columns(self) -> list["Column"]:
        ...
        if response.status_code == 200:
            self.columns = [
                Column(self.nocodb, **c) for c in response.json()["columns"]
            ]
            return self.columns
        else:
            raise Exception(
...
class Column:
    def __init__(self, nocodb: NocoDB | None = None, **kwargs) -> None:
        self.nocodb = nocodb
        self.title: str = kwargs["title"]
        ...
        self.options = []
        if self.data_type == FieldType.SINGLE_SELECT:
            self.options = [Option(**o) for o in kwargs["colOptions"]["options"]]
