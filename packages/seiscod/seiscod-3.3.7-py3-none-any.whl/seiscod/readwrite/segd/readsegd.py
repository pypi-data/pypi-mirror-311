...
:modification 2021: Maximilien Lehujeur
    ...
    - dependency to is no longer required, replaced by dep to tempoo
    - rename the field names slightly
    - do not remove the un-used fields
"""
import warnings
...
import math
import numpy as np
# from readdat.segd.utc_datetime import yjhms_to_utc_datetime, ymdhms_to_utc_datetime
from tempoo.utc import UTC, UTCFromJulday
...
def _bcd(byte):
    ...
    if isinstance(byte, str):  #(native_str,
        try:
            byte = ord(byte)
        except TypeError:
...
def _decode_fraction(bytes_in):
    ...
    # bytes_ord = map(ord, bytes_in)   # old python 2 syntax
    bytes_ord = bytes_in
...
def _decode_asc(bytes_in):
    """Decode ascii."""
    # bytes_ord = map(ord, bytes_in)  # python 2
    bytes_ord = bytes_in
...
def _read_ghb1(fp):
    ...
    ghb1['hour'] = _hour
    ghb1['minute'] = _min
    ghb1['second'] = int(_sec)
    ghb1['microsecond'] = int((_sec % 1) * 1.e6)  # fuck off: will always be 0

    ghb1['time'] = UTCFromJulday(
        year=ghb1['year'],
        julday=ghb1['julday'],
        hour=ghb1['hour'],
        minute=ghb1['minute'],
        second=ghb1['second'],
        microsecond=ghb1['microsecond'])
...
def _read_ghb3(fp):
    ...
    ghb3['source_point_index'] = _decode_bin(buf[13:14])   # added by max, right?
...
def _read_sch(fp):
    ...
    sch['alias_filter_freq_at_m3dB_in_Hz'] = _decode_bcd(buf[12:14])
    sch['alias_filter_slope_in_dB_per_octave'] = _decode_bcd(buf[14:16])
    sch['lowcut_filter_freq_in_Hz'] = _decode_bcd(buf[16:18])
    sch['lowcut_filter_slope_in_dB_per_octave'] = _decode_bcd(buf[18:20])
...
def _read_extdh(fp, size):
    ...
    extdh['threshold_hold_per_var'] = _threshold_types[_thv]
    ...
    _gps_seconds = _gps_microseconds / 1.e6
    _gps_time = _gps_time + _gps_seconds
    print(_gps_time, type(_gps_time))
...
def _read_traceh(fp, skip=False):
    """Read trace header."""
    if skip:
        fp.seek(20, 1)
        return None
...
def _read_traceh_eb2(fp):
    ...
    buf = fp.read(32)
    traceh = OrderedDict()
    traceh['receiver_easting'] = _decode_dbl(buf[0:8])
    traceh['receiver_northing'] = _decode_dbl(buf[8:16])
    traceh['receiver_elevation'] = _decode_flt(buf[16:20])
...
def _read_traceh_eb6(fp):
    ...
    try:
        ...
        warnings.warn(f'unknown subunit_type {_st}')
        traceh['subunit_type'] = _subunit_types[0]

    _ct = _decode_bin(buf[17:18])
    traceh['channel_type'] = _channel_types[_ct]
    # 18-19 : not used
    traceh['sensor_sensitivity_in_mV_per_m_per_s_per_s'] = _decode_flt(buf[20:24])
    # 24-31 : not used
    return traceh
...
def _read_traceh_eb7(fp):
    ...
    try:
        ...
        warnings.warn(f'unknown control_unit_type {_cut}')
...
def _read_trace_data(fp, npts, headonly=False):
    if headonly:
        # return empty array,
        # seek from current position
        # to mimic the default behavior
        fp.seek(npts * 4, 1)  # offset and whence keywords not supported by _io.BufferReader?
        return np.array([])

    buf = array('f')
    # replaces fromstring as recommended by a warning message
    buf.frombytes(fp.read(npts * 4))
...
def _read_trace_data_block(fp, npts, headonly=False, skip=False):
    traceh = _read_traceh(fp, skip=skip)
    if skip:
        # skip is stronger tha headonly, even header is not read
        return traceh, np.array([])
    ...
    trace_data = _read_trace_data(fp, npts, headonly=headonly)
    return traceh, trace_data
...
def read_segd(filename, headonly=False, index_only=None):
    ...
    with open(filename, 'rb') as fp:
        ...
        if index_only is not None:
            if not 0 <= index_only < extdh['total_number_of_traces']:
                raise IndexError(f"{index_only} is out of range [0, {extdh['total_number_of_traces']}[")
        ...
        delta = extdh['sample_rate_in_us'] / 1e6
        npts = extdh['number_of_samples_in_trace']

        raw_trace_list = []  # Stream()
        total_number_of_traces = extdh['total_number_of_traces']

        # if total_number_of_traces < 1:
        #     raise ValueError("no traces found")

        for n in range(total_number_of_traces):
            skip = (index_only is not None) and (n < index_only)

            # read data array and header
            traceh, trace_data = _read_trace_data_block(fp, npts=npts, headonly=headonly, skip=skip)
            if skip:
                continue
            ...
            trace_dict = {
                'npts': npts,
                'network': "",
                'station': str(traceh['unit_serial_number']),
                'channel': _band_code(1. / delta) + _instrument_orientation_code[traceh['sensor_code']],
                'location': "",
                'delta': delta,
                'starttime': generalh['time'],
                'segd': _build_segd_header(generalh, sch, extdh, extrh, traceh)}
            raw_trace = RawTrace(trace_dict=trace_dict, trace_data=trace_data)
            raw_trace_list.append(raw_trace)

            if index_only is not None and n >= index_only:
                break

    if not headonly:
        # check if all traces can be converted to int
        convert_to_int = np.all([np.all(np.mod(raw_trace.trace_data, 1) == 0) for raw_trace in raw_trace_list])

        if convert_to_int:
            for raw_trace in raw_trace_list:
                raw_trace.data = raw_trace.data.astype(np.int32)
