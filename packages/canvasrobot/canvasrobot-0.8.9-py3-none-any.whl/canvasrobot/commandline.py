...
@click.option("--db_update", default=False, help="If True force database update.")
def run(reset_api_keys, db_update):
    path = Path(__file__)
    # /Users/ncdegroot/.local/share/uv/tools/canvasrobot/lib/python3.13/site-packages/canvasrobot/databases
    path = go_up(path, levels=4)
    path = path / "database"
    print(path)
    db_path = os.path.join(path, "databases")
    robot = cr.CanvasRobot(reset_api_keys,
                           db_update,
                           db_folder=path,)
