...
from clickpy.queries.keywords_downloads_resume_query import KeywordsDownloadsResumeQuery
from clickpy.queries.keywords_packages_resume_query import KeywordsPackagesResumeQuery
from clickpy.queries.keywords_releases_resume_query import KeywordsReleasesResumeQuery
from clickpy.queries.package_downloads_resume_query import PackageDownloadsResumeQuery
from clickpy.queries.package_releases_resume_query import PackageReleasesResumeQuery
...
class ClickPyClient(object):
    ...
    async def get_keywords_releases_resume(self, keyword: str) -> KeywordsReleasesResumeQuery.Model:
        return await self.query(KeywordsReleasesResumeQuery(keyword=keyword))

    async def get_keywords_packages_resume(self, keyword: str) -> KeywordsPackagesResumeQuery.Model:
        return await self.query(KeywordsPackagesResumeQuery(keyword=keyword))

    async def get_keywords_downloads_resume(self, keyword: str) -> KeywordsDownloadsResumeQuery.Model:
        return await self.query(KeywordsDownloadsResumeQuery(keyword=keyword))
