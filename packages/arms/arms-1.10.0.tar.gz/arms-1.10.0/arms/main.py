...
def run_process(tpl_name, is_patch=False):
    ...
    if not dot_env_json.get('ARMS_PROJECT_NAME'):
        ...
        if opt_choice == '微服务项目':
            dot_env_json['ARMS_PROJECT_NAMESPACE'] = Prompt.ask(
                '请输入项目命名空间')
        dot_env_json['ARMS_PROJECT_NAME'] = Prompt.ask('请输入项目名称')
    # 支持层级选择
    ...
    for old_proj_name, new_proj_name in join_dot_env_and_query(dot_env_json, index_json):
        ...
        #     midname = str(WordSeed(WordStyle.lower_snake,
        #                   WordSeed.of(new_proj_name).tokens))
        #     out_path = Path('..')
        #     for p in curpath.rglob('*'):
        #         if p.is_file() and (out_path / p).is_file():
        #             new_file_name = dump_file_name(
        #                 p.parts[-1], project=midname)
