...
class TwitterAccountAsync:
    ...
    async def bind_account_v1(self, url: str) -> dict | Coroutine[Any, Any, dict[str, LiteralString | Any]]:
        ...
        async def get_oauth_token() -> str:
            ...
            token = _response.url.split("oauth_token=")
            if len(token) > 1:
                return token[1]
        ...
        return await process_confirm_url(confirm_url)
    ...
    async def verify_credentials(self) -> dict:
        r = await self.session.get(
            "https://api.x.com/graphql/UhddhjWCl-JMqeiG4vPtvw/Viewer",
            params={
                ...
                'fieldToggles': '{"isDelegate":false,"with  AuxiliaryUserLabels":false}',
            }
        )
        return self._verify_response(r)
    ...
    async def screen_name(self) -> str:
        ...
        return _data["data"]["viewer"]["user_results"]["result"]["legacy"]["screen_name"]
    ...
    async def user_id(self) -> str:
        ...
        """
        _data = await self.verify_credentials()
        return _data["data"]["viewer"]["user_results"]["result"]["legacy"]["rest_id"]
    ...
    async def name(self) -> str:
        ...
        """
        _data = await self.verify_credentials()
        return _data["data"]["viewer"]["user_results"]["result"]["legacy"]["name"]
    ...
    async def location(self) -> str:
        ...
        """
        _data = await self.verify_credentials()
        return _data["data"]["viewer"]["user_results"]["result"]["legacy"]["location"]
    ...
    async def description(self) -> str:
        ...
        """
        _data = await self.verify_credentials()
        return _data["data"]["viewer"]["user_results"]["result"]["legacy"]["description"]
    ...
    async def followers_count(self) -> int:
        ...
        """
        _data = await self.verify_credentials()
        return _data["data"]["viewer"]["user_results"]["result"]["legacy"]["followers_count"]

    async def friends_count(self) -> int:
        """
        :return: number of friends/following
        """
        _data = await self.verify_credentials()
        return _data["data"]["viewer"]["user_results"]["result"]["legacy"]["friends_count"]
    ...
    async def registration_date(self) -> str:
        ...
        """
        _data = await self.verify_credentials()
        return _data["data"]["viewer"]["user_results"]["result"]["legacy"]["created_at"]
    ...
    async def is_verified(self) -> bool:
        ...
        """
        _data = await self.verify_credentials()
        return _data["data"]["viewer"]["user_results"]["result"]["legacy"]["verified"]

    async def is_blue_verified(self) -> bool:
        """
        :return: blue verification status
        """
        _data = await self.verify_credentials()
        return _data["data"]["viewer"]["user_results"]["result"]["is_blue_verified"]
    ...
    async def tweet_details(self, tweet_id: int | str) -> dict:
        variables = {"focalTweetId": tweet_id}
        return await self.gql("GET", Operation.TweetDetail, variables)
    ...
    async def user_tweets(self, username: str, limit: int = 40) -> list[dict[str, str | Any]]:
        ...
        while len(tweets_data) < limit:
            try:
                ...
                raise RateLimitError(error.reset_in, collected_data=tweets_data)
    ...
    async def get_user_id(self, username: str) -> str:
        user_data = await self.get_user_by_screen_name(username)
