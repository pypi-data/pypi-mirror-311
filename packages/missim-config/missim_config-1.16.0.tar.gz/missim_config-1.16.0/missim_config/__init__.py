...
class MissimConfig(BaseModel):
    ...
    with_discovery_server: bool = Field(
        default=False, description="Run the discovery server as part of MIS-SIM."
    )
    discovery_server_ip: str = Field(
        default="0.0.0.0",
        description="IP address of the discovery server.",
    )
    own_ip: str = Field(
        default="0.0.0.0",
        description="The IP address of MIS-SIM's primary network interface. This is where DDS will bind to.",
    )
