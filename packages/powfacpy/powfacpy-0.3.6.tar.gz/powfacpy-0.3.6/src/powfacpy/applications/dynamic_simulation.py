...
class DynamicSimulation(ApplicationBase):
    ...
    def get_eigenvalues_of_current_state(
        ...
        if commod_parameters:
            self.act_prj.set_attr(commod, commod_parameters)
        commod.Execute()

        elmres: ElmRes = commod.ResultFile
        comres: ComRes = self.act_prj.app.GetFromStudyCase("ComRes")
    ...
    def create_dyn_sim_event(
        ...
        event_obj = self.act_prj.create_in_folder(
    ...
    def get_dsl_model_parameter_names(self, dsl_model):
        ...
        of a dsl model.
        """
        dsl_model = self.act_prj._handle_single_pf_object_or_path_input(dsl_model)
        try:
        ...
        for dsl_model in dsl_models:
            ...
            if parameter_names_incl_dsl_lookup:
                for parameter_name in parameter_names_incl_dsl_lookup:
                    if not self.is_dsl_lookup_arrays_and_matrices_name(parameter_name):
                        parameter_names.append(parameter_name)

            if parameter_names:
                parameter_names_and_values = self.act_prj.get_attr(
    ...
    def set_parameters_of_dsl_models_in_composite_model(
        ...
        if not single_dict_for_all_dsl_models:
            for dsl_model, parameter_value_dict in models_params_dict.items():
                dsl_model = self.act_prj.get_unique_obj(
                    dsl_model + ".ElmDsl",
                    parent_folder=composite_model,
                    include_subfolders=True,
                )
                self.act_prj.set_attr(dsl_model, parameter_value_dict)
        else:
            dsl_models = self.get_dsl_models_inside_composite_model(composite_model)
            for dsl_model in dsl_models:
    ...
    def is_dsl_lookup_arrays_and_matrices_name(string: str):
        ...
        for (
            dsl_lookup_name
        ) in DynamicSimulation.get_dsl_lookup_arrays_and_matrices_names():
