...
for old_key in supported_and_unsupported_simulation_types_and_names.keys():
    new_dict[old_key + ":"] = supported_and_unsupported_simulation_types_and_names[
        old_key
    ]
    ...
    def create_results_variables_python_file(self):
        ...
        with open(self.path_python_res_var_file, "w") as out_file:
            ...
            for (
                pf_sim_type,
                powfacpy_sim_type,
            ) in supported_and_unsupported_simulation_types_and_names.items():
                if powfacpy_sim_type:
                    ...
                    for res_var_file in pf_res_var_txt_files:
                        with open(
                            self.path_pf_res_var_txt_files + "//" + res_var_file, "r"
                        ) as in_file:
                            out_file.write("\n")
                            elm_class = res_var_file.split(".")[0]
                            self.write_elm_class_in_simulation_type(
                                elm_class, in_file, out_file, pf_sim_type
                            )
...
"""
    ...
    def write_elm_class_in_simulation_type(
        self, elm_class: str, in_file, out_file, pf_sim_type: str
    ):
        ...
        out_file.write(
            2 * self.indentation_increment + "class " + elm_class + "(Enum):\n"
        )
        out_file.write(
            3 * self.indentation_increment + "_init_ = 'value __doc__'\n\n"
    ...
    def write_variable_line(self, line, used_attr, out_file):
        ...
        if not attr_with_underscores_instead_of_colon in used_attr:
            ...
            out_line = (
                ...
                + attr_with_underscores_instead_of_colon
                + " = "
                + '"'
                + attr_and_unit_and_description[0]
                + '", "'
            )
