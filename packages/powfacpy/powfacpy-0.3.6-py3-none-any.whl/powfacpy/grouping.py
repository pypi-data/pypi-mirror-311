...
class GroupOfObjects:
    ...
    def get_by_loc_name_with_class(self, obj: str) -> filter:
        return filter(
            lambda x: fnmatch(x.loc_name + "." + x.GetClassName(), obj), self.objs
        )
...
class Subsystem:
    ...
    def __init__(
        self, elms: list[PFGeneral] = [], name: str = "", parent: Subsystem = None
    ) -> None:
        self.name: str = name
            ...
            for ss in self.children:
                if start_depth < 1:
                    if not stop_at(ss):
                        if include(ss):
                            yield ss
                        for subsubsystem in ss.get_subsystems(
                            include,
                            stop_at,
                            depth=depth - 1,
    ...
    def get_subsytems_by_names(
        self, names, stop_at_names, depth: int = inf, start_depth=0
    ) -> Generator[Subsystem]:
        return self.get_subsystems(
            lambda x: x.name in names, lambda x: x.name in stop_at_names
        )

    def get_subsystems_by_categories(
        self,
        categories: list[str],
        ...
        stop_at_categories_fun = lambda x: any(
            [x in stop_at_categories for x in self.categories]
        )
        return self.get_subsystems(
            categories_fun, stop_at_categories_fun, depth, start_depth
        )
    ...
    def is_in_elms_of(
        self, subsystem: Subsystem, depth: int = inf, start_depth: int = 0
    ):
        ...
        if start_depth < 1 and all([x in subsystem.elms for x in self.elms]):
            ...
            for ss in subsystem.children:
                found_ss = self.is_in_elms_of(
                    ss, depth=depth - 1, start_depth=start_depth - 1
                )
                if found_ss:
                    return found_ss
        return None
...
class PFZoneInterface(powfacpy.PFActiveProject):
    ...
    def __init__(self, pf_app: powfacpy.PFApp, elms: list[PFGeneral] = []) -> None:
