...
class DataParser:
    ...
    def _load_data(self):
        ...
        if load_encrypted:
            ...
            logging.info(f"Starting to retrieve local data file from {self.raw_data_path}.")
    ...
    def _extract_df_debug_mode(self):
        if self.debug:
            ...
            if self.user_config['dataparsers']['subset_vehicle_segment']:
                self.trips = self.trips.sort_values(['trip_start_weekday', 'season', 'vehicle_segment_string']).reset_index(drop=True)
                number_lines_per_cluster = round(self.number_lines_debug / (len(self.trips.trip_start_weekday.unique()) * len(self.trips.season.unique()) * len(self.trips.vehicle_segment_string.unique())))
                self.trips = self.trips.groupby(['trip_start_weekday', 'season', 'vehicle_segment_string']).head(number_lines_per_cluster).reset_index(drop=True)
            else:
                self.trips = self.trips.sort_values(['trip_start_weekday', 'season']).reset_index(drop=True)
                number_lines_per_cluster = round(self.number_lines_debug / (len(self.trips.trip_start_weekday.unique()) * len(self.trips.season.unique())))
                self.trips = self.trips.groupby(['trip_start_weekday', 'season']).head(number_lines_per_cluster).reset_index(drop=True)
        return self.trips
    ...
    def _set_greater_than_filter(dataset: pd.DataFrame, greater_than_filter_dict: dict):
        ...
        for greater_col, greater_elements in greater_than_filter_dict.items():
            greater_than_filter_cols[greater_col] = (
                dataset[greater_col] >= greater_elements.pop()
            )
    ...
    def _set_smaller_than_filter(
        ...
        for smaller_col, smaller_elements in smaller_than_filter_dict.items():
            smaller_than_filter_cols[smaller_col] = (
                dataset[smaller_col] <= smaller_elements.pop()
            )
    ...
    def _complex_filters(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        complex_filters = complex_filters.join(
            self._filter_inconsistent_travel_times(dataset=data)
        )
        complex_filters = complex_filters.join(
            self._filter_overlapping_trips(dataset=data)
        )
        return complex_filters

    @staticmethod
    def _filter_inconsistent_speeds(
        dataset: pd.DataFrame, lower_speed_threshold, higher_speed_threshold
    ) -> pd.Series:
        """
        ...
        """
        dataset["average_speed"] = dataset["trip_distance"] / (
            dataset["travel_time"] / 60
        )
    ...
    def _filter_inconsistent_travel_times(dataset_in: pd.DataFrame) -> pd.Series:
        ...
        dataset["travel_time_ts"] = (
            (dataset["timestamp_end"] - dataset["timestamp_start"])
            .dt.total_seconds()
            .div(60)
            .astype(int)
        )
...
class IntermediateParsing(DataParser):
    ...
    def _complex_filters(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        complex_filters = complex_filters.join(
            self._filter_inconsistent_travel_times(dataset_in=data)
        )
        complex_filters = complex_filters.join(
            self._filter_overlapping_trips(dataset=data)
        )
        complex_filters = complex_filters.join(
            self._filter_consistent_hours(dataset=data)
        )
        complex_filters = complex_filters.join(
            self._filter_zero_length_trips(dataset=data)
        )
        return complex_filters
    ...
    def _assign_season(self, trips):
        ...
        if self.season == "all" and self.user_config["global"]["consider_temperature_cycle_dependency"]["annual"] is True:
            trips['season'] = trips['timestamp_end'].apply(
                self.__identify_season)
        elif pd.Series(self.season).isin(['winter', 'spring', 'fall', 'summer'])[0] and self.user_config["global"]["consider_temperature_cycle_dependency"]["annual"] is False:
            trips["season"] = self.season
        elif self.season == "None":
            trips['season'] = "not assigned"
        elif pd.Series(self.season).isin(['winter', 'spring', 'fall', 'summer'])[0] and self.user_config["global"]["consider_temperature_cycle_dependency"]["annual"] is True:
            raise ValueError('You can not generate a profile of one season with annual selection')
        else:
            raise ValueError('The season you specified does not match any known season or option')
        return self.season
    ...
    def filter_for_season(self, trips):
        self.trips = trips[trips['season'] == self.user_config[
            'dataparsers']['seasons']['season_filter']]
        print(
            f"{(len(trips)-len(self.trips))} rows ({(len(self.trips)/(len(trips)-len(self.trips))*100):.2f}%) have been dropped because they did not took place in your choosed season.")
        return self.trips
    ...
    def _subset_area_type(self):
        if self.user_config["dataparsers"]["subset_area_type"]:
            ...
            n_vehicles = len(self.trips.unique_id.unique())
            logging.info(
                f"The subset contains only vehicles for the area type {area_type} for a total of {n_vehicles} individual "
                f"vehicles."
            )
