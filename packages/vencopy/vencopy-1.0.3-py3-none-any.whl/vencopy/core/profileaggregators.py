...
class ProfileAggregator:
    ...
    def aggregate_profiles(self):
        ...
        workflow.
        """
        if self.user_config["global"]["consider_temperature_cycle_dependency"]["season"] == "all":
            seasons = ["spring", "summer", "winter", "fall"]
            for season in seasons:
                logging.info(f"Aggreagting a weekly profile for season: {season}")
                ids_season = self.activities[self.activities.season == season]["unique_id"].unique()
                self.drain_weekly[season] = self.aggregator.perform_aggregation(profile=self.drain.loc[ids_season], profile_name=f"drain_{season}", method="flow")
                self.charging_power_weekly[season] = self.aggregator.perform_aggregation(profile=self.charging_power.loc[ids_season], profile_name=f"charging_power_{season}", method="flow")
                self.uncontrolled_charging_weekly[season] = self.aggregator.perform_aggregation(profile=self.uncontrolled_charging.loc[ids_season], profile_name=f"uncontrolled_charging_{season}", method="flow")
                self.max_battery_level_weekly[season] = self.aggregator.perform_aggregation(profile=self.max_battery_level.loc[ids_season], profile_name=f"max_battery_level_{season}", method="state")
                self.min_battery_level_weekly[season] = self.aggregator.perform_aggregation(profile=self.min_battery_level.loc[ids_season], profile_name=f"min_battery_level_{season}", method="state")
        else:
...
class Aggregator:
    ...
    def __compose_week_profile(self):
        ...
        if self.weekday_profiles.empty:
            raise ValueError(r"The 'weekday_profiles' DataFrame seems to be empty for one or more seasons. Please add more columns to debug mode.")
        ...
        self.weekday_profiles = pd.DataFrame(self.weekday_profiles, columns=[self.profile_name])
        logging.info(f"Finished generating a weekly profile for the vehicle segment {self.user_config['dataparsers']['vehicle_segment'][self.dataset]} with {self.profile_name}.")
    ...
    def __write_output(self):
        ...
        if self.user_config["global"]["write_output_to_disk"]["aggregator_output"]:
            root = Path(self.user_config["global"]["absolute_path"]["vencopy_root"])
            folder = self.dev_config["global"]["relative_path"]["aggregator_output"]
            self.dev_config["global"]["additional_label"] = "_" + self.profile_name + "_"
            file_name = create_file_name(
                dev_config=self.dev_config,
                user_config=self.user_config,
                file_name_id="output_profileaggregator",
    ...
    def perform_aggregation(
        ...
        logging.info(f"Needed time to aggregate {self.profile_name}: {elapsed_time_agg}.")
        self.profile = None
