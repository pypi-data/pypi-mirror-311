...
class DiaryBuilder:
    def __init__(
        self, configs: dict, data: dict, is_week_diary: bool = False
    ):
        """
    ...
    def create_diaries(self):
        ...
        self.max_battery_level = self.distributor.discretise(
            activities=self.activities,
            profile_name="max_battery_level_start",
            method="dynamic",
        )
        self.min_battery_level = self.distributor.discretise(
            activities=self.activities,
            profile_name="min_battery_level_end",
            method="dynamic",
        )
        ...
        logging.info(f"Needed time to discretise all columns: {needed_time}.")
...
class TimeDiscretiser:
    ...
    def __delta_battery_level_charging(self, data: pd.DataFrame, column: str):
        ...
        if column == "max_battery_level_start":
            data["charge_per_bin"] = (
                self.activities.available_power * self.time_resolution / 60
            )
            ...
            data["charge_per_bin"] = (
                self.activities.available_power * self.time_resolution / 60 * -1
            )
            data.loc[data["trip_id"].isna(), "value_per_bin"] = data.loc[
                data["trip_id"].isna(), :
            ].apply(
    ...
    def __enforce_battery_limit(
        self, delta_battery: list, how: str, lim: float
    ) -> list:
        """
    ...
    def __uncontrolled_charging_parking(self):
        ...
        self.data_to_discretise["number_full_bins_uncontrolled_charging"] = (
            self.data_to_discretise.loc[
                ...
                "time_delta_uncontrolled_chaging",
            ].dt.total_seconds()
            / 60
    ...
    def __charge_rate_per_bin(
        ...
        # it has 3 bins reducing the discretised duration to 45 minutes instead of 50 minutes.
    ...
    def discretise(self, activities, profile_name: str, method: str) -> pd.DataFrame:
        ...
        logging.info(
            f"Needed time to discretise {self.column_to_discretise}: {elapsed_time_diary_builder}."
        )
