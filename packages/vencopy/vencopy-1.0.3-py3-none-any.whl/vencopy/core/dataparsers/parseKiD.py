...
class ParseKiD(IntermediateParsing):
    ...
    def _load_unencrypted_data(self):
        ...
        logging.info(f"Finished loading {len(self.raw_data)} " f"rows of raw data of type .dta.")
    ...
    def __add_string_columns(self, weekday=True, purpose=True, purpose_from=True, vehicle_segment=True):
        """
        ...
        if purpose_from:
            self._add_string_column_from_variable(col_name="purpose_from_string", var_name="trip_purpose_from")
        if vehicle_segment:
    ...
    def process(self) -> pd.DataFrame:
        ...
        if self.user_config["dataparsers"]["seasons"]["filter_season_post_assignment"]:
            self.filter_for_season(self.trips)
        ...
        self.data['activities'] = self._cleanup_dataset(activities=self.data['activities'])
        self.write_output()
