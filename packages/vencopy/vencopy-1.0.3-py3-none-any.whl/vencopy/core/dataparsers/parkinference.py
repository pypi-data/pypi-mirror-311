...
class ParkInference:
    ...
    def add_parking_rows(self, trips: pd.DataFrame) -> pd.DataFrame:
        ...
        logging.info(
            f'Finished activity composition with {self.activities_raw["trip_id"].fillna(0).astype(bool).sum()} trips '
            f'and {self.activities_raw["park_id"].fillna(0).astype(bool).sum()} parking activites including vehicle which did not perform any trips on the survey date.'
        )
    ...
    def _calculate_vehicle_numbers(self, data):
        ...
        if self.user_config["dataparsers"]["subset_vehicle_segment"]:
            vehicle_segment = self.user_config["dataparsers"]["vehicle_segment"][self.dataset]
            if self.dataset == "KiD":
                percentages_immobile_vehicles = self.user_config["postprocessor"]["immobile_vehicles"][self.dataset][vehicle_segment]
            else:
                percentages_immobile_vehicles = self.user_config["postprocessor"]["immobile_vehicles"][self.dataset]
        else:
            #TODO: line below needed?
            percentages_immobile_vehicles = self.user_config["postprocessor"]["immobile_vehicles"][self.dataset]
    ...
    def _calculate_seasonal_vehicle_numbers(self, data):
        for season in self.seasons:
            self.seasonal_vehicle_numbers[season] = self._calculate_vehicle_numbers(data)
        return self.seasonal_vehicle_numbers

    def _create_structure_with_weekday(self, vehicle_numbers):
        immobile_vehicles = pd.DataFrame()
        for key, value in vehicle_numbers.items():
            immobile_vehicles_df_structure = pd.DataFrame(np.nan, index=range(value), columns=self.activities_raw.columns)
            immobile_vehicles_df_structure["trip_start_weekday"] = int(key)
            immobile_vehicles = immobile_vehicles.append(immobile_vehicles_df_structure).reset_index(drop=True)
        immobile_vehicles.loc[:, "weekday_string"] = immobile_vehicles.loc[:, "trip_start_weekday"].replace(
            self.dev_config["dataparsers"]["replacements"][self.dataset]["trip_start_weekday"]
        )
    ...
    def _create_unique_id(self):
        ...
        random_ids = np.random.choice(range(1000000, 10000000), size=pool_size, replace=False)
        ...
        self.immobile_vehicles['unique_id'] = filtered_ids[:len(self.immobile_vehicles)]
        return self.immobile_vehicles

    def _set_vehicle_segment(self):
        if self.user_config["dataparsers"]["subset_vehicle_segment"]:
            if self.dataset == "KiD" or self.dataset == "VF":
                self.immobile_vehicles['vehicle_segment_string'] = self.user_config["dataparsers"]["vehicle_segment"][self.dataset]
        return self.immobile_vehicles
    ...
    def _add_parking_activity_immobile_vehicles(self):
        if self.season == "all" and self.user_config["global"]["consider_temperature_cycle_dependency"]["annual"] is True:
            for season in self.seasons:
                immobile_season = self._create_structure_with_weekday(self.seasonal_vehicle_numbers[season]['immobile_vehicles'])
                immobile_season['season'] = season
                self.immobile_vehicles = pd.concat([self.immobile_vehicles, immobile_season])
        elif pd.Series(self.season).isin(self.seasons)[0]:
            self.immobile_vehicles = self._create_structure_with_weekday(self.vehicle_numbers['immobile_vehicles'])
        ...
        self.activities_raw = pd.concat([self.activities_raw, self.immobile_vehicles]).reset_index(drop=True)
        return self.activities_raw

    def _include_profiles_immobile_vehicles(self):
        if self.season == "all" and self.user_config["global"]["consider_temperature_cycle_dependency"]["annual"] is True:
            self.data['vehicles_number'] = self._calculate_seasonal_vehicle_numbers(self.activities_raw.groupby(by=["trip_start_weekday", "season"]))
        else:
            self.data['vehicles_number'] = self._calculate_vehicle_numbers(self.activities_raw.groupby(by="trip_start_weekday"))
