...
class GridModeller:
    def __init__(self, configs: dict, data):
        ...
        self.temperature_factors = self.user_config["global"]["consider_temperature_cycle_dependency"]["day_night_factors"]

    def __update_first_purpose_string(self):
        """
        Update the 'purpose_string' column in the DataFrame
        ...
        self.activities.loc[self.activities["park_id"] == min_park_id, "purpose_string"] = self.activities.loc[
    ...
    def __assign_grid_via_purposes(self):
        ...
        """
        logging.info("Starting with charge connection replacement of location purposes.")
    ...
    def __home_probability_distribution(self, set_seed: int) -> pd.DataFrame:
        ...
        households = households.drop_duplicates(subset="unique_id").copy()  # 73850 unique HH
        power = list((self.user_config["gridmodellers"]["grid_availability_distribution"][purpose]).keys())
        probability = list(self.user_config["gridmodellers"]["grid_availability_distribution"][purpose].values())
        urng = np.random.default_rng(set_seed)  # universal non-uniform random number
        rng = DiscreteAliasUrn(probability, random_state=urng)
    ...
    def __add_grid_losses(self):
        ...
        if self.user_config["gridmodellers"]["losses"] & self.user_config["global"]["consider_temperature_cycle_dependency"]["annual"] & (self.season == 'all'):
            if self.user_config["global"]["consider_temperature_cycle_dependency"]["daily"]:
                self.activities["available_power"] = (
                    self.activities["rated_power"] - (
                        ...
                        * self.activities.apply(
                            lambda row: self.user_config["gridmodellers"]["loss_factor"].get(
                                row['season'] + "_factor"
        ...
        elif self.user_config["gridmodellers"]["losses"] & (self.season in self.seasons):
            if self.user_config["global"]["consider_temperature_cycle_dependency"]["daily"]:
                self.activities["available_power"] = (
                    self.activities["rated_power"] - (
                        self.activities["rated_power"]
