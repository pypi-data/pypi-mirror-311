...
class Partition(BandStructure_builder, Config_builder, Crystal_builder, CrystalDefect_builder, Dataset_builder, 
    ...
    @staticmethod
    def _compute_lag_msd(pos: np.ndarray) -> (np.ndarray, np.ndarray):
        """
        Compute the Mean Squared Displacement (MSD) using lag time differences.
        
        Parameters:
        pos (numpy.ndarray): Array of shape (particles, time, coordinates) representing the positions of particles.
        
        Returns:
        Dt_r (numpy.ndarray): Array of time differences (lag times) for which MSD is calculated.
        MSD (numpy.ndarray): Array of MSD values for each particle over the lag times.
        """
        n_particles, n_time, n_dim = pos.shape
        Dt_r = np.arange(1, n_time)
        MSD = np.zeros((n_particles, len(Dt_r)))
        
        for i, Dt in enumerate(Dt_r):
            diffs = pos[:, Dt:, :] - pos[:, :-Dt, :]
            sqdist = np.sum(diffs**2, axis=-1)
            MSD[:, i] = np.mean(sqdist, axis=-1)
        
        return Dt_r, MSD

    @staticmethod
    def _compute_initial_msd(pos: np.ndarray) -> (np.ndarray, np.ndarray):
        """
        Compute the Mean Squared Displacement (MSD) relative to the initial position.
        ...
        MSD = np.mean(
            np.linalg.norm(pos - pos[:, 0, :][:, np.newaxis, :], ord=2, axis=-1)**2,
            axis=0
        )
    ...
    @staticmethod
    def _compute_fft_msd(pos: np.ndarray) -> (np.ndarray, np.ndarray):
        """
        ...
        S2 = np.sum(
            np.fft.ifft(np.abs(np.fft.fft(pos, n=2 * n_time, axis=-2))**2, axis=-2)[:, :n_time, :].real,
            axis=-1
        ) / (n_time - np.arange(n_time)[None, :])
        
        D = np.square(pos).sum(axis=-1)
        D = np.append(D, np.zeros((pos.shape[0], 1)), axis=-1)
        
        S1 = (2 * np.sum(D, axis=-1)[:, None] - np.cumsum(
            np.insert(D[:, :-1], 0, 0, axis=-1) + np.flip(D, axis=-1), axis=-1
        ))[:, :-1] / (n_time - np.arange(n_time)[None, :])
    ...
    def MSD(self, pos: np.ndarray, lag: bool=True, fft: bool=True) -> (np.ndarray, np.ndarray):
        """
        Calculate the Mean Squared Displacement (MSD) for a set of particle positions using different methods.
        
        Parameters:
        pos (numpy.ndarray): Array of shape (particles, time, coordinates) representing the positions of particles.
        lag (bool): If True, calculate MSD using lag time differences. Default is True.
        fft (bool): If True, calculate MSD using the Fast Fourier Transform (FFT) method. Default is True.
