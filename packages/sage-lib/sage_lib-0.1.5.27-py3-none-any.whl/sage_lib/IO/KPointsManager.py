...
class KPointsManager(FileManager, AtomicProperties):
    ...
    def band_path(self, path):
        """
        Setter for band path.
        Stores the path as segments of pairs.
        """
    ...
    def special_points(self, points_dict):
        ...
        Configures the special lattice points dictionary.
        """
    ...
    def set_band_path(self, special_points: dict, segments: list, divisions:int=30):
        """
        Set the band path using special lattice points and their connecting segments.

        Parameters:
            special_points (dict): Dictionary of special lattice points {label: [kx, ky, kz]}.
            segments (list): List of tuples representing segments between points [(label1, label2), ...].
        """
    ...
    '''    
    def set_band_path(self, band_path, divisions:int=10):
        self.comment = 'KPOINTS for band structure'
        self.num_kpoints = divisions
        self.mesh_type = 'Line Mode'
        self.coordinate_system = 'Reciprocal'
        self.paths = [ [list(self.special_lattice_points[pp])+[pp], list(self.special_lattice_points[pi])+[pi]] for pp, pi in zip(band_path[::2],band_path[1::2] )]
    '''
