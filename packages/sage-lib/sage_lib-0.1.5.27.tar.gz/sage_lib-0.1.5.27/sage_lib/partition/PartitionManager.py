try:
    from sage_lib.master.FileManager import *   
...
class PartitionManager(FileManager, AtomicProperties): 
    ...
    """
    def __init__(self, file_location: Optional[str] = None, name: Optional[str] = None, **kwargs):
        """
    ...
    def N(self):
        ...
        if self._containers is None:
            return 0
        elif isinstance(self._containers, list):
            return len(self._containers)
        elif isinstance(self._containers, np.ndarray):
    ...
    def uniqueAtomLabels(self):
        ...
        """
        if '_containers' not in self.__dict__:
            raise AttributeError("Attributes PartitionManager.containers must be initialized before accessing uniqueAtomLabels.")
        elif not isinstance(self._uniqueAtomLabels, list) and isinstance(self._containers, list):
            uniqueAtomLabels = set()
            for c in self._containers:
                # Ensure that each container has the attribute AtomPositionManager
            ...
            self.uniqueAtomLabels = list(uniqueAtomLabels)
            return self._uniqueAtomLabels
        else:
            return self._uniqueAtomLabels

    @property
    def uniqueAtomLabels_order(self):
        """
        ...
        """
        if '_containers' not in self.__dict__:
            raise AttributeError("Attributes PartitionManager.containers must be initialized before accessing uniqueAtomLabels_order.")
        elif not isinstance(self._uniqueAtomLabels_order, dict) and isinstance(self._containers, list):
            self._uniqueAtomLabels_order = {n: i for i, n in enumerate(self.uniqueAtomLabels)}
    ...
    @error_handler
    def read_traj(self, file_location:str=None, add_container:bool=True, container_index: Optional[int] = None, verbose=False, *args):
        """
        Reads a trajectory file and stores each frame along with its time information.
    ...
    def read_XYZ(self, 
        ...
        for i, line in enumerate(lines):
            if line.strip().isdigit():
                ...
                if num_atoms > 0:
                    # Create and populate SingleRun object
                    sr = SingleRun(file_location)
                    sr.AtomPositionManager = AtomPosition()
                    sr.AtomPositionManager.read_XYZ(
                        lines=lines[i:i + num_atoms + 2],
