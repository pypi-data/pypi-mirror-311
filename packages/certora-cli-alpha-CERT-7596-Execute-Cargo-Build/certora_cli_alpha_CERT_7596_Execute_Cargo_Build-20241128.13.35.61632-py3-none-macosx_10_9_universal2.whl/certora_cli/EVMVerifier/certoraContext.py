...
def get_local_run_cmd(context: CertoraContext) -> str:
    ...
    if Attrs.is_solana_app():
        if hasattr(context, 'solana_file') and context.solana_file:
            run_args.append(str(context.solana_file))
        else:
            run_args.append(context.files[0])
...
class ProverParser(AttrUtil.ContextAttributeParser):
    ...
    def format_help() -> str:
        ...
        # Using sys.stdout.write() as print() would color some strings here
...
def __alter_args_before_argparse(args_list: List[str]) -> None:
    ...
    for idx, arg in enumerate(args_list):
        if isinstance(arg, str):
            pattern = r"^[\"\']*-[^-]"  # a string that starts 0 or more quotes followed by a single hyphen
...
def build_solana(context: CertoraContext) -> None:
    if len(context.files) != 1:
        raise Util.CertoraUserInputError(f"expected a single Solana file ({Util.CARGO_FILE} or {Util.SOLANA_EXTENSIONS}"
                                         f" file). Got: {context.files}")
    file_path = Path(context.files[0])
    if file_path.name != Util.CARGO_FILE:
        return
    context.cargo_dir = file_path.parent
    context.cargo_file = file_path
    set_cargo_name(context)
    run_solana_compiler(context)


def run_solana_compiler(context: CertoraContext) -> None:
    try:
        context_logger.info("Executing build process...")
        env = os.environ.copy()

        external_build_path = Path(context.external_build) if Path(context.external_build).exists() \
            else context.cargo_dir.joinpath(Util.CARGO_EXTERNAL_BUILD)

        if external_build_path.exists():
            cmd = f'chmod +x {external_build_path.name}'
            Util.run_shell_command(cmd, env=env, cwd=context.cargo_dir)

            cmd = f'./{external_build_path.name}' if not str(external_build_path).startswith('/') \
                else str(external_build_path.name)
            Util.run_shell_command(cmd, env=env, cwd=context.cargo_dir)

        else:
            cmd = f"cargo +solana build-sbf"
            Util.run_shell_command(cmd, env=env, cwd=context.cargo_dir)

        context.solana_file = \
            Path(f"{context.cargo_dir}/target/{context.solana_target}/release/{context.cargo_name}.wasm")
        shutil.copy(context.solana_file, context.cargo_dir)

    except Exception as e:
        raise RuntimeError(f"Failed to build Solana file\n{e}")
