...
def compress_files_solana(zip_file_path: Path, context: CertoraContext, *resource_paths: Path) -> bool:
    with zipfile.ZipFile(zip_file_path, 'w', zipfile.ZIP_DEFLATED) as zip_obj:
        cargo_directory = context.cargo_dir
        for path in cargo_directory.rglob('*.rs'):
            if cargo_directory / 'target' in path.parents or cargo_directory / '.certora_internal' in path.parents:
        ...
        if context.solana_file.exists():
            zip_obj.write(context.solana_file, context.solana_file.relative_to(context.solana_file.parent))
...
class CloudVerification:
    ...
    def prepare_auth_data(self, cl_args: str) -> Optional[Dict[str, Any]]:
        ...
        if Attrs.is_solana_app():
            ...
            solana_jar_settings = []
            if hasattr(self.context, 'solana_file') and self.context.solana_file:
                solana_jar_settings.append(self.context.solana_file.name)  # assuming root is cargo_directory
    ...
    def __compress_and_upload_zip_files(self) -> bool:
        ...
        if self.context.is_tac:
            ...
            if hasattr(self.context, 'solana_file') and self.context.solana_file:
                paths.append(Util.get_certora_metadata_file())
                result = compress_files_solana(self.ZipFilePath, self.context, *paths)
            else:
                result = compress_files(self.ZipFilePath, *paths,
                                        short_output=Ctx.is_minimal_cli_output(self.context))
        elif Attrs.is_wasm_app():
