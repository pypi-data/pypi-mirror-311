...
def run_solana_compiler(context: CertoraContext) -> None:
    try:
        ...
        if external_build_path.exists():
            cmd = f'chmod +x {external_build_path}'
            Util.run_shell_command(cmd, env=env, cwd=context.cargo_dir)

            cmd = f'./{external_build_path}' if not str(external_build_path).startswith('/') \
                else str(external_build_path)
            Util.run_shell_command(cmd, env=env, cwd=context.cargo_dir)
        ...
        # Not sure if we need to copy this somewhere
        # context.wasm_file =
        # Path(f"{context.cargo_dir}/target/{context.wasm_target}/release/{context.cargo_name}.wasm")
        # shutil.copy(context.wasm_file, context.cargo_dir)
