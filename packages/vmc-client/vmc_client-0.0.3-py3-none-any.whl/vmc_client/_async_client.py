...
class AsyncAPIClient:
    ...
    def _raise_api_exception_from_text(self, text: str, e: Exception = None):
        ...
        if body["code"] == VMC_CODE.API_TIMEOUT:
            raise APITimeoutError(context=body) from e
        if body["code"] == VMC_CODE.API_CONNECTION_ERROR:
            raise APIConnectionError(context=body) from e
        if body["code"] != VMC_CODE.SUCCESS:
            raise VMCException(vmc_code=body["code"], msg=body.get("msg", "")) from None
    ...
    def _should_retry(self, response: httpx.Response) -> bool:
        ...
        if "code" not in json:
            return False
        if json["code"] in (
            VMC_CODE.API_TIMEOUT,
            VMC_CODE.API_CONNECTION_ERROR,
            VMC_CODE.API_RATE_LIMIT,
        ):
            return True
        return False
    ...
    async def _request(
        ...
        try:
            response = await self._client.send(request, stream=options.stream)
