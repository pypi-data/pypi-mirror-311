...
class EntityItem(QtWidgets.QTreeWidgetItem):
    ...
    def __init__(self, tree, data=None, display_name=None, editable=False):
        ...
        self.display_name = data[1]['Name'] if data else display_name
        self.name = re.search('[^\/]+$', data[0]).group(0) if data else re.sub('[\s.-]', '_', display_name)
        self.oid = data[0] if data else None
        self.icon = data[1]['_style']['icon'] if data else None
        ...
        if editable:
            ...
            self.validator = QtGui.QRegExpValidator(QtCore.QRegExp(rx))
            ...
            try:
                ...
                if len(items) == 0 and list_index == 2:
                    if self.header_text in ('Asset Families', 'Assets'):
                        ...
                        items = self.page_widget.session.cmds.Flow.get_mapped_rows(map_oid)
                        if items:
                ...
                for item_data in items:
                    ...
                    if self.header_text == 'Films' or self.page_widget.is_source_task_mode():
                        if self.default_value and self.default_value == item_data[1]['Name']:
                            self.setCurrentItem(item)
                    elif self.default_value and self.default_value in item_data[1]['Name']:
                        self.setCurrentItem(item)
            except KeyError:
                None
