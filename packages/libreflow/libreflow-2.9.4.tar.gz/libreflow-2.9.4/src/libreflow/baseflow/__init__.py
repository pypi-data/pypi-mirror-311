...
from .kitsu import KitsuAPIWrapper, KitsuBindings, KitsuUsers, KitsuTasks, KitsuTaskAvailableStatutes
...
class LoginPageWidget(CustomPageWidget):
    def build(self):
        ...
        self.kitsu_url = self.session.cmds.Flow.get_value(f'{self.project_oid}/admin/kitsu/server_url')
    ...
    def on_connection_validated(self):
        ...
        if not logged_in:
            text = "<font color=#fba071><b>Login failed... Check credentials !</font></b>"
        ...
        wizard_default_page = self.session.cmds.Flow.call("/Home/Wizard", 'get_default_page', {}, {})
        if wizard_default_page is not None:
            if wizard_default_page == "Users":
                self.page.show_action_dialog(f"{self.project_oid}/admin/users/create_users")
            elif wizard_default_page == "Tasks":
                self.page.show_action_dialog(f"{self.project_oid}/admin/project_settings/task_manager/default_tasks/add_kitsu_tasks")
        ...
        if child_value is self.project_id:
            ...
            if self.gazu_api.host_is_valid():
                self.project_type.set(
                    self.gazu_api.get_project_type()
                )
...
class ProjectSettings(flow.Object):
    ...
    def __init__(self, parent, name):
        ...
        base_action = self.root().get_object(f'{self.root().project().oid()}/import_files')
        
        if isinstance(base_action, ImportFilesAction):
    ...
    def compute_child_value(self, child_value):
        if child_value is self.root_dir:
            ...
            if "ROOT_DIR" in os.environ:
                self.root().session().log_warning("ROOT_DIR was defined by the environement !")
