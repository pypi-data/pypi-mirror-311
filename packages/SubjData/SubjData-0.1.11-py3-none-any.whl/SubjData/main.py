...
from SubjData.download import download_datasets
from SubjData.process import process_datasets
...
def create_target_dataset(target, hf_token=None):
    
    with open('SubjData/resources/overview_dict.json', 'r') as f:
        dict_overview = json.load(f)
    
    relevant_datasets = [dataset[0] for dataset in dict_overview[target]]
...
def create_category_dataset(category, hf_token=None):
    
    with open('SubjData/resources/overview_dict.json', 'r') as f:
        dict_overview = json.load(f)
        
    with open('SubjData/resources/category_dict.json', 'r') as f:
        category_dict = json.load(f)
    
    targets = category_dict[category]
    relevant_datasets = list(set([dataset[0] for target in targets for dataset in dict_overview[target]]))
    dict_of_datasets = download_datasets(relevant_datasets, hf_token)
    dict_of_processed_datasets = process_datasets(dict_of_datasets)
    target_dataset = pd.DataFrame()
    for df in dict_of_processed_datasets.values():
...
def get_target_info(target):
    ...
    with importlib.resources.open_text('SubjData.resources', 'overview_dict.json') as file:
        dict_overview = json.load(file) 
        
    with importlib.resources.open_text('SubjData.resources', 'instruction_dict.json') as file:
        dict_overview = json.load(file) 
    
#     with open('SubjData/resources/overview_dict.json', 'r') as f:
#         dict_overview = json.load(f)
        
#     with open('SubjData/resources/instruction_dict.json', 'r') as f:
#         instruction_dict = json.load(f)
    
    relevant_datasets = dict_overview[target]
...
def get_category_info(category):
    ...
    relevant_datasets = [dataset for target in category_dict[category] for dataset in dict_overview[target]]
    unique_datasets = list(set([dataset[0] for dataset in relevant_datasets]))
    targets = category_dict[category]
    
    targets_sums = {target: None for target in targets}
    for target in targets:
        targets_sums[target] = np.sum([dataset[1] for dataset in dict_overview[target]])
