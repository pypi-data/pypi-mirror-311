...
if not hx_config.yinying_appid or not hx_config.yinying_token:
    ...
    scheduler.add_job(func=check_update,trigger='interval',hours=12,id="huanxin996")
...
msg_ml = on_command("yinying_chat", aliases={"c_call"},rule=Rule(chek_rule_base),  priority=15, block=True)
...
async def at(matcher: Matcher, event: MessageEvent, bot: Bot):
    ...
    try:
        msg = str(await get_answer_at(event, bot))
        await send_msg(matcher=matcher,event=event,content=msg)
    except Exception as e:
        if groupid:
...
async def ml(matcher: Matcher, event: MessageEvent, bot: Bot, msg: Message = CommandArg()):
    groupid = get_groupid(event)
    try:
        mag = str(await get_answer_ml(event,msg,msg))
        await send_msg(matcher=matcher,event=event,content=msg)
    except Exception as e:
        if groupid:
            img = await error_oops()
...
async def set_global(matcher: Matcher, bot:Bot, event: MessageEvent,events: Event, s: T_State):
    ...
    if s["last"]:
        ...
        if s["last"] == "修改TF":
            config = config_in_global()
            config_name = s["set"]
            get_config = await json_get(config,config_name,None)
