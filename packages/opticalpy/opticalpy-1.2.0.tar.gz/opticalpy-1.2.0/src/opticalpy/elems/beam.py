...
def WhiteRay(wavelengths:list[u.Quantity|float]|np.ndarray,
    ...
    white = optgroup.OpticalGroup(scene=scene, group=group, origin=origin, rotation=rotation, label=label)
    for i, wave in enumerate(wavelengths):
        r = ray.Ray(wavelength=wave, group=white, origin=[0,0], rotation=0, label=f"{label} {i}")
    return white
