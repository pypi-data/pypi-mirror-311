...
class YieldCurve:
    ...
    def __init__(self, list_tenor:list[str], list_rate:list[float], date_curve:datetime=None, date_convention:VALID_CONVENTIONS="ACT/360",date_format="%Y-%m-%d"):
        ...
        df = pd.DataFrame({
            "TENOR": ["0D"] + list_tenor,  # Add 0D tenor at start
            "STRIPPEDRATES": [np.nan] + list_rate  # Add NaN rate for 0D
        })
        ...
        interpolated_df = pd.concat([zero_day, interpolated_df], ignore_index=True)
