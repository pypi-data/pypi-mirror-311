...
class FloatLeg:
    ...
    def __init__(
        ...
        ESTR_coumpounded:compounded.Compounded=None,
        relative_delta=None,
    ):
        ...
        self.ESTR_coumpounded = ESTR_coumpounded
    ...
    def compute_cash_flow(self, date_value:datetime):
        ...
        for i in range(0, len(self.cashflow.end_date), 1):
            if (
                ...
                self.cashflow.day_count[self.cashflow.index[i]] = core.day_count(
                    pd.Timestamp(date_value),
                    list(self.cashflow.end_date)[i],
        ...
        self.cashflow["forward_simple_rate"] = self.cashflow.apply(
            lambda x: financial.zc_to_simplerate(x["forward_zc"], x["day_count"]), axis=1
        )
        ...
        self.accrued_coupon_float = self.accrued_coupon(
    ...
    def accrued_coupon(self,
        ...
        relative_delta=None,
            ...
            ESTR=self.ESTR_coumpounded.df
            date_min = min(ESTR["DATES"])
            date_max = max(ESTR["DATES"])
            SDate = financial.previous_coupon_date(self.cashflow["start_date"], self.cashflow["end_date"], pd.Timestamp(valuation_date))
            ...
            if SDate < date_min or SDate > date_max:
                FRate = self.yield_curve_object.forward_rates(
                    financial.previous_coupon_date(self.cashflow["start_date"], self.cashflow["end_date"], pd.Timestamp(valuation_date)),
