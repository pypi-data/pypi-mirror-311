...
class EconomicIndexDataReader(_DataReader):
    ...
    def get_house_info_df(self,start_date,end_date):
        ...
        approved_house_df = approved_house_df[
            (approved_house_df['ITEM_CODE1'].str.startswith('I43AB')) 
            & (approved_house_df['ITEM_CODE2'] == '1')
            ]
        approved_house_df = approved_house_df.pivot(index='TIME', columns='ITEM_NAME1', values='DATA_VALUE').loc[:,['용도별','주거용','상업용']]
        ...
        building_house_df = building_house_df[
            (building_house_df['ITEM_CODE2'].str.startswith('I47AB')) 
            & (building_house_df['ITEM_CODE1'] == '1')
            ]
        building_house_df = building_house_df.pivot(index='TIME', columns='ITEM_NAME2', values='DATA_VALUE').loc[:,['용도별','주거용','상업용']]
