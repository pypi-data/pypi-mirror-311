    ...
    def _validate_output_read_line(
        ...
        if section in ("Commands",):
            ...
            if commands_index < len(commands) and first_word_of_line == commands[commands_index]:
                ...
                self.assertGreater(
                    len(line.replace(first_word_of_line, "", 1).lstrip()),
        ...
        self.fail(f"Section name {section} is not supported")
    ...
    def validate_output(
        ...
        try:
            ...
            self.assertNotEqual(
                "Start",
                section,
                msg=f"Command {help_command}: incomplete help text: {returned_string}",
            )
            ...
            if not err:
                self.fail(
                    msg=(
                        f"ERROR: Command {help_command} returned error at execution. Output:"
                        f" {returned_string} Error: {str(exception)}"
                    )
                )
            else:
                self.fail(
