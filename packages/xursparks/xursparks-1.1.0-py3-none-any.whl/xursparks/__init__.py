...
def generateTablePaths(dataStorage, options):
	...
	for date in pd.date_range(edate, sdate):
		generatedTablePaths.append(generateTablePath(dataStorage) \
			+ date.strftime(dataStorage["partition_date_format"]) + "/")
...
def readTableInSearchMode(dataStorage, schema, options, dataFrame):
	...
	for path in generateTablePaths(dataStorage, options):
		if "xlsx" in dataStorage.get('format'):
			path = path + options.get('filename')
...
def writeToTargetTable(dataStorage, dataset, writeOptions, jdbcWriteOptions, processDate):
	...
	if(dataStorage.get("format").upper() == "JDBC"):
		...
		options = {**jdbcOptions, **jdbcWriteOptions}
		...
		path = generateTablePath(dataStorage) \
				+ writeDate.strftime(dataStorage.get("partition_date_format")) \
				+ "/"		
		...
		lower_case_dict = {k.lower(): v for k, v in writeOptions.items()}
		if("filename" in lower_case_dict):
			path = path + writeOptions.get("filename")
...
def writeToTargetTableJdbc(targetTable):
	print("WRITING TO JDBC")
	options = getDictionarFromProperties("jdbc_options")

	try:
		targetTable.write.options(**options) \
			.format(getJdbcOptionProperties("format")) \
			.mode(getJdbcOptionProperties("mode")) \
			.save()
	except Exception as e:
		raise e
...
def registerPartition(**kwargs):
	...
	partition_column = targetTable.get("partition_name").lower()
	partition_value = processDate.strftime(targetTable.get("partition_date_format"))
	partition_location = generateWritePath()
...
def generateWritePath():
	path = f'{targetTable["path"]}{targetTable["partition_name"].lower()}={writeDate.strftime(targetTable["partition_date_format"])}/'
	return path
