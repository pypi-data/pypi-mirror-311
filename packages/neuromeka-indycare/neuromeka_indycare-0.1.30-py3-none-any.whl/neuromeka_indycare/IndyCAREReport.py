...
TIMER_RECONNECT_TO_SISH = 10  # (seconds)
...
class Reporter(ProcessTemplate, ABC):
    ...
    def __init__(self, _zip_log_event, _close_recorder_process, _msg_queue):
        ...
        self._sish_output = Queue()
        self._sish_process = None
        self._sish_conty_addr = ''
    ...
    def __close_sish_process(self):
        if self._sish_process and self._sish_process.is_alive():
            self._sish_process.terminate()
            self._sish_process = None
            self._sish_conty_addr = ''
            # self._sish_process.join()
            
    def __connect_to_sish(self):
        try: 
            self.__close_sish_process()
            ...
            args = [
                'ssh', '-p', f'{self._config["sish_port"]}', 
                '-R', f'1000:localhost:{conty_port}',
                f'{self._config["sish_user"]}@{self._config["sish_server"]}'
            ]

            _clear_queue(self._sish_output)

            # Start the ssh_worker process
            self._sish_process = Process(target=self.__ssh_worker, 
                                            args=(self._config["sish_pass"], 
                                            args, 
                                            self._sish_output))
            self._sish_process.start()
            
            ansi_escape = re.compile(r'\x1b\[.*?[@-~]')
            
            self._sish_conty_addr = ''
            while self._sish_conty_addr == '':
                try:
                    output = self._sish_output.get(timeout=1)
                    # print(output, end='')
                    ...
                    for line in lines:
                        ...
                        if 'TCP:' in clean_line or 'TCP Alias:' in clean_line:
                            ...
                            self._sish_conty_addr = f'{self._config["sish_server"]}:{address_port}' 
                            self.Reporter_log.info("CONNECT TO SISH server success...")
                            return
                        
                        elif 'Connection refused' in clean_line or 'Connection timed out' in clean_line:
                            self.Reporter_log.info("CONNECT TO SISH server failed: Connection refused")
                            self.__close_sish_process()
                            return
                
                except queues.Empty:
                    if not self._sish_process.is_alive() and self._sish_conty_addr is not None:
                        break
            ...
            self.Reporter_log.error("SISH connection fail", str(e))

    def __check_sish_connection(self):
        ansi_escape = re.compile(r'\x1b\[.*?[@-~]')
        result = True
        try:
            output = self._sish_output.get(timeout=1)
            # print('__check_sish_connection: ', output)
            lines = output.splitlines ()
            for line in lines:
                clean_line = ansi_escape.sub('', line).strip()
    ...
    def main_process(self, interval):
        """ Check sish connection is valid """
        # self.__connect_to_sish()
        ...
        Daliworks_urls = []
        pre_violationType = -1
        pre_sish_address = self._sish_conty_addr
        ...
        while not self._stop_requested:
            try:
                ...
                if (time.time() - pre_timeSishConnection) >= TIMER_RECONNECT_TO_SISH and Daliworks_urls: 
                    reconnect_sish = False
                    if self._sish_process and self._sish_process.is_alive and self._sish_conty_addr != '':
                        result = self.__check_sish_connection()
                        if result:
                            pass
                        else:
                            reconnect_sish = True
                    ...
                    if reconnect_sish:
                        self.Reporter_log.error("SISH is not running...")
                        self.__connect_to_sish()
                    
                    pre_timeSishConnection = time.time()
                
                # conty_remote_connected = ""
                
                if (time.time() - pre_timeTransmitPeriod) >= self._config["mqtt_transmit_period"] \
                    or pre_sish_address != self._sish_conty_addr:
                    ...
                    try:
                        ...
                        if self.FW_version == "v3":
                            data = {
                                ...
                                "ngrokIp": self._sish_conty_addr, # this is old key,but Daliworks use this key
                                "remoteContyIp": self._sish_conty_addr, # new key
                            ...
                            data = {
                                ...
                                "tempMotor": ",".join(map(str, temperatures_np)),
                                "ngrokIp": self._sish_conty_addr,
                                "remoteContyIp": self._sish_conty_addr,
                                # "ngrokIsContyConnected": conty_remote_connected,
                                "fileURL": Daliworks_urls[0] if Daliworks_urls else "",
                                "terminalURL": Daliworks_urls[1] if Daliworks_urls else "",
                                "camURL": Daliworks_urls[2] if Daliworks_urls else ""
