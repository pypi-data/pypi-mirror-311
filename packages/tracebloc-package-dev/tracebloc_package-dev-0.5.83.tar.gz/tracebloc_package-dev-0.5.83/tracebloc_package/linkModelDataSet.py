...
class LinkModelDataSet:
    ...
    def __not_supported_parameters(self, parameter, framework='pytorch', message=False):
        if message:
            ...
            print_statement = f"The parameter {parameter} is not supported on {framework}\n"
        text = colored(
            print_statement,
            "red",
        )
    ...
    def rotation_range(self, rotation_range: int):
        ...
        if self.__framework != SKLEARN_FRAMEWORK:
            ...
            self.__not_supported_parameters("rotation_range", framework=SKLEARN_FRAMEWORK)
            self.__remove_error_method()
    ...
    def channel_shift_range(self, channel_shift_range: float):
        ...
        if self.__framework == PYTORCH_FRAMEWORK and self.__image_type != "rgb":
            ...
            self.__not_supported_parameters("channel_shift_range", framework=SKLEARN_FRAMEWORK)
            self.__remove_error_method()
        else:
            error_msg = (
    ...
    def horizontal_flip(self, horizontal_flip: bool):
        ...
        if self.__framework == TENSORFLOW_FRAMEWORK:
            ...
            self.__not_supported_parameters("horizontal_flip", framework=self.__framework)
            self.__remove_error_method()
    ...
    def feature_interaction(self, dict):
        ...
        default: None
        """
        feature_list = ['BP_CurrentCurve', 'BP_VoltageCurve', 'BP_ReferenceCurveLength', 'BP_PSF', 'BP_UIP', 'BP_Energy',
                        'BP_gunResistance', 'BP_MaxResistance', 'BP_ResistanceDrop', 'BP_MeanVoltage', 'BP_MeanPulse',
                        'BP_MeanCurrent', 'BP_meanCurrentSec', 'BP_meanCurrentPrim', 'BP_MeanResistance', 'BP_Length',
                        'BP_RmaxTime', 'BP_ProtocolLogNo', 'BP_spatterTime', 'BP_MonitorStdActualValue',
                        'BP_MonitorStdReferenceValue', 'WP_WeldJobName', 'WELD_ID', 'WP_STCReference', 'WP_GunResistance',
                        'VEHICLE_ID', 'WP_WeldTime', 'WP_TimeMillisecond', 'WP_TimerName', 'WP_TipWearState',
                        'WP_UnixEpoch', 'WP_TipDressCount', 'WP_TipWearCount', 'WP_DiDt', 'WP_Expulsion', 'WP_Energy',
                        'WP_Comment', 'measurement_id', 'inspection_id', 'time_stamp_x', 'nugget1', 'nugget2',
                        'indentation1', 'indentation2', 'frontplate1', 'frontplate2', 'stack1', 'stack2', 'pass_fg',
                        'reason', 'cscans', 'meta_x', 'time_stamp_y', 'inspector_id', 'inspection_mode', 'meta_y',
                        'update_status', 'min_size', 'setup_size', 'min_ind', 'max_ind', 'stack_3T', 'safety_fg',
                        'robot_id', 'robot_desc', 'schedule', 'cscans_integers', 'cscan1', 'cscan2', 'resistance']
        method_list = ['product', 'ratio', 'sum', 'polynomial', 'outlier simulation', 'shuffle', 'pca']
        if dict['feature1'] not in feature_list:
            ...
            error_msg = "feature_interaction: Invalid feature name provided\n"
            self.__print_error(error_msg)
        elif dict['method'] in method_list:
            error_msg = "feature_interaction: Invalid method name provided\n"
            self.__print_error(error_msg)
        else:
            self.__feature_interaction.append({'feature1': dict['feature1'],
    ...
    def __getParameters(self):
        parameters = {
            ...
            "feature_interaction": self.__feature_interaction,
        }
