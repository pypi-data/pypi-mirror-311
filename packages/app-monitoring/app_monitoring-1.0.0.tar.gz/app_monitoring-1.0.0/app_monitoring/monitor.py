...
STEPS = OrderedDict({
    ...
    "Initializing Outputs": {"Success": ["Outputs initialized successfully"], "Failed": ["Failed to initialize Outputs"]},
...
def display_status():
    ...
    # Print steps with "Success" or "Failed" first
    for step, status in STATUS.items():
        if status in ("[Success]", "[Failed]"):
            print(f"{step:<60} {status}")

    # Print steps still in progress
    for step, status in STATUS.items():
        if status in ["[\\]", "[|]", "[/]", "[-]"]:
            print(f"{step:<60} {status}")
