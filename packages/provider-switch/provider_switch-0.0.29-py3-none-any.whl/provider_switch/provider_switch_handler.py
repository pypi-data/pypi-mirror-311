...
class ProviderSwitchHandler(object):
    ...
    def _request(
        self, method: str, url: str, headers: dict, params: dict, timeout: int = None
    ) -> dict:  # noqa: C901
        ...
        validated = self.current_provider.check_response(body)
        ...
        return ResponseHelper(self.current_provider).standardize_response(body)
    ...
    def _multiple_requests(
        ...
        for i in range(self.num_of_pages):
            response.append(
                self._request(method=method, url=url, headers=headers, params=params)
            )
        ...
        try:
            ...
            if self.is_pagination is True:
                return self._multiple_requests(
                    method=method, url=url, headers=req_headers, params=params
                )
