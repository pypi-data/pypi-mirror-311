...
from .utilx import is_list_or_tuple, get_combinations, \
    name_str, is_generator, check_bool_var, is_bool_var
...
class CpModelSolver(ICpModel):
    ...
    def addVar(self, lb: int = 0, ub: int = INFINITY,
        ...
        if vtype == Vtype.CONTINUOUS or vtype == Vtype.INTEGER:
            var1: CpVar = CpVar(tempModel, Domain(lb, ub), name, self)
        elif vtype == Vtype.BINARY:
            var1: CpVar = CpVar(tempModel, Domain(lb, ub), name, self)
        self.__all_vars.append(var1)
        return var1

    def addVars(self, *indices, lb: int = 0, ub: int = INFINITY, vtype: Vtype = Vtype.CONTINUOUS,
                name: str = "") -> tupledict:
        li = []
        for ind in indices:
    ...
    def addGenConstrMin(self, target: IVar, exprs: List[IVar]):
        self.__model.add_min_equality(target, exprs)  # type: ignore
    ...
    def addConstrOr(self, constrs: List[cp_model.BoundedLinearExpression], ok_num: int = 1, cmpType: CmpType = CmpType.EQUAL, name: str = "", M=None):
        """
        约束的满足情况
