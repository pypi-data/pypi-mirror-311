...
class Model:
    ...
    def __init__(self, solver_id: Literal["SCIP", "CBC", "SAT", "CLP", "CPLEX", "GLOP", "BOP", "GLPK"] = "SCIP", name=""):
        super().__init__()
    ...
    def statistics(self):
        ...
        """
        print("统计信息：\n========")
        print(f"约束数量：{self.numConstraints()}")
        print(f"变量数量：{self.numVars()}")
        print(
            f"整数变量数量：{self.numVars(Vtype.INTEGER)+self.numVars(Vtype.BINARY)}")
        print("========")
