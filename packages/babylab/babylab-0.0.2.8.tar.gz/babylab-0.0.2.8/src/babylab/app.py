...
from src.babylab import api, utils

app = Flask(__name__, template_folder="templates")
app.secret_key = os.urandom(24)
app.permanent_session_lifetime = datetime.timedelta(minutes=10)

creds = api.redcap_login()
app.config["API_URL"] = creds["API_URL"]
app.config["API_KEY"] = creds["API_KEY"]
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        try:
            ...
            return redirect(url_for("index", login_status="incorrect"))
...
@app.route("/")
@app.route("/index")
def index(login_status: str = None):
    """Index page"""
    if login_status == "missing":
        flash("Credentials file is missing", "error")
    if login_status == "illformed":
        flash("Credentials file is ill-formed", "error")
    if login_status == "incorrect":
...
def dashboard(records: api.Records = None):
    ...
    ppts = utils.get_participants_table(records)
    apts = utils.get_appointments_table(records)
...
def participants(records: api.Records = None):
    ...
    if records is None:
        try:
            ...
            return render_template("index.html", login_status="incorrect")
    df = utils.get_participants_table(records)
...
def record_id(records: api.Records = None, ppt_id: str = None):
    ...
    appts = utils.get_appointments_table(records.participants.records[ppt_id])
...
def participant_modify(ppt_id: str, records: api.Records = None, data: dict = None):
    ...
    if records is None:
        data = api.Records().participants.records[ppt_id].data
        dicts = api.get_data_dict()
        for k, v in data.items():
            dict_key = "participant_" + k
            if dict_key in dicts and v:
...
def appointments(records: api.Records = None):
    ...
    if records is None:
        try:
            ...
            return render_template("index.html", login_status="incorrect")
    df = utils.get_appointments_table(records)
    classes = "table table-striped table-hover table-sm table-condensed"
    df["appointment_id"] = [
...
def appointment_modify(
    ...
    if records is None:
        data = api.Records().appointments.records[appt_id].data
        dicts = api.get_data_dict()
        for k, v in data.items():
            dict_key = "appointment_" + k
            if dict_key in dicts and v:
