...
from src.babylab import api, calendar


def get_participants_table(records: api.Records) -> DataFrame:
...
def get_appointments_table(records: api.Records) -> DataFrame:
    ...
    if records.appointments.records:
        df = records.appointments.to_df().sort_values("date")
        dicts = api.get_data_dict()
        for col_name, col_values in df.items():
