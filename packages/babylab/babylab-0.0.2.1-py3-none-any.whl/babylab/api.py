...
def redcap_login(file: str = ".env") -> None:
    ...
    """
    try:
        if not load_dotenv(dotenv_path=file):
            raise CredentialsMissingException
        creds = dotenv_values()
        if not all(["API_URL" in creds.keys(), "API_KEY" in creds.keys()]):
            raise CredentialsKeysException
        r = requests.post(
            creds["API_URL"],
            data=OrderedDict(
                {
        ...
        CredentialsAuthException,
        CredentialsKeysException,
        CredentialsMissingException,
    ):
        creds = OrderedDict({"API_URL": "", "API_KEY": ""})
    return creds
...
def post_request(
    ...
    **kwargs,
    ...
    """
    creds = redcap_login(file)
    fields = OrderedDict(fields)
    fields["token"] = creds["API_KEY"] if "API_KEY" in creds.keys() else ""
    fields.move_to_end("token", last=False)
    try:
        r = requests.post(creds["API_URL"], data=fields, timeout=timeout, **kwargs)
        r.raise_for_status()
...
def get_redcap_version(**kwargs) -> str:
    ...
    return post_request(fields=fields, **kwargs).content.decode("utf-8")
