...
class Competition:
    def __init__(self, username, course_name, course_round, competition_name):
    ...
    def __document_classification(self):
        ...
        # 예측을 클래스 레이블로 변환 (반올림을 통해 정수로 변환)
        predicted_labels = torch.argmax(prediction, dim=1) # 또는 적절한 변환 방법 사용
        
        # 정확도 계산
        correct = (predicted_labels == answer).sum().item()  # 정답과 일치하는 수
