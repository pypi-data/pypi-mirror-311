...
except MafftNotFoundError as e:
    print(e)
    ...
    for i in tqdm(os.listdir(save_dir), desc="Extracting and Aligning sequences..."):
        ...
        for name in all_info.keys():
            if name != ref_file:
                ...
                output_file_path = split_abspath_pro.split('.')[0] + '_align.fasta'
                mafft_cmd = "{} --auto {} > {}".format(mafft_path, input_file_path, output_file_path)

                # 调用mafft进行比对
                subprocess.run(mafft_cmd, shell=True, stdout=False, stderr=subprocess.DEVNULL)
