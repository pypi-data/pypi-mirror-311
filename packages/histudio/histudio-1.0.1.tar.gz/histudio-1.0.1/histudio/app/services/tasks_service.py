...
from histudio.app.models.entity import Entity
...
from histudio.app.utils import cache, fields, query as query_utils
...
def reset_task_data(task_id):
    ...
    for (
        ...
        if task_status_short_name.lower() == "wip":
            ...
            wip_date = comment.created_at

        if task_status_short_name.lower() == "retake":
            retake_date = comment.created_at

        if task_status_short_name.lower() == "awf":
            awf_date = comment.created_at

        if task_status_short_name.lower() == "app":
            app_date = comment.created_at
