...
from histudio.app.models.attachment_file import AttachmentFile
from histudio.app.models.comment import Comment
from histudio.app.models.department import Department
from histudio.app.models.notification import Notification
from histudio.app.models.project import Project
from histudio.app.models.task import Task

from histudio.app.services import (
    assets_service,
    base_service,
    breakdown_service,
    entities_service,
...
from histudio.app.services.exception import (
    AttachmentFileNotFoundException,
    WrongParameterException,
    AssetNotFoundException,
)

from histudio.app.utils import cache, date_helpers, events, fs, fields
from histudio.app.stores import file_store
from histudio.app import config
...
def create_comment(
    person_id, task_id, task_status_id, text, checklist, files, created_at, wipCount, currentRoute, previousTaskId, wip_id
):
    """
    ...
    if currentRoute == "Assets":
        if wipCount >= "1":
            _manage_previous_status_change(previousTaskId, created_at, files, checklist, author, wip_id)
...
def _manage_previous_status_change(previousTaskId, created_at, files, checklist, author, wip_id):
    task = tasks_service.get_task_with_relations(previousTaskId)
    task_status = tasks_service.get_task_status(wip_id)
    text = "This Task has Been Automatically Assigned To You"
...
def _manage_status_change(task_status, task, comment):
    ...
    if status_changed:
        ...
        if task_status["short_name"] == "wip":
            # new_data['count'] = 1
            generate_auto_mandays_count(new_data, task, count='start')
        #
        if task_status["short_name"] != "wip":
            generate_auto_mandays_count(new_data, task, count='stop')
            # new_data['count'] = 0
    ...
    if status_changed:
        if (task_status["short_name"] == "wip"
            ...
            new_data["wip_date"] = datetime.datetime.utcnow()

    if status_changed:
        if (task_status["short_name"] == "retake"
        ):
            new_data["retake_date"] = datetime.datetime.utcnow()

    if status_changed:
        if (task_status["short_name"] == "awf"
        ):
            new_data["awf_date"] = datetime.datetime.utcnow()

    if status_changed:
        if (task_status["short_name"] == "app"
        ):
            new_data["app_date"] = datetime.datetime.utcnow()
...
def generate_auto_mandays_count(new_data, task, count):
    if count == "start":
        new_data['wip_date'] = dt.utcnow()
        new_data["count"] = 1

    if count == "stop":
        new_data["count"] = 0
        current_date = dt.utcnow()
        mandays_count = task.get('mandays_count', 0)  # Use get with a default value

        # Get wip_date from task
        wipdate = task.get('wip_date', None)

        # Check if wipdate is None or not a string
        if wipdate is None or not isinstance(wipdate, str):
            wipdate = "1970-01-01T00:00:00"  # Default date if wipdate is invalid or None
        
        try:
            date1 = dt.strptime(wipdate, "%Y-%m-%dT%H:%M:%S")
        except (ValueError, TypeError):
            print("Invalid wipdate format, using default date")
            date1 = dt.strptime("1970-01-01T00:00:00", "%Y-%m-%dT%H:%M:%S")

        date2 = current_date
        time_difference = date2.date() - date1.date()
        
        if mandays_count is None or mandays_count == "NoneType":
            mandays_count = 0
        
        new_data['mandays_count'] = int(time_difference.days + mandays_count)

        print(time_difference.days, "##########################", date2,  "[[[[", time_difference.days)
        print(current_date, wipdate, "OOOKAAAYY")
        print(date1.date(), "current_date")
        print(date2.date(), "wipdate")
