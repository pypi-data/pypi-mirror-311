...
from histudio.app.utils import thumbnail as thumbnail_utils
from histudio.app.stores import auth_tokens_store, file_store
...
def dump_database(store=False):
    ...
    backup_path = "/home/hi/VFX_DATABASE_BACKUPS"

    # Create a filename with the current date and time
    now = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S")
    filename = f"histudio-db-backup-{now}.sql.gz"

    if store:
        print("this 1st print---------------1111111111111111111111111")
        # Use temporary directory path
        filename = os.path.join(tempfile.gettempdir(), filename)
    else:
        # Store the file in the backup_path
    ...
    if store:
        # Store the backup and remove the temporary file
        backup_service.store_db_backup(os.path.basename(filename), filename)
        os.remove(filename)
        print(f"Postgres dump added to store (dbbackup/{os.path.basename(filename)}).")
    else:
        print("this 2nd print---------------2222222222222222222222222222222222222222222222222")
        # Use backup_path instead of realpath to show the exact location
        print(f"Postgres dump created ({os.path.join(backup_path, os.path.basename(filename))}).")
