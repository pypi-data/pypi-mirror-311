...
from histudio.app.mixin import ArgsMixin
from histudio.app.utils import permissions
...
class CommentTaskResource(Resource):
    ...
    def post(self, task_id):
        ...
        wipid = TaskStatus.query.filter(TaskStatus.short_name.ilike("hold")).first()
        wip_id = wipid.id
    ...
    def get_arguments(self):
        ...
        parser.add_argument("wipCount", default="", location=location)
        parser.add_argument("previousTaskId", default="", location=location)
        parser.add_argument("currentRoute", default="", location=location)
    ...
    def get_allowed_comments_only(self, comments, person):
        ...
        for comment in comments:
            try:
                ...
                ) or person["id"] in task["assignees"] or person["role"] == "user":
                    print("WORKINGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG")
                    # Allow comment if the person is assigned to the task or the role is 'user'
                    allowed_comments.append(comment)
            except permissions.PermissionDenied:
                pass
            except KeyError:
