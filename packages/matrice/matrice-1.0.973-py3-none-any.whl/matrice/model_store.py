...
class ModelArch:
    ...
    def __init__(
        ...
        if not error:
            self.model_info_id = model_info.get("_id", self.model_info_id)
            ...
            self.model_family_name = self.model_family.model_family_name
    ...
    def _get_model_info(self):
        ...
        Example
        -------
        >>> model_info, error, message = model_arch._get_model_info()
        >>> if error:
        >>>     print(f"Error: {error}")
        >>> else:
    ...
    def get_export_formats(self):
        ...
        if self.model_info_id is not None:
            path = (
                f"/v1/model_store/get_export_formats?modelInfoId={self.model_info_id}"
            )
        else:
...
class ModelFamily:
    ...
    def __init__(self, session, model_family_name=None, model_family_id=None):
        ...
        self.model_keys = {
            model["modelKey"]: model["modelName"] for model in family_data["models"]
        }
...
class BYOM:
    ...
    def get_user_code_details(self, model_family_name):
        model_family_id = self._get_model_faimly_id(model_family_name)
    ...
    def get_allowed_actions(self, model_family_name):
    ...
    def start_test_cases_by_type(self, model_family_name, test_cases_type="failed"):
        path = f"/v1/model_store/run_selected_test_cases?modelFamily={model_family_name}&testType={test_cases_type}"
        resp = self.rpc.put(path=path)
        return handle_response(
            resp,
            "Successfully run the selected test cases",
            "An error occured while running the selected test cases",
        )

    def start_selected_test_cases(self, model_family_name, model_key, test_cases):
    ...
    def integrate_model_actions(
        self, model_family_name, model_key, actions
    ):
        ...
        Args:
            model_family_name (str): Name of the model family (e.g. "RESNET-89", "EfficientNet V2")
            model_key (str): Specific model key (e.g. "resnet18", "efficientnet_v2_l") 
            actions (list): List of action dictionaries with structure:
                [
                    {
                        "actionType": str,  # e.g. "model_predict", "train_model" 
                        "batchSize": int,   # batch size for the action
