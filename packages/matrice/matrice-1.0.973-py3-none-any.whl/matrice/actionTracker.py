...
class ActionTracker:
    ...
    def save_benchmark_results(self, latency_ms, batch_size=1):
        def get_model_arch():
            is_exported = (
                "pytorch"
                not in self.action_details.get("runtimeFramework", "pytorch").lower()
            )
            model_train_doc = self.get_model_train(is_exported)
            model_arch = ModelArch(
                self.session, model_info_id=model_train_doc["_idModelInfo"]
            )
            return model_arch

        def get_export_format():
            if self.action_type != "model_export":
                return self.action_details.get(
                    "runtimeFramework", self.action_details.get("exportFormat", "")
                )
            else:
                export_formats = self.action_details.get("exportFormats", "")
                if type(export_formats) is list:
        ...
        payload = {
            ...
            "modelFamily": self.action_details.get("modelFamily", model_arch.model_family_name) ,
            "modelKey": self.action_details.get("modelKey", model_arch.model_key),
            "paramsInMillion": self.action_details.get("paramsInMillion", model_arch.params_millions),
    ...
    def calculate_metrics(
        ...
        try:
            # TODO update if not pytorch and trained and add more metrics types
            is_exported = (
                "pytorch"
                not in self.action_details.get("runtimeFramework", "pytorch").lower()
            )
            if project_type == "classification":
                metrics = get_classification_evaluation_results(
                    ...
                    outputs,
                    targets,
                    self.get_index_to_category(is_exported),
                )
                ...
                raise ValueError(
                    f"Unsupported metrics type: {project_type}, supported types are classification and object_detection"
                )
        ...
        try:
            if project_type == "classification":
                for output in outputs:
                    ...
                    results.append(
                        {"category": str(predicted_class), "confidence": confidence}
                    )
            elif project_type == "detection":
                for output in outputs:
                ...
                raise ValueError(
                    f"Unsupported metrics type: {project_type}, supported types are classification and detection"
                )
        except Exception as e:
    ...
    def _reformat_inference_targets(self, targets, project_type):
        ...
        try:
            if project_type == "classification":
                results = [
                        {"category": str(target.cpu().item()), "confidence": 1}
    ...
    def store_inference_results(
        self, images, outputs, targets, split_type, project_type
    ):
        ...
        try:
            ...
            samples = [
                {
                    "_idModel": self.action_details.get("_idModel"),
                    "modelType": (
                        "exported"
                        if "pytorch"
                        not in self.action_details.get(
                            "runtimeFramework", "pytorch"
                        ).lower()
                        else "trained"
                    ),
                ...
                for prediction, target in zip(
                    outputs[:num_samples], targets[:num_samples]
                )
            ]

            try:
                presigned_urls = self.rpc.post(
                    path="/v1/model/add_test_sample", payload=samples
                )
            except Exception as e:
            ...
            try:
                # Convert bytes array to list and handle image data
                for image_tensor, presigned_url in zip(images, presigned_urls):
                    ...
                    # Rewind the buffer to the beginning (required to read from it)
                    img_byte_arr.seek(0)
                ...
                log_error(__file__, "store_inference_results", str(e))
                print(f"Failed to upload images to S3: {str(e)}")
        except Exception as e:
            log_error(__file__, "store_inference_results", str(e))
