...
class ActionTracker:
    ...
    def log_error(
        self, filename, function_name, error_message
    ):
    ...
    def _remove_duplicate_results(self, list_of_result_dicts):
        ...
        for result in list_of_result_dicts:
            key = (result.get("splitType"), result.get("metricName"), result.get("category"))
            if key not in seen:
    ...
    def calculate_metrics(
        self, split_type, outputs, targets, metrics_type, images = None, predictions = None, save_inference_results = False
    ):  
        # TODO update if not pytorch and trained and add more metrics types
        is_exported = "pytorch" not in self.action_details.get("runtimeFramework", "pytorch").lower() 
        if metrics_type == "classification":
        ...
        elif metrics_type == "object_detection":
            ...
            raise ValueError(f"Unsupported metrics type: {metrics_type}, supported types are classification and object_detection")
        
        if save_inference_results:
            self.store_inference_results(images, predictions, targets)
        return metrics

    def store_inference_results(self, images, predictions, targets): # TODO
        for image, prediction, target in zip(images, predictions, targets):
            break
            # Convert image and prediction to numpy arrays
            ...
            try:
                self.rpc.post(path="/v1/model_store/inference_results", payload=payload)
            except Exception as e:
