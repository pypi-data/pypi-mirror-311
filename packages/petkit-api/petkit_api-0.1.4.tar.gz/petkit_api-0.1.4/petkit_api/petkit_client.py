...
class PetKitClient:
    ...
    async def login(self) -> None:
        ...
        LOGGER.debug(f"Logging in at {login_url} with region {self.region}")
        ...
        self.token_expiration = datetime.now() + timedelta(
            seconds=response["result"]["session"]["expiresIn"],
        )
    ...
    async def check_token(self) -> None:
        ...
        if (self.token or self.token_expiration) is None:
            ...
            self.token_expiration - current_dt
        ).total_seconds() < TOKEN_EXPIRATION_TIME:
            ...
            if devices:
                for device in devices:
                    ...
                    if device["type"] in WATER_FOUNTAIN_LIST:
                        wf_instance, wf_id = await self._handle_water_fountain(
                            device=device,
                            has_relay=group_has_relay,
                            header=header,
                        )
                    ...
                    if device["type"] in FEEDER_LIST:
                        feeder_instance, feeder_id = await self._handle_feeder(
                            device=device,
                            header=header,
                        )
                    ...
                    if device["type"] in LITTER_LIST:
                        if device["type"] == "T6":
                            litter_box_instance, litter_box_id = (
                                await self._handle_litter_box_smart(
                                    device=device,
                                    header=header,
                                )
                            )
                    ...
                    if device["type"] in PURIFIER_LIST:
                        purifier_instance, purifier_id = await self._handle_purifier(
                            device=device,
                            header=header,
                        )
    ...
    async def _handle_water_fountain(
        self,
        device: dict[str, Any],
        has_relay: bool,
        header: dict[str, str],
        ...
        wf_url = f"{self.base_url}{device_type}/{Endpoint.DEVICE_DATA}"
        ...
        if has_relay:
            ...
            if device["id"] not in self.last_ble_poll:
                ...
                if (
                    current_dt - self.last_ble_poll[device["id"]]
                ).total_seconds() >= BLE_POLL_INTERVAL:
                    can_poll = True
                else:
                    can_poll = False
            if can_poll:
                ...
                relay_devices = await self._post(
                    ble_url,
                    header,
                    data={
                ...
                if relay_devices["result"]:
                    ...
                    if ble_available and main_online:
                        LOGGER.debug(
                            f'Fetching water fountain({device["id"]}) details page at {wf_url}',
                        )
                        ...
                        conn_success = await self.start_ble_connection(
                            conn_url,
                            header,
                            ble_data,
                            ble_connect_attempt,
                        ...
                        if conn_success:
                            ...
                            poll_success = await self.poll_ble_connection(
                                poll_url,
                                header,
                                ble_data,
                                ble_poll_attempt,
                            ...
                            if poll_success:
                                ...
                                self.ble_sequence += 1
                                try:
                                    await self.initial_ble_commands(
                                        device_details,
                                        relay_tc,
                                    )
                                    ...
                                    fountain_data = await self._post(
                                        wf_url,
                                        header,
                                        data,
                                    )
                    ...
                    if not main_online:
                        LOGGER.warning(
                            "Unable to use BLE relay: Main relay device is reported as being offline. Fetching latest available data.",
                        )
    ...
    async def _handle_feeder(
        ...
        data = {"id": device["id"]}
        LOGGER.debug(f'Fetching data for feeder({device["id"]}) at {feeder_url}')
        feeder_data = await self._post(feeder_url, header, data)
        LOGGER.debug(f" Feeder data response:\n{json.dumps(feeder_data, indent=4)}")
        ...
        if device["type"] in ["D3"]:
            ...
            LOGGER.debug(
                f"Sound data response:\n{json.dumps(sound_response, indent=4)}",
            )
    ...
    async def _handle_litter_box(
        ...
        LOGGER.debug(
            f"Litter box record response:\n{json.dumps(device_record, indent=4)}",
        )
        ...
        LOGGER.debug(
            f"Litter box statistics response:\n{json.dumps(device_stats, indent=4)}",
        )
    ...
    async def _handle_litter_box_smart(
        self,
        device: dict[str, Any],
        header: dict[str, str],
    ) -> (LitterBox, int):
    ...
    async def _handle_pets(self, header: dict[str, str]) -> dict[int, Pet]:
        ...
        details_data = {"userId": self.user_id}
    ...
    async def start_ble_connection(
        self,
        conn_url: str,
        header: dict[str, Any],
        ble_data: dict[str, Any],
        ble_connect_attempt: int,
    ) -> bool:
        """Used to initiate the BLE relay connection."""
        # Stop trying to connect via BLE relay after 4 attempts
        if ble_connect_attempt > BLE_CONNECT_ATTEMPT:
            LOGGER.debug(
        ...
        if conn_resp["result"]["state"] != 1:
            ...
            await self.start_ble_connection(
                conn_url,
                header,
                ble_data,
                ble_connect_attempt,
    ...
    async def poll_ble_connection(
        ...
        if poll_resp["result"] != 0:
            ...
            ble_poll_attempt += 1
            await asyncio.sleep(3)
            await self.poll_ble_connection(
                poll_url,
                header,
                ble_data,
                ble_poll_attempt,
    ...
    async def initial_ble_commands(
        self,
        device: dict[str, Any],
        relay_type: int,
    ) -> None:
    ...
    async def create_ble_data(
        self,
        command: FountainCommand,
        device: Fountain | None = None,
    ) -> str:
        ...
        if command == FountainCommand.LIGHT_OFF:
            ...
            data_list = await self.fountain_command_data_creator(
                device=device,
                command=command,
                setting=[0],
            )
            byte_list = await self.create_ble_byte_list(
                command=-35,
                data_list=data_list,
            )
        ...
        return urlencode.quote(b64_encoded, "utf-8")

    async def fountain_command_data_creator(
        self,
        device: Fountain,
        command: FountainCommand,
        setting: list,
        ...
        if command in FOUNTAIN_SETTINGS_COMMANDS:
            light_up = await self.short_to_byte_list(
                input=device_data["settings"]["lampRingLightUpTime"],
            )
            light_out = await self.short_to_byte_list(
                input=device_data["settings"]["lampRingGoOutTime"],
            )
            disturb_start = await self.short_to_byte_list(
                input=device_data["settings"]["noDisturbingStartTime"],
            )
            disturb_end = await self.short_to_byte_list(
                input=device_data["settings"]["noDisturbingEndTime"],
            )
    ...
    async def create_ble_byte_list(
        self,
        command: int,
        data_list: list[int],
    ) -> list[int]:
    ...
    @staticmethod
    async def short_to_byte_list(input_val: int) -> list:
        """Take a short and creates a list with bytes represented in int format."""
        byte_list: list = []
        i: int = 0
        while i < 2:  # noqa: PLR2004
            i2: int = i + 1
            byte_list.append((input_val >> (16 - (i2 * 8))) & 255)
            i = i2
        return byte_list

    async def get_litter_box_record(
        self,
        dev_id: int,
        dev_type: str,
        header: dict[str, Any],
        ...
        url = f"{self.base_url}{dev_type}/{Endpoint.DEVICE_RECORD}"
        data = {"day": str(datetime.now().date()).replace("-", ""), "deviceId": dev_id}
        return await self._post(url, header, data)

    async def control_water_fountain(
        self,
        ...
        if not water_fountain.group_relay:
            raise PetKitError(
                f'{water_fountain.data["name"]} does not have a valid PetKit device to use as a BLE relay.',
            )
        ...
        if command == FountainCommand.PAUSE:
            ...
            if pow_status == 0:
                raise PetKitError(
                    f'{water_fountain.data["name"]} is already paused.',
                )
            if water_fountain.data["mode"] == 1:
                ble_data = await self.create_ble_data(
        ...
        # make sure light is on if brightness is being set
            ...
            if water_fountain.data["settings"]["lampRingSwitch"] != 1:
                raise PetKitError(
                    f'{water_fountain.data["name"]} indicator light is Off. You can only change light brightness when the indicator light is On.',
                )
        ...
        if conn_success:
            ...
            if poll_success:
                ...
                raise BluetoothError(
                    "BLE polling step failed while attempting to send the command to the water fountain",
                )
        else:
            raise BluetoothError(
                "BLE connection step failed while attempting to send the command to the water fountain",
            )
    ...
    async def call_pet(self, feeder: Feeder) -> None:
        ...
        header = await self.create_header()
        data = {"deviceId": feeder.id}
        await self._post(url, header, data)

    async def control_litter_box(
        self,
        litter_box: LitterBox,
        command: LitterBoxCommand,
    ) -> None:
        ...
        if litter_box.type == "t4":
            if command == LitterBoxCommand.START_CLEAN:
                ...
                if "workState" in litter_box.device_detail["state"]:
                    ...
                    if (state["workMode"] == 0) and (state["workProcess"] == 20):
                        ...
                        raise PetKitError(
                            "Unable to call start cleaning command while litter box is in operation.",
                        )
            if command == LitterBoxCommand.PAUSE_CLEAN:
                ...
                self.manual_pause_end[litter_box.id] = datetime.now() + timedelta(
                    seconds=660,
                )

        if command == LitterBoxCommand.POWER:
            # If litter box is currently turned on then you want the command to turn it off
            value = 0 if litter_box.device_detail["state"]["power"] == 1 else 1
        ...
        if litter_box.type == "t3":
            if command == LitterBoxCommand.START_CLEAN:
                ...
                record = await self.get_litter_box_record(
                    litter_box.id,
                    litter_box.type,
                    header,
                )
                if record["result"]:
                    ...
                    if last_item["enumEventType"] == "clean_over":
                        if (last_item["content"]["startReason"] in [0, 1, 2, 3]) and (
                            last_item["content"]["result"] == 3  # noqa: PLR2004
                        ):
                            await self.control_litter_box(
                                litter_box,
                                LitterBoxCommand.RESUME_CLEAN,
                            )
                            self.manually_paused[litter_box.id] = False
                            self.manual_pause_end[litter_box.id] = None
    ...
    async def control_purifier(
        ...
        if command == PurifierCommand.POWER:
            ...
            value = 0 if purifier.device_detail["state"]["power"] in [1, 2] else 1
        else:
    ...
    async def check_manual_pause_expiration(self, id_device: int):
        """Check to see if manual pause has expired and litter box resumed the cleaning on its own."""
        current_dt = datetime.now()
        current_end = self.manual_pause_end[id_device]
        if (current_end - current_dt).total_seconds() <= 0:
            self.manual_pause_end[id_device] = None
            self.manually_paused[id_device] = False
    ...
    async def dual_hopper_manual_feeding(
        self,
        feeder: Feeder,
        amount1: int = 0,
        amount2: int = 0,
        ...
        """
        invalid_amount1 = (amount1 < MIN_FEED_AMOUNT) or (amount1 > MAX_FEED_AMOUNT)
        invalid_amount2 = (amount2 < MIN_FEED_AMOUNT) or (amount2 > MAX_FEED_AMOUNT)
        if invalid_amount1 or invalid_amount2:
            raise PetKitError(
                "Invalid portion amount specified. Each hopper can only take a portion value between/including 0 to 10",
            )
        url = f"{self.base_url}{feeder.type}/{Endpoint.MANUAL_FEED}"
        header = await self.create_header()
        data = {
    ...
    async def update_feeder_settings(
        self,
        feeder: Feeder,
        ...
        data = {"id": feeder.id, "kv": json.dumps(setting_dict)}
        await self._post(url, header, data)

    async def update_litter_box_settings(
        self,
        litter_box: LitterBox,
        setting: LitterBoxSetting | None = None,
        value: int | None = None,
    ) -> None:
        ...
        header = await self.create_header()
        setting_dict = {setting: value}
        data = {"id": litter_box.id, "kv": json.dumps(setting_dict)}
        await self._post(url, header, data)
    ...
    async def update_pet_settings(
        ...
        header = await self.create_header()
        setting_dict = {setting: value}
        data = {"petId": int(pet.id), "kv": json.dumps(setting_dict)}
        await self._post(url, header, data)

    async def update_purifier_settings(
        self,
        purifier: Purifier,
        ...
        header = await self.create_header()
        setting_dict = {setting: value}
        data = {"id": purifier.id, "kv": json.dumps(setting_dict)}
        await self._post(url, header, data)
    ...
    async def cancel_manual_feed(self, feeder: Feeder) -> None:
        ...
        if feeder.type in ["d4s", "d4sh"]:
            if feeder.last_manual_feed_id is None:
                raise PetKitError(
                    "Unable to cancel manual feeding. No valid last manual feeding ID found.",
                )
            data = {
                "day": str(datetime.now().date()).replace("-", ""),
                "deviceId": feeder.id,
    ...
    async def food_replenished(self, feeder: Feeder) -> None:
        ...
        if feeder.type not in ["d4s", "d4sh"]:
            raise PetKitError(
                "The food_replenished method is only used with D4s (Gemini) or D4Sh (YumShare) feeders.",
            )
        url = f"{self.base_url}{feeder.type}/{Endpoint.REPLENISHED_FOOD}"
        header = await self.create_header()
        data = {"deviceId": feeder.id, "noRemind": 3}
        await self._post(url, header, data)

    async def fresh_element_calibration(
        self,
        feeder: Feeder,
        ...
        header = await self.create_header()
        value = 1 if command == FeederCommand.START_CALIBRATION else 0
        data = {"action": value, "deviceId": feeder.id}
        await self._post(url, header, data)
