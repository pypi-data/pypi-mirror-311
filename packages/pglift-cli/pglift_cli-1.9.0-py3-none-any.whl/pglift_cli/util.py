...
def pass_postgresql_instance(f: Callable[P, None]) -> Callable[P, None]:
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> None:
        ...
        assert isinstance(instance, system.PostgreSQLInstance), instance
        context.invoke(f, instance, *args, **kwargs)
...
def get_postgresql_instance(
    ...
    try:
        return system.PostgreSQLInstance.system_lookup(name, version, settings)
...
def get_instance(name: str, version: str | None, settings: Settings) -> system.Instance:
    ...
    return system.Instance.from_postgresql(pg_instance)
...
) -> system.PostgreSQLInstance | tuple[system.PostgreSQLInstance, ...]:
    """Return one or more system.PostgreSQLInstance, possibly guessed if there
    is only one on system, depending on 'param' variadic flag (nargs).
    """

    settings = _site.SETTINGS

    def guess() -> system.PostgreSQLInstance:
...
def instance_lookup(
    ...
    if isinstance(pg_value, tuple):
        return tuple(system.Instance.from_postgresql(i) for i in pg_value)
    return system.Instance.from_postgresql(pg_value)
...
class Obj:
    ...
    @cached_property
    def postgresql_instance(self) -> system.PostgreSQLInstance:
        if isinstance(self._instance, system.PostgreSQLInstance):
    ...
    def instance(self) -> system.Instance:
        return system.Instance.from_postgresql(self.postgresql_instance)
