...
from pglift import postgresql, privileges, profiles, roles
...
@async_command
async def create(
    obj: Obj, instance: system.PostgreSQLInstance, role: interface.Role
) -> None:
...
@async_command
async def alter(
    obj: Obj, instance: system.PostgreSQLInstance, rolname: str, **changes: Any
) -> None:
    """Alter a role in a PostgreSQL instance"""
    with obj.lock, audit():
        async with postgresql.running(instance):
            # We exclude `in_roles` here so that it doesn't conflict with `memberships`
            # until the field is effectively removed (currently deprecated).
            values = (await roles.get(instance, rolname)).model_dump(
                by_alias=True, exclude={"in_roles"}
            )
...
@async_command
async def ls(
    instance: system.PostgreSQLInstance, output_format: OutputFormat | None
) -> None:
...
@async_command
async def drop(
    instance: system.PostgreSQLInstance, roledropped: interface.RoleDropped
) -> None:
    """Drop a role"""
    async with postgresql.running(instance):
