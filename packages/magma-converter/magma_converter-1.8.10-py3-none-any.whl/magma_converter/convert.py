...
from magma_database import Sds, Station
from magma_database.helper import reset
...
from .database import db, DatabaseConverter
            ...
            if trace.stats.channel[0] not in ['M', 'O']:
                ...
                if sds.save(min_completeness=min_completeness, **kwargs) is True:
                    ...
                    if (self.plot_seismogram is True) and (file_exists is False):
                        PlotSeismogram(sds=sds, plot_type=self.plot_type).plot_with_gaps(plot_gaps=self._plot_gaps).save()

                    if self.save_to_database is True:
                        station_id: int = DatabaseConverter.update_station(station=sds.stations)
                        sds_id: int = DatabaseConverter.update_sds(sds=sds.results)
                        self.station_ids.append(station_id)
                        self.sds_ids.append(sds_id)
                        print(f"💽 {date_str} :: Database updated. Station id: {station_id}. SDS id: {sds_id}")
