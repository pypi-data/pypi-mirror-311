...
from majorshare.http_client import DataApi
import majorshare.upass as upass
...
def pro_api(token='',server='', timeout=30):
    """
    ...
    if token is not None and token != '':
        pro = DataApi(token=token,server=server, timeout=timeout)
        return pro
    else:
...
def get_yfinance_proxy(token=''):
    if token == '' or token is None:
        token = upass.get_token()
    
    url = f'https://datashop.majors.ltd:8889/goodip?token={token}'
    try:
        with urllib.request.urlopen(url) as response:
            if response.status == 200:
                ...
                if 'error' in data:
                    ...
                    if 'msg' in data and 'result' in data:
                        print('服务器消息：', data['msg'])
                        print('注意返回的proxy_url非永久有效，失效后需要重新运行get_yfinance_proxy函数进行鉴权。')
                        return data['result']
                    else:
                        raise ValueError("服务器内部错误，请向幂级数反馈！谢谢！")
            else:
                raise ValueError("无法访问服务器！请检查网络")
    except urllib.error.URLError as e:
        raise ValueError(f"网络访问失败: {e.reason}")
