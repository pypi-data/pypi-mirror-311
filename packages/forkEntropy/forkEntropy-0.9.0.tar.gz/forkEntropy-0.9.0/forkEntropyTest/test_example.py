...
def main():
    root_path = "forkEntropyTest/testData\downloaded_PRrawdata\*.json"
    repos_json = glob.glob(root_path)
    replenishedCon = "forkEntropyTest/testData\commitChanges\*.json"
    ...
    print(results_df)
    results_df.to_csv('forkEntropyTest/testResult/output_.csv', index=False)