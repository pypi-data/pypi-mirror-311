...
def isRemoteOllamaHost(host):
    #host = "http://localhost:11434"
    host = re.sub("^.*?://", "", host.strip())
    host = re.sub(":[0-9]+?$", "", host)
    return False if host.lower() in ("127.0.0.1", "localhost") else True
...
def getOllamaServerClient(server="main"):
    ...
    return Client(host=config.ollamaChatHost if server=='chat' else config.ollamaToolHost)
