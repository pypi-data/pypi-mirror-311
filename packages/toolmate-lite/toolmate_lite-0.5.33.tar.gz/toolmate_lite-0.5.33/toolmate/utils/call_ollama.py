...
class CallOllama:
    ...
    @check_ollama_errors
    def checkCompletion():
        if shutil.which("ollama") and not isRemoteOllamaHost(config.ollamaToolHost):
            Downloader.downloadOllamaModel(config.ollamaToolModel)
        if config.useAdditionalChatModel and not isRemoteOllamaHost(config.ollamaChatHost):
            Downloader.downloadOllamaModel(config.ollamaChatModel)
        if not shutil.which("ollama") and not (isRemoteOllamaHost(config.ollamaToolHost) or isRemoteOllamaHost(config.ollamaChatHost)):
