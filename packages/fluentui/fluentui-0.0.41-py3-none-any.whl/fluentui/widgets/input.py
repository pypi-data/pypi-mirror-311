...
class Textarea(AbsScrollAreaMix, QTextEdit):
    ...
    def setText(self, text: str):
        ...
        for img in html.css('img[src^="http://"], img[src^="https://"]'):
            ...
            name = url.toString()
            if doc.resource(TextDocument.Resource.Image, name):
                continue

            # 占位图片源，防止请求重复的图片
            doc.addResource(TextDocument.Resource.Image, name, True)
    ...
    def setDocument(self, document: QTextDocument):
        ...
        document.resource(TextDocument.Resource.Image, "name")
    ...
    def __downloaded(self, reply: QNetworkReply):
        ...
        try:
            doc = self.document()
            doc.addResource(TextDocument.Resource.Image, name, image)
